...
const MODULE_SPEC = [
  ...
  {
    re: /(\batt\b|chấm công|attendance|điều chỉnh công|ca làm)/i,
    spec: "docs/SPEC/SPEC-04 ATT.md",
  },
...
function verifyGuide(b) {
  ...
  const feApps = [
    ...new Set(
      paths.map((p) => (p.match(/^apps\/(auth|console|app|web)\b/) || [])[1]).filter(Boolean),
    ),
  ];
  const commands = [];
  ...
  if (touchesApi) {
    commands.push(
      `bash scripts/lane-db-setup.sh ${b.id.toLowerCase()}  # DB cô lập (chống shared-DB drift)`,
    );
    commands.push(
      `export LANE_DB=mediaos_${b.id.toLowerCase()} && pnpm --filter @mediaos/api test`,
    );
  }
  feApps.forEach((a) =>
    commands.push(`pnpm --filter @mediaos/${a} test && pnpm --filter @mediaos/${a} typecheck`),
  );
...
async function computeStatus() {
  ...
  if (!inProgress.length && ready.length) {
    risks.push({
      level: "info",
      message: `Không có việc đang làm nhưng ${ready.length} việc READY — chọn 1 để mở.`,
    });
  }
  if (inProgress.length > 1) {
    risks.push({
      level: "warn",
      message: `${inProgress.length} việc in_progress cùng lúc — mô hình v2 khuyến nghị tuần tự 1/phiên.`,
    });
  }
  inProgress.forEach((b) => {
    if (b.zone === "red")
      risks.push({
        level: "warn",
        message: `Việc ĐỎ đang mở (${b.id}) — cần người chốt, KHÔNG auto-commit.`,
      });
  });
  ready.forEach((b) => {
    if (b.zone === "red")
      risks.push({
        level: "info",
        message: `Việc ĐỎ chờ làm (${b.id}) — chuẩn bị FULL gate + người duyệt.`,
      });
  });
  ...
  backlog.forEach((b) =>
    (b.depends_on || []).forEach((d) => {
      if (!byId[d])
        risks.push({
          level: "warn",
          message: `${b.id} phụ thuộc \`${d}\` KHÔNG tồn tại trong backlog — kẹt vĩnh viễn.`,
        });
    }),
  );
  if (dirty.length > 60) {
    risks.push({
      level: "warn",
      message: `${dirty.length} file dirty — diff lớn, khó review/red-zone dễ lẫn. Cân nhắc chia nhỏ.`,
    });
  }
  if (dirty.some((l) => /drizzle|_journal|migration/i.test(l))) {
    risks.push({
      level: "warn",
      message: "Có thay đổi migration trong dirty — vùng ĐỎ, RLS+FORCE phải trước backfill.",
    });
  }
...
function serveDocRaw(req, res) {
  ...
  if (abs !== DOCS_DIR && !abs.startsWith(DOCS_DIR + path.sep))
    return send(res, 403, JSON.stringify({ error: "forbidden" }));
  if (!DOC_EXT.has(path.extname(abs).toLowerCase()))
    return send(res, 415, JSON.stringify({ error: "unsupported type" }));
  let st;
  try {
    st = fs.statSync(abs);
  } catch {
