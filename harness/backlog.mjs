...
export const backlog = [
  ...
  },
  {
    ...
    done_when: [
      ...
      "Mention: chỉ mention người có quyền xem task (ngoài scope → cảnh báo/không cho); comment/mention phát event TASK_COMMENT_CREATED/TASK_COMMENT_MENTIONED (registry §9.5, canonical — KHÔNG dùng alias TASK_MENTIONED) qua outbox",
  ...
  },
  {
    ...
    done_when: [
      "Seed notification_events theo Event code registry §9.5 CANONICAL (TASK_ASSIGNED/ASSIGNEE_CHANGED/STATUS_CHANGED/PRIORITY_CHANGED/DUE_DATE_CHANGED/COMMENT_CREATED/COMMENT_MENTIONED/DUE_SOON/OVERDUE, PROJECT_MEMBER_ADDED/REMOVED, PROJECT_CLOSED, NOTIFICATION_CREATED/READ) — event chưa bật MVP để enabled=false (KHÔNG bỏ khỏi catalog); dùng TASK_COMMENT_MENTIONED, KHÔNG alias TASK_MENTIONED",
      "Seed template IN_APP locale vi cho mọi event enabled=true (§10.4: title + short_template + variables_schema); ON CONFLICT DO NOTHING",
      "Seed permission NOTI (view own · count unread · mark read · mark all read · config event/template/channel · view delivery-log) + role mapping (mọi user có own-notification; admin có config); allowlist cặp sensitive nếu cần",
      "Int-spec: seed idempotent chạy lại; event/template khớp registry (không mã lạ); admin thấy cặp config NOTI qua /auth/me; FULL gate security-reviewer + database-reviewer PASS",
    ],
  },
  {
    id: "S4-NOTI-BE-1",
  ...
  },
  {
    ...
    done_when: [
      "Migration nối tiếp head (SAU S4-NOTI-DB, tuần tự): dashboard_widgets (widget_code, widget_name, widget_type, source_modules, required_permission, status) · dashboard_widget_configs (config theo company/role/user/dashboard_type) · dashboard_widget_cache (cache_key, widget_code, dashboard_type, data, expires_at)",
  ...
  // bỏ (không gap cứng). BE enabler seed kèm vì FE screen không tồn tại được nếu thiếu.
  {
    ...
    zone: "green",
    status: "todo",
    paths: ["apps/app/src/routes/leave/**", "apps/app/src/i18n/**", "packages/web-core/src/lib/**"],
    skills: ["code-review"],
    depends_on: [],
