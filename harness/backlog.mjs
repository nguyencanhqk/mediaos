...
export const backlog = [
  ...
  },
  {
    ...
    done_when: [
      "ĐỌC docs/plans/S4-DASH-SEED-1.md §0 TRƯỚC KHI CODE — plan v1 bị plan-reviewer BLOCK, v2 đã sửa 4 lỗi (GRANT DELETE, vị trí seeder, số migration, thiếu cặp audit-log). Đừng lặp lại.",
      "OWNER CHỐT (Option B — gate widget bằng quyền MODULE NGUỒN): KHÔNG seed cặp engine per-widget ('*:dashboard-widget'). dashboard_widgets.required_permission_code lưu chuỗi SPEC verbatim 'DASH.WIDGET.VIEW_*' (thoả DB-07 §8.5); BE resolve → cặp read của module nguồn qua DASH_WIDGET_GATE_PAIR. Cặp module nguồn phải TRA từ migration seed THẬT (grep 'INSERT INTO permissions'), KHÔNG đoán theo tên — vd 'read:attendance' có thể không tồn tại",
      ...
      "Seed ĐÚNG 8 cặp quyền DASH mới: view-employee/manager/hr/admin:dashboard · view:dashboard-config · update:dashboard-config · refresh:dashboard-cache · view:dashboard-audit-log (DASH.AUDIT_LOG.VIEW có ở CẢ DB-07 §10.2 lẫn SPEC-07). GIỮ NGUYÊN 'read:dashboard' (mig 0100). Chỉ enumerate 4 role canonical employee/manager/hr/company-admin — super-admin KHÔNG enumerate (sẽ RAISE vì roles company_id IS NULL không có row đó)",
      ...
      "Int-spec RED-trước, deny-path đi đầu, gate `hasDb && LANE_DB`: (E1) employee/manager có 0 grant *:dashboard-config, không view-admin/view-hr:dashboard, không view:dashboard-audit-log · (E2) KHÔNG tồn tại cặp '*:dashboard-widget' · (E3 chống pair-drift) MỖI giá trị DASH_WIDGET_GATE_PAIR phải resolve ra row (action,resource_type) THẬT trong bảng permissions — sai tên thì RED tại đây thay vì silent-403 ở DASH-BE · catalog == đúng 7 widget · seed lại 2-3 lần idempotent, data_scope không drift · admin thấy cặp DASH qua /auth/me · (cross-tenant) PHẢI plant company thứ 2 + 1 config row thật rồi assert vắng mặt dưới GUC company A — ở N=1 không plant thì test xanh-giả. FULL gate security-reviewer + database-reviewer PASS",
    ],
  },
  {
    id: "S4-DASH-BE-1",
