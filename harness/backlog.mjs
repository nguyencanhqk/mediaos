...
export const backlog = [
  ...
  },
  {
    ...
    paths: [
      ...
      "docs/RELEASE/**",
      "docs/permission-matrix-spec.md",
      "docs/plans/S6-SEC-ORGSCOPE-1.md",
    ],
    skills: ["code-review"],
    depends_on: ["S6-SEC-ORG-1"],
    plan: "docs/plans/S6-SEC-ORGSCOPE-1.md",
    src: [
      "KHÔNG migration — đây là việc CODE thuần (đính chính: khác S6-SEC-PERMVERB-1 vốn cần migration)",
      "docs/plans/S6-SEC-ORG-1.md §7 N-1 — cả 3 reviewer FULL gate (security-reviewer MEDIUM-2 · rls-tenant-isolation-tester · completion-evaluator) độc lập chỉ về cùng chỗ",
      "apps/api/src/org/org.repository.ts listEmployees — chỉ `withTenant` + `eq(users.companyId)`, KHÔNG ép data_scope; trả id·email·fullName·status + team membership của MỌI user chưa xoá",
      "MẪU ĐÚNG có sẵn ở sibling: apps/api/src/employees/employees.service.ts:88-95 — `dataScope.resolveAndAssert(...)` rồi `buildEmployeeScopeCondition(scope, ctx)` AND vào SELECT (DataScopeService: apps/api/src/permission/data-scope.service.ts)",
      "⚠️ CÂU HỎI THIẾT KẾ PHẢI TRẢ LỜI TRƯỚC KHI CODE: `buildEmployeeScopeCondition` dựng vị từ trên employee_profiles, còn listEmployees SELECT từ `users`. Chọn: (a) join employee_profiles rồi lọc — nhưng user KHÔNG có hồ sơ sẽ biến mất khỏi màn RBAC (hồi quy!), (b) dựng biến thể vị từ hình-users, hay (c) giữ Company-only + CHẶN cấp scope hẹp ở role-admin. Quyết định phải ghi vào plan kèm lý do",
      "role-admin.service.ts:309-312 — admin gán được Own/Team/Department, ceiling chỉ chặn System ⇒ đường mở lỗ là CÓ THẬT, không phải giả định",
      "Pin hiện có CHỈ phủ role HỆ THỐNG: ca cuối `test/integration/org-directory-permission.int-spec.ts` — role tenant đúc lúc chạy KHÔNG được phủ",
      "Consumer FE cần rà kèm: apps/console/src/lib/org-api.ts:62 + rbac-api.ts:52 (màn RBAC dùng /org/employees làm subject gán role — thu hẹp scope ở đây có thể làm admin không gán role được cho người ngoài scope)",
      "memory: reused-method-must-be-actor-scoped · read-path-gate-pair-must-match-download-pair",
    ],
    done_when: [
      "RED TRƯỚC: test seed role scope `Team` CÓ `read:user` → gọi GET /org/employees phải chỉ thấy người trong scope. Trên code hiện tại test ĐỎ (nay trả toàn tenant) — chứng minh bằng log",
      "Quyết định thiết kế (a)/(b)/(c) ở src[] được ghi TƯỜNG MINH trong plan kèm lý do; nếu chọn (c) thì phải có cơ chế CHẶN thật ở role-admin, không chỉ ghi chú",
      "KHÔNG hồi quy màn RBAC console: user chưa có employee_profile vẫn phải liệt kê được cho company-admin (scope Company) — có test khoá",
      "Nới pin ở org-directory-permission.int-spec: bỏ giới hạn `is_system = true` sau khi repo đã ép scope, hoặc nêu rõ vì sao vẫn cần giữ",
      "docs/plans/S6-SEC-ORG-1.md §7 N-1 đánh dấu đóng + permission-matrix-spec cập nhật CHỐT /org",
      "FULL gate security-reviewer PASS; verify tier = lint+typecheck+build + check.sh --all với LANE_DB",
    ],
  },
  {
  ...
  },
  {
    ...
    skills: ["code-review"],
    depends_on: ["S6-SEC-ORGTEAMSCOPE-1"],
    plan: "docs/plans/S6-SEC-IDENTITY-PROJ-1.md",
    src: [
      "N-1 (/org/employees, #302) · N-2 (chốt động từ, #305) · N-1c (/org/teams/:id/members, KI-049) = BA lần vá CÙNG một lớp lỗi ở ba route khác nhau trong ba tuần ⇒ vá lẻ không hội tụ",
      "Gốc: PermissionGuard chỉ trả lời 'CÓ cặp quyền không', không đọc data_scope ⇒ mọi route chỉ dựa vào guard đều thừa hưởng khoảng hở",
      "ĐÃ LOẠI đường 'guard tự phơi data_scope ra request' (plan S6-SEC-ORGTEAMSCOPE-1 §3.3): guard phơi ra thì handler VẪN tự chọn dùng hay không — N-1c chính là ca guard gate đúng/handler không bound. Thêm một thứ tuỳ chọn không biến bug im lặng thành bug ồn ào",
      "Census 2026-07-29 ('40+ điểm ở 7 module') ĐÃ BỊ ĐO LẠI VÀ BÁC BỎ — con số THẬT (script AST-lite, bắt cả alias `alias(users,…)` và `getTableColumns(users)` mà grep thẳng TRƯỢT): **79 điểm ở 31 file / 12 module** = 52 PROJECTION + 11 PREDICATE (ilike/eq — lớp oracle RIÊNG, không cùng lớp lỗ) + 6 ORDER BY + 8 hỗn hợp. Module thiếu trong census cũ: me · users · tasks · integrations/lms · recycle-bin · user-invites",
      "Census cấp MODULE (có @RequirePermission + có chuỗi data_scope trong service) KHÔNG dùng được để kết luận: module org hiện ra 'CÓ' chỉ vì N-1 thêm cho employees trong khi teams/members vẫn trống",
      "✅ ĐÃ ĐO PROD 2026-07-30 (mediaos/funtime, 46 user sống) — done_when 'ĐO PROD' COI NHƯ XONG cho pha này. Kết quả then chốt: role `employee` = **45/46 user sống**, giữ read:attendance@Company · read:leave@Company · read:employee@Own · read:project@Own · read:task@Own, và **KHÔNG có view:user nào**. SA=6, company-admin=1, MỌI role khác = 0 user",
      "⚠️ GUARD ĐÃ VERIFY: permission.guard.ts KHÔNG hề đọc data_scope — khái niệm 'scope' duy nhất trong guard là scopeKeys của PAT. Luận điểm gốc của WO ĐÚNG, nay có bằng chứng file:dòng chứ không phải suy luận",
      "ĐÃ KIỂM CHỨNG LÀ KÍN (đừng đo lại): GET /attendance tự bound `userId: query.userId ?? actor.id` + assertCanManage (attendance.service.ts:457-463) · GET /leave/balances scope=all đòi manage:leave (leave.service.ts:122-128) · /auth/users nhận sẵn vị từ scope (auth-users.repository.ts:111) · /org/units+units/tree phơi headUserName CÓ CHỦ ĐÍCH, có chữ ký ở test/foundation/route-verdicts.ts",
      "⚠️ DỮ KIỆN THIẾT KẾ QUAN TRỌNG NHẤT cho cơ chế: hai module kín ở trên kín bằng **assert thứ hai trong service** (`assertCanManage`/`assertCan`) hoặc bằng **tự-bound theo actor.id**, KHÔNG bằng vị từ scope. Một cơ chế 'mọi điểm chiếu PHẢI nhận vị từ scope' sẽ bắt SAI hàng loạt call-site vốn đã an toàn ⇒ cơ chế phải nhận NHIỀU DẠNG CĂN CỨ (scoped-predicate · self-bound-by-actor · waiver-đã-ký kiểu route-verdicts · machine/job không có actor HTTP), không chỉ một",
      "ĐÃ TÁCH RA VÀ VÁ RIÊNG 2026-07-30 (S6-SEC-IDENTITYBOUND-1): KI-051 /recycle-bin/employees (S2, lỗ SỐNG 45 user) + KI-052 /org/teams leaderUserName (S3). Còn lại làm nợ CÓ SỐ HIỆU cho WO này: **KI-053** role-admin.repository.ts:158-159 · **KI-054** login-log.repository.ts:72-73 + security-event.repository.ts:84-88 (auth-logs-viewer ghi 'Company-scope' trong docstring nhưng KHÔNG resolve data_scope)",
    ],
    done_when: [
  ...
  {
    id: "S6-SEC-IDENTITYBOUND-1",
    module: "AUTH",
    layer: "SEC",
    title:
      "N-1d/N-1e (KI-051 · KI-052) — bound hai đường chiếu danh tính còn hở: /recycle-bin/employees (lỗ SỐNG, 45/46 user giữ cặp) + /org/teams leaderUserName",
    zone: "red",
    status: "done",
    paths: [
      "apps/api/src/recycle-bin/**",
      "apps/api/src/org/**",
      "apps/api/test/integration/identity-bound-scope.int-spec.ts",
      "apps/console/src/lib/recycle-bin-api.ts",
      "apps/console/src/routes/recycle-bin.spec.tsx",
      "docs/RELEASE/**",
      "docs/plans/S6-SEC-IDENTITYBOUND-1.md",
    ],
    skills: ["code-review"],
    depends_on: ["S6-SEC-ORGTEAMSCOPE-1"],
    plan: "docs/plans/S6-SEC-IDENTITYBOUND-1.md",
    src: [
      "TÁCH khỏi S6-SEC-IDENTITY-PROJ-1 (owner chốt 2026-07-30): pha ĐO của WO cơ chế tìm ra một lỗ SỐNG, mà lỗ sống thì không đi theo lịch của WO S3 'hoãn trong cửa sổ RC'",
      "KI-051: GET /recycle-bin/employees gate read:employee rồi chiếu userFullName+userEmail của MỌI hồ sơ xoá mềm, service không resolve scope nào. Role SEEDED `employee` giữ read:employee@Own với 45/46 user sống và KHÔNG có view:user ⇒ mỗi nhân viên đọc được danh bạ nhân sự đã nghỉ. Cùng lớp KI-049 nhưng 45 người giữ cặp thay vì 0. Phơi nhiễm lúc phát hiện = 0 hàng (0 hồ sơ xoá mềm) — chặn bởi THIẾU DỮ LIỆU, không phải bởi lớp kiểm soát",
      "KI-052: GET /org/teams chiếu leaderUserName không bound — hình dạng N-1c ở phương thức BÊN CẠNH trong chính org.repository.ts mà S6-SEC-ORGTEAMSCOPE-1 vừa vá ⇒ bằng chứng vá-theo-route không quét hết file",
      "0 migration · 0 grant mới · 0 đổi seed. Ai mất quyền xem: KHÔNG AI (SA+company-admin đều view:user@Company)",
      "⚠️ BẪY FE đã bắt được: apps/console deletedEmployeeSchema khai userFullName/userEmail BẮT BUỘC ⇒ server bỏ khoá là ZodError runtime dù HTTP 200 ⇒ vỡ TRẮNG trang thùng rác cho đúng role mà bản vá bảo vệ (memory apifetch-drops-pagination-bare-array). Đã đổi .optional() + khoá bằng ca console, và ca đó ĐÃ CHỨNG MINH ĐỎ khi gỡ .optional()",
      "⚠️ KHÁC N-1c: contract leaderUserName là .nullable() HỢP LỆ (team chưa có trưởng nhóm) ⇒ null KHÔNG phân biệt được 'chưa có' với 'ngoài scope' ⇒ cơ chế hỏng-ồn-ào của N-1c KHÔNG áp dụng được ở đây, bắt buộc bỏ khoá qua identityInScope",
    ],
    done_when: [
      "✅ RED chứng minh trước khi vá: identity-bound-scope.int-spec.ts 4 đỏ / 4 xanh — ca read:employee@Own không view:user trả 2/2 danh tính",
      "✅ GREEN 8/8; hồi quy org+recycle-bin+permission 468/468, foundation 162/162, console recycle-bin 10/10",
      "✅ Ca đối chứng CẢ HAI phía: Company thấy đủ danh tính + thiếu cặp gate vẫn 403 (vá không nới route)",
      "✅ RELEASE-02: KI-051/052 mở+đóng kèm số đo; KI-053/054 mở làm nợ có số hiệu",
      "FULL gate security-reviewer PASS trước merge (vùng đỏ — người chốt)",
    ],
  },
  {
    ...
    paths: [
      ...
      "docs/permission-matrix-spec.md",
      "docs/RELEASE/**",
      "docs/plans/S6-SEC-PERMVERB-1.md",
    ],
    skills: ["code-review"],
    depends_on: ["S6-SEC-ORG-1"],
    plan: "docs/plans/S6-SEC-PERMVERB-1.md",
    src: [
      "docs/plans/S6-SEC-ORG-1.md §7 N-2 + permission-matrix-spec.md CHỐT /org (bảng 3 role lệch)",
      "GỐC RỄ = tách từ vựng: 0444_s2_authseed1_canonical_roles_perms.sql:87-90 cố ý giới thiệu `view:user` ('KHÁC legacy read:user'), trong khi /org vẫn gate `read:user`",
      "ĐO PROD 2026-07-27: `hr-manager`(…009) có read:team + GHI team nhưng thiếu read:user · `hr`(…011) có view:user nhưng thiếu CẢ read:user lẫn read:team · `manager`(…010) thiếu cả ba. Cả ba đang 0 user ⇒ ảnh hưởng sống = 0, sửa được sạch TRƯỚC khi có người dùng",
      "✅ ĐÃ THI CÔNG 2026-07-29 — KẾT LUẬN: **KHÔNG CÓ MIGRATION**. ADR docs/DECISIONS/DECISIONS-06_Permission_Verb_Canonical.md (D-41 chốt `view:user`); nhật ký docs/plans/S6-SEC-PERMVERB-1.md. Đo PROD cho thấy pha EXPAND đã xảy ra từ 0444 ⇒ cả 7 user SỐNG giữ cặp danh bạ (SA 6 + company-admin 1) đều có CẢ HAI động từ @Company ⇒ đổi gate là no-op; 0 grant mới, 0 revoke",
      "⚠️ VẾ 'backfill 3 role' của WO seed BỊ SỐ ĐO BÁC BỎ: `manager` ĐÚNG thiết kế §13 (Manager `-` cho AUTH.USER.VIEW ⇒ backfill = mở rộng quyền ngoài đặc tả) · `hr-manager` media-era ngoài §13 · chỉ `hr` lệch thật VÀ đã có sẵn view:user. `project-manager` MẤT quyền (0 user, media-era, cố ý không backfill)",
      "⚠️ WO seed ĐẾM THIẾU PIN: ghi 'còn 2 pin literal' nhưng thực tế có BA — org-directory-permission.int-spec.ts cũng seed cặp bằng literal (~9 vị trí). Thêm: artifact docs/_review/S6-SEC-ROUTEMAP-1-route-census.json phải regen bằng ROUTE_CENSUS_WRITE=1 (memory route-census-runtime-gate)",
    ...
    done_when: [
      ...
      "Nếu giữ `read:user`: backfill PER-PAIR cho hr(…011)/manager(…010)/hr-manager(…009) đúng cặp cần, data_scope tường minh. Nếu chuyển sang `view:user`: đổi gate ở org.controller + expand-contract 2 release (thêm grant TRƯỚC, đổi enforce SAU), KHÔNG revoke cùng lúc",
      "Liệt kê TRƯỚC/SAU: mỗi role đọc được gì, ai được thêm quyền — nêu rõ ai được MỞ RỘNG so với hôm nay (đây là đường ngược với S6-SEC-ORG-1 nên phải soi kỹ hơn)",
      "Test canonical seed-pin không vỡ (memory canonical-seed-pin-regression) — nếu vỡ thì cập nhật pin kèm bằng chứng, không nới cho xanh",
      "permission-matrix-spec.md: bảng 3 role lệch chuyển thành ĐÃ ĐÓNG; docs/DB cập nhật nếu catalog đổi",
      "FULL gate security-reviewer + database-reviewer (thiếu agent thì GHI RÕ thay bằng gì); RED-first cho vế deny",
    ],
  },
  {
    id: "S6-SEC-NOTITX-1",
  ...
  },
  {
    ...
    plan: "docs/plans/S6-SEC-XTENANTFK-1.md",
    src: [
      "PHÁT HIỆN + ĐO trong S6-QA-TENANTWRITE-1 (2026-07-29). Gốc: kiểm tra FK của Postgres chạy với quyền hệ thống và KHÔNG áp RLS — đó là hành vi thiết kế của PG, không phải bug của ta",
    ...
    done_when: [
      "ĐO LẠI trên PROD (không chỉ lane): số FK một-cột giữa 2 bảng tenant + số hàng ĐANG lệch tenant cho mỗi cặp — hàng lệch có thật thì phải quyết xoá hay sửa, có chữ ký",
      "Ưu tiên theo BỀ MẶT THẬT: cặp nào có endpoint nhận id từ client (task_assignees, project_members, team_members, task_watchers…) làm TRƯỚC; cặp chỉ ghi từ server làm sau hoặc ký waiver",
      "Chốt chống MỌC THÊM: một kiểm tra chạy được theo lệnh đếm FK một-cột giữa 2 bảng tenant, tăng so với baseline đã ký ⇒ ĐỎ. Không có chốt này thì vá xong lại mọc (đúng lớp lỗi KI-036/KI-028)",
      "Mở rộng lưới `tenant-isolation.int-spec.ts`: ca (b) hiện chỉ phủ team_members — data-drive nó theo danh sách FK thay vì viết tay từng cặp",
      "FULL gate security-reviewer + rls-tenant-isolation-tester; RELEASE-02 KI-046 đóng kèm số đo trước/sau",
    ],
  },
  {
    id: "S6-SEC-MV-1",
  ...
  },
  {
    ...
    paths: [
      ...
      "docs/RELEASE/**",
      "docs/DEVOPS/**",
      "docs/plans/S6-SEC-DBFENCE-1.md",
    ],
    skills: ["code-review"],
    depends_on: [],
    plan: "docs/plans/S6-SEC-DBFENCE-1.md",
    src: [
      "RELEASE-02 KI-028 §đo lại 2026-07-28 — containment 27/7 chỉ phủ 16/74 tenant; 58 company + 201 user của 26/7 chưa từng vào phép đo",
      'apps/api/vitest.config.ts:11 — `const db = process.env.LANE_DB ?? "mediaos"`: thiếu LANE_DB thì test trỏ THẲNG DB PROD. Đây là nguồn rò, không phải triệu chứng',
      "apps/api/test/integration/tenant-isolation.int-spec.ts:12 — `describe.skipIf(!hasDb)` (KHÔNG đòi LANE_DB) ⇒ vẫn chạy + seedCompany vào PROD; cleanupTenants nằm ở afterAll nên run crash (KI-014) là mất dọn",
      "Đo 2026-07-28 trên PROD: 74/75 company khớp mẫu test · 226 user active · 55 hash argon2/bcrypt thật · 33 giữ role toàn cục · GIAO = 13 company-admin + 5 platform-admin",
      "scripts/s6sec1-contain-test-tenants.sql — script containment đã có, chạy đúng nhưng trên tập 16 company; tái dùng được sau khi mở rộng phạm vi",
      "funtime nguyên vẹn 46 user (35 active + 11 locked) — mọi thao tác purge PHẢI chứng minh 0 dòng funtime bị chạm",
    ],
    done_when: [
      "BỊT NGUỒN RÒ TRƯỚC (không có bước này thì purge vô nghĩa): thiếu LANE_DB ⇒ int-spec phải SKIP hoặc ĐỎ, tuyệt đối KHÔNG rơi về DB 'mediaos'. Cân nhắc bỏ hẳn default 'mediaos' trong vitest.config và bắt buộc LANE_DB, HOẶC chặn cứng khi tên DB đích trùng DB của .env.prod",
      "RED-proof nguồn rò: chạy một int-spec có seed mà KHÔNG set LANE_DB ⇒ phải skip/đỏ và ĐO ĐƯỢC là 0 company mới sinh trong 'mediaos'. Trước khi vá, chính ca này sinh company thật — chứng minh cả hai chiều",
      "Quét lại TOÀN BỘ spec gate bằng `hasDb` mà THIẾU `LANE_DB` (không chỉ tenant-isolation) — liệt kê số, sửa hết; đây là lớp lỗi, không phải một file",
      "PURGE SAU KHI ĐÃ BỊT: xoá 74 tenant test + 226 user; ưu tiên thu hồi 18 tài khoản vừa-đăng-nhập-được-vừa-role-toàn-cục (5 platform-admin) TRƯỚC, phần rác còn lại sau",
      "Bằng chứng trước/sau chạy trên PROD: company khớp mẫu test = 0 · user test active = 0 · user test giữ role toàn cục = 0 · funtime VẪN 46 user (35 active + 11 locked) và 0 dòng bị script chạm",
      "Chốt hồi quy để KHÔNG tái diễn lần thứ ba: một kiểm tra chạy được theo lệnh (script/CI) đếm company khớp mẫu tenant test trong DB PROD, khác 0 ⇒ ĐỎ. Lần đóng 27/7 thiếu đúng chốt này nên rác mọc lại trong 2 ngày",
      "FULL gate security-reviewer + rls-tenant-isolation-tester PASS; RELEASE-02 KI-028 đóng KÈM số đo, RELEASE-01 §CRITICAL/HIGH chấm lại",
    ],
  },
  {
    id: "S6-SEC-ROTATE-1",
    ...
    paths: [
      ...
      "apps/api/demo-seed-base.mjs",
      "apps/api/demo-seed-full.mjs",
      "apps/api/demo-seed-dashboard.mjs",
      "apps/api/seed-operator.mjs",
      "scripts/**",
      "harness/**",
      ".github/workflows/**",
      "docs/RELEASE/**",
      "docs/plans/S6-SEC-ROTATE-1.md",
    ],
    skills: ["code-review"],
    depends_on: [],
    plan: "docs/plans/S6-SEC-ROTATE-1.md",
    src: [
      "FULL gate của S6-SEC-DBFENCE-1 (2026-07-28) — chứng minh THỰC NGHIỆM: nối superuser vào PROD `mediaos` bằng đúng literal trong repo → THÀNH CÔNG",
      "gh repo view → nguyencanhqk/mediaos visibility=PUBLIC",
      "Đếm file tracked: 17 file chứa literal của `mediaos` (SUPERUSER) · 7 của `mediaos_app` · 6 của `mediaos_worker`",
      "mediaos.ps1:242-245 `Invoke-Roles` CHỦ ĐỘNG ALTER ROLE về literal dev ⇒ nguồn TÁI NHIỄM: rotate xong mà không sửa hàm này thì lần chạy sau khôi phục lỗ hổng (đúng lớp lỗi KI-036)",
      'docker-compose.yml:29 `"${POSTGRES_PORT:-5432}:5432"` ⇒ bind 0.0.0.0',
      "apps/api/demo-seed-{base,full,dashboard}.mjs + seed-operator.mjs mặc định ghi thẳng `mediaos` (PROD) — NẰM NGOÀI hàng rào vitest của KI-028",
      "Đây là lý do bán kính KI-028 lớn: mọi default trong repo đều là chìa khoá thật",
    ],
    done_when: [
  ...
  },
  {
    ...
    done_when: [
      "SHIPPED (mig 0495, idx 175 when 1717500870000, nối tiếp head 0494): bảng user_preferences theo SPEC-09 §15.2 / DB-08 §8.16 (id UUID gen_random_uuid() · company_id NOT NULL DEFAULT literal-GUC · user_id NOT NULL FK users · locale · timezone · theme CHECK IN (system/light/dark) · date_format · time_format · default_landing · density CHECK IN (comfortable/compact) · favorite_modules JSONB · me_layout_config JSONB · created/updated_at + created_by/updated_by) + UNIQUE(company_id, user_id); KHÔNG deleted_at (upsert-config)",
      "BẤT BIẾN #1: ENABLE + FORCE RLS + policy tenant_isolation literal-GUC (mẫu 0479) TRƯỚC mọi INSERT; app GRANT SELECT,INSERT,UPDATE (KHÔNG DELETE) + worker SELECT; đăng ký rls-registry case user_preferences; drizzle schema user-preferences.ts additive (KHÔNG db:generate drop); schema/index.ts append khối WO",
      "Seed module ME vào modules (mirror 0435, module_group='Experience', sort_order=80, is_active/is_mvp=true, is_core=false, ON CONFLICT (module_code) WHERE deleted_at IS NULL DO NOTHING); OWNER CHỐT 5 pair is_sensitive=false: ('access','me')[ME.ACCESS cổng nav] + ('view','user-preference') + ('update','user-preference') + ('update','avatar') + ('update','notification-preference'), grant per-pair data_scope Own cho CẢ 4 role canonical (§13 per-(permission,role), resolve theo thuộc tính, per-pair DELETE-wrong-scope + INSERT ON CONFLICT, verify fail-LOUD mirror 0466/0476) = 20 role_permissions; KHÔNG wrap nghiệp-vụ-nguồn (ME-DEC-002/§11.2); mapping pair→code ghi docs/permission-matrix-spec.md §9",
      "Int-spec RED-trước lane DB cô lập (me-user-preferences-seed.int-spec + rls-registry): cross-tenant deny user_preferences + cross-user CONTRACT trong-tenant (RLS KHÔNG cô lập user ⇒ IDOR deny ép ở ME-BE, it.todo) + upsert idempotent theo UNIQUE + seed-assert (module ME + 5 pair + 20 grant Own); migration-smoke clean 0000→0495 xanh; gate hasDb && LANE_DB; FULL gate database-reviewer + security-reviewer + rls-tenant-isolation-tester + silent-failure-hunter PASS",
    ],
  },
  {
    id: "S5-ME-BE-1",
    ...
    done_when: [
      ...
      "AUDIT ANOMALY PERSIST (§12.4 + BẤT BIẾN #2 append-only): >1 employee active → MeCurrentPersonResolver ghi audit object_type='user' ở withTenant RIÊNG đã COMMIT TRƯỚC khi throw ConflictException 409 — audit KHÔNG cùng tx với nhánh throw (throw = rollback ⇒ mất audit); int-spec j2 assert audit_logs GIỮ dòng anomaly sau 409 (it thường, KHÔNG it.fails); ME_ERROR_CODES.DATA_INCONSISTENT import từ @mediaos/contracts (không hard-code); int-spec canonical DUY NHẤT test/integration/me-personal-hub.int-spec.ts (xoá bản trùng src/me/me.int.spec.ts)",
    ],
  },
  {
    id: "S5-ME-BE-2",
  ...
  // ═══ S5-TASK-UX2 2026-07-18 — owner benchmark MISA AMIS: board dự án + sidebar cây phòng ban ═══
  // Owner gỡ hoãn post-MVP của wave S5-TASK-UX (2026-07-13): chốt cột pipeline tuỳ biến + full tab
  // parity + quyền per-project. Thứ tự đợt: A(pipeline) → B(sidebar) → D1(tab shell) → 🔴C(quyền)
  // → D2-D5(Lịch/Tài liệu/Gantt/Biểu mẫu). Mockup duyệt hướng: artifact 7a4b7be2.
  // Memory: task-ux-reference-benchmark · task-redesign-2026-07-reuse-map.
  {
    id: "S5-TASK-PIPELINE-1",
    module: "TASK",
    layer: "FULL",
    title:
    ...
    paths: [
      ...
      "docs/API Design/API-06_TASK_API_Design.md",
      "docs/DB/DB-06 TASK Database Design.md",
      "apps/api/src/tasks/**",
      "apps/api/src/foundation/seed/**",
      "apps/api/migrations/**",
      ...
      "packages/web-core/src/lib/**",
      "apps/app/src/i18n/**",
      "docs/plans/S5-TASK-PIPELINE-1.md",
    ],
    skills: ["code-review"],
    depends_on: ["S5-TASK-HRCODE-1"],
    src: [
      "Owner benchmark MISA AMIS 2026-07-18: board 7 cột pipeline sản xuất (Ý Tưởng→Thiết Kế→Quay→Hậu Kỳ→Thumbnail→Duyệt Video→SEO) — KHÔNG phải trạng thái task",
      "HẠ TẦNG ĐÃ CÓ: tasks.state_id + FK + index (mig 0420:153,164-166); project_states RLS+FORCE (0420:52-58); CRUD state 4 route đầy đủ; state_group backlog/unstarted/started/completed/cancelled. FE chưa nối 1 dòng nào ⇒ phần lớn là NỐI DÂY",
      "BẪY CHẶN (plan-reviewer BLOCK bản 1): mig 0420:230-241 ĐÃ backfill 5 state cho MỌI project (CROSS JOIN) ⇒ nhánh fallback KHÔNG áp cho dự án cũ. Và 0420:244-260 backfill state_id từ cột `status` LEGACY (chạy TRƯỚC 0478 tạo task_status) trong khi Kanban đọc task_status (task-core.repository.ts:123) ⇒ state_id LỆCH PHA; task tạo sau 0420 có state_id=NULL (insertTaskCoreTx không ghi) ⇒ đổi board mà không backfill sẽ DỒN HẾT TASK VỀ MỘT CỘT",
      "Quyết định owner: nới FSM (Done→{In Progress,In Review,Cancelled}) + auto-map qua TaskActionsService.changeStatus. Lý do: board 7 cột ⇒ user không bao giờ bấm đổi trạng thái riêng ⇒ task_status đóng băng 'Todo' ⇒ countsByStatus/mv_dashboard_task_status sai, isOverdue LUÔN true (task-core.repository.ts:136-137), completed_at luôn NULL",
      "Trái SPEC-06 hiện hành (§366 'Kanban hiển thị theo cột TRẠNG THÁI', nghiệm thu 15-16, TASK-TC-025, §21) ⇒ PHẢI có ADR + sửa spec TRƯỚC khi code (CLAUDE.md §1 spec thắng code)",
      "Bẫy pin: TASK_PERMISSION_COUNT=23 (task-permissions.const.ts:51) + int-spec assert 23 — thêm pair update-state:task phải sửa CÙNG COMMIT (memory canonical-seed-pin-regression); grant per-(role,pair) theo khuôn 0485:114-235, KHÔNG INSERT...SELECT blanket (memory permissions-0005-bulk-grant-trap)",
      "Đường ghi state_id thứ ba: TasksService.updateTaskFields (tasks.service.ts:308-359) đã guard stateInProjectTx + có test NHƯNG không route nào tới (PATCH /tasks/:id đi taskCore.updateTask) = code chết → để WO dọn riêng, KHÔNG guard 2 nơi",
    ],
    done_when: [
      "ADR + SPEC-06/API-06/DB-06 cập nhật XONG TRƯỚC khi merge code",
      "Migration 0500 backfill: 0 task có project_id còn state_id NULL; task task_status='Done' trỏ state nhóm 'completed' — đếm trước/sau trên lane DB",
      "Dự án hiện hữu: board columnMode:'state', thẻ nằm ĐÚNG cột theo task_status thật (KHÔNG dồn 1 cột) — regression quan trọng nhất, fixture CẤM tự set state_id",
      "Kéo sang cột nhóm completed ⇒ task_status='Done' + completed_at + event + audit; kéo cùng nhóm ⇒ no-op không phát event rác; kéo NGƯỢC completed→started THÀNH CÔNG (FSM đã nới)",
      "move-state double-gate: @Own kéo thẻ người khác ⇒ 404; task workflow-driven ⇒ 400; state khác project/tenant ⇒ 4xx. ATOMIC: FSM từ chối ⇒ state_id KHÔNG đổi",
      "Task tạo mới trong dự án có pipeline ⇒ state_id = is_default",
      "Pair update-state:task seed per-(role,pair); TASK_PERMISSION_COUNT=24; verify grant bằng query DB THẬT (không grep)",
      "FULL gate (crown: đổi luật FSM + đổi dữ liệu) + check.sh --lane-db xanh + NGƯỜI CHỐT",
    ],
  },
  {
    id: "S5-TASK-NAV-TREE-1",
    module: "TASK",
    layer: "FE",
    title:
    ...
    paths: [
      ...
      "apps/app/src/routes/tasks/**",
      "packages/web-core/src/lib/**",
      "apps/api/src/tasks/projects.controller.ts",
      "packages/contracts/src/task.ts",
      "apps/app/src/i18n/**",
    ],
    skills: ["code-review"],
    ...
    src: [
      "Owner yêu cầu 2026-07-18: sidebar tổ chức theo sơ đồ cây phòng ban, mỗi phòng ban có nút chức năng bên cạnh mở menu",
      "ĐÃ CÓ: SidebarItemMeta.children (web-core registry.ts:684) + filterSidebarItems ĐÃ đệ quy (:689-695) — chỉ ModuleSidebar.tsx không đọc children ⇒ sửa 1 bộ dựng. project.departmentId + departmentName đã có trong DTO; GET /org/units/tree đã sống (read-open)",
      "THIẾU: listTaskProjectsQuerySchema (task.ts:484) không có filter departmentId ⇒ cần thêm query param BE",
      "BẪY: ROUTE_REGISTRY.showInSidebar là metadata CHẾT — ModuleSidebar chỉ đọc sidebar-registry.ts; thêm mục nav phải sửa CẢ HAI chỗ (đã từng làm mồ côi SCREEN-001)",
      "BẪY: 3 đề xuất sidebar TASK xung đột trên giấy (UI-09 §11.2 · FRONTEND-11 §8.1 · UI-02 §9) và KHÔNG cái nào khớp TASK_SIDEBAR 4 mục đang chạy ⇒ WO này phải RECONCILE thành 1 doc chuẩn, không thêm đề xuất thứ tư",
      "Mục 'Cài đặt quyền cho dự án' trong menu ⋯ KHÔNG thuộc đợt này — phụ thuộc đợt C (data-scope Project chưa có trong engine). Hiển thị mục này khi chưa có engine = hứa suông",
    ],
    done_when: [
      "ModuleSidebar dựng cây đệ quy từ children (gập/mở, giữ trạng thái); dự án lồng dưới đúng phòng ban theo departmentId",
      "Menu ⋯ mỗi phòng ban: xem báo cáo · thêm dự án (prefill departmentId) · sắp xếp — mỗi mục gate quyền tương ứng, thiếu quyền thì ẨN (UI-02 §5.3 không lộ menu không có quyền)",
      "Filter departmentId trên GET /projects (+ contracts) có int-spec cross-tenant",
      "Doc sidebar TASK hợp nhất 3 đề xuất xung đột thành 1 bản chuẩn; sidebar-registry + ROUTE_REGISTRY đồng bộ (không mục mồ côi)",
      "FE test: loading/error/empty; phòng ban 0 dự án vẫn hiện; cây sâu ≥3 cấp không vỡ layout",
      "LIGHT gate + typecheck/lint xanh",
    ],
  },
  {
    ...
    depends_on: ["S5-TASK-PIPELINE-1"],
    src: [
      "Owner benchmark MISA AMIS: tab bar trong dự án + toolbar + rail avatar bên phải lọc theo người thực hiện (gồm 'Chưa giao')",
    ...
    done_when: [
      ...
      "Toolbar dùng CHUNG bộ lọc giữa tab Bảng và Danh sách (đổi filter ở tab này giữ khi sang tab kia)",
      "Rail avatar: lọc theo người thực hiện + 'Chưa giao', đếm đúng, bật/tắt nhiều người",
      "Quyền: tab Báo cáo ẩn khi thiếu view-report:project (useCanExact — pair sensitive)",
      "FE test loading/error/empty cho từng tab; LIGHT gate",
    ],
  },
  {
    id: "S5-TASK-DETAIL-1",
    ...
    depends_on: ["S5-TASK-PIPELINE-1"],
    src: [
      "Owner gửi ảnh màn chi tiết task MISA AMIS 2026-07-18; khảo sát đối chiếu cho thấy 4 mục nằm TRONG SPEC-06 nhưng chưa implement (khác với nhóm ngoài-spec cần owner duyệt mở rộng)",
      "GAP 1 (vi phạm SPEC §13.12:1150-1151 yêu cầu cột Dữ liệu cũ/mới): TaskActivityTimeline.tsx:91 chỉ in `log.message ?? label`, BỎ HOÀN TOÀN old_values/new_values ⇒ không dựng được dòng 'Duyệt Video ← Kịch bản'. Dữ liệu ĐÃ CÓ trong task_activity_logs (task-activity.ts:32-46), chỉ thiếu render",
    ...
    done_when: [
      "Timeline render old→new cho mọi loại thay đổi (đổi cột, trạng thái, assignee, hạn, ưu tiên) — đúng SPEC §13.12; tên cột trong lịch sử là tên TẠI THỜI ĐIỂM đó, không đổi theo khi cột bị rename",
  ...
  },
  {
    ...
    paths: [
      ...
      "packages/web-core/src/lib/**",
      "apps/app/src/i18n/**",
      "docs/plans/S5-TASK-PROJROLE-1.md",
    ],
    skills: ["code-review"],
    depends_on: ["S5-TASK-PIPELINE-1"],
    src: [
      "Owner chốt 2026-07-18 (task-ux-reference-benchmark): 'Quyền per-project THẬT SỰ: projectRole trở thành nguồn quyền, không chỉ nới read-scope như hiện nay' — đợt 🔴C, sau A/B/D1 (đã ship #234-#243)",
      "RÀO D-22 (DECISIONS-01:346, chốt 02/07): GIỮ 5 bậc data_scope — quyền per-project qua project_members ở SERVICE LAYER TASK, KHÔNG thêm bậc 'Project' vào engine. API-06 §6.3 đã đặc tả sẵn mô hình 2 lớp: RBAC trước, project role sau",
      "HIỆN TRẠNG (khảo sát 2026-07-19): project_role CHỈ là dữ liệu + rule last-Owner (projects.service.ts:518-521,565-568). Quyền per-project THẬT đang neo owner_employee_id 1-người qua assertGovern (projects.service.ts:628-651). Membership = nhánh OR trong buildReadScopeExists (task-core.repository.ts:198-224) KHÔNG đọc role — áp cho CẢ write qua assertInScopeForWrite (task-core.service.ts:561 + task-actions.service.ts:544) ⇒ member Viewer về lý thuyết lọt nhánh write (chưa nổ vì grant write emp/mgr đang hoãn)",
      "TASK_DEFERRED_GRANTS (task-permissions.const.ts:153-157) hoãn create/update:task emp@Own+mgr@Team + delete:task mgr@Team VÌ route sống pair-only — điều kiện un-defer ghi rõ: 'grant trong migration CÙNG release với enforcement scope+membership'. Đợt C CHÍNH LÀ release đó (update/delete đã có assertInScopeForWrite; create chưa có enforcement — phải thiết kế create-scope trước khi flip; delete:task mgr GIỮ HOÃN vì §9 đòi 'nếu là creator/owner' = relation-check chưa thiết kế)",
      "XUNG ĐỘT SPEC phải reconcile trong ADR: SPEC-06 §14.6 nói Manager 'Quản lý task VÀ THÀNH VIÊN' nhưng §9 ma trận + API-06 §6.3 nói quản lý thành viên = Owner-only ⇒ ADR chốt 1 đường (khuyến nghị Owner-only, khớp hành vi assertGovern hiện tại + least-privilege)",
      "SPEC-06 §9:599 note: route quản trị cột pipeline (project_state) 'không tồn tại kiểm tra owner dự án' — thêm tầng role per-project cho states CRUD là QUYẾT ĐỊNH MỚI phải vào sổ DECISIONS trước khi code (chính note đó yêu cầu)",
      "updateProject đổi ownerEmployeeId KHÔNG sync project_members (projects.service.ts:301-310 chỉ validate active) ⇒ D-25 phải chốt quan hệ owner_employee_id ↔ role Owner + sync khi reassign",
      "FE: ProjectMemberTable ĐÃ đầy đủ thêm/đổi role/xóa (apps/app/src/routes/tasks/ProjectMemberTable.tsx, gate manage-member pair) nhưng nằm trong ProjectDetailPage:514, KHÔNG có mục 'Cài đặt quyền cho dự án' ở menu ⋯ (TaskSidebarTree.tsx:130-185 chỉ 3 mục) — đợt B cố ý hoãn chờ đợt C. FE cần myProjectRole từ server để gate per-project (KHÔNG hard-code so sánh role phía client ngoài giá trị server trả)",
      "Bẫy pin: đổi grant = migration 0501 DO-block per-(role,pair) khuôn 0499/0485 (KHÔNG INSERT...SELECT blanket) + sửa CÙNG COMMIT task-permissions.const.ts (TASK_GRANT_MATRIX/TASK_DEFERRED_GRANTS/TASK_EXPECTED_GRANT_COUNTS) + lật assert DENY trong task-permissions-seed.int.spec (bẫy canonical-seed-pin-regression)",
    ],
    done_when: [
      "ADR DECISIONS-04 (D-23..D-28) + SPEC-06 (§8.3/§9/§14.6/§18.6)/API-06 (§6.3)/DB-06 cập nhật XONG TRƯỚC khi merge code — reconcile xung đột Manager-quản-thành-viên",
      "Ma trận role×action per-project ghim trong ADR và ép ở service layer: membership-branch WRITE đòi role Owner/Manager; collab (comment/checklist/file-upload) đòi ≥Member (Viewer read-only); READ giữ mọi Active member — deny-path test đủ 4 role",
      "Governance (close/delete/manage-member/update-đổi-chủ) chuyển neo: scope<Company ⇒ Active member role Owner (KHÔNG còn so owner_employee_id 1-người); fail-closed khi dự án 0 Owner; last-Owner rules giữ nguyên; reassign owner sync project_members trong cùng tx",
      "project-states CRUD thêm tầng role per-project (scope<Company ⇒ Owner/Manager của ĐÚNG project) — theo quyết định mới trong ADR",
      "Migration 0501 un-defer create/update:task (emp@Own + mgr@Team, 4 hàng) khuôn DO-block 0499; delete:task mgr GIỮ HOÃN; const pins + seed int-spec lật DENY→ALLOW cùng commit; POST /tasks enforce create-scope (Own⇒assignee=self; Team⇒assignee trong team; projectId + scope<Company ⇒ PHẢI là member Owner/Manager) TRƯỚC khi grant có hiệu lực",
      "Contracts + DTO: myProjectRole (nullable) trên project detail + list; FE gate mục menu/nút theo useCan + myProjectRole server trả",
      "FE: tab/màn 'Thành viên & quyền' trong vỏ workspace (?tab=members — reuse ProjectMemberTable + bảng chú giải quyền theo role) + mục 'Cài đặt quyền' ở menu ⋯ node dự án sidebar (ẨN khi thiếu quyền theo UI-02 §5.3); loading/error/empty; i18n vi",
      "Deny-path RED trước: Viewer/Member update task người khác trong project ⇒ 404 + Manager/Owner ⇒ OK; Viewer comment ⇒ 403/404; member thường gọi manage-member ⇒ 403; Manager gọi close/delete ⇒ 403; Owner-role (khác owner_employee_id) close ⇒ OK (đổi hành vi có chủ đích); cross-tenant mọi đường mới ⇒ 404; RLS không rò",
      "FULL gate (crown: permission+migration) + bash harness/check.sh --lane-db xanh + NGƯỜI CHỐT trước merge",
    ],
  },
  {
    id: "S5-TASK-BOARD-UX-1",
    module: "TASK",
    layer: "FE",
    title:
      "Board: bấm thẻ mở chi tiết trong panel TRƯỢT PHẢI (?task=, giữ ngữ cảnh board) + tạo nhanh đáy mỗi cột pipeline (gõ tiêu đề, Enter là tạo thẳng vào cột)",
    zone: "yellow",
    status: "done",
    paths: [
    ...
    skills: ["code-review"],
    depends_on: ["S5-TASK-PIPELINE-1", "S5-TASK-DETAIL-1"],
    src: [
      "Owner 2026-07-20 gửi ảnh board MISA AMIS + ảnh board MediaOS hiện tại: thiếu ĐÚNG hai thao tác hằng ngày — bấm thẻ xem chi tiết, và thêm việc ngay trên cột",
      "KanbanCard (TaskKanbanPage.tsx) render div CHỈ có draggable+onDragStart — KHÔNG onClick/navigate ⇒ thẻ board là ngõ cụt; điều hướng chi tiết chỉ tồn tại ở các view BẢNG (task-columns.tsx, MyTasksPage, TaskSubtaskPanel navigate /tasks/$taskId)",
      "BE ĐÃ SẴN cho tạo-nhanh, không phải viết mới: createTaskCoreSchema nhận `stateId` (contracts task.ts) và task-core.service.ts §3c ghi thẳng comment 'nút + Thêm công việc đáy cột board' — server tự suy task_status từ state_group nên FE KHÔNG gửi status",
      "GATE ĐÔI dễ bỏ sót: §3c đòi THÊM update-state:task khi có stateId tường minh (403 TRƯỚC khi tạo). Gate FE chỉ create:task ⇒ nút hiện rồi bấm là 403",
      "CHỈ cột columnMode:'state' có tạo nhanh. Cột status-mode cũ: server từ chối stateId ⇒ task rơi vào cột is_default = SAI cột người dùng vừa bấm, tệ hơn là không có nút",
      "Bẫy cache đã vá kèm: taskKeys.kanban KHÔNG nằm dưới prefix tasks/list ⇒ TaskFormDrawer.onSuccess (chỉ invalidate list/detail) làm task tạo từ tab Danh sách không lên board tới 15s staleTime (refetchOnWindowFocus tắt) — người dùng đọc là 'tạo xong mà board trống'",
      "packages/ui chưa có primitive side-panel (chỉ Dialog modal giữa màn) ⇒ thêm Sheet. Bẫy MODAL LỒNG NHAU: Dialog con (Sửa/Xoá task) render như descendant trong cây sheet, cả hai cùng nghe Esc trên document ⇒ 1 lần Esc đóng CẢ HAI nếu không nhường",
    ],
    done_when: [
      "Bấm thẻ board mở panel chi tiết trượt phải; board vẫn thấy phía sau; Esc/nút X/bấm nền đóng panel",
      "Nội dung panel = CHUNG một TaskDetailContent với trang /tasks/:taskId (không nhân bản khối nội dung, không lệch tính năng giữa 2 lối vào)",
      "Trạng thái panel nằm trên URL (?task=<id>): copy link ra đúng board + đúng task; mở=push/đóng=replace nên Back không mở lại panel; đổi tab bỏ ?task=",
      "Mở được bằng bàn phím (thẻ role=button + tabIndex, Enter/Space) — thẻ là div nên phải tự nối phím",
      "Mỗi cột pipeline có ô tạo nhanh: Enter tạo task với ĐÚNG projectId+stateId của cột, ô tự xoá và giữ mở để gõ tiếp, Esc đóng, ô trống không gọi API",
      "Gate tạo nhanh = create:task VÀ update-state:task (mirror server §3c); thiếu một trong hai ⇒ ẩn nút. Cột status-mode KHÔNG có nút",
      "Tạo xong board tự làm mới (invalidate taskKeys.kanban tường minh) — kể cả khi tạo từ TaskFormDrawer ở tab Danh sách",
      "LIGHT gate (FE thuần, không chạm migration/permission seed/RLS) + typecheck/lint/build xanh; test deny-path gate đôi + payload stateId + invalidation",
    ],
  },
  {
    id: "S5-TASK-INLINE-1",
    ...
    layer: "FE",
    title:
      "Màn chi tiết task: bố cục lại (dự án + trạng thái + ưu tiên lên dải đầu, hết trùng lặp) + sửa-tại-chỗ người phụ trách, và việc con sửa người/hạn ngay trên dòng",
    zone: "yellow",
    status: "done",
    paths: ["apps/app/src/routes/tasks/**", "apps/app/src/i18n/**"],
    skills: ["code-review"],
    depends_on: ["S5-TASK-BOARD-UX-1"],
    src: [
      "Owner 2026-07-20 (ảnh drawer MediaOS + ảnh chi tiết MISA AMIS): muốn sửa trực tiếp trong màn chi tiết nếu có quyền, dự án+trạng thái đẩy lên đầu, việc con chọn người/hạn ngay khi bấm avatar",
      "PHÁT HIỆN: status/priority/deadline ĐÃ inline sẵn từ S4-FE-TASK-2 (TaskStatusSelect lưu ngay khi đổi) — vấn đề thật là BỐ CỤC: chúng bị chôn trong thẻ 'Thao tác nhanh' rồi lưới OverviewCard ngay dưới LẶP LẠI cùng thông tin ở dạng chỉ-đọc ⇒ người dùng thấy 'Người phụ trách'/'Trạng thái' hai lần, không rõ chỗ nào sửa được",
      "Người phụ trách là thứ DUY NHẤT chưa inline: TaskAssignControl cũ dùng Select + nút 'Đổi' (2 bước cho 1 thao tác)",
      "Việc con: assignee/dueAt là chữ chết trên dòng (TaskSubtaskPanel), sửa phải bấm bút chì mở dialog = 3 cú bấm. SubtaskListItemDto ĐÃ mang đủ mainAssigneeEmployeeId/assigneeName/dueAt ⇒ render control inline được ngay, KHÔNG cần API mới",
      "QUYẾT ĐỊNH QUYỀN (không phải chuyện style): task cha đi 4 route action riêng (assign/change-status/change-priority/change-deadline) giữ đúng cửa cũ — dùng PATCH sẽ lặng lẽ NỚI vì employee có update:task@Own nhưng KHÔNG có assign/update-priority/update-deadline. NGƯỢC LẠI việc con đi PATCH (đúng cửa EditSubtaskDialog đang dùng) — đi route assign sẽ LÀM MẤT khả năng nhân viên tự sửa việc con của mình",
      "Việc con: canEdit PHẢI gồm item.canOpen (D-39 đọc thừa hưởng, GHI không thừa hưởng) — thiếu vế này thì con ngoài phạm vi đọc riêng vẫn hiện nút sửa và mỗi lần bấm là 403",
      "HOÃN có chủ đích — đổi DỰ ÁN: giữ chỉ-đọc ở dải đầu. PATCH mỗi {projectId} đổi project_id nhưng GIỮ state_id trỏ cột dự án CŨ (task-core.service.ts: nhánh ghi projectId không đụng state; applyStateChangeTx chỉ chạy khi có stateId) ⇒ thẻ rơi vào cột mặc định board mới + DB mang tham chiếu chéo dự án. Lỗ này ĐANG tồn tại qua dialog Sửa (task-form-schema.ts luôn gửi projectId, không bao giờ gửi stateId) — WO riêng vá cho đúng, KHÔNG đẩy nó vào chỗ dễ bấm hơn",
    ],
    done_when: [
      "Dải đầu: tên dự án + trạng thái + ưu tiên đứng trên cùng; trạng thái/ưu tiên đổi được tại chỗ",
      "Mỗi thông tin xuất hiện ĐÚNG MỘT LẦN — hết cặp 'Thao tác nhanh' + lưới chỉ-đọc lặp lại",
      "Người phụ trách: chọn là gán ngay, không còn nút 'Đổi'; vẫn đi route assign (assign:task)",
      "Mỗi ô thiếu quyền ⇒ render chữ CHỈ-ĐỌC, không phải control disabled (UI-02 §5.3)",
      "Việc con: bấm avatar mở picker có tìm kiếm → chọn là lưu; bấm ngày → sửa tại chỗ, Enter/blur lưu, Esc huỷ; cả hai đi PATCH và invalidate đủ subtasks·detail cha·board·report",
      "Việc con canOpen=false ⇒ chỉ-đọc (không avatar bấm được, không ô ngày bấm được)",
      "LIGHT gate + typecheck/lint/build xanh; test khoá 2 ranh giới quyền (cha=route action · con=PATCH) + deny-path từng ô",
    ],
  },
  {
    id: "S5-TASK-AVATAR-1",
    ...
    layer: "FULL",
    title:
      "Avatar người phụ trách trong TASK (Nhóm C của hệ avatar): nối 3 mắt xích đứt (repo SELECT · service ký URL · contract) cho board/chi tiết/việc con/my-tasks/rail",
    zone: "yellow",
    status: "done",
    paths: [
      "apps/api/src/tasks/**",
      "apps/api/src/foundation/files/avatar-presign.service.ts",
      "packages/contracts/src/task.ts",
      "apps/app/src/routes/tasks/**",
    ],
    skills: ["code-review"],
    ...
    src: [
      ...
      "Chuỗi đứt 3 mắt xích: (1) task-core.repository JOIN employee_profiles ae ĐÃ CÓ SẴN nhưng không SELECT ae.avatar_url; (2) không đường TASK nào gọi AvatarPresignService (chỉ hr-read.service gọi); (3) taskCoreResponseSchema không khai field",
      "BẪY BẢO MẬT: `employee_profiles.avatar_url` là cột ĐA-NGƯỜI-GHI (profile-change-request ghi verbatim) ⇒ ký mù = IDOR đọc file nội-tenant. Bắt buộc đi qua AvatarPresignService (findVerifiedAvatarsTx khớp ĐÚNG cặp employeeId↔fileId + owner_user_id=created_by). Cột thô đặt tên `assigneeAvatarRaw` để không ai nhầm là URL hiển thị; mapper MẶC ĐỊNH null nên quên nối resolver là mất ảnh, KHÔNG BAO GIỜ lộ fileId",
      "BẪY NESTED-TX: resolveEmployeeAvatars tự mở withTenant ⇒ gọi trong tx caller sẽ treo trên PgBouncer transaction-mode. Vá bằng tham số `callerTx?` (tái dùng kết nối đang mở) thay vì bắt mọi caller tách đôi luồng",
      "BẪY GHI-ĐÈ-CACHE: useTaskActionMutation.onSuccess setQueryData(result.task) GHI ĐÈ TOÀN BỘ cache chi tiết ⇒ 4 route action + reload() cũng PHẢI ký avatar, nếu không mỗi lần đổi trạng thái là ảnh biến mất tới lần refetch sau. Đúng lớp lỗi 'field im lặng không tới FE ở một phần ba số đường' mà docblock toDto đã bị 2 lần",
      "BẪY THỨ TỰ THAM SỐ: TaskCoreService được vài unit spec dựng bằng `new TaskCoreService(...)` theo VỊ TRÍ ⇒ dependency mới phải đứng CUỐI; chèn giữa làm lệch mọi dependency sau mà tsc chỉ báo 'thiếu 1 tham số'",
    ],
    done_when: [
      "Board/chi tiết/việc con/my-tasks/rail lọc đều hiện ảnh thật; không có ảnh ⇒ chữ cái đầu (fail-soft, không vỡ trang)",
      "Ký THEO LÔ + khử trùng người: 1 người phụ trách N thẻ chỉ tốn 1 hàng ký",
      "4 route action + reload() cũng trả avatar (không xoá ảnh khỏi cache sau mỗi thao tác)",
      "DTO KHÔNG BAO GIỜ chứa fileId thô — chỉ URL đã ký hoặc null",
      "typecheck/lint/build + unit API/FE xanh",
    ],
  },
  {
    id: "S5-TASK-CARDSUB-1",
    module: "TASK",
    layer: "FE",
    title: "Thẻ board: nút trỏ xuống bung danh sách việc con ngay trên thẻ (tải lười theo thẻ)",
    zone: "green",
    status: "done",
    paths: ["apps/app/src/routes/tasks/**", "apps/app/src/i18n/**"],
    skills: ["code-review"],
    depends_on: ["S5-TASK-SUBTASK-1"],
    src: [
      "Owner 2026-07-20 (ảnh MISA): thẻ '8/8 ⌄' bung ra danh sách các bước",
      "Payload board CHỈ có 2 con số (countSubtaskProgressByParentIdsTx là aggregate GROUP BY), KHÔNG có danh sách con và KHÔNG có endpoint lấy-hàng-loạt ⇒ phải gọi GET /tasks/:id/subtasks cho TỪNG thẻ. Board vài chục thẻ ⇒ TẢI LƯỜI (enabled: expanded) là bắt buộc, không phải tối ưu",
      "Cache dùng CHUNG khoá taskKeys.subtasks(taskId) với panel việc con ở màn chi tiết ⇒ mở chi tiết rồi quay ra board là cache-hit",
      "Trạng thái bung giữ Ở BOARD theo taskId (không trong từng thẻ): kéo-thả làm thẻ remount sang cột khác, state cục bộ sẽ tự thu lại giữa chừng thao tác",
      "Nút nằm TRONG thẻ vốn đã có onClick mở chi tiết ⇒ phải stopPropagation cho CẢ chuột LẪN bàn phím (thẻ là role=button nghe Enter/Space)",
    ],
    done_when: [
      "subtaskTotal>0 mới có nút; bung ra list có trạng thái + tiêu đề + avatar người thực hiện",
      "Chỉ gọi API khi bung; bung thẻ thứ hai của cùng task là cache-hit",
      "Bấm nút/bấm trong danh sách KHÔNG mở panel chi tiết",
      "Kéo-thả thẻ đang bung không làm nó tự thu",
    ],
  },
  {
    id: "S5-TASK-MOVEPROJ-1",
    module: "TASK",
    layer: "FE",
    title:
      "VÁ BUG state_id mồ côi khi đổi dự án + đường đổi dự án ĐÚNG MỘT chỗ (hộp thoại bắt chọn cột đích)",
    zone: "yellow",
    status: "done",
    paths: ["apps/app/src/routes/tasks/**", "apps/app/src/i18n/**"],
    skills: ["code-review"],
    depends_on: ["S5-TASK-INLINE-1"],
    src: [
      "BUG CÓ SẴN (không phải do WO này sinh ra): task-form-schema.ts taskFormToUpdatePayload LUÔN gửi projectId và KHÔNG BAO GIỜ gửi stateId. Server đổi project_id nhưng không đụng state_id (nhánh ghi project không gọi applyStateChangeTx) ⇒ task ở dự án MỚI mà cột trỏ dự án CŨ. Board mới không khớp cột nào nên thả vào cột mặc định (task-kanban.service fallback), DB mang tham chiếu chéo dự án tới lần kéo-thả kế tiếp. Im lặng: không lỗi, không cảnh báo, không test nào phủ",
      "Owner ban đầu muốn dropdown dự án ngay ở màn chi tiết; ĐÃ TỪ CHỐI ở S5-TASK-INLINE-1 vì làm vậy chỉ đẩy bug vào chỗ dễ bấm hơn — vá trước rồi mới mở",
      "BẪY khi vá: KHÔNG khoá ô dự án bằng `disabled` trên ô đã register — react-hook-form có thể trả undefined ⇒ payload gửi projectId:null ⇒ GỠ task khỏi dự án. Hỏng NẶNG HƠN bug đang vá và trông y hệt 'không làm gì'. Cách đúng: bỏ hẳn projectId khỏi taskFormToUpdatePayload",
      "D-36a — chỉ task GỐC KHÔNG CÓ việc con mới đổi được dự án (server 400 SUBTASK_*_PROJECT_LOCKED); chặn sớm kèm giải thích thay vì để bấm rồi ăn lỗi",
      "Gửi kèm stateId đòi THÊM update-state:task (task-core.service §3c) ⇒ KHÔNG prefill task.stateId vào form Sửa, nếu không mỗi lần sửa tiêu đề đều đòi quyền đó và người chỉ có update:task mất khả năng sửa",
    ],
    done_when: [
      "Đổi dự án đi ĐÚNG MỘT đường (TaskMoveProjectDialog), bắt buộc chọn cột đích khi dự án đích có pipeline",
      "PATCH gửi projectId + stateId trong CÙNG một lần ⇒ hết state_id mồ côi",
      "Form Sửa KHÔNG còn gửi projectId (chọn dự án chỉ khi TẠO), có dòng chỉ đường",
      "Task là việc con / có việc con ⇒ chặn kèm giải thích; nút ẩn khi thiếu update:task hoặc update-state:task",
      "Invalidate CẢ HAI board (dự án cũ mất thẻ, dự án mới thêm thẻ)",
    ],
  },
  {
    id: "S5-TASK-COVER-1",
    ...
    paths: [
      ...
      "apps/api/test/integration/**",
      "packages/contracts/src/task.ts",
      "packages/contracts/src/task-file.ts",
      "packages/web-core/src/**",
      "apps/app/src/routes/tasks/**",
      "apps/app/src/i18n/**",
    ],
    skills: ["code-review"],
    depends_on: ["S5-TASK-AVATAR-1"],
    src: [
      "Owner 2026-07-20 chốt phương án: ảnh bìa CHỌN TỪ TỆP ĐÃ ĐÍNH KÈM (nút 'Đặt làm ảnh bìa' trên mỗi ảnh trong khu Tệp đính kèm), KHÔNG dựng luồng upload riêng",
      "ĐÍNH CHÍNH 2026-07-20 (khi build): mô tả gốc ghi linkType='Cover' là SAI và tự mâu thuẫn với lời hứa KHÔNG MIGRATION — 'Cover' KHÔNG nằm trong CHECK chk_file_links_link_type (mig 0433:159, chỉ Avatar/Attachment/Contract/Proof/Document/Import/Export/Other) và không có trong FILE_LINK_TYPE_VALUES; dùng nó BẮT BUỘC migration ALTER CHECK + đổi enum. PHƯƠNG ÁN THẬT (owner chốt): ảnh bìa = chính dòng Attachment của task được bật is_primary. unique index uq_file_links_primary_per_entity_type (0433:174) đã ép sẵn đúng MỘT bìa/task ⇒ giữ trọn lời hứa không migration",
      "TÍNH CHẤT AN TOÀN THEN CHỐT: bìa CHÍNH LÀ dòng attachment nên 'chỉ nhận tệp đã đính kèm' là CẤU TRÚC, không phải lệnh kiểm có thể quên. Kiểm kèm: image/* + Uploaded + scan Clean|NotRequired (ngưỡng CHẶT như DOWNLOADABLE_SCAN, KHÔNG phải ≠Infected — biên an toàn không được lỏng hơn đường tải nó thay thế) + VỊ TỪ ĐỘC QUYỀN (tệp còn link sống ở entity KHÁC ⇒ từ chối). Vị từ độc quyền là chốt CHỐNG LEO THANG ĐỌC: đường tải thật đi qua FilePolicy.decideForLinkedFile = AND khắt-khe-nhất trên MỌI link, nên tệp link cả vào HR/Employee cả vào task (ảnh CCCD/hợp đồng) đang 403 khi tải sẽ hiện làm bìa cho cả board nếu thiếu nó",
      "Đọc: FileRepository.findVerifiedTaskCoversTx(companyId, taskIds, tx) lọc moduleCode='TASK'/entityType='task'/linkType='Attachment' + is_primary + image/Uploaded/scan Clean|NotRequired + NOT EXISTS(link sống ở entity khác), rồi CoverPresignService ký. SELF-DEFENDING: is_primary là cột ĐA-NGƯỜI-GHI (POST /foundation/files/:id/links nhận isPrimary verbatim, chỉ chặn Infected) nên MỌI ràng buộc phải ở đường ĐỌC. ⚠️ CẤM thêm fl2.company_id vào NOT EXISTS — ở NOT EXISTS mọi điều kiện thêm là fail-OPEN. Fail-soft: ký lỗi ⇒ null ⇒ thẻ không bìa, KHÔNG 500 board",
      "Ghi: KHÔNG dùng FilesService.link/unlink (mô tả gốc sai — sẽ tạo dòng link THỨ HAI cho cùng tệp, danh sách đính kèm hiện trùng). Đúng: UPDATE cờ is_primary trên CHÍNH dòng link đã có (file_links có GRANT UPDATE, 0433:181) — hạ cờ bìa cũ + nâng cờ mới trong CÙNG tx. findPrimaryLinkTx CHỈ query file_links, CẤM join files: TaskFileService.delete chỉ soft-delete bảng files nên link vẫn sống với is_primary=true, join vào sẽ làm primary MỒ CÔI thành vô hình ⇒ 23505→500 mọi lần. Kèm audit_logs object_type='file_link' (đã có trong CHECK mig 0440 ⇒ không cần migration)",
      "DTO: thêm coverUrl vào taskCoreResponseSchema (.nullable().optional(), additive) rồi thread qua CHÍNH mapper + 4 đường đọc + 4 route action y như assigneeAvatarUrl vừa làm ở S5-TASK-AVATAR-1 (đường đi đã chứng minh)",
      "VÌ SAO TÁCH WO RIÊNG: chạm file-link + presign = kiểm soát truy cập tệp ⇒ FULL gate (security-reviewer) theo CLAUDE.md §6, không gộp đuôi phiên khác",
    ],
    done_when: [
      "POST /tasks/:taskId/files/:fileId/cover + DELETE /tasks/:taskId/files/cover (KHÔNG phải /tasks/:taskId/cover — controller prefix là tasks/:taskId/files; @Delete('cover') phải khai TRƯỚC @Delete(':fileId') kẻo bị che); chỉ nhận file ĐÃ đính kèm task đó + là ảnh + Uploaded + scan Clean|NotRequired + không gắn ở entity khác",
      "Đặt bìa mới thay bìa cũ (hạ cờ cũ + nâng cờ mới trong CÙNG tx, KHÔNG unlink/link) — không vi phạm unique, không để 2 bìa, không 500 khi đồng thời (advisory xact lock + bắt 23505/40P01/40001 → 409)",
      "coverUrl trả trên board + chi tiết (URL đã ký, KHÔNG BAO GIỜ fileId thô); không có bìa ⇒ null",
      "FE: nút 'Đặt làm ảnh bìa'/'Gỡ ảnh bìa' trong TaskFilePanel (chỉ hiện trên tệp ảnh); thẻ board render ảnh bìa; deny-path ẩn nút",
      "int-spec deny-path (apps/api/test/integration/ — KHÔNG phải src/, *.int-spec.ts trong src/ không khớp glob nào của vitest.config.ts ⇒ chạy 0 ca mà gate vẫn PASS): file KHÔNG thuộc task ⇒ 404; cross-tenant ⇒ 404; không phải ảnh ⇒ 415 (KHÔNG phải 400); scan chưa sạch ⇒ 409; thiếu file-upload:task ⇒ 403; cờ is_primary bật vòng qua service ⇒ đường đọc VẪN trả null; primary mồ côi sau soft-delete file vẫn hạ được",
  ...
  },
  {
    ...
    src: [
      ...
      "ĐO 2026-07-24 (code đã viết xong, cờ CHƯA bật): có ĐÚNG 3 điểm tạo phiên (grep SESSION_COOKIE_NAME) = sign-in · sso · verify-email. verify-email THIẾU gate ⇒ OTP 'register' phát trước lúc flip vẫn đổi được phiên trong 10 phút OTP_TTL_MS, vòng qua SSO-only (đóng sign-up + resend-otp KHÔNG bịt được). Đã vá trong WO này",
      "BẪY break-glass: guard chỉ kiểm ADMIN_EMAILS khác rỗng, KHÔNG kiểm tài khoản đó có mật khẩu biết được — ensureUserForSso tạo user JIT với mật khẩu ngẫu nhiên 24 byte, mà forgot-password đã 403 dưới SSO-only ⇒ nếu admin chưa từng đặt mật khẩu LMS thì break-glass CHẾT đúng lúc cần. Owner phải login thử khi cờ CÒN TẮT trước khi flip",
      "KHÔNG smoke được cục bộ: next.config.ts không cho override distDir ⇒ next dev/build đều ghi thẳng .next mà NSSM MediaOS-LMS đang phục vụ (cùng họ landmine lms-next-build-shares-prod-dist). Vì vậy bản vá thiết kế INERT khi cờ tắt: deploy trước, bật sau",
    ],
    done_when: [
  ...
  },
  {
    ...
    done_when: [
      "Card Đào tạo trong /me (số khoá đang học + % gần nhất, fail-soft khi 403/502 — KHÔNG kéo sập overview) + trang /me/training loading/error/empty đủ 3 trạng thái; user không quyền access:lms không thấy card/menu (<PermissionGate> cấu hình theo useCanExact (KHÔNG useCan), KHÔNG hard-code role)",
  ...
  // ── Seed 2026-07-24 từ nhận xét owner (kèm ảnh): "LMS không dùng chung cấu trúc hiển thị" ─────
  // Owner đối chiếu LMS ↔ MediaOS và chỉ ra 3 điểm: thanh trạng thái trên, bố cục sidebar, thông báo.
  // Đo thực tế: UI-1 (token) + UI-2 (primitive) KHÔNG phủ được — cái lệch là KHUNG (topbar full-width
  // vs sidebar full-height) và NGUỒN DỮ LIỆU thông báo (hai kho tách rời). Tách 3 WO: UI-3 = khung
  // (owner chọn "port cấu trúc shell"); NOTI-1/2 = hợp nhất thông báo về module NOTI (owner chọn).
  {
    id: "S5-LMS-UI-3",
    module: "LMS",
    layer: "FE",
    ...
    skills: ["code-review"],
    depends_on: [],
    plan: "docs/plans/S5-LMS-UI-3.md",
    src: [
      "ĐO 2026-07-24: MediaOS ProtectedShell.tsx:133 = h-dvh flex-col ⇒ GlobalTopbar full-width TRƯỚC, ModuleSidebar nằm DƯỚI. LMS app/(app)/layout.tsx:57 = SidebarProvider + AppSidebar variant='inset' ⇒ sidebar cao full màn từ y=0, header chỉ chiếm nửa phải. Chiều cao đã khớp từ UI-2 (--header-height 14) — cái lệch là ĐIỂM BẮT ĐẦU",
      "CHẶN kỹ thuật: SidebarTrigger gọi useSidebar() ⇒ KHÔNG đưa SiteHeader ra ngoài SidebarProvider được. Cách đi: đổi TRỤC wrapper (className 'h-dvh min-h-0 flex-col' — tw-merge cho min-h-0 thắng min-h-svh; flex-col là direction nên không xung đột với display flex) rồi bọc [sidebar + inset] trong 1 div flex row",
      "components/ui/sidebar.tsx:234 là chỗ CHẶN thứ hai: sidebar desktop 'fixed inset-y-0 h-svh' luôn dán mép trên viewport, đè topbar. Sửa 1 chỗ: top-(--sidebar-top) + h-[calc(100svh-var(--sidebar-top))], khai mặc định SIDEBAR_TOP='0rem' cạnh SIDEBAR_WIDTH (dòng 30-32/136-137) ⇒ mọi layout khác ((auth)/(public)/docs) giữ nguyên hành vi cũ, chỉ (app) truyền 3.5rem",
      "anatomy đích (GlobalTopbar.tsx:93-145): [trigger][logo+tên cty][divider][tên app] … [Khóa học của tôi][AppSwitcher][Theme][Chuông][divider][NavUser]. LMS đang để AppSwitcher ở cụm TRÁI (site-header.tsx:58,61) và brand chỉ hiện ở nhánh !hasSidebar ⇒ gộp 2 nhánh làm 1 để /chat không có brand kép",
      "sidebar: bỏ SidebarHeader logo (app-sidebar.tsx:182-196 — brand lên topbar), bỏ sidebarStyle đổi --sidebar-primary theo khu (dòng 176-178, admin đỏ/learning xanh — MediaOS không có khái niệm này), thêm prop label cho NavMain → SidebarGroupLabel theo ngôn ngữ ModuleSidebar.tsx:208 (text-[11px] uppercase tracking-wider)",
      "R1 cuộn: đặt overflow-y-auto trên SidebarInset ⇒ document hết cuộn; trang nào dùng sticky top-* bên trong sẽ neo theo inset. Smoke trang dài: /course · learn · /manage-courses · /manage-exam/{id}/grading",
      "R2: đọc 3 layout con TRƯỚC khi sửa — app/(app)/course/[slug]/learn/layout.tsx · manage-exam/[id]/layout.tsx · admin/exams/[id]/layout.tsx (có thể tự dựng h-screen/sticky riêng ⇒ chồng chiều cao)",
      "kỷ luật track LOCAL: apps/lms trong .gitignore dòng 8, pnpm-lock + Next 15 riêng, KHÔNG import @mediaos/ui ⇒ port cấu trúc chứ không chia sẻ component (docs/plans/S5-LMS-UI-1.md §2). Ship = m prod-update lms (build VÀ restart) + backup data/app.db; verify ở localhost:3400, KHÔNG phải domain (memory lms-next-build-shares-prod-dist)",
    ],
    done_when: [
      "Đặt cạnh MediaOS: topbar LMS chạy hết chiều ngang, sidebar bắt đầu DƯỚI topbar, brand ở topbar, thứ tự nút cụm phải giống nhau, sidebar cùng bề rộng và có nhãn nhóm",
      "Thu gọn sidebar cho ra rail icon (không ẩn hẳn) + tooltip đúng; mobile drawer sheet vẫn mở/đóng bình thường",
      "npx tsc --noEmit 0 lỗi + eslint sạch trên file chạm; KHÔNG đụng route/API/quyền — diff nằm trọn ở lớp trình bày",
  ...
  },
  {
    ...
    skills: ["code-review"],
    depends_on: [],
    plan: "docs/plans/S5-LMS-NOTI-1.md",
    src: [
      "ĐO 2026-07-24: hạ tầng ĐÃ CÓ — POST /internal/v1/notifications/events (internal-notifications.controller.ts) nhận event chuẩn hoá rồi engine tự resolve recipient + dedupe + delivery_log. Việc của WO này là mở đường cho MỘT caller ở NGOÀI tiến trình api (LMS), không phải xây engine",
      "CHẶN 1 — danh tính: intake là defense-in-depth CẦN CẢ HAI (JwtAuthGuard toàn cục ⇒ thiếu Bearer là 401 TRƯỚC InternalGuard; company_id lấy từ req.user.companyId, controller dòng 68-70 chặn body spoof). LMS là service, KHÔNG có JWT user ⇒ phải chốt: service-account user (có companyId thật) hay guard riêng cho caller máy. ĐÂY LÀ ĐIỂM CROWN — company_id TUYỆT ĐỐI không được lấy từ body (BẤT BIẾN #1)",
      "CHẶN 2 — người nhận: internalEventRecipientSchema (contracts/notification.ts:117) chỉ có mode UserIds/EmployeeIds, đều là UUID MediaOS. LMS chỉ biết EMAIL (payload SSO = email + name + jti; users LMS không lưu mediaos userId). Hai lối: (a) thêm mediaos userId vào payload SSO + cột ở LMS rồi gửi UserIds — schema BE không đổi, phần việc dồn sang NOTI-2; (b) nới schema thêm mode Emails — chạm trust boundary crown, cần deny-path riêng",
      "OWNER CHỐT 2026-07-24: đi lối (a) — GIỮ NGUYÊN internalEventRecipientSchema. Lý do: không nới trust boundary của intake (mode Emails mở thêm một đường resolve người nhận từ chuỗi client gửi), và ánh xạ userId ổn định hơn email (đổi email không làm mồ côi thông báo). Hệ quả: BE chỉ lo danh tính service + catalog; phần định danh người nhận dồn sang NOTI-2",
      "eventCode phải VERBATIM khớp catalog seed — registerSource() FAIL-LOUD NGAY LÚC BOOT nếu lệch (goal-noti-bridge.registrar.ts:24). Mẫu migration seed catalog gần nhất: 0507 (GOAL_ASSIGNED/GOAL_FINALIZED)",
    ...
    done_when: [
      ...
      "eventCode LMS_* seed trong catalog + template render ra tiếng Việt đúng; boot KHÔNG fail-loud",
      "internalEventRecipientSchema GIỮ NGUYÊN (owner chốt lối (a)) — nếu quá trình làm phát hiện lối (a) bất khả thi thì DỪNG hỏi owner, KHÔNG tự nới sang mode Emails",
      "Coverage vùng chạm ≥80%; bash harness/check.sh --lane-db xanh (deny-path chạy THẬT, không skip)",
    ],
  },
  {
    id: "S5-LMS-NOTI-2",
    ...
    src: [
      "ĐO 2026-07-24: hai hệ thông báo TÁCH RỜI hoàn toàn — LMS AdminNotificationBell.tsx:62 đọc /api/notifications của chính Next app (SQLite riêng) + socket.io path /socket.io event user:notification-created; MediaOS NotificationBadge đọc module NOTI qua API + WS room co:{companyId}. Thông báo LMS chưa bao giờ hiện ở MediaOS",
      "phụ thuộc NOTI-1 vì cả DANH TÍNH (LMS không có JWT user để qua JwtAuthGuard) lẫn CÁCH CHỈ NGƯỜI NHẬN (LMS chỉ biết email) đều do NOTI-1 chốt — làm trước là đoán mò",
      "nếu NOTI-1 chọn lối (a): WO này phải thêm mediaos userId vào payload SSO (lib/platform/auth/sso.ts) + cột trên users LMS + backfill cho user đã có, rồi gửi recipient mode UserIds",
      "điểm phát sự kiện trong LMS: duyệt ghi danh (approval_status='approved') · chấm xong bài thi · gán khoá · job nhắc hạn. Gửi PHẢI best-effort + rate-limit + log server-side khi lỗi, KHÔNG chặn luồng học (mẫu auditSso trong app/api/auth/sso/route.ts: lỗi audit không chặn đăng nhập nhưng không nuốt câm)",
    ...
    done_when: [
      ...
      "Gọi lại cùng sự kiện (retry/double-submit) KHÔNG tạo thông báo trùng (dedupeKey)",
      "MediaOS sập hoặc trả lỗi ⇒ luồng học/duyệt ở LMS vẫn chạy bình thường, lỗi được log server-side (không nuốt câm)",
      "Không loại thông báo nào đến người dùng ở CẢ hai nơi; danh sách sự kiện chuyển/giữ ghi rõ trong plan",
      "npx tsc --noEmit + eslint xanh; smoke end-to-end với tài khoản thật sau deploy",
    ],
  },
  ...
  // ── DOC-2: sinh ra vì owner LẬT CHAT-DEC-004 (02/08). SPEC-15 Draft được viết quanh mệnh đề
  //    "không ai đọc được phòng mình không thuộc, kể cả Super Admin" — mệnh đề đó là tiền đề của
  //    §3.3 · §4.1 · §11 · §18 · §20 · §21 và của chính ChatAccessService. Không sửa được bằng
  //    một dòng ở bảng §22; phải viết lại trước khi bất kỳ WO code nào đọc spec.
  {
    id: "S7-CHAT-DOC-2",
    module: "CHAT",
    layer: "DOC",
    ...
    paths: [
      ...
      "docs/permission-matrix-spec.md",
      "docs/DB/DB-10_Migration_Plan_Initial_Seed_Data_Database_Design.md",
      "docs/plans/S7-CHAT-WAVE.md",
      "harness/backlog.mjs",
    ],
    skills: [],
    depends_on: ["S7-CHAT-DOC-1"],
    plan: "docs/plans/S7-CHAT-WAVE.md",
    src: [
      "owner AskUserQuestion 2026-08-02: DEC-004 = 'Super Admin đọc được mọi phòng (có audit)' — NGƯỢC đề xuất §3.3 bản Draft; DEC-008 = toàn bộ lịch sử; 10 mã còn lại theo đề xuất; thi công trên nhánh wave, KHÔNG merge master",
      "docs/plans/S7-CHAT-WAVE.md §3 (thiết kế đóng khung ngoại lệ) — nguồn của mọi ràng buộc dưới đây",
      "memory: capability-allowlist-hides-admin-screens (KI-058 — cặp nhạy cảm thiếu allowlist ⇒ màn ẩn; test bằng SA KHÔNG tái hiện được) · read-path-gate-pair-must-match-download-pair · canonical-seed-pin-regression",
    ],
    done_when: [
      "SPEC-15: §1 Draft→Approved · §3.3 VIẾT LẠI (bảng 7 ràng buộc đóng khung) · §4.1 mục 5 · §6 dòng BOD/Admin · §9 thêm CHAT-SCREEN-007/008 · §11 bảng 9→10 cặp + is_sensitive hỗn hợp · §12 thêm CHAT-ERR-019/020 · §15 thêm CHAT-API-018/019 · §18 · §20 ca 9/10/11 · §21 nhóm deny-path mới · §22 điền 12 kết quả + ghi chú lịch sử · §24 · §25",
      "permission-matrix §9c: bảng 9→10 cặp + viết lại dòng 'Không có cặp nào cho phép đọc phòng mình không thuộc' + is_sensitive hỗn hợp + ghi chú ngoại lệ ở đầu mục",
      "DB-12 bước D: 9→10 cặp permission, is_sensitive hỗn hợp, verify fail-loud CẢ HAI VẾ (SA có cặp, 3 role canonical còn lại KHÔNG có)",
      "API-13: §3 mục 5 (9→10 cặp) · §4.2 gỡ dòng 'bất kỳ endpoint nào cho phép đọc phòng lạ' + thêm 2 dòng ngoài-phạm-vi mới · §5 khối endpoint · §5.3 MỚI (4 endpoint + 6 ràng buộc thi công)",
      "backlog: cập nhật done_when của DB-1 (10 cặp) · BE-1 (chủ thể test deny-path KHÔNG được là SA) · BE-4 (KHÔNG mở tìm kiếm) · QA-1; seed S7-CHAT-BE-7 + S7-CHAT-FE-5",
      "Rà soát chéo: grep 'kể cả Super Admin' + 'không ai đọc' + '9 cặp' + 'allowlist FE' + '18 mã' trên docs/ VÀ harness/backlog.mjs → 0 chỗ còn khẳng định mệnh đề CŨ. ⚠️ CẤM dùng grep có | head làm bằng chứng PHỦ ĐỊNH — vòng đầu đã kết luận nhầm '0 kết quả' vì head -20 cắt cụt, plan-reviewer bắt được 4 chỗ còn sót (SPEC-15:497 bảng seed mà DB-1 đọc · SPEC-15:615 · title DB-1 · WAVE:49). Số dòng bị cắt trông giống hệt số dòng không tồn tại",
      "plan-reviewer đối kháng PASS trên SPEC-15 + DB-12 sau khi sửa (đây là done_when #5 của DOC-1)",
    ],
  },
  {
    id: "S7-CHAT-DB-1",
    ...
    done_when: [
      ...
      "BACKFILL TRƯỚC KHI ADD CONSTRAINT, nếu không migration ĐỎ trên DB đã có dữ liệu: sync_source là cột MỚI DEFAULT 'manual' trong khi chk_chat_rooms_sync_source ép department→'department', project→'project' ⇒ mọi hàng department/project đang tồn tại vi phạm NGAY lúc ADD CONSTRAINT. Thứ tự: thêm cột → UPDATE SET sync_source = room_type WHERE room_type IN ('department','project') → đếm fail-loud hàng vi phạm TỪNG CHECK (hoặc ADD CONSTRAINT ... NOT VALID rồi VALIDATE) → mới ADD CONSTRAINT. Áp cùng cách cho chk_chat_rooms_type_anchor",
      "attachment_count ĐẶT NGAY TRONG CÂU INSERT (fileIds.length), CẤM UPDATE sau. Câu 'cập nhật cùng transaction ⇒ không cần GRANT UPDATE' trong DB-12 bản 01/08 là SAI — quyền Postgres không đến từ việc nằm cùng transaction; chat_messages chỉ có SELECT,INSERT + column-GRANT 4 cột ⇒ mọi UPDATE ... SET attachment_count bị từ chối ⇒ MỌI TIN CÓ TỆP TRẢ 500",
      "CREATE EXTENSION unaccent WITH SCHEMA public + public.f_unaccent() IMMUTABLE PARALLEL SAFE STRICT (unaccent gốc chỉ STABLE ⇒ dùng thẳng là migration ĐỎ) + cột search_vector GENERATED ALWAYS AS (to_tsvector('simple', public.f_unaccent(coalesce(body,'')))) STORED + GIN. Dùng 'simple' KHÔNG 'english'. QUALIFY schema tường minh: cột generated neo vào OID hàm, phụ thuộc search_path lúc migrate mà hỏng thì hỏng VĨNH VIỄN. Xác minh vai trò chạy migration có quyền CREATE EXTENSION trên PROD",
      "Khai search_vector là GENERATED trong Drizzle schema communication.ts — thiếu là mọi INSERT tin nhắn đỏ 'cannot insert into generated column'",
      "room_code: chốt nullable HOẶC backfill + SET NOT NULL trong CÙNG migration (DB-12 §6.1 ghi 'Bắt buộc (sau backfill)' nhưng không bước nào SET NOT NULL — để lửng là drift)",
      "Seed: module CHAT (ON CONFLICT DO NOTHING) + 10 cặp permission SPEC-15 §11 + grant per-pair data_scope 4 role canonical (DELETE-wrong-scope + INSERT ON CONFLICT + verify fail-LOUD) + sequence_counters 'chat_room' cho MỌI company (thiếu = SequenceNotFoundError ngay phòng đầu, đúng bug QA2-CRIT-002 của task_code)",
      "Cặp thứ 10 ('view','chat-oversight') — CHAT-DEC-004: CHỈ INSERT vào catalog permissions, KHÔNG INSERT role_permissions cho role canonical nào. Verify fail-loud ĐÚNG MỘT VẾ: 0 role canonical giữ cặp. ⚠️ super-admin KHÔNG phải role canonical (4 canonical = roles.company_id IS NULL, SA không có hàng ở đó — dashboard-widget-catalog.const.ts:33 mirror 0481:35). SA nhận cặp qua SuperAdminBootstrapService lúc boot (grant TẤT CẢ cặp catalog) ⇒ khẳng định 'SA có cặp' là int-spec SAU BOOT, KHÔNG phải verify trong migration (viết WHERE code='super-admin' sẽ khớp 0 hàng ⇒ migration luôn đỏ ⇒ người thi công dễ grant nhầm sang company-admin, đúng role SPEC-15 §11 CẤM)",
      "is_sensitive ĐÃ CHỐT ở SPEC-15 §11: false cho 9 cặp thường, TRUE cho riêng ('view','chat-oversight'). ⚠️ allowlist cặp nhạy cảm nằm ở BACKEND: SENSITIVE_CAPABILITY_ALLOWLIST trong apps/api/src/permission/permission.service.ts (KHÔNG phải ở FE — getCapabilities() lọc bỏ mọi cặp sensitive, chỉ allowlist mới cho /auth/me trả key), pin bởi apps/api/src/auth/auth-me-capabilities.int.spec.ts. Thêm cặp vào allowlist CÙNG COMMIT với seed — thiếu là màn CHAT-SCREEN-007/008 ẩn dù DB có quyền (KI-058), và test bằng tài khoản SA KHÔNG tái hiện được",
      "Thêm 'view:chat-oversight' vào CẢ HAI mảng của apps/api/src/permission/permission.service.ts: SENSITIVE_CAPABILITY_ALLOWLIST (:47 — để /auth/me trả key) VÀ SENSITIVE_SCREEN_GATE_PAIRS (:195 — chốt hồi quy sensitive-screen-gate-allowlist.spec làm CI ĐỎ khi quên). Cùng commit với seed",
      "Viết int-spec SAU BOOT khẳng định role 'super-admin' giữ view:chat-oversight và /auth/me trả key — đặt ở apps/api/src/auth/auth-me-capabilities.int.spec.ts (đã có sẵn, gate hasDb && LANE_DB ⇒ CI SKIP, chỉ check.sh --all / REQUIRE_LANE_DB=1 mới ép chạy). Nêu rõ spec TỰ DỰNG role SA hay đòi PLATFORM_SUPERADMIN_EMAIL — đòi env mà lane không có thì spec skip lặng lẽ và B3 quay lại bằng cửa khác",
  ...
  },
  {
    ...
    done_when: [
      ...
      "⚠️ VÁ FULL GATE 03/08/2026 — 2 HIGH + 5 MEDIUM, cả hai HIGH đo được bằng probe trên DB thật:",
      "(HIGH-1) THU HỒI KHÔNG CÒN GỠ file_links. Gỡ link ⇒ decideForLinkedFile thấy links.length===0 ⇒ coi là foundation-owned ⇒ fallback FOUNDATION.FILE.DOWNLOAD (company-admin đang giữ, bulk grant 0435). Probe: người NGOÀI phòng 403 TRƯỚC thu hồi, 302 SAU thu hồi ⇒ thu hồi MỞ RỘNG phạm vi tệp. Nay link giữ sống (tệp vẫn module-owned) + ChatMessageFileResolver.canRead từ chối khi recalled_at IS NOT NULL. SPEC-15 §13.6 đã đính chính. ChatMessagesRepository.unlinkMessageFiles ĐÃ GỠ BỎ",
      "(HIGH-2) Khoá điều phối phải CHÍNH TẮC ở biên ghi. Registry FilePolicyService tra resolver bằng khoá đã lowercase, còn module truy vấn file_links bằng so-chuỗi CHÍNH XÁC ⇒ client khai moduleCode:'chat'/entityType:'Chat_Message' tạo được link 'ma': resolver VẪN cấp quyền tải cho mọi thành viên phòng, nhưng CHAT không thấy để hiển thị, unlinkMessageFiles không chạm, canUnlinkFile luôn false ⇒ grant KHÔNG quan sát được và KHÔNG thu hồi được. FileService.link nay từ chối 400 qua FilePolicyService.canonicalOwnerKey",
      "(MED) canAttach phải ép ĐỦ luật của sendMessage — POST /foundation/files/:id/links là ĐƯỜNG GHI THỨ HAI không đi qua ChatMessagesService: thêm vế phòng-lưu-trữ (CHAT-ERR-005 bị vòng qua), vế tin-đã-thu-hồi, và vế TRẦN CHAT_MAX_ATTACHMENTS_PER_MESSAGE (trần chỉ ở Zod ⇒ vượt trần phá điều kiện tiên quyết của trimToMessageBoundary ⇒ tab Tệp MẤT tệp im lặng)",
      "(MED) decorate() bỏ vế lọc attachmentCount>0 — cột không có GRANT UPDATE nên tệp gắn qua đường FOUNDATION vô hình VĨNH VIỄN trên /messages dù tab Tệp vẫn liệt kê + vẫn ký URL",
      "(MED) signMany nạp link CẢ LÔ bằng MỘT truy vấn (listLinksForFiles) thay vì mỗi tệp một withTenant; logSignedUrls đổi từ best-effort sang FAIL-CLOSED (nuốt lỗi ghi = giữ nguyên trạng thái 'kéo cả kho không để lại dấu vết'); orderBy thêm fileLinks.id (created_at trùng nhau trong một tx ⇒ thứ tự do planner quyết, ảnh đảo chỗ giữa hai lần cuộn)",
      "NỢ CÓ CHỦ: mỗi tệp vẫn tốn 1 dispatch policy (đã bỏ được vế nạp link). Lỗ 'links.length===0 ⇒ tụt fallback FOUNDATION' KHÔNG riêng CHAT (HR contract · task file · avatar · branding) ⇒ WO riêng S7-FND-LINKFALLBACK-1",
    ],
  },
  {
    id: "S7-FND-LINKFALLBACK-1",
    module: "FOUNDATION",
    layer: "BE",
    title:
    ...
    paths: [
      ...
      "apps/api/test/integration/**",
      "apps/api/test/foundation/**",
      "docs/plans/S7-FND-LINKFALLBACK-1.md",
    ],
    skills: ["code-review"],
    depends_on: ["S7-CHAT-BE-3"],
    src: [
      "apps/api/src/foundation/files/file-policy.service.ts (nhánh links.length === 0 → decide() foundation-owned)",
      "apps/api/migrations/0435_foundation_db5_retention_seed_modules.sql:375-380 (company-admin được bulk-grant download:foundation-file)",
      "S7-CHAT-BE-3 FULL gate — HIGH-1, đo bằng probe trên DB thật",
    ],
    done_when: [
      "Tệp có ÍT NHẤT MỘT link module-owned đã soft-delete và 0 link sống ⇒ decideForLinkedFile KHÔNG rơi xuống fallback FOUNDATION.FILE.*, trả deny (lý do riêng, phân biệt được với deny-no-resolver)",
      "Tệp CHƯA BAO GIỜ có link module (foundation-owned thật) ⇒ giữ NGUYÊN hành vi fallback hiện tại — đây là vế đối chứng, thiếu nó thì bản vá chặn nhầm mọi tệp foundation",
      "Chạy lại TOÀN BỘ int-spec của 5 module đăng ký resolver (CHAT · HR contract · HR employee-file · ME avatar · TASK · branding) — đây là đổi hợp đồng dùng chung, bán kính nổ ~15 int-spec chạm luồng link/unlink",
      "RED-trước: gỡ link của HR contract → company-admin KHÔNG tải lại được qua /foundation/files/:id/download",
      "FULL gate PASS",
    ],
  },
  {
  ...
  },
  {
    ...
    done_when: [
      ...
      "OWNER CHỐT 02/08/2026 — đường THU HỒI (rời phòng: đổi phòng ban · nghỉ việc · unlink · bớt project_member) chạy trong CÙNG transaction nguồn, cửa sổ lệch = 0. KHÔNG nuốt lỗi bằng logger.warn ở nhánh này: hỏng phải LOUD (log ERROR + audit resultStatus='Failure'). Đường TẠO PHÒNG được phép ngoài tx vì cấp room_code phải gọi TRƯỚC khi mở tx (SequenceService.nextCode tự mở withTenant, không lồng được — chat-room-code.service.ts jsdoc). OrgRepository.createOrgUnit hiện KHÔNG nhận tx (org.repository.ts:89) ⇒ phải nới chữ ký repo, hoặc ghi rõ writer này ở ngoài tx",
      "Liệt kê DANH SÁCH WRITER đầy đủ trong plan và chốt hook ở method DÙNG CHUNG. ⚠️ Có HAI họ writer ghi employee_profiles, không phải một: HrWriteService VÀ employees.service.ts (createEmployee:218 · updateEmployee:303 — ghi cả orgUnitId lẫn status · deleteEmployee:378, route sống ở employees.controller.ts:45/58/68), cộng RecycleBinService.restoreEmployee. Thiếu họ thứ hai = đổi phòng ban qua PATCH /employees/:id KHÔNG có hook nào",
      "Job đối soát ĐỊNH KỲ theo nhịp system-jobs @SystemJobHandler idempotent so khớp lại toàn bộ phòng sync_source<>'manual' với nguồn, sửa lệch, log số phòng/thành viên đã sửa; SchedulerModule import tường minh ChatModule. ⚠️ KHÔNG có cron 'đêm' trong hệ: mọi handler chạy chung một setInterval (worker-scheduler.config.ts) và PROD đặt SYSTEM_JOBS_POLL_MS=900000 = 15 PHÚT (.env.prod:9), không phải 60s như .env.example. Job là LƯỚI AN TOÀN, không phải đường chính",
      "Vị từ 'tập mong muốn' phải DUY NHẤT, dùng chung cho hook và job. Phòng project phải kèm employee_profiles.status='active' AND deleted_at IS NULL — thiếu vế này thì nhịp job kế tiếp JOIN LẠI người vừa nghỉ việc (changeStatus không chạm project_members), tự phá ca RED 'người nghỉ việc mất mọi đường đọc'",
  ...
  },
  {
    ...
    paths: [
      ...
      "apps/api/src/realtime/**",
      "apps/api/test/integration/**",
      "docs/plans/S7-CHAT-RT-0.md",
    ],
    skills: ["code-review"],
    depends_on: [],
    plan: "docs/plans/S7-CHAT-RT-0.md",
    src: [
      "ĐO 02/08/2026: grep toàn repo `ValkeyIoAdapter|useWebSocketAdapter` = 3 hit, CẢ BA nằm trong chính apps/api/src/realtime/valkey-io.adapter.ts (định nghĩa + logger + jsdoc). KHÔNG có lời gọi `app.useWebSocketAdapter(...)` ở bất kỳ đâu ⇒ override createIOServer() CHƯA BAO GIỜ CHẠY",
      "Hệ quả 1 — KHÔNG CORS: valkey-io.adapter.ts:60-70 là nơi DUY NHẤT set cors từ env.CORS_ORIGIN. app.enableCors() ở main.ts CHỈ áp cho HTTP, KHÔNG áp cho engine.io. socket.io-client thử polling TRƯỚC ⇒ request cross-origin bị chặn ⇒ connect_error 'xhr poll error'. FE dev :5273 vs API :3100 là khác origin, PROD khác domain hẳn",
      "Hệ quả 2 — KHÔNG đa-instance: server.adapter(this.adapterConstructor) không chạy ⇒ socketsJoin/socketsLeave của RealtimeEmitterService KHÔNG vượt sang instance khác. PROD hiện 1 instance nên chưa lộ",
      "Vì sao lỗ sống sót tới giờ: client WS duy nhất trong repo là client NODE (realtime.gateway.io.spec.ts) — Node không thực thi CORS; và test chạy 1 instance nên phần pub/sub không bao giờ bị kiểm",
    ],
    done_when: [
      "main.ts gọi app.useWebSocketAdapter(new ValkeyIoAdapter(app)) + await connectToValkey() TRƯỚC app.listen; thứ tự sai thì adapter gắn sau khi gateway đã tạo server = vô hiệu, phải có test/assert chứng minh thứ tự",
      "CORS Socket.IO đọc đúng CORS_ORIGIN (nhiều origin phân tách bởi dấu phẩy, trim); test chứng minh origin NGOÀI danh sách bị từ chối ở handshake — không phải chỉ 'có cấu hình'",
      "Adapter KHÔNG nối được Valkey → hiện fail-soft im lặng (WARN, rơi in-memory). Đổi thành CẢNH BÁO LOUD lúc boot (log ERROR + đánh dấu trạng thái) vì ở chế độ đó mọi ràng buộc join/leave chỉ đúng trên 1 instance — im lặng = tin nhắn rò sang người vừa bị gỡ ở instance khác",
      "Smoke bằng TRÌNH DUYỆT THẬT (không phải client Node): mở apps/app, thấy đúng 1 kết nối /ws lên, không có lỗi CORS ở console. Client Node KHÔNG thực thi CORS nên int-spec hiện tại KHÔNG chứng minh được điều này",
      "FULL gate PASS",
    ],
  },
  {
    ...
    done_when: [
      ...
      "Emit chat:message / chat:message-recalled / chat:read / chat:room SAU KHI transaction commit — test chứng minh transaction rollback thì KHÔNG có emit. ⚠️ WO NÀY SỞ HỮU CẢ ĐIỂM GỌI (owner chốt 02/08/2026): tự thêm lời gọi emit vào ChatMessagesService.sendMessage/markRead + ChatMessageModerationService.recall — S7-CHAT-BE-2 đã tuyên bố 'chừa nguyên' nên KHÔNG ai khác wire; dựng emitter mà không có nơi gọi = 3/4 sự kiện là dead code",
      "Cổng quyền đường đọc WS: trước khi join phòng chat phải kiểm cặp ('view','chat-room') — đường REST tương đương đã bắt buộc cặp này (chat-rooms.controller.ts). Phòng department/project có thành viên DẪN XUẤT nên membership KHÔNG thay được cặp quyền: người bị thu hồi quyền CHAT vẫn còn hàng chat_room_members. RED-trước bằng role thường (KHÔNG phải SA): là thành viên nhưng thiếu cặp → connect OK, nhận 0 chat:message",
      "Mọi payload .parse() qua schema contracts trong RealtimeEmitterService — cấm io.emit row DB thẳng; tin thu hồi emit ĐÚNG {messageId, roomId, recalledAt}, KHÔNG kèm body (kể cả null) — owner chốt 02/08/2026 theo API-13 §7, thay cho câu 'emit body:null' của bản seed (hai văn bản từng lệch nhau)",
  ...
  },
  {
    ...
    skills: ["code-review"],
    depends_on: ["S7-CHAT-BE-2", "S7-CHAT-RT-1", "S7-CHAT-RT-0"],
  ...
  // ── BE-7 + FE-5: sinh ra từ owner lật CHAT-DEC-004 (02/08). Tách WO RIÊNG là chủ ý thiết kế,
  //    không phải tiện tay: đường đọc thường (BE-1/BE-2) phải giữ được tính chất "gọi assertMember
  //    vô điều kiện" để đọc code là chứng minh được. Nhét thêm một nhánh if vào đó = mất vĩnh viễn.
  {
    id: "S7-CHAT-BE-7",
    module: "CHAT",
    layer: "BE",
    ...
    skills: ["code-review"],
    // BE-3 nằm trong deps CÓ CHỦ Ý: BE-3 là WO thêm URL ký vào DTO tin nhắn. Nếu BE-7 xong trước,
    // BE-3 có thể gắn URL ký vào DTO dùng chung và mở lại đúng lỗ B1 mà không ai review lại BE-7.
    depends_on: ["S7-CHAT-BE-2", "S7-CHAT-BE-3"],
    plan: "docs/plans/S7-CHAT-BE-7.md",
    src: [
      "SPEC-15 §3.3 (bảng 7 ràng buộc đóng khung) · §9 CHAT-SCREEN-007/008 · §12 CHAT-ERR-019/020 · §15 CHAT-API-018/019 · §18 · §20 ca 10/11 · §21 nhóm 'Đọc-vượt membership'",
      "API-13 §5.3 — 6 ràng buộc thi công, TẤT CẢ là điều kiện PASS của FULL gate",
      "owner AskUserQuestion 2026-08-02 (DEC-004 lật ngược đề xuất Draft)",
      "memory: read-path-gate-pair-must-match-download-pair · capability-allowlist-hides-admin-screens · reused-method-must-be-actor-scoped",
    ],
    done_when: [
      "Controller + service RIÊNG, path riêng /chat/oversight/*. CẤM tái dùng handler CHAT-API-001/004/009 kèm cờ isOversight; CẤM thêm tham số bỏ-qua-membership vào ChatAccessService.assertMember. Grep chứng minh đường đọc thường vẫn gọi assertMember vô điều kiện",
      "CHAT-API-018a/b/c + 019, TẤT CẢ là GET. Không POST/PATCH/DELETE nào dưới /chat/oversight/ — SA không gửi/ghim/thu hồi/sửa thành viên được ở phòng mình không thuộc",
      "HAI mô hình transaction KHÁC NHAU, chốt tường minh (API-13 §5.3 ràng buộc 2+3): THÀNH CÔNG → audit resultStatus 'Success' trong CÙNG tx với truy vấn đọc, ghi TRƯỚC khi trả; ép ghi audit lỗi → rollback → 0 byte ra ngoài (CHAT-ERR-020, HTTP 500, CẤM 200-rỗng). TỪ CHỐI → audit 'Denied' trong tx RIÊNG ĐÃ COMMIT rồi mới ném 403 (CHAT-ERR-019)",
      "GIỮ CẢ @RequirePermission('view','chat-oversight',{isSensitive:true}) LẪN @UseGuards(ChatOversightAuditGuard, PermissionGuard) — THỨ TỰ có ý nghĩa. Guard audit chạy TRƯỚC, tự gọi PermissionService.can(); deny → ghi audit 'Denied' trong withTenant tx RIÊNG ĐÃ COMMIT rồi return true để PERMISSIONGUARD mới là bên ném 403 (fail-closed vẫn mặc định). Ba bẫy: (a) ném 403 trong cùng tx ⇒ dòng audit từ chối BỊ ROLLBACK MẤT; (b) chỉ PermissionGuard ⇒ thân controller không chạy ⇒ 0 dòng audit; (c) BỎ PermissionGuard ⇒ route-guard-coverage.e2e-spec ĐỎ ('@RequirePermission trang trí'), bỏ luôn metadata thì route rơi vào needVerdict và phải ký route-verdicts = đưa route nguy hiểm nhất module vào rổ 'không gate'",
      "Lỗi hạ tầng trong ChatOversightAuditGuard → logger.error + return true (KHÔNG allow thêm gì vì PermissionGuard phía sau vẫn quyết, KHÔNG ném 503). Đường THÀNH CÔNG thì ngược lại: mất audit = ROLLBACK, vì ở đó audit là điều kiện để dữ liệu được rời server",
      "Regen route census (ROUTE_CENSUS_WRITE=1) + ký phán quyết cho ~21 route CHAT mới — thiếu là route-census runtime gate ĐỎ (memory route-census-runtime-gate)",
      "KHÔNG cấp quyền tải tệp: ChatMessageFileResolver CẤM đọc cặp chat-oversight, và DTO 018c trả metadata tệp KHÔNG kèm URL ký hạn ngắn. Tái dùng nguyên DTO của CHAT-API-009 (vốn kèm URL ký) = payload oversight tự phát ra khoá đọc tệp không cần membership",
      "018c KHÔNG tái dùng truy vấn có JOIN chat_room_members — SA không có hàng membership nên vị từ visible_from_seq dùng chung sẽ trả RỖNG (hỏng lặng lẽ chiều ngược lại). Oversight đọc toàn dải seq, con trỏ riêng",
      "KHÔNG có /chat/oversight/search (ràng buộc thiết kế, không phải thiếu sót) · KHÔNG emit WS cho phiên đọc-vượt (không join room, không nhận chat:message realtime)",
      "018a hẹp hơn 'liệt kê mọi phòng': từ khoá ≥2 ký tự + trần trang + audit ghi tiêu chí tìm. Không siết là một lần gọi xuất được đồ thị 'ai nhắn riêng với ai' toàn công ty — rộng hơn ngoại lệ owner chốt ('mở ĐÍCH DANH một phòng')",
      "019 phải bó action='chat.oversight.read' AND module_code='CHAT'; không bó là biến một cặp quyền CHAT thành cổng đọc audit TOÀN HỆ THỐNG. Ca test: gieo dòng audit module khác → 019 KHÔNG trả về",
      "RED-trước, CHỦ THỂ LÀ ROLE DỰNG TRONG TEST — KHÔNG dùng tài khoản SA cho CẢ vế deny LẪN vế allow. SA có *:* và được bootstrap grant mọi cặp catalog ⇒ ca positive vẫn XANH kể cả khi guard khai sai resource ('chat-oversite'), sai action, hoặc QUÊN GUARD HOÀN TOÀN; thêm nữa lane không set PLATFORM_SUPERADMIN_EMAIL thì role SA có thể không tồn tại. Đếm audit_logs trước/sau mỗi ca: thành công +1 'Success' · từ chối +1 'Denied' (VẪN +1 SAU KHI request trả 403) · rollback +0 VÀ 0 dữ liệu",
      "FULL gate (security-reviewer + silent-failure-hunter) PASS — WO này là bề mặt rủi ro lớn nhất của cả module",
    ],
  },
  {
    id: "S7-CHAT-FE-5",
    ...
    paths: [
      ...
      "docs/plans/S7-CHAT-FE-5.md",
    ],
    skills: ["frontend-design", "code-review"],
    depends_on: ["S7-CHAT-BE-7", "S7-CHAT-FE-2"],
    plan: "docs/plans/S7-CHAT-FE-5.md",
    src: [
      "SPEC-15 §9 CHAT-SCREEN-007/008 · §3.3 (lối vào tường minh là 1 trong 6 ràng buộc) · §18 (audit không xem được trên UI thì không phải kiểm soát)",
      "memory: capability-allowlist-hides-admin-screens (KI-058) · personal-prefs-must-not-sit-behind-permission-gate",
    ],
    done_when: [
      "Gate bằng useCanExact('view','chat-oversight') — KHÔNG dùng useCan/PermissionGate mặc định. useCan rơi xuống *:resourceType → action:* → *:* (packages/web-core/src/hooks/use-can.ts) ⇒ sẽ hiện lối vào màn NGUY HIỂM NHẤT module cho mọi người giữ wildcard trong khi BE vẫn 403. Ca test: tài khoản chỉ có *:* → KHÔNG thấy lối vào",
      "Cặp NHẠY CẢM (is_sensitive=true) ⇒ phải có trong SENSITIVE_CAPABILITY_ALLOWLIST ở BACKEND (apps/api/src/permission/permission.service.ts) — nếu không /auth/me không trả key và màn ẩn dù DB có quyền. Nghiệm thu bằng tài khoản KHÔNG phải SA nhưng được cấp cặp — test bằng SA KHÔNG chứng minh được gì (SA lọt nhờ *:*)",
      "RELEASE-11 Admin Guide: ghi thẳng rằng phạm vi đọc-vượt BAO GỒM cả phòng direct (tin nhắn riêng) — SPEC-15 §3.3 bắt buộc công bố ranh giới riêng tư thật, không chôn trong code",
      "CHAT-SCREEN-007: tra phòng theo mã/tên → hộp thoại xác nhận 'xem với tư cách quản trị' (nêu rõ hành động này được ghi nhật ký) → mở phòng CHỈ ĐỌC: không ô soạn tin, không nút ghim/thu hồi/sửa thành viên",
      "KHÔNG trộn vào danh sách phòng của CHAT-SCREEN-001 và KHÔNG vào panel nổi — lối vào riêng, để không bao giờ xảy ra do vô ý",
      "CHAT-SCREEN-008: nhật ký đọc-vượt (ai · phòng nào · lúc nào · thành công/bị từ chối), lọc theo người và khoảng thời gian",
      "PermissionGate/useCan, KHÔNG hard-code role; loading/error/empty; i18n vi namespace chat",
      "LIGHT gate PASS",
    ],
  },
  ...
  },
  {
    ...
    skills: ["code-review"],
    depends_on: ["S7-CHAT-FE-3", "S7-CHAT-BE-5", "S7-CHAT-BE-6", "S7-CHAT-FE-4", "S7-CHAT-FE-5"],
    plan: "docs/plans/S7-CHAT-QA-1.md",
    src: [
      "SPEC-15 §20 (12 tiêu chí nghiệm thu) · §21 (12 nhóm scenario, gồm nhóm 'Đọc-vượt membership')",
    ...
    done_when: [
      "Đủ 12 nhóm §21; đặt file đúng glob (apps/api chỉ chạy src/**/*.spec.ts — int-spec phải ở test/integration/**/*.int-spec.ts, sai glob là 0 ca chạy mà gate vẫn PASS)",
      ...
      "Nghiệm thu §20 ca 5 (tìm kiếm không rò phòng ngoài) · ca 9 (role KHÔNG có ('view','chat-oversight') bị chặn mọi đường) · ca 10 (đọc-vượt sinh đúng 1 hàng audit; ép audit lỗi → 0 dữ liệu) · ca 11 (người có cặp gọi /chat/search vẫn chỉ thấy phòng mình là thành viên) · ca 12 (tài khoản chỉ có *:* KHÔNG thấy lối vào CHAT-SCREEN-007) chạy THẬT, bằng chứng RED trước GREEN lưu docs/QA/evidence/",
      "CHỦ THỂ mọi ca oversight — CẢ vế deny LẪN vế allow — là role dựng trong test, KHÔNG phải Super Admin. SA có *:* VÀ được SuperAdminBootstrapService grant mọi cặp catalog ⇒ ca positive dùng SA là TAUTOLOGY: xanh kể cả khi guard khai sai resource, sai action, hoặc quên guard hoàn toàn; và lane không set PLATFORM_SUPERADMIN_EMAIL thì role SA có thể không tồn tại nên ca còn không chạy. Mỗi nhóm phải nêu rõ role chủ thể trong TÊN test",
  ...
  // ══════════════════════════════════════════════════════════════════════════════════════════════
  // WAVE S8-CHAT-UX — nâng cấp giao diện Chat (owner yêu cầu 05/08/2026).
  // Kế hoạch cấp wave + bằng chứng đo thật + 6 quyết định: docs/plans/S8-CHAT-UX-WAVE.md
  // ⚠️ DOC-1 chưa xong thì KHÔNG WO code nào ngoài FE-1 được mở (FE-1 thuần trình bày dữ liệu đã có).
  // ══════════════════════════════════════════════════════════════════════════════════════════════
  {
    id: "S8-CHAT-UX-DOC-1",
    module: "CHAT",
    ...
    paths: [
      ...
      "docs/API Design/API-13_CHAT_API_Design.md",
      "docs/permission-matrix-spec.md",
      "docs/plans/S8-CHAT-UX-WAVE.md",
    ],
    skills: ["code-review"],
    depends_on: [],
    plan: "docs/plans/S8-CHAT-UX-WAVE.md",
    src: [
      "Yêu cầu owner 05/08/2026: chia nhóm hội thoại · ghim hội thoại · avatar nhóm · avatar người gửi · tắt thông báo + đánh dấu chưa đọc · đang gõ/đang online · thả cảm xúc",
      "SPEC-15 §5.2 ĐANG liệt 'Đang gõ / đang online' và 'Thả cảm xúc' là NGOÀI v1 — làm mà không sửa văn bản thì mọi WO sau thi công theo spec đã sai (đúng tiền lệ S7-CHAT-DOC-2 với DEC-004)",
      "ĐO 05/08: phòng department/project có 0 admin (chat-derived-rooms-sync.service.ts không gán role='admin' lần nào) ⇒ luật 'admin phòng đặt avatar' làm avatar VĨNH VIỄN không đặt được ở đúng 2 loại owner vừa yêu cầu ⇒ DEC-016 phải chốt chủ thể khác",
    ],
    done_when: [
      "Owner chốt tường minh 6 quyết định DEC-014…018 trong §3 của WAVE doc (đặc biệt: có đồng ý ĐẢO §5.2 cho typing + reaction không)",
      "SPEC-15: chuyển typing/presence + reaction từ §5.2 sang §5.1; bổ sung §8 (pinned_at · avatar_file_id · reactions) · §9 (CHAT-SCREEN-001 chia mục) · §10 (CHAT-FUNC-016…020) · §11 (ghi chú chủ thể đặt avatar theo loại phòng) · §22 (bảng quyết định)",
      "DB-12 + API-13 cập nhật CÙNG COMMIT — ba nguồn không được lệch nhau",
      "KHÔNG chạy prettier lên docs/** (memory prettier-on-docs-reflows-all-tables: 15 dòng thật → ~1.500 dòng churn)",
      "plan-reviewer PASS trước khi mở DB-1",
    ],
  },
  {
    id: "S8-CHAT-UX-FE-1",
    ...
    paths: [
      ...
      "apps/app/src/components/chat/RoomListPanel.spec.tsx",
      "apps/app/src/components/chat/room-list-sections.ts",
      "apps/app/src/components/chat/room-list-sections.spec.ts",
    ...
    plan: "docs/plans/S8-CHAT-UX-WAVE.md",
    src: [
      "ĐO 05/08: RoomListPanel.tsx:151-204 là <ul> MỘT MỨC phẳng, mọi loại phòng trộn chung, sắp xếp theo roomOrder của store",
      "CHAT-DEC-014 — owner chọn 'nhóm sẵn theo loại phòng', KHÔNG phải thư mục tự đặt (thư mục tự đặt = wave sau, cần 2 bảng + kéo-thả)",
      "memory: duplicate-sibling-key-leaks-dom-node — chia mục sinh nhiều <ul> anh em; key phải duy nhất TOÀN danh sách, không chỉ trong một mục",
    ],
    done_when: [
      "Mục cố định theo roomType, thứ tự: Ghim (chừa sẵn, rỗng ở WO này) · Tin nhắn riêng · Nhóm · Phòng ban · Dự án; mục RỖNG ẩn hẳn (không hiện tiêu đề trống)",
      "Thu/mở từng mục, trạng thái nhớ ở localStorage per-user; mặc định MỞ hết",
      "Số chưa đọc CỘNG DỒN hiện trên tiêu đề mục khi mục đang thu — thu mục lại mà mất dấu tin chưa đọc là đường im lặng",
      "Ô lọc + rổ 'đã lưu trữ' giữ nguyên hành vi cũ; lọc ra kết quả thì chỉ hiện mục CÓ kết quả",
      "Test ĐẾM NODE (không chỉ đếm element): mỗi phòng xuất hiện ĐÚNG 1 lần trong toàn danh sách, không nhân bản qua các mục",
      "LIGHT gate (react-reviewer + typescript-reviewer + quality-gate) PASS",
    ],
  },
  {
    ...
    skills: ["code-review", "security-review"],
    depends_on: ["S8-CHAT-UX-DOC-1"],
    plan: "docs/plans/S8-CHAT-UX-DB-1.md",
    src: [
    ...
    done_when: [
      "Migration đánh số 0543 (head hiện tại 0542), CÓ entry trong meta/_journal.json — thiếu entry là bị BỎ QUA trong im lặng",
  ...
  },
  {
    ...
    paths: [
      ...
      "packages/contracts/src/chat.ts",
      "packages/web-core/src/lib/chat-api.ts",
      "docs/plans/S8-CHAT-UX-BE-1.md",
    ],
    skills: ["code-review"],
    depends_on: ["S8-CHAT-UX-DB-1"],
    plan: "docs/plans/S8-CHAT-UX-BE-1.md",
    src: [
      "ĐO 05/08: mig 0538:258 ĐÃ cấp GRANT UPDATE (muted_until) nhưng grep apps/app không có lối vào nào — quyền có, cột có, UI không",
      "memory: ui-promises-backend-never-reads — bật 'tắt thông báo' trên UI mà đường phát noti không THẬT đọc muted_until thì người dùng vẫn bị làm phiền và không ai biết. Grep ĐƯỜNG QUYẾT ĐỊNH ở cả hai đầu",
      "SPEC-15 §10 CHAT-FUNC-015: phòng đã tắt KHÔNG sinh notification nhưng VẪN tăng badge — hai vế, đừng làm một",
      "memory: server-masking-needs-optional-fe-schema — khoá mới trong chatRoomSchema phải .optional()",
    ],
    done_when: [
      "RED trước: spec chứng minh hôm nay KHÔNG có route nào ghi được pinned_at/muted_until (404), rồi mới thêm",
      "Ghim: PUT/DELETE /chat/rooms/:id/pin — chỉ ghim phòng MÌNH LÀ THÀNH VIÊN (deny-path: ghim phòng không thuộc ⇒ 404, không phải 403 làm lộ tồn tại)",
      "Trần 10 phòng ghim/người ép ở SERVICE + ca đua (2 request song song không vượt trần)",
      "Tắt thông báo: PUT /chat/rooms/:id/mute nhận mutedUntil; CHỨNG MINH bằng test rằng đường phát noti CHAT_MENTIONED/CHAT_DIRECT_MESSAGE thật sự bỏ qua phòng đã tắt, VÀ unreadCount vẫn tăng",
      "Đánh dấu chưa đọc: last_read_seq CHỈ TIẾN (CHAT-ERR-018) ⇒ KHÔNG lùi con trỏ; dùng cờ riêng per-member, không phá bất biến chỉ-tiến",
      "chatRoomSchema thêm pinnedAt/mutedUntil dạng .optional(); rebuild contracts + web-core dist trước typecheck",
      "LIGHT gate PASS",
    ],
  },
  {
    ...
    skills: ["code-review", "security-review"],
    depends_on: ["S8-CHAT-UX-DB-1"],
    plan: "docs/plans/S8-CHAT-UX-BE-2.md",
    src: [
      "Khuôn ĐÃ qua FULL gate 2 lần: me-avatar.service.ts (S5-ME-BE-4) và chat-files.service.ts (S7-CHAT-BE-8) — gate *:foundation-file nằm ở FilesController, FileService KHÔNG gate ⇒ controller có gate RIÊNG gọi thẳng FileService là hợp lệ, KHÔNG cấp cặp quyền mới, KHÔNG migration quyền",
      "ĐO 05/08: ('upload','foundation-file') chỉ có ở SA · company-admin · QUẢN LÝ CẤP CAO (mig 0435:376) ⇒ KHÔNG được bắt người dùng đi đường foundation, sẽ chết đúng như lỗ BE-8 đã vá",
      "CHAT-DEC-016 — chủ thể đặt avatar KHÁC NHAU theo loại phòng vì department/project có 0 admin phòng (đo thật)",
      "memory: read-path-gate-pair-must-match-download-pair — cặp gate đường TẢI avatar phải trùng cặp đường ĐỌC phòng",
    ],
    done_when: [
      "Ownership + state + MIME image/* kiểm ở service (chống IDOR: chỉ gắn file DO CHÍNH MÌNH upload, state Uploaded, non-Infected)",
      "Chủ thể theo DEC-016: group→room admin · department→('update','org-unit') · project→quản lý dự án · direct→422 CẤM (avatar dẫn xuất từ người đối thoại, không lưu cột)",
      "Deny-path RED trước cho TỪNG loại phòng: thành viên thường đặt avatar nhóm ⇒ 403; người ngoài phòng ⇒ 404",
      "downloadUrl ký TƯƠI mỗi lần trả, KHÔNG persist URL (StorageAdapter cấm)",
      "Job đối soát derived-room chạy sau đó KHÔNG xoá avatar — chứng minh bằng int-spec chạy thật job",
      "FULL gate (security-reviewer + silent-failure-hunter) PASS",
    ],
  },
  {
    ...
    paths: [
      ...
      "apps/api/src/chat/chat-messages.controller.ts",
      "apps/api/src/chat/chat.mapper.ts",
      "apps/api/src/chat/**/*.spec.ts",
      "apps/api/src/realtime/realtime-emitter.service.ts",
      "packages/contracts/src/chat.ts",
      "docs/plans/S8-CHAT-UX-BE-3.md",
    ],
    skills: ["code-review"],
    depends_on: ["S8-CHAT-UX-DB-1"],
    plan: "docs/plans/S8-CHAT-UX-BE-3.md",
    src: [
      "CHAT-DEC-018 — bộ emoji ĐÓNG (6 mã), ép bằng CHECK ở DB chứ không chỉ Zod ở app: chuỗi tự do là bề mặt lưu trữ vô hạn + rủi ro render",
      "memory: ws-payload-narrower-than-rest-dto — payload chat:reaction phải qua CÙNG lớp DTO/mask như REST, cấm emit thẳng row",
      "Tổng hợp reaction phải đi kèm trang tin nhắn (1 truy vấn gộp), KHÔNG N+1 theo từng tin",
    ],
    done_when: [
      "React/bỏ react idempotent: PUT hai lần = 1 hàng; DELETE khi chưa react = 204, không 404",
      "Chỉ react được tin trong phòng MÌNH LÀ THÀNH VIÊN (deny-path RED trước: tin phòng khác ⇒ 404)",
      "KHÔNG react được tin đã thu hồi (recalled_at IS NOT NULL) ⇒ 422",
      "DTO tin mang tổng hợp {emoji, count, mine} — không trả danh sách userId thô (rò ai-thả-gì cho cả phòng là quyết định riêng, không làm ngầm)",
      "Đường đọc trang tin: 1 truy vấn gộp reaction, đo bằng test đếm số câu SQL — không N+1",
      "LIGHT gate PASS",
    ],
  },
  {
    ...
    skills: ["code-review", "security-review"],
    depends_on: ["S8-CHAT-UX-DOC-1"],
    plan: "docs/plans/S8-CHAT-UX-RT-1.md",
    src: [
      "RÀNG BUỘC CỨNG: chat-realtime-structure.spec.ts:26 ép 0 @SubscribeMessage trong TOÀN BỘ apps/api/src (quét đệ quy, đã bỏ comment). CHAT-DEC-005 'WS một chiều' — WO này KHÔNG được phá",
      "CHAT-DEC-017: typing đi qua POST /chat/rooms/:id/typing (204, 0 ghi DB, 0 audit) → emitter phát chat:typing. Presence làm ở handleConnection/handleDisconnect — thuần server, không cần event từ client",
      "memory: valkey-shared-across-all-envs-no-channel-prefix — Valkey dùng chung 4 môi trường, KHÔNG có tiền tố kênh. Khoá presence thiếu prefix ⇒ người ở dev-online hiện 'đang online' trên PROD",
      "memory: ws-permission-gate-needs-its-own-room · engineio-cors-never-rejects — cưỡng chế ở allowRequest, không phải chỉ bỏ header",
    ],
    done_when: [
      "Ratchet 0-@SubscribeMessage VẪN XANH sau WO này — chạy lại chat-realtime-structure.spec.ts và dán kết quả vào PR",
      "Typing: chỉ thành viên phòng ping được (deny-path RED trước ⇒ 404); server tiết lưu, KHÔNG ghi DB, KHÔNG audit; payload chỉ {roomId, userId} — không nội dung",
      "Presence: khoá Valkey mang tiền tố môi trường; test A/B chứng minh hai môi trường KHÔNG thấy nhau",
      "Ngắt kết nối bẩn (kill process) không để lại 'đang online' vĩnh viễn — khoá có TTL, không dựa vào handleDisconnect chạy được",
      "Người bị khoá/vô hiệu bị cắt phiên WS thì cũng biến mất khỏi presence",
      "FULL gate (security-reviewer + silent-failure-hunter) PASS",
    ],
  },
  {
    ...
    paths: [
      ...
      "apps/app/src/locales/**",
      "packages/web-core/src/lib/chat-api.ts",
      "docs/plans/S8-CHAT-UX-FE-2.md",
    ],
    skills: ["code-review"],
    depends_on: ["S8-CHAT-UX-BE-1", "S8-CHAT-UX-BE-2", "S8-CHAT-UX-FE-1"],
    plan: "docs/plans/S8-CHAT-UX-FE-2.md",
    src: [
      "CHAT-DEC-014/014/015",
      "memory: react-query-v5-stale-mutationfn-closure — dựng payload trong onClick rồi mutate(payload), đừng đọc state trong mutationFn",
      "memory: personal-prefs-must-not-sit-behind-permission-gate — ghim/tắt thông báo là TUỲ CHỌN CÁ NHÂN, không được giấu sau cổng quyền quản trị",
      "Nút đặt avatar phải ẩn theo ĐÚNG luật DEC-016 của từng loại phòng — hiện nút rồi để BE trả 403 là hứa suông",
    ],
    done_when: [
      "Menu ngữ cảnh mở bằng chuột phải VÀ bằng nút … (bàn phím tới được — không chỉ chuột phải)",
      "Ghim/bỏ ghim cập nhật lạc quan, thất bại thì HOÀN NGUYÊN và báo lỗi (không im lặng)",
      "Vượt trần 10 ghim (409 CHAT-ERR-021): hiện thông điệp nêu rõ trần, không phải lỗi chung chung",
      "Avatar: nhóm/phòng ban/dự án dùng avatar phòng nếu có, không có thì chữ cái đầu + màu suy từ id; direct dùng avatar người đối thoại",
      "Trạng thái tắt thông báo hiện ngay trên dòng phòng (biểu tượng chuông gạch), không phải chỉ trong menu",
      "LIGHT gate PASS",
    ],
  },
  {
    ...
    skills: ["code-review"],
    depends_on: ["S8-CHAT-UX-BE-3", "S8-CHAT-UX-RT-1", "S8-CHAT-UX-FE-2"],
    plan: "docs/plans/S8-CHAT-UX-FE-3.md",
    src: [
      "CHAT-DEC-019 — avatar người gửi lấy từ ROSTER phòng (ký 1 lần), KHÔNG ký theo từng tin: 50 tin = 50 lần ký + 50 hạn lệch nhau",
      "Người ĐÃ RỜI phòng vẫn phải có trong roster (kèm leftAt) — thiếu thì tin cũ của họ mất avatar và mất tên",
      "memory: ws-payload-narrower-than-rest-dto — chỉ báo đang gõ dựng từ payload WS hẹp, đừng giả định nó có mọi khoá như REST",
    ],
    done_when: [
      "Gộp tin liên tiếp cùng người trong cửa sổ N phút: chỉ tin đầu hiện avatar+tên, các tin sau thụt lề — đo bằng test ĐẾM NODE avatar",
      "Tin hệ thống (system) KHÔNG có avatar, KHÔNG gộp",
      "Thanh cảm xúc: 6 emoji cố định + hiện tổng hợp dưới bong bóng; bấm lại để bỏ; cập nhật lạc quan có hoàn nguyên",
      "Chỉ báo 'đang gõ' tự tắt sau 5s không nhận ping; FE tiết lưu ping 3s (không mỗi phím một request)",
      "Chấm online chỉ hiện ở phòng direct và trong danh sách thành viên — KHÔNG hiện ở phòng đông người (nhiễu + rò lịch làm việc)",
      "LIGHT gate PASS",
    ],
  },
  {
    ...
    skills: ["code-review", "security-review"],
    depends_on: ["S8-CHAT-UX-FE-3"],
    plan: "docs/plans/S8-CHAT-UX-QA-1.md",
    src: [
    ...
    done_when: [
      "Ma trận deny-path đủ: ghim/mute/đánh dấu phòng KHÔNG thuộc · đặt avatar khi không đủ tư cách (4 loại phòng) · react tin phòng khác · ping typing phòng khác — tất cả RED-proof trước khi xanh",
      "Cross-tenant: 0 rò pinned_at/avatar/reaction/presence giữa 2 công ty",
      "Ratchet chat-realtime-structure.spec.ts + s7-chat-db1-invariants.int-spec.ts xanh, và KHÔNG bị nới lỏng assert (diff của chính 2 file này phải được soi riêng)",
      "Coverage ≥80% cho cụm chat mới; bash harness/check.sh --all XANH có bằng chứng LANE_DB",
      "Cập nhật docs/TESTABLE-FEATURES.md + ghi nợ migration 0543 cho PROD vào RELEASE (0542 CŨNG chưa áp PROD)",
    ],
  },
...

  {
    ...
    done_when: [
      "✅ Phép đo LẶP LẠI ĐƯỢC: `apps/api/test/foundation/route-http-coverage.e2e-spec.ts` (không cần Postgres, boot app + scan tĩnh file supertest, KHÔNG đếm tay) — đo lại cuối 14/08: **370/499 (74,1%) có bằng chứng phủ, 129 chưa** (baseline lúc dựng: 366/499). ⚠️ 74,1% là **CẬN TRÊN** (khớp verb-set × path-set ở cấp FILE ⇒ sai số dồn về false-positive 'covered') ⇒ 129 là cận DƯỚI của khoảng trống thật.",
      "✅ Xếp hạng risk score (route ghi + nhạy cảm auth/permission/secret/audit lên trước route đọc) — in top-30 mỗi lần chạy script.",
      "◐ Phủ test HTTP thật đợt này: **5 route risk≥5 (đều có ca ALLOW 2xx THẬT) + 2 route risk=3** — ĐÍNH CHÍNH câu 'phủ 5/18 route risk≥5' của vòng trước (sai 2 lần: 2 trong 5 là risk=3, và PATCH security-policy là route CHẾT chỉ có ca ghim 403). Cụ thể: risk 7 `POST /auth/reset-password` · risk 7 `POST /users/activation/accept` · risk 6 `POST /auth/2fa/disable` (`auth-toprisk-http.int-spec.ts`, 13 ca) + risk 5 `PUT /settings/mail-config` · risk 5 `POST /settings/mail-config/test` + risk 3 `GET /settings/security-policy` · risk 3 `GET /settings/mail-config` (`security-mailconfig-http.int-spec.ts`, 10 ca). `PATCH /settings/security-policy` KHÔNG tính đã phủ ⇒ **KI-065**. Sổ khớp: baseline 18 route risk≥5 chưa phủ = 5 phủ thật + 1 ghim-403 không tính + **12 còn nợ**.",
      "✅ Test chạy thật ở LANE_DB (`mediaos_routehttp`) — 23/23 xanh (13 auth-toprisk + 10 security-mailconfig), không skip; `route-http-coverage.e2e-spec.ts` 5/5 (có ratchet).",
      "✅ KI-025 cập nhật số đo mới ở RELEASE-02 (giữ nguyên S2 — còn nợ thật, không hạ severity theo mong muốn) + ghi rõ 74,1% là CẬN TRÊN.",
      "✅ Nợ ĐƯA RA KHỎI VĂN XUÔI: 12 route risk≥5 còn lại → WO `S10-QA-ROUTEHTTP-2` (danh sách đầy đủ ở `src`/`done_when` của WO đó); bug route chết → **KI-065** (hàng riêng, có mức S2 + chủ) + WO `S10-QA-SECPOLICY-GATE-1`.",
    ],
    notes: [
      ...
      "ĐÓNG 14/08/2026 (chạm ngân sách phiên, dừng đúng luật): phép đo cho 370/499 (74,1%) — heuristic RỘNG có chủ đích (scan cấp FILE, không cấp câu lệnh — xem giới hạn ghi trong docblock đầu file coverage script), nên là CẬN TRÊN, không phải số chính xác tuyệt đối.",
      "🔴 PHÁT HIỆN THẬT khi viết test cho `PATCH /settings/security-policy` đã được TÁCH thành **KI-065** (RELEASE-02, hàng riêng có mức S2 + chủ) — KHÔNG còn chôn trong văn xuôi KI-025. Test ghim: `security-mailconfig-http.int-spec.ts:171`; ai vá bug sẽ thấy ca đó ĐỎ CÓ CHỦ Ý và phải LẬT sang ALLOW 2xx, cấm nới assert.",
      "Việc còn nợ cho lane khác (đã seed WO, không để ở notes): (1) `S10-QA-ROUTEHTTP-2` — test HTTP thật cho **12 route risk≥5** còn lại; (2) `S10-QA-SECPOLICY-GATE-1` — quyết định số phận `PATCH /settings/security-policy` (KI-065).",
      "Ratchet đã cắm: `MAX_UNCOVERED_HIGH_RISK = 12` + `MIN_COVERED_COUNT` trong coverage spec ⇒ thêm route risk≥5 không test, hoặc xoá test làm tụt phủ, đều ĐỎ CI. Siết xuống sau mỗi đợt phủ thêm.",
    ],
  },

  {
    // Seed 14/08/2026 từ S10-QA-ROUTEHTTP-1 (vòng sửa): 12 route risk≥5 CÒN NỢ, đưa ra khỏi văn xuôi.
    ...
    skills: ["security-review", "code-review"],
    depends_on: ["S10-QA-ROUTEHTTP-1"],
    src: [
      "Kế thừa `S10-QA-ROUTEHTTP-1` (KI-025). Danh sách dưới đây là SỐ ĐO, không phải ước lượng — output `pnpm --filter @mediaos/api exec vitest run test/foundation/route-http-coverage.e2e-spec.ts` ngày 14/08/2026 (370/499 phủ, 129 chưa, 12 route risk≥5 chưa phủ).",
      "12 ROUTE risk≥5 CÒN NỢ (đủ verb + path + controller + cặp quyền, không rút gọn):",
      "  1. POST   /api/v1/users/invite                              [UserInvitesController#invite]              perm=invite:user",
      "  2. POST   /api/v1/api-keys                                  [ApiKeysController#create]                  perm=manage:api-key",
      "  3. POST   /api/v1/api-keys/:id/revoke                       [ApiKeysController#revoke]                  perm=manage:api-key",
      "  4. POST   /api/v1/auth/users/:id/password/reset             [AuthUsersController#resetPassword]         perm=reset-password:user",
      "  5. POST   /api/v1/auth/users/:id/restore                    [AuthUsersController#restore]               perm=restore:user",
      "  6. DELETE /api/v1/auth/users/:id                            [AuthUsersController#softDelete]            perm=delete:user",
      "  7. POST   /api/v1/permissions/users/:userId/roles           [PermissionAdminController#assignRole]      perm=assign-role:user",
      "  8. DELETE /api/v1/permissions/users/:userId/roles/:roleId   [PermissionAdminController#revokeRole]      perm=assign-role:user",
      "  9. PUT    /api/v1/permissions/object                        [PermissionAdminController#setObjectPermission]    perm=grant-object-permission:permission",
      " 10. DELETE /api/v1/permissions/object                        [PermissionAdminController#removeObjectPermission] perm=grant-object-permission:permission",
      " 11. DELETE /api/v1/auth/roles/:id                            [RoleAdminController#deleteRole]            perm=delete:role",
      " 12. DELETE /api/v1/auth/roles/:id/permissions                [RoleAdminController#revokePermission]      perm=assign:permission",
      "Vì sao phủ tầng service KHÔNG đủ: guard · ZodValidationPipe · response envelope CHỈ chạy khi đi qua HTTP thật. Đây đúng nhóm crown-jewel (gán/thu hồi role, object-grant, xoá mềm user, api-key) mà CLAUDE.md §6 đòi deny-path test.",
      "Khuôn có sẵn để bắt chước, KHÔNG tự nghĩ kiểu mới: `apps/api/test/integration/auth-toprisk-http.int-spec.ts` (ALLOW chứng minh bằng HỆ QUẢ quan sát qua HTTP, cách ly rate-limit bằng user riêng) + `security-mailconfig-http.int-spec.ts`.",
    ],
    done_when: [
      "Mỗi route trong 12 route trên có ÍT NHẤT 1 ca ALLOW 2xx THẬT (đúng quyền ⇒ 2xx + envelope đúng hình) đặt TRƯỚC ca DENY — nhánh thiếu ALLOW làm ca DENY xanh RỖNG (bài học deny-cases-vacuous-without-allow-case)",
      "ALLOW phải chứng minh bằng HỆ QUẢ quan sát được (role thật được gán/thu hồi, user thật `deleted_at`, api-key thật dùng được rồi bị revoke) — KHÔNG chỉ assert mã 2xx",
      "Ca DENY: thiếu quyền ⇒ 403 (dùng actor KHÔNG phải super-admin — super-admin không canonical, test bằng SA là tautology); sai DTO ⇒ 400 ở BIÊN (chứng minh pipe chạy TRƯỚC service)",
      "Ca CROSS-TENANT cho route có `:id`: actor công ty A thao tác trên id của công ty B ⇒ 404/403, KHÔNG rò tồn tại",
      "Chạy THẬT ở LANE_DB (`bash scripts/lane-db-setup.sh routehttp2` → `export LANE_DB=mediaos_routehttp2`) — thiếu LANE_DB thì int-spec SKIP = xanh-giả",
      "Chạy lại `route-http-coverage.e2e-spec.ts`: `uncovered risk≥5` giảm từ 12 xuống ≤0-2 VÀ **SIẾT hằng số `MAX_UNCOVERED_HIGH_RISK` xuống đúng số mới** (ratchet chỉ có giá trị khi được siết; nới lên phải có WO + lý do)",
    ...
    notes: [
      ...
      "Nhóm 7-10 (permission-admin) là crown-jewel: đổi role/object-grant phải kiểm cả AUDIT LOG được ghi (append-only) trong cùng ca.",
    ],
  },

  {
    // Seed 14/08/2026 từ KI-065 (do S10-QA-ROUTEHTTP-1 đào ra khi viết test HTTP thật).
    id: "S10-QA-SECPOLICY-GATE-1",
    module: "AUTH",
    layer: "BACKEND",
    title:
    ...
    depends_on: [],
    src: [
      "KI-065 (RELEASE-02) — mở 14/08/2026, mức S2. Đo bằng HTTP THẬT, không suy đoán: `apps/api/test/integration/security-mailconfig-http.int-spec.ts:171` — actor có ALLOW company-level đầy đủ trên `configure-security-policy:company` VẪN nhận 403 `deny-object-required`.",
      'Cơ chế: `security-policy.controller.ts:34-38` khai `@RequirePermission("configure-security-policy", "company", { isSensitive: true, requiresReauth: true })` nhưng route KHÔNG có `:id` ⇒ `resourceId` luôn `null` ⇒ `permission.decide.ts` tính `needsObjectGrant = true` VĨNH VIỄN; điều kiện thoát còn lại `req.reauthContext` KHÔNG được gán ở BẤT KỲ đâu trong codebase (chỉ có nơi ĐỌC).',
      "Hỏng đúng chiều an toàn (fail-CLOSED) ⇒ KHÔNG phải lỗ bảo mật; hậu quả là ép 2FA / chính sách mật khẩu / chính sách phiên KHÔNG cấu hình được qua API — phải sửa thẳng DB `company_security_policies` (đúng cách đã phải làm ở KI-027).",
      "Hai hướng vá (quyết định THIẾT KẾ, không phải một dòng): (a) bỏ `requiresReauth` nếu chưa có step-up thật; (b) coi singleton là resource: lấy `companyId` làm `resourceId` VÀ gán `req.reauthContext` ở một guard step-up có thật.",
    ],
    done_when: [
      "Chọn dứt khoát (a) hay (b) + ghi lý do vào `docs/DECISIONS/` nếu đổi ngữ nghĩa `needsObjectGrant`/`requiresReauth` (chạm permission engine = crown-jewel, FULL gate)",
      "LẬT ca ghim: `security-mailconfig-http.int-spec.ts:171` từ '403 với mọi actor' sang **ALLOW 2xx thật cho actor đủ quyền** — ca này sẽ ĐỎ CÓ CHỦ Ý khi bug được vá, TUYỆT ĐỐI không nới assert cho khớp (bài học tests-can-pin-a-hole-open)",
      "Thêm ca DENY THẬT sau khi có ALLOW: actor KHÔNG có `configure-security-policy:company` ⇒ 403; actor công ty khác ⇒ không đổi được policy công ty này",
      "Kiểm không leo thang: bản vá KHÔNG được nới `needsObjectGrant` cho các route nhạy cảm KHÁC có `:id` (bài học reviewer-proposed-fix-can-open-holes) — có ca hồi quy chứng minh route sensitive có `:id` vẫn đòi object-grant như cũ",
      "Ghi audit khi đổi chính sách bảo mật (append-only) + giữ nguyên BẤT BIẾN #4 (người gọi PATCH tự vào exempt-list, chống tự-khoá)",
      "Chạy THẬT ở LANE_DB; KI-025 gỡ dòng '`PATCH /settings/security-policy` không tính đã phủ' và KI-065 ĐÓNG ở RELEASE-02",
    ],
    notes: [
...

  {
    ...
    notes: [
      ...
      // ── Vòng plan 1 (19/08/2026, auto-loop wf_70ddd8c2-795): plan-reviewer BLOCK. 8 BLOCKING + 6 WARN.
      // 5 khẳng định nặng nhất đã ĐO LẠI TAY và ĐÚNG — ghi vào đây để vòng sau không phải tìm lại.
      "🚩 PLAN-BLOCK #1 (ĐÃ XÁC MINH) — `isReauthValid` (permission.decide.ts:145-149) đòi `reauthValidUntil instanceof Date`. Valkey CHỈ lưu string ⇒ nếu StepUpWindowService trả chuỗi ISO/số ms thì hàm này false VĨNH VIỄN: 403 im lặng, không exception, không log = tái diễn KI-065 nguyên xi. Unit spec với store GIẢ (trả Date) + int-spec ở LANE_DB (Valkey thường VẮNG ⇒ rơi fallback memory, Date native) đều KHÔNG chạy qua đường serialize/deserialize thật. BẮT BUỘC có spec codec: ghi rồi đọc qua stub trả STRING, assert `instanceof Date` VÀ `isReauthValid(...) === true`.",
      "🚩 PLAN-BLOCK #2 (ĐÃ XÁC MINH) — thứ tự deny ở permission.decide.ts: `deny-object-required` trả ở dòng 93-95 KHI KHÔNG có object ALLOW, TRƯỚC mọi kiểm tra reauth; `deny-reauth-required` chỉ trả khi ĐÃ có object ALLOW (dòng 71-78). ⇒ ca deny (userA→userB), (objectX→objectY), (cross-tenant) mà chỉ assert `403` sẽ XANH RỖNG vì lý do SAI. Mỗi ca deny phải GIỮ object grant đúng cho chủ thể/đối tượng đang đo và assert CHUỖI LÝ DO `deny-reauth-required`, không phải status.",
      "🚩 PLAN-BLOCK #3 (ĐÃ XÁC MINH) — `audit_logs.object_id` là cột **uuid** (db/schema/audit.ts:36). `resourceId` đến từ BODY client; thiếu `.uuid()` trong Zod ⇒ 22P02 ở tầng DB, và theo [[drizzle-wraps-pg-error-code-in-cause]] `err.code` undefined ⇒ **500 chứ không phải 400**. Ép `z.string().uuid()` + ca test 'resourceId không phải UUID ⇒ 400, KHÔNG ghi audit, KHÔNG tạo window'.",
      "🚩 PLAN-BLOCK #4 (ĐÃ XÁC MINH) — PermissionModule ↔ AuthModule đã là vòng `forwardRef` (permission.module.ts:107). ReauthGuard (ở PermissionModule) cần StepUpWindowService (ở AuthModule): inject sai chiều ⇒ DI trả `undefined` ⇒ guard không bao giờ ghi `reauthContext` ⇒ 403 CÂM. Phải (a) export StepUpWindowService trong AuthModule.exports, (b) `@Inject(forwardRef(...))`, (c) fail-fast `moduleRef.get()` trong `onModuleInit` (mẫu SecurityEventWriter, permission.module.ts:166) để DI hỏng thì app CRASH rõ ràng. Lane phải TUẦN TỰ L0→L1→L2→L3, cấm song song.",
      "🚩 PLAN-BLOCK #5 (ĐÃ XÁC MINH) — CẤM tái dùng `TwoFactorService.verifyChallenge` (two-factor.service.ts:264) cho step-up: (a) TOTP sai thì nó rơi xuống tiêu THẬT một recovery code (đánh dấu `used_at`); (b) nó `replayGuard.claim('totp-step', userId:step, 90)` ⇒ mã vừa dùng ĐĂNG NHẬP trong cùng time-step 30s bị coi là SAI khi step-up — vừa khó hiểu cho người dùng vừa là nguồn flake chắc chắn. ADR phải chốt tường minh (khuyến nghị: step-up KHÔNG nhận recovery code, dùng đường verify TOTP thuần).",
      "🚩 PLAN-BLOCK #6 — rate-limit step-up chưa chốt KHOÁ. Nhúng `ip` ⇒ sau cloudflared mọi `req.ip`=`::1` (KI-066) nên 'per-IP' thoái hoá thành bucket TOÀN CÔNG TY = một kẻ gõ sai khoá step-up của MỌI người. Nhúng `email` ⇒ mở đường quấy rối victim. Chốt: khoá `(companyId|userId)` lấy TỪ JWT, ngưỡng qua env có mặc định, `isLocked()` kiểm TRƯỚC khi gọi verify, thành công thì `reset(key)`. Ca test: bucket đang khoá ⇒ từ chối mà spy `verify` = 0 lần.",
      "🚩 PLAN-BLOCK #7 — `POST /auth/step-up` là ORACLE mật khẩu/TOTP MỚI cho phiên đã đăng nhập, cố ý nằm NGOÀI bucket login. Plan chỉ audit khi CẤP window ⇒ mọi lần thử SAI không để dấu vết pháp chứng. `audit_logs.result_status ∈ Success/Failure/Denied/Error` đã có sẵn (mig 0432) nên KHÔNG cần migration. Audit CẢ nhánh sai + dual-write `user_security_events` như 2FA đang làm.",
      "🚩 PLAN-BLOCK #8 — cổng chống tái diễn còn lỗ: khi `hasReauthWriter` lật true, lý do `no-step-up-writer` NHẢ cho MỌI route. Route có `:id` + ReauthGuard nhưng cặp (action,resourceType) không mint được window sẽ lại 403 CÂM = KI-065 lần hai. Cần MỘT registry cặp reveal-class trong src, hai đầu đọc CÙNG nguồn: endpoint step-up validate body theo nó, và `findUnreachableReauthRoutes` thêm lý do thứ tư `pair-not-mintable` + ca thử-ngược.",
      "⚠ WARN vòng 1: (a) lý do `no-reauth-guard` so khớp literal chuỗi 'ReauthGuard' ⇒ đổi tên/bọc mixin là xanh oan — ghim `ReauthGuard.name` ([[index-ratchet-must-pin-definition-not-name]]); (b) action audit phải theo tiền tố module (`auth.step_up_*`), không phải `step_up_granted`; (c) `STEP_UP_TTL_SEC` cần TRẦN, không chỉ mặc định; (d) fallback memory của Valkey làm window VÔ HÌNH giữa các instance đa-tiến-trình; (e) ca cross-tenant int-spec chứng minh YẾU (userId cũng đổi theo companyId) — bằng chứng thật phải là bảng bộ-5 đổi ĐÚNG MỘT thành phần mỗi lần; (f) `POST /auth/step-up` risk≥5 ⇒ phải chạy lại `route-http-coverage.e2e-spec.ts` (MIN_COVERED_COUNT=383, MAX_UNCOVERED_HIGH_RISK=0) sau khi viết int-spec.",
      "✅ QUYẾT ĐỊNH CHỦ DỰ ÁN 19/08/2026 (chốt, vòng plan 2 KHÔNG được mở lại): (D1) yếu tố step-up = **CHỈ TOTP**, qua đường verify THUẦN tự viết — TUYỆT ĐỐI KHÔNG gọi `TwoFactorService.verifyChallenge`. (D2) step-up **KHÔNG nhận recovery code**: đường code mới không được chạm bảng `user_recovery_codes`, và phải có ca test chứng minh sai TOTP KHÔNG tiêu recovery code nào (đếm hàng `used_at` trước/sau). (D3) registry cặp reveal-class khởi tạo **RỖNG** — WO này chỉ xây NĂNG LỰC, KHÔNG gắn `requiresReauth` lên bất kỳ route nào; việc gắn cờ là WO riêng khi có nhu cầu thật.",
      "⚙ HỆ QUẢ CỦA D3 (làm BLOCK #8 dễ hơn, không khó hơn): registry rỗng ⇒ lý do thứ tư `pair-not-mintable` của `findUnreachableReauthRoutes` bắt được MỌI route dám khai `requiresReauth`. Đó là hành vi ĐÚNG và là cổng chống tái diễn KI-065: chừng nào chưa có ai thêm cặp vào registry thì không route nào được phép khai cờ. Ca thử-ngược bắt buộc: thêm một route giả khai `requiresReauth` ⇒ cổng phải ĐỎ với lý do `pair-not-mintable`.",
      "⚙ HỆ QUẢ CỦA D1 — replay marker: step-up phải có namespace replay RIÊNG (KHÔNG dùng lại `totp-step` của login) để (a) chính step-up single-use trong một time-step 30s, (b) mã vừa dùng đăng nhập KHÔNG bị coi là sai khi step-up (nguồn flake mà WARN vòng 1 chỉ ra). Khoá Valkey mới PHẢI đi qua `valkey-key.ts` để mang envScope (KI-067) — cổng runtime sẽ ném nếu quên.",
      "⚙ HỆ QUẢ CỦA D1 — user chưa bật 2FA: `POST /auth/step-up` phải trả lỗi TƯỜNG MINH có mã (dạng `AUTH-ERR-*`, thông điệp 'cần bật 2FA trước'), KHÔNG phải 403 câm và KHÔNG phải 500. Có ca test cho nhánh này. Vì D3 để registry rỗng nên không route nào phụ thuộc step-up ⇒ nhánh này không chặn nghiệp vụ nào.",
      // ── Vòng plan 2 (19/08, auto-loop wf_917673f9-b40): BLOCK lần hai. 7 BLOCKING mới; 5 đã đo lại tay.
      "🚩V2-BLOCK#1 (ĐÃ XÁC MINH) — `ValkeyService.set()` TRẢ `true` KHI VALKEY KHÔNG CẤU HÌNH (permission/valkey.service.ts:99 `if (!this.client) return true;`). Builder viết `if (!await set(...)) throw` sẽ thấy `true` ở môi trường thiếu VALKEY_URL ⇒ 200 kèm reauthValidUntil, window KHÔNG nằm ở đâu cả, guard đọc null ⇒ 403 CÂM = KI-065 nguyên xi. Nhánh lưu PHẢI chọn theo `valkey.isEnabled()` (mẫu LoginRateLimiter/ReplayGuardService), TUYỆT ĐỐI không suy ra thành công từ giá trị trả về của set().",
      "🚩V2-BLOCK#2 (ĐÃ XÁC MINH — LIÊN ĐỚI [[S10-FND-VALKEYSCOPE-2]]) — `ReplayGuardService.claim()` (auth/replay-guard.service.ts:60-61) ghi kép VÔ ĐIỀU KIỆN cho MỌI marker: `setNx(replayKey(...))` VÀ `setNx(legacyReplayKey(...))` = `replay:<marker>:<rest>` KHÔNG có envScope. `LEGACY_UNSCOPED_PATTERNS` (common/valkey/valkey-key.ts:179-182) CHỈ miễn trừ `2fa-jti|totp-step`. ⇒ marker step-up MỚI: ở test `assertKeysScoped` NÉM ⇒ 500; ở PROD cổng TẮT (valkey-key.ts:225 `if NODE_ENV!==test return`) nên ghi khoá unscoped DÙNG CHUNG 4 môi trường = mở lại đúng lỗ KI-067 vừa bịt. Ghi kép legacy cho marker CHƯA TỪNG TỒN TẠI không có giá trị chuyển tiếp nào, chỉ toàn hại.",
      "🚩V2-BLOCK#3 (HỒI QUY KẾ HOẠCH) — plan vòng 2 ĐÁNH RƠI đúng thứ vòng 1 đã chốt: không nhắc `AuthModule.exports += StepUpWindowService`, không nhắc fail-fast `moduleRef.get()` trong onModuleInit, và mục GATE&ROUTING còn ghi có thể song song L2 MÂU THUẪN với cấm song song đã ghim. ⇒ tín hiệu: thêm vòng plan KHÔNG hội tụ, planner rơi ràng buộc đã chốt.",
      "🚩V2-BLOCK#4 — cổng `no-reauth-guard` mới chỉ kiểm TỒN TẠI ReauthGuard. NestJS chạy guard theo tầng rồi theo thứ tự khai ⇒ `@UseGuards(PermissionGuard, ReauthGuard)` đảo thứ tự, hoặc PermissionGuard ở CẤP CLASS + ReauthGuard ở CẤP ROUTE, thì PermissionGuard quyết định TRƯỚC khi window được ghi ⇒ 403 vĩnh viễn mà cổng vẫn XANH. Dữ liệu để kiểm ĐÃ CÓ (route-census.ts:151,186-187 tách theo tầng + giữ thứ tự). Phải đo THỨ TỰ, không chỉ tồn tại.",
      "🚩V2-BLOCK#5 — ngữ nghĩa cửa sổ bỏ ngỏ: ADR để builder tự chọn dùng-lại-được-trong-TTL hay single-use nhưng KHÔNG acceptanceCheck nào phụ thuộc lựa chọn đó ⇒ chọn gì cũng đạt = nghiệm thu rỗng ở đúng thuộc tính an ninh quan trọng nhất. Nếu dùng-lại thì phải có ca đường ĐỌC KHÔNG gia hạn TTL (chống sliding-window biến TTL ngắn thành phiên vô hạn).",
...

  {
    ...
    notes: [
      ...
      "GREEN 13/08/2026: `JsonConsoleLogger` (kế thừa `ConsoleLogger` built-in Nest 11, `json:true`) đăng ký MỘT LẦN ở `main.ts` — Nest override `Logger.staticInstanceRef` toàn cục nên KHÔNG cần sửa từng file gọi log. `ops-log-window.mjs` đọc SONG SONG cả JSON mới lẫn Nest văn xuôi cũ (tail log lẫn hai định dạng ngay sau restart vẫn đếm đúng). Bằng chứng BẺ HỎNG: bơm 19/20/200 dòng lỗi JSON giả qua evaluate()+exitCodeFor() thật — severity/exit code đổi đúng ngưỡng. 34/34 test `ops-log-window.test.mjs` (node --test) + 100/100 test `src/common` (vitest) + typecheck sạch. Ngưỡng `ERROR_SPIKE`/bảng rule ở `ops-alert-rules.mjs` KHÔNG đổi (đã xác minh 0 diff) nên KHÔNG cần sửa ngưỡng — nhưng RELEASE-09 §5 dòng 279 + §7 dòng 358 trích trạng thái `18.4 Logging` VẪN ghi KI-009 'CHƯA'/8/9, mâu thuẫn trực tiếp với RELEASE-02 đã ĐÓNG KI-009 cùng ngày; vá nốt ở lane S10-FND-JSONLOG-1-FIX-DOC9.",
...

  {
    ...
    done_when: [
      ...
      "Int-spec RED-trước, chạy THẬT ở LANE_DB (`bash scripts/lane-db-setup.sh hrstatus` → `export LANE_DB=mediaos_hrstatus`; thiếu LANE_DB ⇒ SKIP = xanh-giả): (a) thiếu `endDate` khi sang resigned ⇒ 4xx VÀ status không đổi VÀ 0 hàng history VÀ 0 audit; (b) đổi thành công ⇒ `status` + `end_date` + 1 hàng `employee_status_histories` + 1 audit 'change-status' trong MỘT lượt gọi; (c) transition sai (terminated→active) ⇒ 422 + 0 ghi; (d) thiếu quyền ⇒ 403 + 0 audit; (e) 2-tenant không đổi chéo",
    ...
    notes: [
      ...
      "📌 ĐỔI TÊN KHI THI CÔNG (17/08/2026, sau 2 vòng plan-review): field là **`endDate`**, KHÔNG phải `effectiveDate` như WO seed. Lý do đo trên SPEC-03: `:1887` (§14.6 bước 4) 'Nhập ngày hiệu lực' áp cho MỌI transition và `:577` (§11.4 probation→Official) cũng vậy; còn `:1889` (bước 6) / `:593` (§11.5) mới là 'ngày nghỉ việc' — HAI khái niệm. WO này chỉ thi công được vế 'ngày nghỉ việc' (cột `employee_profiles.end_date` đã có); vế 'ngày hiệu lực cho mọi transition' cần cột mới trên `employee_status_histories` (hiện chỉ có `changed_at`) ⇒ HOÃN sang WO có migration. Đặt tên `effectiveDate` cho cái này sẽ CHIẾM CHỖ khái niệm kia trên một schema `.strict()`, ép WO sau phải đổi tên phá hợp đồng. Tên `effectiveDate` được GIỮ CHỖ.",
      "📌 Hợp đồng ép HAI CHIỀU (không chỉ 'bắt buộc khi exit'): non-exit mà gửi `endDate` ⇒ 400. Nhận rồi bỏ đi = 'UI hứa, backend không đọc'. Ép ở CẢ Zod (biên HTTP) lẫn service (đường gọi trực tiếp), assert đặt SAU `assertWriteScope` TRƯỚC `withTenant`.",
...

  {
    ...
    notes: [
      ...
      "📌 Đường thoát khi có người bị khoá oan trong lúc chờ WO này: xoá khoá trong Valkey — `docker exec mediaos-valkey valkey-cli DEL 'rl:production:mediaos:ip:{slug}|{email}|{ip}:lock' 'rl:production:mediaos:acct:{slug}|{email}:cnt'` (hình dạng ĐỔI từ 18/08 — S10-FND-VALKEYSCOPE-1 thêm `envScope`; ĐẾM TRƯỚC bằng `--scan --pattern` vì `DEL` trả 0 cũng là hình dạng của pattern SAI, xem KI-067) (đã dùng thật 17/08 để mở khoá cho tài khoản bị kẹt). Tạo lại tài khoản KHÔNG gỡ được khoá — khoá theo email.",
    ],
  },

  {
    ...
    paths: [
      ...
      "apps/api/src/auth/replay-guard.service.ts",
      "apps/api/src/auth/replay-guard.service.spec.ts",
      "apps/api/src/permission/permission.cache.ts",
      "apps/api/src/permission/permission.cache.spec.ts",
      "apps/api/src/permission/permission.g3-4.spec.ts",
      "apps/api/src/permission/valkey.service.ts",
      "apps/api/src/permission/valkey.service.spec.ts",
      "apps/api/src/common/idempotency/**",
      "apps/api/src/realtime/ws-adapter-config.ts",
      "apps/api/src/realtime/ws-adapter-config.spec.ts",
      "apps/api/src/chat/chat-typing.service.ts",
      "apps/api/src/chat/chat-typing.service.spec.ts",
      "apps/api/src/chat/chat-call-cooldown.service.ts",
      "apps/api/src/chat/chat-call-ice.service.ts",
      "apps/api/src/chat/chat-call-ice.service.spec.ts",
      "apps/api/src/chat/chat-call-cooldown.service.spec.ts",
      "apps/api/src/me/me.constants.ts",
      "apps/api/src/me/me-training.service.spec.ts",
      "apps/api/test/**",
      "docs/RELEASE/RELEASE-02_Known_Issues_MVP.md",
      ".env.example",
      "harness/backlog.mjs",
    ...
    done_when: [
      "MỌI không gian khoá Valkey đi qua MỘT chỗ dựng khoá có `envScope` — file mới `apps/api/src/common/valkey/valkey-key.ts` IMPORT `resolveEnvScope` từ `realtime/ws-adapter-config.ts` (KHÔNG copy, KHÔNG viết hàm thứ hai). Hình dạng chuẩn `{namespace}:{envScope}:{phần còn lại}` như `chat:presence` đã đúng; với `idem:` thì envScope nằm NGOÀI sha256 để vận hành SCAN đọc được bằng mắt.",
      "PHẠM VI = 16 họ khoá đã ĐO 18/08 (WO gốc đếm thiếu, chỉ ghi 9): `rl:ip` · `rl:acct` · `rl:forgot:ip` · `rl:forgot:acct` (login-rate-limiter.ts:41,46,56,60) · `2fa|` (auth.service.ts:469) · `2fa-enable|` (two-factor.service.ts:183) · `2fa-disable|` (auth.service.ts:590) · `change-pw|` (auth.service.ts:625) · `replay:*` (replay-guard.service.ts:64, phủ cả `2fa-jti:` lẫn `totp-step:`) · `perm:cap` · `perm:obj` (permission.cache.ts:36,45) · `idem:*` (idempotency.interceptor.ts:129-133) · `chat:typing` (chat-typing.service.ts:82) · `chat:cooldown` (chat-call-cooldown.service.ts:64) · `chat:ice-turn-reject` (chat-call-ice.service.ts:217) · `me:training` (me/me.constants.ts:151). `chat:presence` + kênh socket.io ĐÃ ĐÚNG — giữ nguyên. Bốn khoá dạng ống hiện nằm ở namespace GỐC (không cả tiền tố `rl:`) — gom vào họ `rl:` luôn, đổi hình dạng ở đây MIỄN PHÍ vì khoá cũ dù sao cũng mồ côi lúc deploy.",
      "Test đóng đinh distinctness theo ma trận (env × company × user × resource), KHÔNG chỉ lật envScope: với 4 bộ env thật (production:mediaos · development:mediaos_dev · development:mediaos · test:mediaos_<lane>) MỌI builder cho chuỗi đôi một khác nhau (`new Set(...).size === n`), CỘNG assert tường minh khoá CHỨA companyId và userId (mất companyId = MỘT ô cache dùng chung cho mọi công ty, mà perm cache đứng TRƯỚC RLS nên DB không cứu được), CỘNG ca ALLOW đối chứng (chống deny xanh-rỗng). Mở rộng đúng khuôn `ws-adapter-config.spec.ts:65-87`, không bịa khuôn mới.",
      "🔴 Đường INVALIDATE dùng CÙNG hàm dựng khoá với đường GHI: `permission.cache.ts:164` (`invalidateUser` → `del(capKey(...))`) phải đổi ĐỒNG THỜI với `capKey`. Spec MỚI `permission.cache.spec.ts` (colocated, hiện KHÔNG tồn tại) với fake ValkeyService dạng Map: ca ghi-rồi-invalidate chứng minh DEL trúng ĐÚNG chuỗi vừa SET (lệch một chữ = grant cũ sống tới hết TTL 300s = leo thang quyền IM LẶNG) + ca hai envScope không xoá trúng khoá của nhau.",
      "Census tĩnh: quét `apps/api/src/**/*.ts` + `apps/api/src/**/*.spec.ts` + `apps/api/test/**` không còn literal khoá Valkey dựng tại chỗ ngoài allowlist. Regex phải NEO ĐẦU literal theo đúng tiền tố khoá và loại TƯỜNG MINH nhóm tên-sự-kiện socket.io (`chat:typing`/`chat:message`/`chat:read`/`chat:room`/`chat:call`… còn là TÊN SỰ KIỆN ở realtime-emitter, gateway, spec, comment ⇒ quét thô sẽ đỏ oan rồi bị nới tới vô nghĩa). Allowlist ghi dạng file+lý do NGAY TRONG acceptance, bắt buộc có `realtime/chat-presence-reader.service.ts:43` (literal khoá HỢP LỆ) cùng dedupeKey NOTI, storage key, file-policy Map, module setting.",
      "🔴 CƯỠNG CHẾ RUNTIME — PHẠM VI RÚT GỌN (chốt 18/08 sau vòng 2 plan-review): assert trong ValkeyService CHỈ sống khi `NODE_ENV==='test'` (ném `ValkeyKeyScopeError`). KHÔNG nhánh warn ở development/production · KHÔNG sổ vi phạm cấp module · KHÔNG `setupFiles` mới · KHÔNG đụng `globalSetup` (đang là db-fence KI-028) · KHÔNG đổi một byte hành vi PROD. Lý do cắt: cổng-ở-production kéo theo bốn tầng phụ thuộc (miễn trừ legacy, sổ vi phạm, quota warn, PII trong sổ) mà mỗi tầng lại đẻ lỗ mới; và hợp đồng never-throws của ValkeyService có 6 call site KHÔNG bọc try ⇒ giữ cổng ngoài production là cách duy nhất chắc chắn không biến một khoá sót thành login 500. Cưỡng chế thật nằm ở test/CI — phải GHI ĐÚNG NHƯ VẬY trong KI-067, đừng để người sau tưởng PROD đang được bảo vệ.",
      "🔴 CỔNG PHẢI NEO SEGMENT, CẤM `includes()`: `development:mediaos` là TIỀN TỐ của `development:mediaos_dev` ⇒ khoá của dev-online sẽ LỌT cổng khi tiến trình chạy ở dev local — đúng cặp môi trường WO sinh ra để tách. Kiểm bằng `key.startsWith(namespace + ':' + scope + ':')` hoặc regex `(^|:)scope(:|$)`. Bắt buộc một ca test đánh THẲNG VÀO CỔNG (không chỉ vào builder) cho cặp `development:mediaos` vs `development:mediaos_dev`: khoá của env kia phải BỊ BẮT. Assert đứng TRƯỚC nhánh `if (!this.client) return` để lượt test không có Valkey vẫn là một lượt census.",
      "🔴 2FA: ĐỌC KÉP **VÀ GHI KÉP** đúng MỘT chu kỳ deploy (owner chốt đọc kép 18/08; ghi kép là vế còn thiếu do vòng 2 chỉ ra). Đọc kép để chiều TIẾN an toàn; GHI kép (ghi cả khoá mới lẫn khoá cũ) để chiều ROLLBACK đối xứng — chỉ ghi khoá mới thì sau khi lùi bản, mọi jti/`totp-step` tiêu thụ trong cửa sổ chạy bản mới sẽ claim LẠI được trong TTL 600s = hở đúng tính single-use mà đọc kép dựng ra để chặn. `legacyReplayKey()` khai trong `valkey-key.ts`, nằm trong allowlist census VÀ trong miễn trừ DUY NHẤT của cổng runtime (regex `^replay:(2fa-jti|totp-step):`), docblock ghi hạn gỡ. Ca test: (a) jti tiêu ở khoá CŨ ⇒ sau deploy claim=false; (b) jti tiêu ở bản MỚI ⇒ khoá CŨ cũng có marker (rollback-safe); (c) mọi key chưa scoped KHÁC vẫn ném dưới NODE_ENV=test. Seed WO gỡ hai đường legacy ở PR sau.",
      "Thứ tự land: assert runtime là BƯỚC CUỐI, sau khi cả 16 họ khoá đã chuyển (CẤM bật sớm rồi để 15 họ còn lại làm đỏ chính mình — áp lực sửa đỏ sẽ đẩy lane đi nới cổng). Bước đó phải RED-proof bằng vi phạm CỐ Ý (suite ĐỎ), và đo RIÊNG với census tĩnh — hai cơ chế khác nhau, không gộp bằng chứng.",
      "Spec đang đóng đinh hình dạng khoá CŨ phải sửa sang so với chuỗi do builder THẬT sinh ra, CẤM hạ xuống `expect.any(String)`/`stringContaining` lỏng (lớp tests-can-pin-a-hole-open): `permission.g3-4.spec.ts:239,269` · `replay-guard.service.spec.ts:55` · `chat-typing.service.spec.ts:103` · `chat-call-ice.service.spec.ts:407`.",
      "`currentEnvScope()` memo LAZY (gọi `loadEnv()` lúc import sẽ giết fixture int-spec — lớp env-schema-floor-breaks-test-fixtures) VÀ có đường reset `__resetEnvScopeForTests()`; `NODE_ENV` phải đọc LAZY mỗi lần kiểm, không memo. Lý do: vitest chỉ làm mới module registry theo FILE, không theo ca ⇒ không reset thì ca sau chạy bằng scope mồi từ ca trước và xanh OAN. Có ca pin đổi `NODE_ENV` giữa hai ca TRONG CÙNG một file.",
      "Acceptance KHÔNG neo SỐ DÒNG: đo `grep -c 'export function resolveEnvScope' apps/api/src` == 1. Kèm docblock ở `ws-adapter-config.ts` báo `common/valkey/valkey-key.ts` phụ thuộc hàm này — hướng common/ → realtime/ là NGHỊCH TẦNG và CỐ Ý (từ chối move vì churn), ghi ra để lần sau không ai sửa-cho-đúng-tầng bằng một bản sao thứ hai.",
      "🔴 RUNBOOK CŨ PHẢI ĐỔI CÙNG PR — nếu không, thủ tục MỞ KHOÁ KHẨN CẤP chết IM LẶNG: `harness/backlog.mjs` (notes của S10-AUTH-IPTRUST-1, lệnh `valkey-cli DEL 'rl:ip:{slug}|{email}|{ip}:lock' 'rl:acct:{slug}|{email}:cnt'` — đã dùng THẬT 17/08) và `.env.example` (mô tả hình dạng `rl:ip:{slug}|{email}|{ip}`) phải đổi sang hình dạng có envScope. MỌI lệnh dọn/mở khoá trong KI-067 phải SCAN/EXISTS ĐẾM TRƯỚC rồi mới DEL, kèm câu 'DEL trả 0 = pattern SAI, KHÔNG phải đã sạch' (lớp known-issue-workaround-may-never-have-run).",
      "KI-067 vào RELEASE-02 (mở + đóng cùng PR, khuôn KI-066) nêu đủ: db0 dùng chung 4 môi trường · 16 họ khoá không scoped · 253/288 khoá là cache QUYẾT ĐỊNH QUYỀN · bằng chứng khoá đăng nhập chéo môi trường. PHẦN VẬN HÀNH viết sẵn: (a) mọi lockout `rl:*` bị VÔ HIỆU ngay lúc deploy + lệnh DEL NEO-SLUG chỉ khớp khoá CŨ (`rl:ip:funtime|*`…) + CẤM `FLUSHDB` (giết presence + trạng thái adapter socket.io) + nói rõ pattern neo-slug khớp khoá cũ của CẢ 4 môi trường vì chung db0 chung slug — đó là chủ ý; (b) `idem:*` mồ côi ⇒ client retry vắt qua mốc deploy sẽ CHẠY THẬT lần hai (đẻ bản ghi trùng), KHÔNG phải phát lại — đừng gọi là 'tự lành'; (c) rollback làm khoá MỚI mồ côi + reset lockout LẦN HAI, kèm lệnh dọn chiều ngược; (d) một dòng nói rõ cổng runtime CHỈ sống ở test/CI.",
      "KHÔNG đụng (ngoài phạm vi): ngưỡng rate-limit · TRUST_PROXY (đã đóng ở S10-AUTH-IPTRUST-1) · migration/schema · FE · `vitest.config.ts` globalSetup (db-fence KI-028) · không thêm `setupFiles`. Không tách môi trường bằng số db Valkey hay `keyPrefix` của ioredis (hai cơ chế cô lập song song = không ai biết cái nào đang cưỡng chế thật).",
      "Verify vùng đỏ: `pnpm --filter @mediaos/api typecheck` xanh · `bash scripts/lane-db-setup.sh valkeyscope` → `export LANE_DB=mediaos_valkeyscope` → `bash harness/check.sh --all` XANH THẬT (không phải 'XANH KHÔNG ĐỦ BẰNG CHỨNG') · test chạy với `TURBO_FORCE=1`. Coverage: CI KHÔNG chạy `--coverage` (ratchet chạy tay) ⇒ ĐO MỘT LẦN bằng `pnpm --filter @mediaos/api test -- --coverage --coverage.include='src/common/valkey/**'` rồi dán số vào PR — đừng hứa ngưỡng không ai cưỡng chế. Bằng chứng hành vi nằm ở unit spec với fake store dạng Map (VALKEY_URL thường VẮNG trong test và các consumer đều gác bằng `isEnabled()`), KHÔNG ở int-spec.",
      "FULL gate theo CLAUDE.md §6: security-reviewer + database-reviewer + silent-failure-hunter (+ santa-method cho lane crown) — hoặc ghi TƯỜNG MINH một dòng miễn trừ `database-reviewer` để người chốt vùng đỏ biết mình đang miễn trừ cái gì (đây là tầng cache đứng TRƯỚC RLS).",
      "🔴 CHIỀU ROLLBACK CHO CACHE QUYỀN (vòng 3): sau deploy `invalidateUser` chỉ DEL khoá ĐÃ scoped nên 253 khoá `perm:cap` hình dạng CŨ vẫn sống hết TTL 300s — thu hồi quyền trong cửa sổ đó KHÔNG chạm khoá cũ, và lùi bản trong cửa sổ ⇒ bản cũ đọc lại grant TRƯỚC thu hồi (im lặng: không log, không exception). Chọn MỘT và ghi rõ: (i) `invalidateUser` DEL kèm khoá legacy đúng một chu kỳ (hình dạng legacy vào miễn trừ cổng + allowlist census + test assert DEL nhận CẢ HAI chuỗi), HOẶC (ii) runbook chạy NGAY lúc deploy: SCAN/đếm rồi DEL riêng `perm:cap:`/`perm:obj:` hình dạng CŨ (pattern KHÔNG khớp khoá mới) + câu CẤM rollback trong 300s nếu chưa dọn.",
      "🔴 BẢNG CHÂN TRỊ CỦA GHI-KÉP `setNx` (vòng 3): code hiện tại `replay-guard.service.ts:39-45` là `if (res !== null) return res; return claimMem(...)`. Với hai lượt setNx, tổ hợp `new=true, legacy=null` (Valkey rớt giữa chừng) rơi xuống memory và trả TRUE cho jti có thể đã tiêu ở khoá cũ = FAIL-OPEN. Quy định tường minh: claim thành công ⇔ `newRes === true && legacyRes === true`; bất kỳ `null` nào ⇒ fail-soft xuống memory NHƯNG memory khoá theo chuỗi ĐÃ scoped; thứ tự ghi mới-trước-legacy-sau là CỐ Ý; TTL hai vế bằng nhau. Bắt buộc có ca test `legacy setNx → null`.",
      "🔴 GHIM HÌNH DẠNG MỚI CỦA `replay:` (vòng 3): miễn trừ `^replay:(2fa-jti|totp-step):` sẽ nuốt luôn khoá MỚI nếu builder chọn `replay:2fa-jti:{envScope}:{jti}` ⇒ cả họ `replay:` vĩnh viễn nằm ngoài cổng = cổng XANH RỖNG. Hình dạng MỚI bắt buộc là `replay:{envScope}:{2fa-jti|totp-step}:…`, và có ca test chứng minh một khoá `replay:` chưa scoped mà KHÔNG khớp hai hình dạng legacy thì VẪN BỊ NÉM.",
      "🔴 CENSUS PHẢI BÓC COMMENT TRƯỚC KHI KHỚP (vòng 3): regex neo-đầu sẽ khớp VĂN XUÔI đã tồn tại trong đúng phạm vi quét — `login-rate-limiter.ts:51-52` · `permission.cache.ts:20-21` · `login-rate-limiter.spec.ts:131` · `forgot-password-rate-limit.spec.ts:61,100` · `test/integration/auth-toprisk-http.int-spec.ts:25` (backtick trong comment, khớp y hệt literal thật — lớp gitleaks-prose-colon-false-positive). Bóc comment/docblock trước khi khớp (hoặc chỉ khớp literal ở vị trí đối số) và liệt kê đủ dòng còn sót vào allowlist file+lý do. Không làm thì lane gặp ~6 đỏ oan ngay vòng đầu rồi sẽ nới regex tới vô nghĩa.",
      "MỘT PR DUY NHẤT, CẤM deploy giữa các lane (vòng 3): thủ tục mở khoá khẩn cấp đã DÙNG THẬT 17/08 — nếu phần đổi hình dạng `rl:*` land và deploy TRƯỚC phần cập nhật runbook thì lệnh mở khoá tài liệu hoá SAI trong cửa sổ đó (`DEL` trả 0, người vận hành đọc thành đã-sạch).",
      "Cổng phải IMPORT `currentEnvScope()` từ `common/valkey/valkey-key.ts`, KHÔNG tự gọi `resolveEnvScope(loadEnv(), …)` lần thứ hai (vòng 3): hai bên sẽ lệch đúng lúc test lật `NODE_ENV` hoặc gọi `__resetEnvScopeForTests()` ⇒ cổng so với scope KHÁC scope đã dựng khoá (đỏ giả hoặc xanh oan). Phép đo: `grep -c 'resolveEnvScope(' apps/api/src` == đúng 2 call site (`resolveValkeyChannelKey` + `currentEnvScope`). Neo segment phải ESCAPE envScope (tên DB có thể chứa `.`/`-`) hoặc dùng `key.split(':')` / `startsWith(prefix + ':')`.",
      "SEED WO GỠ WORKAROUND NGAY TRONG PR NÀY (vòng 3): thêm WO tiếp nối vào `harness/backlog.mjs` — gỡ `legacyReplayKey()`, gỡ miễn trừ regex của cổng, gỡ dual-write, gỡ DEL legacy của `invalidateUser`; điều kiện `sau ≥1 chu kỳ deploy ổn định`. Docblock hạn-gỡ KHÔNG đủ (lớp known-issue-workaround-may-never-have-run). CẤM sửa-đỏ bằng cách bọc try/catch quanh call site ValkeyService hoặc thêm mục allowlist: mọi mục allowlist MỚI phải do người chốt vùng đỏ duyệt (anti-bandaid-guard chỉ bắt `catch {}` rỗng, không bắt `catch { log }`).",
    ],
    notes: [
      ...
      "📌 HAI VÒNG plan-review đã BLOCK (wf_d26482a4-7f6 và wf_1ed558c4-2c7 — 748k token, 0 dòng code). Kế hoạch vòng 1 ở `docs/plans/S10-FND-VALKEYSCOPE-1.md` (GAP-ANALYSIS 16 họ khoá còn dùng được). Toàn bộ phát hiện của hai vòng ĐÃ được đối chiếu vào `done_when` ở trên — LẬP KẾ HOẠCH BÁM done_when, đừng dựng lại từ đầu và đừng mở rộng cơ chế: vòng 2 block chính vì cổng-runtime-ở-production (thứ Đội 1 tự thêm, ngoài chữ WO) đẻ ra bốn tầng phụ thuộc mới. Nếu vòng 3 còn block ở kế hoạch thì DỪNG lập kế hoạch, chuyển sang code+test trực tiếp (lớp plan-review-rounds-inject-new-holes).",
      "⚠️ CẤM phát biểu ở VERIFY rằng mọi int-spec đi qua Valkey là một lượt census: `VALKEY_URL` thường VẮNG trong test và consumer đều gác bằng `isEnabled()` (login-rate-limiter.ts:115-117, replay-guard.service.ts:39, chat-call-cooldown.service.ts:101) ⇒ phần lớn đường không hề gọi ValkeyService. Nghiệm thu dựa vào int-spec sẽ PASS RỖNG.",
      "📌 VÒNG 3 (wf_9633557d-82c) BLOCK tiếp — tổng 3 vòng, ~1.02M token, 0 dòng code. Toàn bộ phát hiện đã vào done_when. DỪNG lập kế hoạch qua auto-loop (lớp plan-review-rounds-inject-new-holes): triển khai TUẦN TỰ bám done_when, review bằng gate FULL sau khi có diff thật. Phát hiện vòng 3 đáng giá nhất: lỗ `perm:cap` chiều rollback (mục cùng tên) và fail-open trong bảng chân trị `setNx` — hai thứ chỉ lộ ra khi đã có hình hài code, nên tiếp tục vá kế hoạch trên giấy là sai công cụ.",
    ],
  },

  {
    // Seed 18/08/2026 TRONG PR của S10-FND-VALKEYSCOPE-1: chu kỳ chuyển tiếp đọc-kép/ghi-kép + DEL legacy
    // + miễn trừ cổng runtime là workaround CÓ HẠN. Docblock "sẽ gỡ sau" một mình KHÔNG đủ (bài học
    // known-issue-workaround-may-never-have-run) nên việc gỡ có Work Order riêng ngay từ đầu.
    id: "S10-FND-VALKEYSCOPE-2",
    module: "FOUNDATION",
    layer: "BE",
    title:
    ...
    paths: [
      ...
      "apps/api/src/common/valkey/valkey-key-census.spec.ts",
      "apps/api/src/auth/replay-guard.service.ts",
      "apps/api/src/auth/replay-guard.service.spec.ts",
      "apps/api/src/permission/permission.cache.ts",
      "apps/api/src/permission/permission.cache.spec.ts",
    ...
    skills: ["security-review"],
    depends_on: ["S10-FND-VALKEYSCOPE-1"],
    src: [
      "Ba workaround land cùng S10-FND-VALKEYSCOPE-1, mỗi cái đóng một cửa sổ CHỈ tồn tại quanh mốc deploy: (1) `ReplayGuard.claim` đọc+ghi CẢ `replay:{envScope}:{marker}:…` lẫn `replay:{marker}:…`; (2) `CachedPermissionRepository.invalidateUser` DEL kèm `legacyPermCapKey`; (3) `LEGACY_UNSCOPED_PATTERNS` trong `valkey-key.ts` cho hai họ đó đi qua cổng runtime.",
      "Giá đang trả: mỗi lần claim 2FA tốn 2 round-trip Valkey (đường bước-2 đăng nhập), và cổng runtime có một cửa hẹp mà mọi khoá `replay:`/`perm:` chưa scoped lọt qua được.",
    ],
    done_when: [
      "ĐIỀU KIỆN MỞ: đã qua ≥1 chu kỳ deploy PROD ổn định kể từ khi S10-FND-VALKEYSCOPE-1 lên, VÀ `valkey-cli --scan --pattern 'replay:2fa-jti:*'` + `'perm:cap:*'` (hình dạng CŨ, không có envScope) trả 0 dòng. Dán số đo vào PR.",
      "Gỡ `legacyReplayKey`, `legacyPermCapKey`, `legacyPermObjKey` khỏi `valkey-key.ts` + gỡ `LEGACY_UNSCOPED_PATTERNS` (mảng RỖNG hoặc xoá hẳn nhánh miễn trừ trong `assertKeysScoped`).",
      "`ReplayGuard.claim` còn MỘT lượt `setNx`; ca test đọc/ghi kép chuyển thành ca khẳng định CHỈ ghi khoá mới. `invalidateUser` DEL đúng một khoá; sửa assert ở `permission.g3-4.spec.ts` cho khớp (so chuỗi THẬT, KHÔNG nới assert).",
      "Ca test của cổng: khoá `replay:`/`perm:` chưa scoped giờ PHẢI bị ném (trước đây được miễn trừ) — chứng minh cửa hẹp đã đóng.",
      "KI-067 cập nhật một dòng: chu kỳ chuyển tiếp đã kết thúc, ngày nào.",
    ],
    notes: [
