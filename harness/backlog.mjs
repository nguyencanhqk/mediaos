...
export const backlog = [
  ...
  },
  {
    ...
    layer: "FULL",
    title:
      "Ảnh bìa cho công việc — chọn từ TỆP ĐÃ ĐÍNH KÈM (file_links linkType='Cover'), hiện trên thẻ board + màn chi tiết",
    zone: "red",
    status: "todo",
    paths: [
      "apps/api/src/tasks/**",
    ...
    src: [
      ...
      "KHÔNG CẦN MIGRATION: tái dùng file_links polymorphic đã có — linkType='Cover' + isPrimary=true; unique index uq_file_links_primary_per_entity_type đã ép ĐÚNG MỘT bìa cho mỗi task. Task hiện hard-code linkType 'Attachment' + isPrimary false (task-file.service.ts)",
      "TÍNH CHẤT AN TOÀN THEN CHỐT (thiết kế để giảm bề mặt IDOR): chỉ nhận fileId ĐÃ là Attachment SỐNG của CHÍNH task đó (repo.findLinkedFileTx) ⇒ đặt bìa KHÔNG cấp quyền đọc file mới, chỉ đánh dấu một tệp đã truy cập được. Kèm kiểm image/* + Uploaded + scan≠Infected",
      "Đọc: mirror khuôn avatar — thêm FileRepository.findVerifiedTaskCoversTx(companyId, taskIds, tx) trả {taskId,fileId,storagePath} lọc moduleCode='TASK'/entityType='task'/linkType='Cover' + image/Uploaded/không-Infected, rồi ký qua storage adapter. Fail-soft: ký lỗi ⇒ null ⇒ thẻ không có bìa, KHÔNG 500 board",
      "Ghi: FilesService.link (linkType 'Cover', isPrimary true) + FilesService.unlink cho bìa cũ TRƯỚC khi link mới (replace semantics, tránh đụng unique) — khuôn me-avatar.service.ts:158-215",
    ...
    done_when: [
      "POST /tasks/:taskId/files/:fileId/cover + DELETE /tasks/:taskId/cover; chỉ nhận file ĐÃ đính kèm task đó + là ảnh + Uploaded + không Infected",
      "Đặt bìa mới thay bìa cũ (unlink trước link) — không vi phạm unique, không để 2 bìa",
      ...
      "int-spec deny-path: đặt bìa bằng file KHÔNG thuộc task ⇒ 404; file cross-tenant ⇒ 404; file không phải ảnh ⇒ 400; thiếu quyền ⇒ 403",
