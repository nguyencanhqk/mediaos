...
export const meta = {
  ...
  plan:
    'Kế hoạch tổng thể (rebuild 2026-06-22) = docs/IMPLEMENTATION-01..10 + docs/ISSUE-BOARD-01 + docs/PROJECT-BASELINE-01 — 7 sprint (S0–S6), EPIC-00→11, 112 story / 869 point. backlog.mjs CHỈ giữ WO sprint hành (S0–S1); pull sprint kế từ ISSUE-BOARD §18. Điểm vào: docs/plans/MVP-MASTER-PLAN.md.',
  foundation:
  ...
  brain: 'Điều phối đa-agent (decompose/route/review/escalate) dùng .claude/workflows/parallel-lanes.mjs · auto-loop .claude/workflows/auto-loop.mjs.',
};
...
export const backlog = [
  ...
  // IMPLEMENTATION-03 · EPIC-00/09 + DB nền + AUTH-DB + CI/env. Mục tiêu: team code/build/test/deploy-dev được.
  {
    ...
    module: 'PROJECT',
    layer: 'DOC',
    title: 'Governance: chuẩn hoá board/label/DoR/DoD + chốt backlog harness theo ISSUE-BOARD-01 (master-plan pointer)',
    zone: 'green',
    status: 'todo',
  ...
  },
  {
    ...
    module: 'DEVOPS',
    layer: 'DEVOPS',
    title: 'CI BE/FE: đối chiếu lint·typecheck·test·build + migration-check + path-filter (api/auth/console/app) với DEVOPS-02',
    zone: 'green',
    status: 'todo',
  ...
  },
  {
    ...
    module: 'DEVOPS',
    layer: 'DEVOPS',
    title: 'Hạ tầng local: đối chiếu docker compose (Postgres/PgBouncer/Valkey/MinIO) + .env.example với DEVOPS-03/04',
    zone: 'green',
  ...
  },
  {
    ...
    module: 'FOUNDATION',
    layer: 'DB',
    title: 'Đối chiếu schema nền (companies·modules·settings·sequence·audit·files·file_links·holidays) + RLS+FORCE với DB-01/DB-08',
    zone: 'red',
    status: 'todo',
  ...
  },
  {
    ...
    module: 'FOUNDATION',
    layer: 'DB',
    title: 'Seed module catalog (MVP active · Phase inactive) + default system/company settings idempotent (ON CONFLICT)',
    zone: 'red',
    status: 'todo',
  ...
  },
  {
    ...
    module: 'AUTH',
    layer: 'DB',
    title: 'Đối chiếu AUTH/RBAC schema (users·sessions·password_reset·login_log·roles·permissions·user_roles·role_permissions) + seed matrix với DB-02',
    zone: 'red',
    status: 'todo',
  ...
  },
  {
    ...
    module: 'FRONTEND',
    layer: 'FE',
    title: 'Đối chiếu API client + query layer + error mapper (401/403/422/500 · request-id · idempotency) với FRONTEND-04',
    zone: 'green',
    status: 'todo',
  ...
  },
  {
    ...
    skills: ['code-review'],
    depends_on: ['S1-FND-AUDIT-1', 'S1-FND-SETTING-1', 'S1-FND-FILE-1', 'S1-FND-SEQ-1', 'S1-FND-MODULE-1'],
