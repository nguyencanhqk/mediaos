...
export const backlog = [
  ...
  },
  {
    ...
    done_when: [
      ...
      "Seed module ME vào bảng modules (mirror 0435, ON CONFLICT DO NOTHING, status active, module_group Experience, sort_order 80 theo DB-10 §10.1); seed permission catalog ME đúng ME-DEC-002 + DB-10 §12.8 (SỬA 2026-07-13 — bản trước TRÍCH NGƯỢC ME-DEC-002, plan-reviewer chặn): cặp ME.ACCESS (gate khu vực /me) + cặp riêng cho chức năng THỰC SỰ thuộc ME: preference view/update (ME.PREFERENCE.VIEW_OWN/UPDATE_OWN) · avatar update (ME.AVATAR.UPDATE_OWN) · notification-preference update (ME.NOTIFICATION_PREFERENCE.UPDATE_OWN) — tất cả is_sensitive=false, grant per-pair data_scope Own cho MỌI role canonical (bài học §13 per-(permission,role); resolve theo thuộc tính + verify fail-LOUD mirror 0466/0476); KHÔNG seed wrapper cho nghiệp vụ nguồn ATT/LEAVE/TASK/NOTI/profile (SPEC-09 §11.2 — dùng permission nguồn); chuỗi (action,resource_type) cụ thể chốt trong plan + GHI mapping pair→permission-code vào docs/permission-matrix-spec.md CÙNG PR (bài học pair-drift S3-FE, web-core PERMISSION_CODE_TO_PAIR hạ nguồn phải khớp); plan-reviewer soi",
