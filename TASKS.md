...
| G8 | Approval · Defect · Eval · KPI | 🛠️+🤖 | L | 🟡 G8-1 Approval multi-level (1–3 cấp APR-001/002) **✅ MERGED master** (`7856e88` --no-ff, §5.4): append-only `approval_requests`=nguồn sự thật + step=projection (ADR-0016) + gate fix H1/M1 + E2 audit. Reconcile khi land: journal idx 50–52 (when bump 085000+ vì đụng g13), audit CHECK UNION **44 type** (43 + `approval_rule`). Verify `mediaos_g8` fresh: chain `0000→0082` sạch, **988 pass / 0 fail**. Defect/Eval/KPI ☐ |
...
- [x] **G10-4** 🤖🟢 (M) Meeting + biên bản + **task sau họp** → ghi vào **Task Hub G9** (`task_type=meeting_action`), KHÔNG bảng riêng. — **BE ✅** (lane g10 band 0050s, mig `0053`). Phần booking/double-booking/RLS/masking đã land trước ở `0052` (merge `cfab97e`); G10-4 này bù 2 nửa còn lại: **biên bản** (`meeting_notes` — UPDATE được, KHÔNG DELETE grant, audit `meeting_note`) + **action-item sau họp** ghi thẳng `tasks` (`task_type='meeting_action'`, BẤT BIẾN #4 — KHÔNG bảng task riêng) qua `TasksService.createMeetingActionTask` (lõi `createHubTask` dùng chung với office; **POST /tasks vẫn office-only** — không mở meeting_action cho tạo tay) + link `meeting_tasks` (unique cặp) + audit `MeetingActionTaskCreated` + outbox `meeting.action_task.created`. Authz: chỉ **organiser**, cuộc họp còn sống (`assertOrganizerActiveMeeting`). REST: `GET/POST/PATCH /meetings/:id/notes`, `GET/POST /meetings/:id/action-items`. Contracts: `meetingNoteSchema`/`createMeetingActionSchema`/`meetingActionItemSchema`. RLS+FORCE+tenant policy cho 2 bảng mới; đăng ký rls-harness (chống false-green). **RED trước** (deny-path organiser/cancelled/not-found + action→meeting_action + masking). Verify DB-cô-lập `mediaos_g10` (chain `0000→0053` áp sạch): meeting spec **24/24** · **full api 1169 pass / 16 skip** · typecheck 4/4 · lint 0 error · prettier sạch. _1 fail duy nhất = `secret-rotation`↔`platform-accounts-reveal` tranh chấp bảng global `encryption_keys` (PURPOSE `platform_account`) khi vitest chạy song song file — xanh khi chạy riêng (14/14), KHÔNG do G10-4 (diff không chạm crypto)._ LIGHT gate (self-review DB/security/silent-failure: RLS mirror 0052, audit union additive, FK-guard qua createHubTask, link idempotent onConflictDoNothing).
