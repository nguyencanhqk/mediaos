...
| AC-5 (api-keys/PAT) | `0310–0319` | 🟡 **đang dùng (CHƯA land)** — Personal Access Token + auth-pipeline TOÀN CỤC. Mig `0310_ac5_api_keys` (idx 97 when 1717500320000 — re-stamp khi land): `api_keys` (MUTABLE column-grant: app UPDATE chỉ `last_used_at`/`revoked_at`) + `api_key_usages` (APPEND-ONLY) + RLS+FORCE + `resolve_api_key_by_hash` SECURITY DEFINER (auth cross-tenant HẸP) + audit CHECK DO-block UNION +`api_key` + seed `manage:api-key` (sensitive) grant company-admin. Hook `guard-migration-band` cho ac5 = `[[310,319]]`. |
