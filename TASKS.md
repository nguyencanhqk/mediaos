...
| AC-4 (ui-config) | `0300–0309` | 🟡 **đang land** — Admin Control Plane N3: TENANT self-service branding/navigation/i18n (companyId từ JWT, KHÔNG cross-tenant operator). Mig `0300_ac4_ui_config` idx **99** when 1717500340000 (> AC-7 1717500330000, re-stamp mỗi rebase): 3 bảng per-tenant FORCE-RLS `tenant_branding` (1-row UNIQUE company_id, upsert) + `ui_navigation_config` (UNIQUE company_id,key + module_key nullable → ẩn menu nếu module tắt + soft-delete) + `i18n_overrides` (UNIQUE company_id,locale,namespace,key + soft-delete); RLS USING+WITH CHECK + index company_id; audit CHECK DO-block UNION +`tenant_branding`/`ui_navigation`/`i18n_override` (sync AUDIT_OBJECT_TYPES); perm mới `view:branding`+`manage:branding`+`manage:ui-navigation`+`manage:i18n-override` **is_sensitive=FALSE** (PRD §5.y — KHÔNG requiresReauth, tránh reveal-class trap) grant company-admin (00000001). Menu effective gate qua FeatureFlagService (KHÔNG bảng song song). 3 bảng vào rls-registry + cleanupTenants. Hook `guard-migration-band` cho ac4 = `[[300,309]]`. Verify `mediaos_ac4` chain 0000→latest sạch: api **2146 pass/0 fail** (rls 3 + deny 10 + module-gate 2 + contract 11) + admin typecheck/test 56/build. FULL gate (database+security+silent-failure+santa). KHÔNG land. |
| AC-5 (api-keys/PAT) | `0310–0319` | ✅ **ĐÃ LAND** (master `75a0b35`) — Personal Access Token + auth-pipeline TOÀN CỤC. Mig `0310_ac5_api_keys` (idx 97 when 1717500320000): `api_keys` (MUTABLE column-grant: app UPDATE chỉ `last_used_at`/`revoked_at`, +UNIQUE `token_hash`) + `api_key_usages` (APPEND-ONLY) + RLS+FORCE + `resolve_api_key_by_hash` SECURITY DEFINER (auth cross-tenant HẸP) + audit CHECK DO-block UNION +`api_key` + seed `manage:api-key` (sensitive) grant company-admin. Hook `guard-migration-band` cho ac5 = `[[310,319]]`. |
| AC-8 (observability) | `0340–0344` | 🟡 **đang land** — Admin Control Plane N6: audit viewer (tenant-self + operator cross-tenant) + queue/dead-letter monitor (READ-ONLY). Mig `0340_ac8_observability` idx **101** when 1717500360000 (> AC-6 1717500350000, re-stamp mỗi rebase). GUC HẸP MỚI `app.platform_audit_read` (cross-tenant primitive #2 — KHÔNG mở rộng `app.platform_admin` chỉ-companies của 0230) + 3 policy `*_platform_audit_read` FOR SELECT TO mediaos_app USING-only (OR-combine với *_tenant_iso, SELECT-only by construction = KHÔNG WITH CHECK cross-tenant ⇒ append-only #2 + isolation nguyên vẹn) trên audit_logs/outbox_events/dead_letter_events. Perm `view:audit-log`(grant company-admin 001) + `view:platform-audit`(grant platform-admin …f0), is_sensitive=TRUE, **isSensitive-ONLY KHÔNG requiresReauth** (tránh reveal-class trap; step-up qua OperatorReauthGuard keyed sentinel PLATFORM_AUDIT_SCOPE). Audit reuse object_type='company' (action operator.audit_read/operator.queue_read, KHÔNG đổi CHECK). `withPlatformReadContext` SELECT-only by contract (TÁI DÙNG AC-9). KHÔNG bảng mới ⇒ rls-registry/cleanupTenants KHÔNG đổi. Hook `guard-migration-band` cho ac8 = `[[340,344]]`. Verify `mediaos_ac8` chain 0000→latest sạch (api 2262 pass + admin 81 + contracts 131). FULL gate. KHÔNG land. |
| AC-7 (module-registry) | `0330–0339` | 🟡 **đang land** — Admin Control Plane N4: catalog GLOBAL `system_modules` (no-RLS, mirror permissions/subscription_plans) — lớp "module" TRÊN feature-flag G16-3, KHÔNG store on/off thứ 3 (toggle = `company_feature_flags`). Mig `0330` idx **98** when 1717500330000 (> AC-5 1717500320000, re-stamp mỗi rebase). Perm mới `manage:module-toggle` (is_sensitive, **isSensitive-only — KHÔNG requiresReauth**, step-up qua OperatorReauthGuard) + `view:system-module`, grant tường minh role …f0; audit reuse object_type='company' (action `operator.module_toggled`, KHÔNG đổi CHECK). Hook `guard-migration-band` cho ac7 = `[[330,339]]`. Verify `mediaos_ac7` chain 0000→latest sạch. |
...
- [~] **AC-8** 🛠️🔋 (M) **Observability** — audit viewer (per-tenant → cross-tenant qua GUC read-only) + queue/dead-letter monitor. **CROWN** FULL+security. _(dep: AC-0b)_ — **🟡 IMPLEMENTED trên lane `feat/ac8-observability` (off `aceaeac`, CHƯA land master).** Mig `0340_ac8_observability` (idx 101, when 1717500360000): GUC HẸP MỚI `app.platform_audit_read` (KHÔNG mở rộng `app.platform_admin`) + 3 policy `*_platform_audit_read` FOR SELECT TO mediaos_app USING-only (OR-combine, SELECT-only by construction — KHÔNG WITH CHECK ⇒ append-only #2 + isolation giữ) trên audit_logs/outbox_events/dead_letter_events; perm mới `view:audit-log`(→company-admin 001) + `view:platform-audit`(→platform-admin f0), is_sensitive=TRUE, ON CONFLICT DO NOTHING + grant TƯỜNG MINH; audit object_type KHÔNG đổi (reuse 'company'). `withPlatformReadContext` (SELECT-only by contract, TÁI DÙNG AC-9). BE AuditReadService(listOwnTenant withTenant / listCrossTenant withPlatformReadContext + recordOperatorAction 'operator.audit_read') + QueueMonitorService('operator.queue_read') — redact before/after object_type nhạy cảm (audit-redact.helper), row-cap [1..100], step-up keyed sentinel PLATFORM_AUDIT_SCOPE (fail-closed). Routes `/tenant/audit`(view:audit-log) + `/admin/platform/audit`+`/admin/platform/queue`(@OperatorOnly+OperatorReauthGuard+view:platform-audit, isSensitive-ONLY KHÔNG requiresReauth — tránh reveal-class trap). FE apps/admin `/operator/audit`·`/operator/queue`·`/tenant/$companyId/audit` + observability-api + i18n audit/observability + nav additive. Verify `mediaos_ac8` chain 0000→0340 sạch: **api 2262 pass/0 fail** (AC-8 deny/leak/GUC/step-up/redact/row-cap 24 + contract 11 + adjacent platform 0 regress) + **admin 81 pass + contracts 131 pass**, typecheck 3/3, lint MY-files 0, build admin OK. KHÔNG land lane này.
...
> Tách `apps/web` (1 SPA) → **3 product app** `studio` + `people` + `console` + **`apps/auth`** (đăng nhập trung tâm/SSO) + 2 shared package `packages/{ui,web-core}`. **GIỮ 1 backend** (`apps/api`) — KHÔNG tách microservices. Đường cắt tự nhiên = **7 category** trong nav registry `apps/web/src/lib/nav.ts`. 📋 Kế hoạch đầy đủ (bản đồ di chuyển theo file thật · thiết kế SSO cookie-subdomain · rủi ro · ước lượng): **`docs/frontend-split-plan.md`**. **✅ PHASE 1 ĐÓNG 2026-06-17** — FS-0 + FS-1a + FS-1b đã LAND. **✅ WAVE 2 ĐÓNG 2026-06-18** — FS-2 `apps/people` + FS-3 `apps/studio` + FS-4 `apps/console` đã LAND master (tip `c885f87`); 3 product app chạy độc lập qua SSO `apps/auth`, apps/web còn launcher rỗng. **CÒN: FS-5 cutover** (DNS/TLS wildcard/launcher/CI per-app + XOÁ apps/web).
...
- [x] **FS-1a (Wave 1) ✅ MERGED master** (`22fdab8` --no-ff, off `11a3dfc`; lane+worktree dọn) — 🛠️🔋🔋 (L) **Nền phiên SSO ở `apps/api`** (CROWN auth boundary). Thêm vòng đời phiên cookie-subdomain **KHÔNG tách api**: `POST /auth/refresh` cookie-first (rotation + **reuse-detection** → thu hồi cả **family** buộc login lại) + `POST /auth/logout` (thu hồi family + xoá cookie, chỉ token CÒN SỐNG mới uỷ quyền — chống forced-logout) + login/2fa-verify **phát refresh cookie `HttpOnly`/`Secure`/`Domain`/`SameSite=Strict`** + CSRF cookie (đọc được) + **CSRF double-submit** (header `x-csrf-token` hằng-thời-gian) cho endpoint cookie-based + `GET /auth/redirect-allowed` (validate `?redirect` allowlist origin EXACT, từ chối userinfo/relative/scheme nguy hiểm — chống open-redirect). CORS allowlist credentials **đã có** ở `main.ts` (`CORS_ORIGIN` tường minh, `credentials:true`, không `*`). **Mig 0400** (band feauth `0400s`, **re-stamp khi land → idx 101 when 1717500360000** vì master tiến ac4/ac6 sau fork) = `refresh_tokens.family_id uuid NOT NULL DEFAULT gen_random_uuid()` (login=family mới · rotation=kế thừa · reuse/logout=thu hồi cả họ) + partial index `(company_id, family_id) WHERE revoked_at IS NULL`; RLS+FORCE sẵn từ 0004, KHÔNG backfill, KHÔNG đổi audit CHECK (reuse `auth` object_type). Access/Bearer in-memory cho REST/WS **giữ nguyên**; luồng body refreshToken (mobile) tương thích ngược. **CONTRACT cho 1b** (`packages/contracts/auth.ts`): `refreshToken` optional · `authRefreshResponseSchema` · `redirectAllowedResponseSchema` · hằng `REFRESH_COOKIE_NAME`/`CSRF_COOKIE_NAME`/`CSRF_HEADER_NAME`. **DoD ĐẠT:** chain `0000→latest` sạch trên `LANE_DB=mediaos_feauth` · deny-path TRƯỚC + happy-path xanh (auth-session 13 + auth 8 + cookie/redirect unit 29) · **full api 2143 pass/0 fail** (outbox parallel-flake pre-existing, xanh serial) · typecheck 10/10 · audit `auth.token_refreshed`/`auth.token_reuse_detected`/`auth.logout`. **FULL gate + santa CONVERGED:** security BLOCK (HIGH TOCTOU `refresh` thiếu FOR UPDATE → vá `.for("update")`; userinfo redirect bypass → vá) + code-reviewer HIGH (forced-logout: token revoked vẫn thu hồi family → vá guard live-token) đều FIXED → SAFE-TO-LAND/SHIP. _(dep: FS-0)_
- [x] **FS-1b ✅ MERGED master** (`b3e9146` --no-ff, off `701437a`; lane+worktree dọn) — 🛠️🔋 (M) **`apps/auth` SPA + wiring `web-core`** (CROWN auth FE). Tiêu thụ contract FS-1a. **`packages/web-core`** (`lib/api-client.ts` + `lib/session.ts`): mọi fetch `credentials:'include'`; **silent-refresh khi load** (`bootstrapSession` → `refreshAccessToken` cookie-first đọc cookie `mediaos_csrf` → header `x-csrf-token` → nạp access token in-memory → `/me` → `setUser`; dedupe cấp bootstrap StrictMode) + **refresh-on-401 SINGLE-FLIGHT** (N request 401 đồng thời → ĐÚNG 1 `/auth/refresh`, chờ chung 1 promise rồi REPLAY 1 lần với token mới — chống reuse-detection thu hồi family) + **KHÔNG vòng lặp** (replay đi thẳng `finishResponse`, refresh dùng `fetch` thẳng không đệ quy, `skipAuth` bỏ qua) + **redirect `auth.<domain>?redirect=<đích>`** khi refresh fail + **epoch guard** (logout-trong-lúc-refresh KHÔNG commit token oan) + `logoutSession` (thu hồi family + xoá cookie + xoá store + redirect, best-effort) + `getAuthRedirectUrl`/`configureAuthAppUrl` + store `setAccessToken` (CHỈ access token in-memory, refresh token KHÔNG bao giờ chạm JS). **`apps/auth`** Vite SPA mỏng (scaffold từ `apps/admin`, dùng `@mediaos/{web-core,ui}`, KHÔNG router/query): login + 2FA challenge (chuyển khỏi `apps/web`) → đăng nhập xong (cookie SSO server đặt) gọi `authApi.checkRedirect` (GET `/auth/redirect-allowed`, server là nguồn allowlist DUY NHẤT) → `window.location` về target đã whitelist / landing mặc định (chống open-redirect, KHÔNG tự phán ở client). **`apps/web`** chuyển luồng mới: `main.tsx` boot `bootstrapSession()` (silent-refresh trước mount, chưa phiên → redirect auth, không nháy login); xoá route/màn `/login` + `TwoFactorChallengeForm` (dời sang auth); guard `throw redirect({ href: getAuthRedirectUrl() })` (full-page external, dừng pipeline); logout = `logoutSession` toàn cục. **Dev `*.localhost`** (`.env.example`: `CORS_ORIGIN` + `AUTH_COOKIE_DOMAIN=localhost` + `AUTH_COOKIE_SECURE=false` + `AUTH_REDIRECT_ALLOWLIST` + `VITE_API_URL=api.localhost` + `VITE_AUTH_APP_URL=auth.localhost:5275` + `VITE_DEFAULT_APP_URL=web.localhost:5273`; vite `allowedHosts:[".localhost"]`). **DoD ĐẠT:** build 5/5 + typecheck sạch + lint 0 error · **test: web-core 27 (+22: api-client 18 single-flight/no-loop/epoch/redirect + session 4 + auth-api 5) + apps/web 300 (−9 login dời) + apps/auth 9 = 336**. **FULL gate (security + react + silent-failure) + santa adversarial pass trên queue = SHIP**: fixed 2 HIGH security (redirecting-flag-không-reset → reset macrotask; 2FA raw err.message → friendlyError mapping) + 3 HIGH react (busy stuck → finally ×2; 2FA error role="alert"; guard không throw → `redirect({href})`) + 2 HIGH silent-failure (contract-drift 200/sai-schema + /me non-401 → console.error tín hiệu operator). **GOTCHA:** `authApi.login` PHẢI `skipAuth:true` (401 sai-mật-khẩu KHÔNG được kích refresh→loop ở chính apps/auth); web-core test `environment:node` → `vi.stubGlobal` fetch/document/window + `vi.resetModules` reset state module-level. _(dep: FS-1a)_ ⭐ **Phase 1 ĐÃ ĐÓNG 2026-06-17** (FS-1a `22fdab8` + FS-1b `b3e9146` trên master `b3e9146`) → Wave 2 mở.
- [x] **FS-2 ✅ MERGED master** (`c472249` --no-ff, off `61835e5`) — 🤖🟢 (M) **`apps/people`** (CROWN payroll) — Vite SPA `people.localhost:5277` mirror boot SSO apps/web (bootstrapSession + authGuard `throw redirect({href})`, access token in-memory). Di chuyển **byte-identical (git R100)** routes org/hr/payroll + components hr/payroll/org-chart + api libs (org/positions/employees/attendance/leave/salary-profile/payroll-period/payslip/bonus-penalty) + i18n (org/hr/payroll) khỏi `apps/web`; khai NAV_ITEMS subset (hr+attendance+payroll); launcher chỉ render category CÓ app. dep `@xyflow/react` (org-chart). **org-api/employees-api GIỮ ở web** (studio code còn ở web tiêu thụ tới FS-3) + dup canonical sang people. Bỏ chuông thông báo khỏi shell people (notification-api thuộc studio §4). **DoD ĐẠT:** people typecheck + **147 test** + lint + vite build; web typecheck + test + build xanh. **CROWN gate (security + silent-failure) CLEAN** — crown surface (payslip masking server-side, guard, token in-memory) không lỗi lane-introduced; áp 2 fix nhỏ (boot `.catch`→redirect, launcher category filter). 0 migration. _(dep: FS-1)_
- [x] **FS-3 ✅ MERGED master** (`78d531b` --no-ff, off `7cc169b`) — 🤖🟢 (M) **`apps/studio`** (work+process+goals) — Vite SPA `studio.localhost:5276`. Di chuyển R100 routes tasks/media/chat/workflows/dashboard/kpi + components + api libs + workflow-builder + i18n (tasks/channels/dashboard/kpi/projects/workflows) khỏi web; NAV_ITEMS subset work+process+goals. deps `@xyflow/react` (workflow canvas) + `recharts` (cô lập vào studio). **socket.io-client KHÔNG thêm** (chat REST-only, 0 importer). **Shared-tới-cutover (copy sang studio, GIỮ ở web):** notification-bell + notification-api + chat.json (chuông shell, web còn dùng) · org-api + employees-api (dropdown) · channels-api (console platform-accounts cũng dùng). `kpi-history.tsx` bỏ chỉ thị `eslint-disable react-hooks/*` mồ côi (apps/studio ngoài scope react-hooks như apps/admin; config-protection chặn nới scope). **DoD ĐẠT:** studio typecheck + **89 test** + lint + build; web xanh. **LIGHT gate (react-reviewer) APPROVED** (0 CRITICAL/HIGH). 0 migration. _(dep: FS-1)_
- [x] **FS-4 ✅ MERGED master** (`c885f87` --no-ff, off `7cc169b`) — 🤖🟢 (M) **`apps/console`** (nhóm `system`, TENANT `aud=user` — **KHÁC** operator plane `apps/admin` aud=operator) — Vite SPA `console.localhost:5278`. Di chuyển R100 settings/company + platform-accounts (secret reveal) + break-glass routes/components/api + i18n (settings/break-glass) khỏi web; NAV_ITEMS subset system. **channels-api dup sang console** (platform-accounts dùng channelsApi) + GIỮ ở web. Bỏ chuông thông báo khỏi shell. **Cutover XOÁ `apps/web` = WAVE 3 (FS-5), KHÔNG ở Wave 2** — web GIỮ làm launcher rỗng (router chỉ còn `/`, NAV_ITEMS=[]). **DoD ĐẠT:** console typecheck + **42 test** + lint + build; web xanh. **CROWN gate (security + silent-failure) CLEAN** — secret-reveal invariant giữ nguyên (R100, masking server-side), guard intact, aud=user boundary sạch. 0 migration. _(dep: FS-1)_ ⭐ **WAVE 2 ĐÓNG 2026-06-18** (master tip `c885f87`): merged tuần tự people→studio→console, reconcile hot-file `apps/web/router.tsx`+`lib/nav.ts` mỗi merge → web còn `/` + NAV_ITEMS rỗng. **Merged tree: typecheck 4/4 + test people 147/studio 89/console 42/web 28 = 306 + lint 4/4 + vite build 4/4 XANH.** .env.example +CORS+AUTH_REDIRECT_ALLOWLIST 3 origin mới. **KNOWN DEBT (pre-existing, không lane-introduced):** `employees-api.ts` (web+people) dùng fetch tự chế không gắn Bearer/credentials → màn employee 401 dưới SSO; vá đồng bộ (web+people+studio dup) ở change riêng (dùng web-core apiFetch như org-api; FormData upload cần override Content-Type). **Notification bell** tạm vắng ở people/console (nâng lên package dùng chung để gắn lại).
- [~] **FS-5** 🔧🔋 (M) **Cutover — phần CODE ĐÃ LAND (lane `feat/fs5-cutover`), phần OPS = runbook scaffold** _(dep: FS-2 · FS-3 · FS-4)_. **Nợ Wave 2 ĐÓNG:** (1a CROWN auth) `employees-api.ts` (web/people/studio byte-identical) chuyển sang web-core `apiFetch` (Bearer+credentials+refresh-on-401); `uploadImport` FormData fetch thẳng giữ Bearer + KHÔNG set Content-Type — FULL gate security+silent-failure SAFE-TO-LAND. (1b) **notification-bell→@mediaos/ui** + **notification-api→@mediaos/web-core** + i18n namespace `notifications` RIÊNG (nhúng CORE_RESOURCES) → **gắn lại chuông cho cả 4 app** (web/studio/people/console), bỏ dup web/studio; ui +peer `@tanstack/react-query` — LIGHT react-reviewer APPROVED. **Launcher (option A — giữ apps/web, đổi vai):** `apps/web` thành **launcher root-domain**: `lib/launcher-apps.ts` registry 3 app (URL từ `VITE_{STUDIO,PEOPLE,CONSOLE}_URL`, default dev *.localhost) + `hasAnyCapability` (mirror wildcard useCan) gate tile; `home.tsx` render card cross-origin `<a>`; root-layout → `<Outlet/>`. **DỌN dead shared-tới-cutover ở web:** lib/{channels-api,employees-api,org-api(+spec),nav}.ts + chat.json (people/studio giữ bản đang dùng). **Phần OPS (CHƯA land hạ tầng thật — chờ domain/CI):** runbook `docs/ops/fs5-cutover-runbook.md` (DNS · TLS wildcard `*.<domain>` · reverse proxy · cookie/CORS/redirect prod · CI per-app · dev *.localhost · checklist + rollback) + `.env.example` khối "FS-5 CUTOVER — TEMPLATE PROD" (placeholder `mediaos.example`). **DoD ĐẠT (FE-only, 0 migration):** typecheck 7/7 + lint clean + test web-core 27/auth 9/web 22/studio 89/people 147/console 42 + vite build 5/5 XANH. **CÒN (ops thật):** chốt domain prod · CI/CD per-app · DNS/LB/TLS provider · Vault KMS. **apps/web GIỮ làm launcher** (không xoá — option A); nếu sau xoá web: move 4 spec test-package (use-can/permission-gate/button/two-factor-api) sang web-core/ui + TwoFactorSettings.tsx sang product app.
