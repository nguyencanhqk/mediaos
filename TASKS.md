...
- [ ] **FS-1b** 🛠️🔋 (M) **`apps/auth` SPA + wiring `web-core`** — SPA mỏng (login + 2FA + set-password) tiêu thụ contract FS-1a; `web-core` **silent-refresh khi load + refresh-on-401** (xếp hàng request, `credentials:'include'`) + redirect `auth.<domain>` khi fail; dev `*.localhost`. **FULL gate (auth FE).** ⭐ CHỐT CHẶN cùng FS-1a — phải xong trước khi tách product app. _(dep: FS-1a)_
