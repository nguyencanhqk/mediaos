...
| G14 | Dashboard & Report | 🤖 AI-bulk 🟢 | M | 🟡 G14-1 Dashboard **✅ MERGED master** (`633ba22`): dashboard stats + permission masking + FE. G14-2 Report **✅ MERGED** (`1aef3ff`, mig `0101` idx 60). G14-3 MV + Alerts **🟡 WIP** (worktree g14-mat): mig `0102` (idx 65, when 1717500128000) mv_dashboard_task_status + mv_dashboard_output (UNIQUE index → REFRESH CONCURRENTLY) + MvDashboardService (company_id mandatory) + AlertsService (overdue/channel-risk, threshold constants) + DashboardRefreshService (CONCURRENTLY qua workerPool, non-concurrent lần đầu) + controller append (GET mv-stats/alerts, POST refresh) + contracts append (mvStatsQuerySchema/alertSchema) + 21 unit test xanh (typecheck sạch). Chờ verify DB cô lập + int-spec DB. |
