...
| G11 | Attendance · Leave | 🤖 AI-bulk 🟢 | M | 🟡 BE xong (attendance `6181a05` + leave `4b7ea4a`); verify DB cô lập `mediaos_g11` **229 pass** (deny-path 4 + RLS 2-tenant 165 + unit); gate BLOCK→**vá F1–F5** `82fd27f` (TOCTOU FOR UPDATE · check-out perm · mapError leak · scope=all · overnight checkout — [`docs/reviews/g11-gates.md`](docs/reviews/g11-gates.md)); FE xong branch `feat/g11-fe` (167 web test). **Còn:** push + merge master §5.4 (rebase+reconcile audit CHECK/journal) · follow-up F6 pagination/F7 period-lock immutability(trước G12) ✅ (`0064` trigger BEFORE UPDATE attendance_periods chặn locked→open · RED→GREEN DB-cô-lập `mediaos_g11f7`: period-lock 4/4 + regression tenant-iso 165/attendance-perm 4/rls-guards 3 xanh; lane g11f7)/F8 cleanup. 4 fail pre-existing G6-2/G2-6 (ngoài G11). |
