...
| AC-9 (db-ops) | `0345–0349` | 🟡 **đang land (LANE CUỐI)** — Admin Control Plane N6 db-ops operator CHỈ-ĐỌC. **ADR-0020** (mở rộng 0019 Tầng 1): P1 migration-status (đọc `drizzle.__drizzle_migrations` qua runRaw + GRANT SELECT cho app role + đối chiếu `_journal.json`, READ-ONLY) · P2 **data browser TENANT-SCOPED qua `withTenant(targetCompanyId)`** (RLS company_id=current ÉP — KHÔNG GUC mới/BYPASSRLS; **all-tenant Tầng 3 role-DB-read-only DEFER**) · P3 break-glass SoD gate · P4 export scaffold (**worker materialize file DEFER**, như AC-6 delivery). Mig `0345_ac9_db_ops` idx **103** when 1717500380000 (> AC-8 1717500370000, re-stamp mỗi rebase). 3 bảng **GLOBAL no-RLS operator-scoped** (`target_tenant_id` KHÔNG company_id ⇒ tự loại khỏi rls-guards/rls-coverage-assert, KHÔNG vào rls-registry/cleanupTenants): `db_ops_grants` (MUTABLE FSM column-grant) + `db_ops_grant_approvals` (APPEND-ONLY, `REVOKE UPDATE,DELETE`, SoD UNIQUE+CHECK self-approve) + `db_export_jobs`. SoD ép 3 tầng (UNIQUE+CHECK+COUNT DISTINCT≥2). **Allowlist default-DENY** bảng+cột (`packages/contracts/db-ops-allowlist.ts`) — LOẠI TRỪ verbatim secret_ciphertext/payslips/salary/*_totp/webhook-secret/encryption_keys/token_hash/break_glass*+db_ops*.reason. Perm `read:db-browser` + `manage:db-ops` is_sensitive=TRUE, **isSensitive-ONLY KHÔNG requiresReauth** (tránh reveal-class trap; step-up qua OperatorReauthGuard + resolveWindow tường minh ở controller; tenant-scoped=target id thật, all-tenant=sentinel PLATFORM_DB_OPS_SCOPE), grant role …f0. Audit reuse object_type='company' (action operator.db_read/db_export/db_grant_*, KHÔNG đổi CHECK); **fail-closed** (audit lỗi ⇒ client KHÔNG nhận data, mirror AuditReadService). `withTransaction` mới (plain tx no-GUC cho bảng global no-RLS). Hook `guard-migration-band` cho ac9 = `[[345,349]]`. Verify `mediaos_ac9` chain 0000→0345 sạch: contracts 14 + metadata 37 + allowlist 7 + sod 8 + deny 6 + migration-status 3 + regression (tenant-iso **306/0-reg** + rls-guards + rls-coverage-assert) GIỮ XANH; admin typecheck+lint+test 81+build. FULL gate + break-glass review. KHÔNG land (orchestrator land cuối). |
...
- [~] **AC-9** 🛠️🔋 (L) **DB ops** — migration status viewer + **data browser read-only allowlist bảng/cột (LOẠI secret/PII)** + export job; audit từng row, break-glass-style SoD approval (band `0345–0349s`). **CROWN, blast-radius cao nhất — land CUỐI.** _(dep: AC-0b, sau AC-8)_ — **🟡 IMPLEMENTED trên lane `feat/ac9-db-ops` (off `7cc169b`, CHƯA land master).** **ADR-0020** (mở rộng 0019 **Tầng 1** `withTenant(targetCompanyId)` — data browser TENANT-SCOPED, RLS company_id=current ÉP; **all-tenant Tầng 3 role-DB-read-only DEFER**, lý do hạ tầng pool/role provisioning). Mig `0345_ac9_db_ops` (idx 103, when 1717500380000): 3 bảng **GLOBAL no-RLS operator-scoped** (`target_tenant_id` KHÔNG company_id ⇒ tự loại khỏi rls-guards/rls-coverage-assert, KHÔNG vào rls-registry/cleanupTenants) — `db_ops_grants` (MUTABLE FSM, `REVOKE UPDATE,DELETE` + column-GRANT cột vòng đời) + `db_ops_grant_approvals` (APPEND-ONLY `REVOKE UPDATE,DELETE`, SoD `UNIQUE(grant_id,approver)` + `CHECK(approver<>requester)` + `CHECK(required_approvals>=2)`) + `db_export_jobs`; perm `read:db-browser`+`manage:db-ops` is_sensitive=TRUE ON CONFLICT DO NOTHING + grant …f0 TƯỜNG MINH; GRANT SELECT `drizzle.__drizzle_migrations` cho app role (migration-status). Audit object_type KHÔNG đổi (reuse 'company': operator.db_read/db_export/db_grant_*). `withTransaction` mới (plain tx no-GUC cho bảng global no-RLS). BE: MigrationStatusService(runRaw READ-ONLY) · DataBrowserService(assertGrantActive fail-closed 403 → withTenant(target) project cột allowlist + clampLimit [1..100] + audit MỖI read **fail-closed**) · DbOpsGrantService(SoD FOR UPDATE, UNIQUE 23505→409, self-approve→403, expired→410, COUNT DISTINCT≥2 flip 'active', deny-audit NGOÀI tx) · DbExportJobService(scaffold; **worker DEFER**). **Allowlist default-DENY** (`packages/contracts/db-ops-allowlist.ts`) — LOẠI TRỪ verbatim secret_ciphertext/payslips/salary/*_totp/webhook-secret/encryption_keys/token_hash/break_glass*+db_ops*.reason; bảng/cột ngoài → 400. Controller `/operator/db-ops/*` @OperatorOnly + OperatorReauthGuard + PermissionGuard + **isSensitive-ONLY KHÔNG requiresReauth** (tránh reveal-class trap; step-up resolveWindow tường minh — tenant-scoped=target id thật, all-tenant=sentinel PLATFORM_DB_OPS_SCOPE). FE apps/admin `/operator/db-ops` (4 panel: migration-status + data-browser tenant+table picker DataTable + break-glass grants request/approve/revoke + export jobs) + db-ops-api + i18n db-ops.json + nav+router additive. Verify `mediaos_ac9` chain 0000→0345 sạch: contracts 14 + metadata 37 + allowlist 7 + sod 8 + deny 6 + migration-status 3 + regression (tenant-iso **306/0-reg** + rls-guards + rls-coverage-assert GIỮ XANH), api typecheck 0, admin typecheck+lint+test 81+build OK. FULL gate + break-glass review. KHÔNG land lane này.
