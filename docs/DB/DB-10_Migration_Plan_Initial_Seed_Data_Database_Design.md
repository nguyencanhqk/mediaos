...
| RECRUIT | Tuyển dụng | Phase 2 | false — hàng pre-seed từ `0435`; **`0560` (S12-RECRUIT-DB-1, đã merge 31/08/2026) GIỮ false**, guard chỉ kiểm TỒN TẠI nên forward-compatible (pin `migration-smoke` giữ `RECRUIT`); bật `true` ở **S12-RECRUIT-FE-1** bằng `UPDATE` tường minh + gỡ `RECRUIT` khỏi `EXTENSION_INACTIVE_MODULES` cùng commit |
...
### 12.11 RECRUIT permissions *(Phase 2 — ĐÃ seed ở `S12-RECRUIT-DB-1`, mig `0560`: 16 cặp + 42 grant §9f, verify fail-loud)*
