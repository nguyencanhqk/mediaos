...
| **0526 / 0527 / 0528** (đợt D — ĐÃ CHẠY, S5-GOAL-DB-2) | `0526` tạo `task_templates` + `task_template_items` + RLS+FORCE; `0527` seed pair `('manage','task-template')` (manager=Department, company-admin=Company, employee/hr KHÔNG) + verify; `0528` PURE audit UNION-ADD `'task_template'` (neo 2 tầng `object_type = ANY`) | tách khỏi wave lõi. ⚠️ Số dự kiến ban đầu (0508–0510) ĐÃ BỊ wave LMS chiếm ⇒ thực tế lấy **0526–0528** (head lúc chạy = idx 192 / `0525`). Luôn đọc `migrations/meta/_journal.json` THẬT lúc chạy. Audit tách RIÊNG (0528) để probe test đọc-cả-file mà không chạm seed catalog global |
