...
2. **Composite tenant FK** `(company_id, x_id) REFERENCES t (company_id, id)` cho **mọi** FK chéo bảng nghiệp vụ (mẫu `0535`; bảng đích phải có `UNIQUE (company_id, id)` — `employees`/`employee_profiles` đã có từ `0535`). **FK về `users` (`*_by`, nullable) CŨNG composite**: `(company_id, col) REFERENCES users (company_id, id) ON DELETE SET NULL (col)` — khuôn `0538:315-334` (CHAT: "6 cột FK mới, KHÔNG cái nào được để một cột"). Lý do (phát hiện ở review ROOM 29/08/2026, áp ngược cho ASSET): `fk-tenant-census.ts` đếm **mọi** FK một-cột giữa hai bảng có `company_id` (không lọc nullable) ⇒ FK đơn cột mới làm `xtenant-fk-ratchet` assert (a) ĐỎ và lớp T **không được ký waiver**; bản chất là lỗ KI-046 (FK bỏ qua RLS — `assigned_by` trỏ được user tenant khác). Mọi dòng "FK `users.id` SET NULL" trong §6 đọc theo luật này; sàn `FK_SINGLE_COL_PAIRS_FLOOR = 423` **không hạ**.
   ...
   - **Đính chính khi thi công (`0549`, plan-review S11-ASSET-DB-1 29/08/2026): `*_by` của 4 bảng SỔ (`asset_assignments` · `asset_maintenances` · `asset_inventories` · `asset_inventory_items`) là `ON DELETE NO ACTION`, KHÔNG `SET NULL (col)`.** RI action chạy ở tầng owner, bỏ qua allowlist cột ⇒ `DELETE FROM users` sẽ ghi đè `assigned_by`/`opened_by`/`checked_by` — đúng cột cố ý không grant (bất biến #2; tiền lệ `chat_calls`). `NO ACTION` hợp lệ với census; teardown không hỏng vì `cleanupTenants()` xoá 6 bảng trước `users`. Mọi dòng "FK `users.id` SET NULL" ở §6.3–6.6 đọc là `NO ACTION`; 2 bảng mutable (`asset_categories`, `assets`) giữ `SET NULL (col)`.
...
| --- | --- | --- |
| **A** (`0549`) | Tạo 6 bảng (`company_id … ON DELETE CASCADE`; composite FK nội bộ `NO ACTION`, §4.2) + `UNIQUE (company_id, id)` ở bảng đích + CHECK + index §6 · **ENABLE/FORCE RLS + policy literal-GUC cả 6 bảng** · GRANT: `asset_categories`/`assets` = `SELECT, INSERT, UPDATE` (không DELETE — soft delete); 4 bảng sổ = `SELECT, INSERT` + UPDATE **cấp cột** §6.3–6.6 (**không** phát `GRANT UPDATE ON <table>` rồi mới grant cột; nhớ `REVOKE` cấp bảng về sau **xoá sạch** column-grant — `revoke-table-grant-wipes-column-grants`) · đăng ký `rls-registry` · **VERIFY fail-loud (`RAISE EXCEPTION`, khuôn `0506` bước 6)**: 6 bảng `relrowsecurity AND relforcerowsecurity` + có policy; app role **0 quyền DELETE** trên 4 sổ; tập cột UPDATE của app role trên 4 sổ **đúng bằng** allowlist §6.3–6.6 (so bằng **`aclexplode(relacl/attacl)`** — KHÔNG `information_schema.column_privileges`: view trộn ACL cấp bảng vào cấp cột và lọc `pg_has_role(grantor)` ⇒ tập rỗng khi đổi owner; khuôn `0540:137-139`; đính chính 29/08/2026) · verify **DƯƠNG đúng-bằng 26 composite FK** qua `pg_constraint` (quên hẳn FK thì `fk-tenant-census` im lặng) · predicate partial unique so **đúng chuỗi `pg_get_expr(indpred)`** · **cùng commit**: thêm 6 bảng vào `apps/api/test/helpers/seed.ts` `cleanupTenants()` theo thứ tự con→cha (`asset_inventory_items` → `asset_inventories` → `asset_maintenances` → `asset_assignments` → `assets` → `asset_categories`) | RLS TRƯỚC mọi INSERT (bất biến #1); `fk-tenant-census` không được đỏ; drizzle schema `apps/api/src/db/schema/assets.ts` parity (RLS/grant/partial-index chỉ ở SQL). Thiếu `cleanupTenants` = đỏ hàng loạt `afterAll` (`drop-table-must-clean-test-teardown`) |
| **B** (`0550`) | **Module `ASSET`: hàng ĐÃ TỒN TẠI từ `0435:297`** (`Extension`, `is_active=false`, `sort_order=10`) ⇒ `INSERT … ON CONFLICT DO NOTHING` là NO-OP; **thi công `0550` chỉ verify hàng tồn tại và GIỮ `is_active=false`** (tiền lệ `0538` CHAT: bật cờ khi chưa có endpoint = hứa suông; pin `migration-smoke` `EXTENSION_INACTIVE_MODULES` giữ `ASSET`). Bật ở `S11-ASSET-FE-1` bằng `UPDATE` tường minh + gỡ pin smoke cùng commit (đính chính 29/08/2026) · seed role hệ thống **`asset-manager`** (`company_id NULL`, `is_system=true`, **`requires_two_factor=false` khai tường minh** — không để mặc định ngầm, `company-admin` là `true` từ `0120`; id cố định mới, `ON CONFLICT DO NOTHING` — tiền lệ `0019` `hr-manager`) · **11 cặp** permission SPEC-13 §11 `is_sensitive=false` `ON CONFLICT (action, resource_type) DO NOTHING` · grant per-(role, pair) theo ma trận **§9d** (resolve role theo `name + company_id IS NULL + deleted_at IS NULL`, DELETE-wrong-scope + INSERT ON CONFLICT, verify fail-loud đếm đúng **28** hàng — mirror `0506`) · **UNION-ADD** 5 giá trị vào CHECK `audit_logs.object_type` — **clone nguyên khối `0545`** (đính chính 29/08/2026: KHÔNG clone `0506` bước 4 — bản đó parse `\{[^}]*\}` trên TOÀN BỘ constraintdef, chưa neo tầng-1, chính là bẫy `audit-check-union-parse-anchor-trap`; `0545`/`0528` neo 2 tầng vào `object_type = ANY(…)`, fail-closed, NO-LOSS/NO-GAIN), rồi verify regex biên `~ '[,{'']asset['',}]'` cho **từng** giá trị như `0506:265` — KHÔNG tự viết parser mới + `AUDIT_OBJECT_TYPES` cùng commit. **Ghi nhận cho audit (security-reviewer 29/08/2026):** `asset-manager` giữ 11 cặp @Company như `company-admin` nhưng `requires_two_factor=false` (company-admin = true từ `0120`) ⇒ có đường ghi tài sản/đọc tài chính toàn công ty không qua 2FA — chốt có chủ đích (như `hr`/`hr-manager`); đổi ở AUTH nếu chính sách 2FA mở rộng. `acknowledged_at` có thêm `chk_asset_assignments_ack_v1 CHECK (acknowledged_at IS NULL)` ở v1 (column-grant chỉ chặn UPDATE, không chặn INSERT) — WO cấp phát 2 bước DROP CHECK này. Role `asset-manager` là role **cộng thêm**: không có cặp ME / `(read,dashboard)` — user chỉ mang role này không mở được `/me` (pin EXACT-SET 12 role ở `dash-seed2-manager-hr-grant` giữ nguyên). **Không** có `asset_inventory_item`: đánh dấu dòng audit dưới aggregate `asset_inventory` (`object_id = inventory_id`) | `super-admin` KHÔNG enumerate (nhận qua bootstrap). `asset-manager` **không** vào danh sách canonical (`DashCanonicalRole`/`NOTI_CANONICAL_ROLES`/pin `auth-seed-canonical-roles` giữ 4 role) |
