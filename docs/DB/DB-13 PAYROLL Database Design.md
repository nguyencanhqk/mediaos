...
| 🔁 **`S15-PAYROLL-DB-1B` (mig `0574`)** — `enforce_bonus_penalty_freeze()` thêm nhánh **(F)** + trigger thành **`BEFORE INSERT OR UPDATE`** (`CREATE OR REPLACE TRIGGER`, giữ tên) | **(F)** nhả consume (`x → NULL`) CHỈ khi kỳ CŨ ∈ {`CollectingData`, `Calculated`}; gắn (`NULL → x`, và INSERT hàng đã consume) CHỈ khi kỳ MỚI **còn sống** (`deleted_at IS NULL`) và ∈ tập đó. Kỳ đọc `FOR SHARE` qua `public.payroll_periods` lọc `company_id`. Message `bonus_penalty_freeze_guard:<tag>: …`, tag **ĐÓNG** `frozen` (A)(B)(D) · `rebind` (C) · `status-terminal` (E) · `period-frozen` · `not-found`. (A)–(E) giữ nguyên logic. **KHÔNG** thêm «INSERT chỉ `Pending` sạch» (khác T3 `payroll_advances`) | nợ B2 của DB-2 §3.5.a: không vế trạng-thái-kỳ ⇒ nhả khoản đã trừ ở kỳ `Locked`/`Paid` rồi gắn kỳ sau = **trừ lương hai lần**; gắn vào kỳ ≥ `Reviewing` hoặc kỳ đã xoá mềm = `consumed` mà không bao giờ vào dòng lương. Nhả không lọc `deleted_at` để còn cứu khoản ra được. Insert-shape bị bỏ vì BEFORE INSERT bắn TRƯỚC CHECK — giết fixture dựng hàng đã duyệt + các ca ghim tên CHECK. ⚠️ Máy tính (v1 và BE-3) PHẢI khoá kỳ `FOR UPDATE` TRƯỚC khi ghi `bonus_penalties` — đảo thứ tự là nguy cơ 40P01 |
| — (🔁 **`S15-PAYROLL-BE-3`**, mig `0575`) | **THÊM** CHECK `bonus_penalties_four_eyes_check` `status <> 'Approved' OR decided_by IS DISTINCT FROM created_by` (VALIDATED; preflight RAISE nếu còn hàng tự duyệt) | chốt cuối four-eyes của thưởng/phạt (nợ security-review DB-1B MEDIUM): trước đó chỉ có `decided_pair_check` ⇒ INSERT/UPDATE thô hàng `Approved` tự duyệt được máy tính lương nhặt vào lương. `IS DISTINCT FROM` để `decided_by` NULL (FK `SET NULL`) không nổ. Service 027 tiền-kiểm; race map 409 PAYROLL-ERR-012 `self-approval`. Cùng migration: vá hàng hệ thống `NGHI_KHONG_LUONG` → `earning` công thức âm (tắt ĐÚNG trigger `salary_component_system_freeze` trong một khối DO, khuôn `0574`) |
...
chk bonus_penalties_four_eyes_check       status <> 'Approved' OR decided_by IS DISTINCT FROM created_by   -- mig 0575 (S15-PAYROLL-BE-3)
...
>
> 🔁 **ĐÍNH CHÍNH 14/09/2026 (`S15-PAYROLL-DB-2`, mig `0572` — thực thi thật khác khối SQL trên ở HAI chỗ):**
>
> 1. **Kỳ v1 đã `Locked` cũng vi phạm `paid_pair_check`** — khối trên chỉ backfill `Paid`, trong khi vế trái `paid_pair_check` có `Locked` và kỳ đi `Paid → Locked` có `paid_by` NULL. Không hạ `Locked` được (terminal). **Owner chốt 14/09 lối A:** thêm bước **(3b)** `UPDATE … SET paid_by = published_by, paid_at = published_at WHERE status = 'Locked' AND paid_by IS NULL` (đếm trước/sau, verify khớp). **Dấu hàng di sản máy đọc được:** `paid_at = published_at AND paid_by = published_by` — BE-4/FE-3 không hiện kỳ di sản «đã chi» cho người không có dòng chi.
> 2. **`submitted_pair_check` và `approved_pair_check` cũng phải thêm `'Published'` vào vế trái.** Không có thì hàng `Published` thiếu `submitted_*` lọt DB (`published_pair_check` chỉ ép `approved_*` + `published_*`). `0572` DROP+ADD **cả bốn** CHECK cặp; khối VERIFY so `pg_get_constraintdef` **đúng bằng** chuỗi kỳ vọng (không `LIKE`).
>
> Toàn bộ chuỗi chạy trong **một khối `DO`** (nguyên tử bất kể migrator gói tx thế nào), đếm **không lọc `deleted_at`** (CHECK soi cả hàng xoá mềm). ⚠️ **Cấm rollback artifact BE về trước mốc `0572`** khi chưa có migration đảo: BE cũ ghi `'Paid'` thiếu `paid_by` ⇒ 500, và lọc `{Paid, Locked}` ⇒ `/me/payslips` `200 []`.
>
...
> | Nền | `LUONG_CO_BAN` · `PHU_CAP` · `NGHI_KHONG_LUONG` | `earning` (🔁 `NGHI_KHONG_LUONG` là `earning` công thức ÂM từ seedVersion v4 / mig `0575` — trước đó `deduction` làm TNCN tính trên tiền nghỉ không lương, `S15-PAYROLL-BE-3` O-5) | `formula`/`profile_item` | `false` |
> | 🔁 **Seed ở BE-2** (`seedVersion v2`) | `THUONG` (earning, `SYS_BONUS_AMOUNT`) · `PHAT` (deduction, `SYS_PENALTY_AMOUNT`) · `TAM_UNG` (deduction, `SYS_ADVANCE_AMOUNT`) | theo mã | `formula` | `false` |
>
> 🔁 **BA thành phần `THUONG` · `PHAT` · `TAM_UNG` CHƯA SEED ở DB-1 — CÓ CHỦ ĐÍCH** (security review 11/09/2026). Giá trị của chúng là **đầu vào THEO DÒNG** (thưởng/phạt đã duyệt của kỳ; tạm ứng `Approved` chưa khấu trừ), mà không gian tên `SYS_*` của SPEC-11 §13.6 D — khai **ĐÓNG** — **không có biến nào** biểu diễn được. Hai lối sai đều tệ: seed bằng REF **trần** (`BONUS_AMOUNT`…) là tự tay tạo lỗ **shadowing** (theo grammar, REF trần = *mã thành phần* ⇒ tenant tạo được hàng cùng tên và CHE đầu vào engine, mà `code_shape_check` chỉ cấm tiền tố `SYS_`/`TL_`/`GT_` nên không bắt); seed `fixed_amount = 0` là fail-open im lặng (khoản thưởng biến mất, `net ≥ 0` vẫn đúng). ⇒ **nợ `S15-PAYROLL-BE-2`**: mở rộng `SYS_*` ở SPEC-11 §13.6 D rồi seed ba hàng này cùng lượt, **bump `seedVersion`**. Ca `E13` của `s15-payroll-db1-seed.int-spec.ts` ghim chủ đích này.
> ✅ **Trả nợ ở `S15-PAYROLL-BE-2` (13/09/2026):** SPEC-11 §13.6 D mở rộng `SYS_*` bằng ba biến trên rồi seed ba hàng (`seedVersion v1 → v2`); ca `E13` lật thành «có mặt, công thức là CHÍNH biến `SYS_*`».
> 🔴 **Seeder chạy MỖI LẦN BOOT — không được hoàn tác chỉnh sửa mẫu của người dùng** (plan-review BE-2 B2): không tạo lại `MAU_MAC_DINH` đã xoá mềm; chỉ link thành phần **vừa được chèn** trong chính lượt seed; lấy cùng advisory lock `payroll-catalog:<company>` với route ghi catalog/mẫu. Assert (5) = «mẫu mặc định (nếu còn sống) chứa đủ 4 nút tổng hợp»; assert (6) = «tập mã `is_system` sống ĐÚNG BẰNG hằng TS».
> 🔁 **Assert (5) nâng + (7) mới — `S15-PAYROLL-DB-1B` (seedVersion `v3`, file `payroll-master-data.integrity.ts`):** (5) = mẫu mặc định do seeder tạo (nếu còn sống) phải **BIÊN DỊCH** được như đường TÍNH — `compileGraph(requireEngineNodes)` trên CÙNG hàng `componentsTx` + CÙNG ánh xạ `templateGraphComponent` của 053/054; thành phần ngưng dùng/xoá mềm trong mẫu ⇒ ném (053/054 cũng từ chối). (7) = **NỘI DUNG** mọi hàng `is_system` sống có mã thuộc hằng (`kind` · `value_type` · `formula` · `pit_deductible` · `fixed_amount` · `is_active`) == đúng giá trị seeder chèn, báo ĐÍCH DANH mã + trường; KHÔNG so `name`/`sort_order` (người dùng sửa được). (6) chỉ so TẬP MÃ nên mù với lệch nội dung.
> 🔴 **Nợ phát hiện ở plan-review BE-2 (B1) — WO `S15-PAYROLL-DB-1B`:** công thức seed `LUONG_CO_BAN` chia pro-rate theo `SYS_PRESENT_DAYS` trong khi `NGHI_KHONG_LUONG` trừ thêm ngày nghỉ không lương ⇒ **trừ HAI LẦN** (quyết định owner 2026-09-01 ở SPEC-11 §13.4: tử số = `present + unpaid`, kẹp trần 1). Hàng bị trigger đóng băng ⇒ vá bằng migration có `DISABLE/ENABLE TRIGGER` + assert (7) công thức == hằng. `S15-PAYROLL-BE-3` phụ thuộc WO này.
> ✅ **Trả nợ ở `S15-PAYROLL-DB-1B` (mig `0574`, 14/09/2026):** `LUONG_CO_BAN = MIN(SYS_BASE_SALARY * (SYS_PRESENT_DAYS + SYS_UNPAID_LEAVE_DAYS) / SYS_WORK_DAYS, SYS_BASE_SALARY) * SYS_PAY_RATIO / 100`; `NGHI_KHONG_LUONG` GIỮ NGUYÊN. Thứ tự nhân/chia chốt BẰNG SỐ ĐO (51.584 ca so số học chính xác bằng BigInt: khớp 100%). Migration tắt ĐÚNG trigger `salary_component_system_freeze` + UPDATE hàng mang chuỗi cũ CHÍNH XÁC + bật lại trong MỘT khối DO; preflight fail-closed khi có hàng công thức THỨ BA hoặc đã có dòng lương tính theo mẫu (`template_fingerprint`); ghi đè của người dùng mang chuỗi cũ chỉ được BÁO, không sửa. Kèm index RI `payroll_template_components_company_component_idx (company_id, component_id)` (database-review BE-2 LOW-2).
> 🔁 **Công thức seed dùng ĐÚNG ba họ của §13.6 D và `FUNC` hợp lệ của §13.6 A**, không dùng tên trần: `SYS_BASE_SALARY` · `SYS_PAY_RATIO` · `SYS_PRESENT_DAYS` · `SYS_WORK_DAYS` · `SYS_UNPAID_LEAVE_DAYS` · `SYS_INSURANCE_SALARY` · `TL_*` · và `BH_TRAN_BHXH/BHYT/BHTN(...)` để **KẸP TRẦN** căn cứ đóng (§13.7 B — nhân thẳng không kẹp thì người lương cao bị trừ **vượt trần** và ba cột `si_cap`/`hi_cap`/`ui_cap` thành trang trí), `TNCN_LUY_TIEN(...)` cho thuế. Ca **E11** census MỌI REF của mọi công thức đã seed — đó là ca biến `code_shape_check` thành chốt THẬT.
> 🔁 **Hàng `is_system` còn được đóng băng bằng TRIGGER `salary_component_system_freeze`** (mig `0570`), không chỉ bằng CHECK. CHECK `system_not_deletable` chỉ gác `deleted_at`; một câu `UPDATE ... SET kind='deduction', value_type='fixed', fixed_amount=0 WHERE code='TONG_KHAU_TRU'` lọt qua **cả ba** CHECK và hạ cấp nút engine ⇒ **`net = gross` cho cả công ty** mà mọi bất biến SQL vẫn xanh. Trigger cho sửa `name`/`sort_order`, đóng băng phần còn lại (kể cả `is_active` và `pit_deductible`).
...
>
> 🔁 **ĐÍNH CHÍNH 11/09/2026 (security review `S15-PAYROLL-DB-1`) — bảng này VẪN PHẢI vào `RetentionService.PROTECTED_TABLES`.** Bản trước viết «KHÔNG cần», lập luận rằng tiêu chí của tập đó là «thiếu `GRANT DELETE` ⇒ retention ăn `42501` uncaught». Đó là **phân loại, không phải an toàn**: tiêu chí THẬT ghi ở đầu docblock của tập là «bảng `runCleanup` **TUYỆT ĐỐI KHÔNG được xoá**». Đường khai thác đo được: `POST /foundation/retention-policies` nhận `entityType` **chuỗi tự do** (contract chỉ ép regex, KHÔNG có allowlist bảng) · `_deleteEligible` lọc theo **`created_at < cutoff`, KHÔNG theo `deleted_at`** ⇒ hard-delete hàng cấu hình **ĐANG SỐNG** · `mediaos_app` CÓ DELETE ⇒ lệnh chạy thật ⇒ mẫu còn nhưng danh sách thành phần RỖNG («bảng 0 cột mà không lỗi»), và mẫu do tenant tạo + mọi `formula_override`/`is_visible`/`sort_order` **mất vĩnh viễn** (không soft-delete; `audit_logs` chỉ giữ vết lần SỬA). Thêm vào tập KHÔNG đụng API-053 (route tự phát DELETE, không qua retention) ⇒ chi phí bằng 0.
> ⚠️ Ngoài ra `fk-tenant-census` và ratchet GRANT **sẽ thấy một DELETE mới** ⇒ WO DB cập nhật pin/ratchet **cùng commit**, kèm comment trỏ về đoạn này.
...
> 🔁 **ĐÍNH CHÍNH 14/09/2026 (`S15-PAYROLL-DB-2`, mig `0572` — plan-review vòng 1 B1/B2/M1/M2/M3): trigger ở đây KHÔNG đứng một mình được — thực thi thành BA trigger:** **T1 `payroll_payment_batch_freeze`** (`BEFORE INSERT OR UPDATE` trên đợt: `payroll_period_id` **bất biến ở MỌI trạng thái** — đổi kỳ của một đợt `Draft` đã có dòng làm luật PHỦ tính dòng kỳ cũ cho kỳ mới; đợt `Completed` terminal, chỉ sửa `note`; cấm INSERT thẳng `Completed`) · **T2 `payroll_payment_line_guard`** (`BEFORE INSERT OR UPDATE` trên dòng: đóng băng thêm `batch_id`/`user_id`/3 snapshot/`deleted_by`; cấm INSERT vào và chuyển sang/ra đợt `Completed`; phiếu phải **cùng user** với dòng và **cùng kỳ** với đợt; đọc đợt `FOR SHARE`; lookup rỗng ⇒ fail-closed) · **T3 `payroll_advance_freeze_guard`** (§14.1 — khuôn `bonus_penalty_freeze_guard` + INSERT chỉ ở `Pending` sạch có `created_by`, bind/nhả consume chỉ khi kỳ ∈ `{CollectingData, Calculated}` đọc `FOR SHARE`). Message `<trigger>:<tag>: …` với tag thuộc danh sách ĐÓNG — map lỗi ở SPEC-11 §12.1. **T1 còn cấm xoá mềm đợt có dòng sống** (tag `has-active-lines` — dòng giữ slot `payslip_uq` dưới đợt «đã biến mất» làm phiếu bị khoá vĩnh viễn) và T2 coi đợt đã xoá mềm là `not-found`: gỡ dòng TRƯỚC, xoá đợt SAU. ⚠️ **Xoá CỨNG user đã chạm hàng đóng băng sẽ ABORT**, không `SET NULL` êm — FK `SET NULL (completed_by/decided_by/created_by)` bắn UPDATE mà trigger coi là sửa vết. Fail-safe có chủ đích (user chỉ xoá mềm, bất biến #2); runbook ops không được xoá cứng user để «dọn». Bổ sung CHECK ngoài bảng §14: `payroll_advances_deducted_bound_check` · `payroll_advances_four_eyes_check` · `payroll_payment_lines_bank_pair_check`; index `payroll_advances_company_period_idx`.
>
