...
3. **Append-only**: `candidate_stage_events` — app role `GRANT SELECT, INSERT`, **không UPDATE/DELETE** (bất biến #2). `interview_participants` chỉ INSERT. Không bảng RECRUIT nào có DELETE cho app role. `mediaos_worker` nhận `SELECT` trên cả 8 bảng theo KHUÔN `0549`/`0552` (đồng nhất verify 2d; role `NOBYPASSRLS`, vẫn phải qua GUC) — RECRUIT v1 **chưa có** system job tiêu thụ; nếu tới `S12-RECRUIT-BE-1`/Phase sau vẫn không có job nào đọc `candidates`/`offers` thì thu hồi SELECT worker trên hai bảng PII/lương đó (ghi nhận security review 31/08, MED-6).
...
>
> **Đo lúc thi công DB-1 (31/08/2026, PG 17):** `lower`/`regexp_replace` **không leakproof** (`pg_proc.proleakproof = f`) ⇒ dưới **FORCE RLS**, planner không được đẩy qual chứa chúng vào `Index Cond` trước hàng rào policy — biểu thức nằm ở `Filter`, còn index vẫn được dùng làm **access path theo tiền tố `company_id`** (Bitmap Index Scan ~số hàng ứng viên của tenant — chấp nhận được ở quy mô này). Parity biểu thức (khác một ký tự là mất) chứng minh ở EXPLAIN **owner** (không RLS): `Index Cond` chứa đủ hai vế. Cả hai tầng đo nằm ở ca H1 của `s12-recruit-db1-invariants.int-spec.ts`. BE-1 **vẫn phải dùng đúng biểu thức** — đổi biểu thức là mất luôn cả đường parity lẫn khả năng nâng cấp sau (generated column + index thường nếu quy mô đòi).
>
