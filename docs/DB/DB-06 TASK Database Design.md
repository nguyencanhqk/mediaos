...
### 4.16 `parent_task_id` là đường sống — cây công việc con đúng 1 cấp

> **Đổi 20/07/2026** — quyết định gốc: [DECISIONS-05](<../DECISIONS/DECISIONS-05_Task_Subtask_And_Leaf_Counting.md>) (D-31…D-41). Trước đợt này `parent_task_id` chỉ là cột dự phòng ("sub-task nếu dùng"); **từ nay nó là đường dữ liệu SỐNG** và mọi truy vấn đếm của TASK phụ thuộc vào nó.

Công việc con là **task thật** trên `tasks.parent_task_id`, **đúng 1 cấp** (cha phải có `parent_task_id IS NULL`), **bắt buộc cùng `project_id` với cha**.

#### Hai vị từ "con" — đặt tên tường minh, KHÔNG được hợp nhất (D-32)

| Tên | Điều kiện | Dùng cho |
| --- | --- | --- |
| `ACTIVE_CHILD` (**cấu trúc**) | `c.parent_task_id = t.id AND c.company_id = t.company_id AND c.deleted_at IS NULL` — **mọi trạng thái, kể cả `Cancelled`** | xóa lan · luật độ sâu · câu hỏi "task này có phải là cha không" |
| `COUNTABLE_CHILD` (**đếm**) | `ACTIVE_CHILD` **AND** `c.task_status IS DISTINCT FROM 'Cancelled'` | định nghĩa **lá** · mẫu số tiến độ · lọc board theo người phụ trách |

```sql
-- LÁ (D-34): task chưa xóa và KHÔNG có COUNTABLE_CHILD nào.
-- Task không có con thì chính nó là lá.
NOT EXISTS (
  SELECT 1 FROM tasks c
   WHERE c.parent_task_id = tk.id
     AND c.company_id     = tk.company_id      -- BẤT BIẾN #1: defense-in-depth trên RLS
     AND c.deleted_at IS NULL
     AND c.task_status IS DISTINCT FROM 'Cancelled'
)
```

**Vì sao phải là hai vị từ, không phải một:** nếu "lá" dùng `ACTIVE_CHILD` thì một task cha `Todo` đang **quá hạn** mà có đúng 1 con `Cancelled` sẽ rớt khỏi mọi con số ⇒ dự án hiện *"0 việc phải làm, 0 quá hạn"* trong khi cha vẫn sống và trễ hạn. Ngược lại, nếu xóa lan dùng `COUNTABLE_CHILD` thì con `Cancelled` thành **mồ côi**. Cả hai vị từ được pin bằng test — đừng gộp cho gọn.

**`company_id` trong subquery là bắt buộc** ở mọi câu dùng hai vị từ trên (BẤT BIẾN #1 — ép ở tầng DB, không dựa kỷ luật dev).

#### Bất biến cây và luật khóa hàng (D-33)

Bốn phép kiểm khi gán `parent_task_id = P` cho task T: **(a)** `P ≠ T` · **(b)** P tồn tại, cùng company, chưa xóa · **(c)** `P.parent_task_id IS NULL` · **(d)** T không có `ACTIVE_CHILD` *(chỉ khi update)*.

**Khóa hàng là MỘT PHẦN của bất biến, không phải tối ưu hiệu năng.** Dưới `READ COMMITTED`, bốn phép kiểm-rồi-ghi **không serialize**: `PATCH A {parent:B}` ‖ `PATCH B {parent:A}` sẽ commit cả hai và tạo **chu trình**. Luật khóa — **một luật cho mọi đường ghi, không ngoại lệ**:

```sql
SELECT id FROM tasks
 WHERE company_id = $1 AND id = ANY($2)   -- $2 = TOÀN BỘ tập hàng thao tác sẽ chạm
 ORDER BY id                              -- thứ tự khóa TOÀN CỤC, id tăng dần
   FOR UPDATE
```

rồi **đọc lại sau khóa** mới kiểm. Giữ **đúng một** câu (node `LockRows` nằm trên `Sort`; tách ra là mất bảo đảm thứ tự) và khóa **trên một bảng duy nhất, không JOIN**. Tập hàng phải khóa theo từng đường (kể cả **cha CŨ** khi đổi cha) nằm ở DECISIONS-05 D-33 — thiếu cha cũ là một lỗ hổng thật: con sống của một cha không liên quan bị xóa lan, câm.

**Mọi `UPDATE` mang vị từ cấu trúc phải assert số hàng ảnh hưởng**, lệch ⇒ rollback. Lỗi `40P01`/`40001` map thành 409 retry-able, không để rơi ra 500 raw.
...
| `tasks.id` -> `tasks.parent_task_id` | 1-n self-reference | **Công việc con — đường sống, đúng 1 cấp** (§4.16). FK **composite** `(parent_task_id, company_id) → (id, company_id)` chống cha cross-tenant |
...
-- ────────────────────────────────────────────────────────────────────────────
-- Công việc con (§4.16, DECISIONS-05) — thêm 20/07/2026, migration 0503.
-- ────────────────────────────────────────────────────────────────────────────

-- BACKSTOP CROSS-TENANT Ở TẦNG DB (BẤT BIẾN #1).
-- FK gốc `parent_task_id REFERENCES tasks(id)` KHÔNG mang company_id, và RI check
-- của Postgres BỎ QUA RLS ⇒ nếu chỉ có nó thì chống cha cross-tenant hoàn toàn dựa
-- vào app-check. FK composite dưới đây ép ràng buộc xuống tầng DB.
ALTER TABLE tasks
ADD CONSTRAINT uq_tasks_id_company UNIQUE (id, company_id);

ALTER TABLE tasks
ADD CONSTRAINT fk_tasks_parent_same_company
FOREIGN KEY (parent_task_id, company_id) REFERENCES tasks (id, company_id);

-- Vị từ ACTIVE_CHILD / COUNTABLE_CHILD (§4.16) và aggregate tiến độ theo
-- `parent_task_id = ANY(...)`. FK KHÔNG được Postgres tự index ⇒ phải tạo tay.
CREATE INDEX IF NOT EXISTS tasks_parent_active_idx
ON tasks (company_id, parent_task_id)
WHERE deleted_at IS NULL;
```

> **Về `tasks_parent_active_idx`.** Lợi ích thật nằm ở aggregate tiến độ (`parent_task_id = ANY($ids)`) và vị từ lá **theo từng dự án**. Với truy vấn dựng lại materialized view **toàn bảng**, Postgres nhiều khả năng chọn hash anti-join — index này không phải là thứ quyết định ở đó, đừng hứa hẹn quá mức.
>
> **`UNIQUE (id, company_id)` + FK composite khóa bảng khi build.** Đo `count(*)` trước khi áp; hệ đang chạy đơn công ty (N=1) nên bảng nhỏ và build trong migration là chấp nhận được. Nếu số hàng lớn bất ngờ ⇒ **DỪNG**, ghi lý do và giữ app-check, KHÔNG khóa bảng dài trong lúc migrate.
...
16. **Cây công việc con đúng 1 cấp** (§4.16): cha phải có `parent_task_id IS NULL`; task đang có `ACTIVE_CHILD` không được trở thành con; không tự làm cha chính mình (đã có CHECK `chk_tasks_not_self_parent`, service vẫn kiểm để trả 400 sạch thay vì `23514` raw).
17. **Con cùng `project_id` với cha** (cả hai NULL cũng hợp lệ). `project_id` của một cây là **bất biến**: task có con không đổi được `project_id`; task là con không đổi được `project_id` riêng. Vi phạm ⇒ 400, không cascade.
18. **Con luôn `state_id = NULL`** và giữ NULL sau mọi lần đổi trạng thái — việc con không lên board. Ràng buộc này phải chốt ở **phương thức dùng chung** của các đường ghi `state_id`, không rải ở từng route.
19. **Xóa cha kéo theo soft-delete mọi `ACTIVE_CHILD`** (kể cả con `Cancelled`, nếu không sẽ mồ côi), trong **cùng một giao dịch**, tất-cả-hoặc-không. **Hủy (`Cancelled`) cha KHÔNG hủy con.**
20. **Đếm cho dashboard/báo cáo tính theo LÁ** (§4.16): dashboard MV · báo cáo tiến độ dự án · widget tiến độ phải dùng **cùng một vị từ, cùng một release** — lệch pha giữa ba nơi cho hai con số khác nhau trên cùng một màn hình. Các màn **danh sách** việc phải xử lý thì **không** áp đếm-lá (hiện cả cha lẫn con).
...
tasks_parent_active_idx
```

> `tasks_parent_active_idx (company_id, parent_task_id) WHERE deleted_at IS NULL` phục vụ vị từ **lá** (§4.16) của báo cáo/dashboard và aggregate tiến độ việc con theo `parent_task_id = ANY(...)` cho một trang board — **một** truy vấn cho N thẻ, không N+1.
...
3. FK self-reference `tasks.parent_task_id` tạo constraint **sau** khi tạo bảng. Bản **composite** `(parent_task_id, company_id) → tasks(id, company_id)` (§4.16) đòi `UNIQUE (id, company_id)` phải tồn tại **trước** — hai lệnh này đi cùng một migration, đúng thứ tự đó.
