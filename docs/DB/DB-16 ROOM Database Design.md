...
| `booked_by_user_id` | UUID | Không | composite FK → `users (company_id, id)` `SET NULL (booked_by_user_id)` — người thao tác (≠ organizer khi đặt hộ) |
...
ALTER TABLE room_bookings ADD CONSTRAINT room_bookings_booked_by_tenant_fk    FOREIGN KEY (company_id, booked_by_user_id) REFERENCES users (company_id, id) ON DELETE SET NULL (booked_by_user_id);
