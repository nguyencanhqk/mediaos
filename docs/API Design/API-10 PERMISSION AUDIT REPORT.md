...
> **CHỐT 2026-07-02 (S2-FND-DOC-1):** Catalog THỰC THI chuẩn = tuple `(action, resource_type)` namespace `foundation-*` seed `0435` (xem [Matrix §5.8](<API-10 PERMISSION MATRIX.md>)); cờ `sensitive` khớp seed là authoritative. GHI CHÚ 2 gap ĐÃ BIẾT — KHÔNG sửa ở WO này: (1) **7 cặp seed-orphan** (seed 0435 nhưng chưa có endpoint tiêu thụ, vd. `run:foundation-seed`, `view`/`run:foundation-job`, `manage:foundation-retention` — controller retention/seed hiện READ-ONLY, module system-jobs chưa tồn tại); (2) **pair-drift audit-log (H4)**: code gate `('view','audit-log', sensitive)` trong `audit.controller` ≠ cặp catalog seed 0435 `('view','foundation-audit-log')`. Cả 2 xử lý ở WO **FND-AUDIT-PAIR-PIN**. Drift `read`/`update:company` (seed `0005`) nay chỉ còn **catalog-noise** — route company/current live gate `('view'/'update','foundation-company')` (seed 0435), không dùng cặp `company` cũ.
