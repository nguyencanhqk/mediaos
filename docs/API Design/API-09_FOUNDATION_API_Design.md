...
> **CHỐT 2026-07-02 (S2-FND-DOC-1):** Mô hình quyền THỰC THI = tuple `(action, resource_type)` với namespace `foundation-*` khớp seed `0435_foundation_db5_retention_seed_modules.sql` (vd. `view`/`update` × `foundation-company`; `download` × `foundation-file`). Notation `MODULE.RESOURCE.ACTION` (vd. `FOUNDATION.FILE.DOWNLOAD`) trong doc này chỉ là NHÃN hiển thị/ánh xạ 1-1 sang tuple — permission engine KHÔNG dùng chuỗi dotted. Đây là lệch-có-chủ-đích doc↔code đã pin (code thắng).
...
> **CHỐT 2026-07-02 (S2-FND-DOC-1):** 5 internal REST `/internal/v1/foundation/*` (audit-logs · sequences/generate · files/link · settings/resolve · public-holidays/check) THỰC THI = **in-process service call** trong modular monolith — KHÔNG expose HTTP route (không có `@Controller('internal/...')`); đây là lệch-có-chủ-đích đã pin (in-process thắng REST). NGOẠI LỆ: `retention/cleanup-jobs/run` + `seeds/run` là THIẾU THẬT (chưa có trigger/route — retention.controller chỉ `GET retention-policies`, seed.controller chỉ `GET seeds`) ⇒ đây là GAP mở, KHÔNG phải điểm pin.
> **CHỐT 2026-07-02 (S2-FND-DOC-1):** Path/method lệch nhỏ đã CHẤP NHẬN (code thắng): preview mã sequence = `GET /foundation/sequences/:id/preview` (KHÔNG `POST /sequences/{key}/preview`) · check ngày = `GET /foundation/holidays/check-working-day` (KHÔNG `/public-holidays/check`) · trạng thái seed = `GET /foundation/seeds` (KHÔNG `/seed-batches`) · file-link NESTED dưới file = `POST /foundation/files/:id/links` + `DELETE /foundation/files/:id/links/:link_id` (KHÔNG `/file-links` top-level).
...
> **CHỐT 2026-07-02 (S2-FND-DOC-1):** Bảng dotted trên = NHÃN; tuple engine thật ở seed `0435` (`foundation-*`). Cờ `is_sensitive=TRUE` theo seed 0435: `update:foundation-module`, `system-manage:foundation-setting`, `manage:foundation-retention`, `run:foundation-job`, `view:foundation-seed`, `run:foundation-seed`; tất cả cặp `foundation-*` còn lại `is_sensitive=FALSE`.
...
> **CHỐT 2026-07-02 (S2-FND-DOC-1):** OUT-OF-SCOPE ở v2 single-tenant (N=1). `GET /api/v1/foundation/companies` là nhóm multi-company/tenant — CẮT tới khi mở multi-company; không implement, không seed route. Reconcile khi bật multi-company.
...
> **CHỐT 2026-07-02 (S2-FND-DOC-1):** OUT-OF-SCOPE ở v2 single-tenant (N=1). `POST /api/v1/foundation/companies` (tạo company/tenant mới) là nhóm multi-company — CẮT tới khi mở multi-company; không implement, không seed route. Reconcile khi bật multi-company.
...
> **CHỐT 2026-07-02 (S2-FND-DOC-1):** OUT-OF-SCOPE ở v2 single-tenant (N=1). Cả `POST /companies/{company_id}/suspend` VÀ `POST /companies/{company_id}/activate` là thao tác vòng đời tenant liên-công-ty — CẮT tới khi mở multi-company; không implement, không seed route. Reconcile khi bật multi-company.
...
> **CHỐT 2026-07-02 (S2-FND-DOC-1):** `GET /api/v1/foundation/modules/my-apps` (App Switcher/Home Portal) là **Authenticated-only** — controller KHÔNG gắn `@UseGuards(PermissionGuard)`/`@RequirePermission`; service TỰ LỌC theo capability của user (requiredAny + hasAnyCapability), KHÔNG gate bằng một quyền `foundation-*` cụ thể. Đây là lệch-có-chủ-đích đã pin: hiển thị app phụ thuộc capability đã cấp, không phải một permission Foundation đơn lẻ.
...
> **CHỐT 2026-07-02 (S2-FND-DOC-1):** THỰC THI = **302 redirect** tới presigned URL TTL-ngắn (`res.redirect(302, url)` trong `files.controller`), KHÔNG stream nhị phân qua backend và KHÔNG lộ `storage_path`/provider secret cho client. `FilePolicy.canDownload` là chốt quyền (deny → 403 + log). Đây là lệch-có-chủ-đích đã pin (presigned thắng stream).
