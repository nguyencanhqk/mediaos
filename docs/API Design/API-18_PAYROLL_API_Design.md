...
> | 5   | **§15 hàng 014**             | `Approved → Paid`                                | **`Approved → Published`** _(NOTI-EVENT-023 phát Ở ĐÂY)_                                                                    |
...
| **D** | **PDF phiếu lương**                                | 083–085 | `S15-PAYROLL-BE-5B` _(owner O-1 17/09/2026 tách khỏi BE-5)_ | Phiếu người khác · phiếu **của mình** (không cần cặp export) · **batch → signed-URL ZIP**; sinh từ snapshot `payslip_items`, **không tính lại**                                                      |
...
**Track A — Nhân viên · hồ sơ lương v2 · bảng công** _(`S15-PAYROLL-BE-1`, mã 036–043)_
...
**Track B — Thành phần lương · mẫu bảng lương · tỉ lệ luật định** _(`S15-PAYROLL-BE-2`, mã 044–058)_
...
**Track C — Tạm ứng · chi trả · ngân sách · import** _(`S15-PAYROLL-BE-4`, mã 059–077)_
...
**Track D — Tổng quan · báo cáo · PDF** _(`S15-PAYROLL-BE-5`, mã 078–085)_
...
>
> ⚠️ **Bốn bẫy thứ tự khai route** _(cùng lớp `goals/tree` mà v1 đã vấp ở `summary`)_ — route TĨNH phải khai **TRƯỚC** route `{id}` cùng cấp:
>
...
>
> ⚠️ **MƯỜI route mới nhận `Idempotency-Key` do CLIENT sinh — danh sách ĐÓNG** (SPEC-11 §15.1): **039 · 041 · 045 · 050 · 056 · 060 · 067 · 072 · 074 · 085**. Khoá **không** suy từ payload (`idempotency-key-must-be-content-derived` — server tự suy khoá từ payload làm hai lượt khác nhau đụng nhau); chống trùng **nghiệp vụ** vẫn là việc của UNIQUE ở DB (§6.6). ⇒ `@Idempotent()` THỰC TẾ phủ **9 route mới** (danh sách trên trừ 085) **+ 5 route v1** (002 · 007 · 013 · 020 · 024) = **14 route**. **085 (PDF hàng loạt) CỐ Ý KHÔNG** `@Idempotent()` (17/09/2026, owner O-8 — `S15-PAYROLL-BE-5B`): 085 là lấy-hoặc-tạo, trả `202`/`200` ĐỘNG và body có signed-URL, trong khi interceptor chốt mã TRƯỚC handler và cache body 900 s ⇒ phát lại sai mã + URL cũ; bấm hai lần vẫn ra MỘT lô nhờ advisory lock (người, kỳ). **076 (import multipart) CỐ Ý KHÔNG** `@Idempotent()` (15/09/2026, security-reviewer BE-4 M2): interceptor băm `request.body` TRƯỚC `FileInterceptor` ⇒ vân tay rỗng, mù tệp ⇒ cùng key + tệp khác phát lại phản hồi cũ và im lặng bỏ tệp mới — cùng khuôn HR `employees/import`; chống nạp trùng = `warnings: possible-duplicate:<n>` + cổng duyệt.
>
...
| **Fingerprint hai tầng (BE-2)**                              | `GET /payroll/templates/{id}` (051) trả `formulaSetFingerprint` = băm TẬP CÔNG THỨC của mẫu, **không** kèm bản tỉ lệ; dòng lương ghi `SHA-256(formulaSetFingerprint + ':' + statutory_rate_id)`. Băng «mẫu đã đổi» so hai giá trị này cùng `statutory_rate_id` của dòng (SPEC-11 §13.6 G)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| **PAYROLL-API-083..085**                   | ✅ **Đã hiện thực** (`S15-PAYROLL-BE-5B`, 17/09/2026) | PDF phiếu lương — pdfmake + Roboto nhúng, sinh từ snapshot; tệp TẠM (PDF 15′ · ZIP 24 h) giao qua signed-URL do CHÍNH route PAYROLL cấp (route file chung bị resolver PAYROLL chặn); 085 lấy-hoặc-tạo chạy nền qua outbox; `TEMP_FILE_CLEANUP` xoá cả object (owner O-3..O-8)                                                                                                                                                                                                                                                                                                          |
...
| PAYROLL-ERR-028 | `409` | `batch-empty`                                                                                                                              | Hoàn tất (072) một đợt chi trả **RỖNG** (0 dòng còn hiệu lực) — đường phát DUY NHẤT của 028 (SPEC-11 §12.1; bản nháp `no-completed-batch` là mã chết vì 072 là cửa duy nhất vào `Paid`). Đợt không `Completed`, kỳ không `Paid` (PAY-DEC-017)                                     |
...
>
> **Hai bề mặt của bảng tỉ lệ luật định tách theo HTTP, đừng gộp:** **422 022** = _nội dung bản sai_ (thiếu bậc/trần, 7 bậc không liên tục) — người dùng sửa được ngay trong form; **409 033** = _xung đột với dữ liệu đã có_ (trùng `effective_from`, hoặc bản đã được một kỳ dùng ⇒ tạo bản mới). Cùng lớp phân tách «lúc LƯU vs lúc TÍNH» của 018/019 ↔ 020.
>
