...
>
> ⚠️ **MƯỜI route mới nhận `Idempotency-Key` do CLIENT sinh — danh sách ĐÓNG** (SPEC-11 §15.1): **039 · 041 · 045 · 050 · 056 · 060 · 067 · 072 · 074 · 085**. Khoá **không** suy từ payload (`idempotency-key-must-be-content-derived` — server tự suy khoá từ payload làm hai lượt khác nhau đụng nhau); chống trùng **nghiệp vụ** vẫn là việc của UNIQUE ở DB (§6.6). ⇒ `@Idempotent()` sau v2 phủ **10 route mới + 5 route v1** (002 · 007 · 013 · 020 · 024) = **15 route**. **076 (import multipart) CỐ Ý KHÔNG** `@Idempotent()` (15/09/2026, security-reviewer BE-4 M2): interceptor băm `request.body` TRƯỚC `FileInterceptor` ⇒ vân tay rỗng, mù tệp ⇒ cùng key + tệp khác phát lại phản hồi cũ và im lặng bỏ tệp mới — cùng khuôn HR `employees/import`; chống nạp trùng = `warnings: possible-duplicate:<n>` + cổng duyệt.
>
