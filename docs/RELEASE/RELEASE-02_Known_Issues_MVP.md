...
| KI-025 | **133/499 đường dẫn API (26,7%) không có test HTTP nào chạm** (đo lại 14/08 bằng `route-http-coverage.e2e-spec.ts`, thay số 98/346 cũ) — phủ ở tầng service nên guard/DTO/envelope của route chưa từng chạy. Còn 15 route risk≥5 chưa phủ: `auth/reset-password` · `2fa/disable` · `users/invite`+`activation/accept` · `api-keys` create/revoke · `auth/users/:id` password-reset/restore/soft-delete · `permissions` assign-role/revoke-role/object-grant ×4 · `auth/roles/:id` delete/revoke-permission | S2 | Độ phủ test | ❌ | ❌ | Sau MVP |
...
ghi rõ trong docblock đầu file. Baseline mới: **366/499 (73,3%) có bằng chứng phủ, 133 (26,7%) chưa** —
KHÔNG so trực tiếp được với số 28% cũ (cách đo khác nhau).

Xếp hạng rủi ro (route ghi + nhạy cảm auth/permission/secret/audit lên trước route đọc) rồi phủ HTTP thật
đợt này cho 5/18 route risk≥5÷cao nhất — `apps/api/test/integration/security-mailconfig-http.int-spec.ts`
(11 ca, ALLOW trước DENY): `GET/PATCH /settings/security-policy` + `GET/PUT/POST(test) /settings/mail-config`.

🔴 **Phát hiện thật khi viết test (không phải test hỏng):** `PATCH /settings/security-policy` khai
`isSensitive + requiresReauth` nhưng route KHÔNG có `:id` param ⇒ `resourceId` luôn `null` ⇒
`permission.decide.ts` luôn đòi object-level grant (`needsObjectGrant` luôn true) ⇒ **403
`deny-object-required` VĨNH VIỄN cho MỌI actor** kể cả actor có ALLOW company-level đầy đủ — và
`req.reauthContext` (điều kiện còn lại) không được gán ở BẤT KỲ đâu trong codebase (grep xác nhận). Route
này hiện KHÔNG THỂ gọi thành công qua HTTP. Test mới GHIM đúng hành vi 403 hiện tại; KHÔNG tự sửa guard —
cần WO riêng (thiết kế lại gate cho resource không-có-id, hoặc bỏ `requiresReauth` nếu chưa cần step-up).

**CÒN NỢ (ghi số, không cắt lặng lẽ):** 133 route chưa có bằng chứng HTTP, trong đó **15 route risk≥5**
(liệt kê ở hàng bảng KI-025) và ~10 route risk=3-4 (`2fa/status` · `api-keys/scopes` · `users/pending` ·
`users/me` PATCH · `leave/admin/balances/:id/transactions` · 4 route `workflow` approval/checklist).
Danh sách đầy đủ + risk score in ra khi chạy lại script đo (console output, top-30 hiện ra mặc định).
