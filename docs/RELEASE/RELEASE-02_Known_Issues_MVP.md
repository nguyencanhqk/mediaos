...
| **KI-070** | **Bản vá KI-053/054/069 bound CỘT, KHÔNG bound HÀNG — phát biểu ranh giới thay vì để nó thành khoảng trống im lặng.** Sau `S6-SEC-IDENTITY-PROJ-1`, một vai giữ `view:audit-log@Own` **vẫn đọc được toàn bộ** 364 `login_logs` + 65 `user_security_events` của tenant: `user_id`/`actor_user_id` (UUID), IP, user-agent, mốc thời gian, `failure_reason` — chỉ mất email/họ tên. Hơn nữa `LoginLogFilter.userId` vẫn lấy **thẳng từ query param của caller** (`auth-logs-viewer.service.ts` → `login-log.repository.ts` `buildWhere`) nên vẫn dò được lịch sử đăng nhập của một UUID bất kỳ. KI-053 tương tự: vẫn nhận trọn `userId` + `status` + `expiresAt` của mọi thành viên role. **Vì sao mở thay vì im lặng:** đóng KI-054 mà không ghi ranh giới là lặp đúng lỗi mà chính KI-054 tố cáo — "Company-scope" ghi trong docstring như một sự thật trong khi không có gì ép nó. **Workaround:** không cấp `view:audit-log`/`view:role` ở scope hẹp hơn Company cho vai nào (hôm nay 3 vai giữ, cả ba `@Company` ⇒ chưa ai chạm được lớp này). **Việc thật:** cho `data_scope` chặn TẬP HÀNG của hai bảng nhật ký, hoặc quyết định tường minh rằng nhật ký là Company-only rồi ép bằng `resolveAndAssert` + trần scope | S3 | Bảo mật (phân quyền) | ❌ | ❌ | `S10-SEC-AUDITLOGROW-1` — **ĐÃ GIAO 2026-08-20** (mở 2026-08-19 bởi `S6-SEC-IDENTITY-PROJ-1`) |
