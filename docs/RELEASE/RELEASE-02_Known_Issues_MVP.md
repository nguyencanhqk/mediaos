...
| KI-044 | `login_logs`: hàng `blocked/TooManyAttempts` ghi `company_id = NULL` **kể cả khi companySlug HỢP LỆ**, vì `isLoginRateLimited()` (`auth.service.ts:199`) chạy TRƯỚC `resolveCompanyId()` (`:215`). Đo PROD: **165/268 hàng NULL (~62%) thực ra CÓ CHỦ**. Hệ quả sau `S6-SEC-LOGINLOG-1`: company-admin **không còn thấy** các lần brute-force nhắm vào chính công ty mình qua AUTH-API-401. **KHÔNG phải rò rỉ** (không ai đọc được dữ liệu ngoài phạm vi) — là mất **tầm nhìn của bên phòng thủ**; dữ liệu vốn gắn sai chủ từ S2, 0532 chỉ làm hậu quả lộ ra. Phát hiện độc lập bởi CẢ HAI reviewer của FULL gate. ⚠️ Vá đúng = gắn đúng chủ khi slug resolve được, **KHÔNG nới lại `USING`**; nhưng đổi thứ tự đường login có đánh đổi chưa cân: đẩy truy vấn DB lên trước bộ chặn tần suất ⇒ bề mặt DoS, và chạm cân bằng timing chống dò tenant | S3 | Bảo mật (quan sát) | ❌ | ❌ | `S6-SEC-LOGINLOG-2` (mở 2026-07-28) |
...
| **KI-045** | **Sau rotate của `S6-SEC-ROTATE-1` (28/7), đường chạy int-spec bằng `LANE_DB` KHÔNG còn nối được DB** — hàng rào deny-path/IDOR/cross-tenant trở thành *không chạy được* trên máy này. Hai chỗ còn giữ mật khẩu TIỀN-rotate: `scripts/lane-db-setup.sh:24` (`DEV_PW="${OWNER_DB_PASSWORD:-changeme_dev_only}"`) và `apps/api/test/db-target.ts:97-99` (dựng 3 URL lane từ `changeme_app_only`/`changeme_dev_only`/`changeme_worker_only`). Role thật đã đổi mật khẩu ⇒ `28P01 password authentication failed`; db-fence fail-closed nên **toàn bộ suite api từ chối chạy**. **Nguy hiểm ở chỗ im lặng:** `check.sh --lane-db` khi `lane-db-setup.sh` hỏng thì **chỉ in cảnh báo rồi chạy tiếp KHÔNG có `LANE_DB`** ⇒ kết luận rơi về "XANH KHÔNG ĐỦ BẰNG CHỨNG", dễ bị đọc thành xanh. Chỉ dẫn ngay trong `apps/api/.env` ("cần DB cho test ⇒ dùng LANE_DB + `check.sh --lane-db`") nay **trỏ vào đường đã gãy**. **Đường vòng đã kiểm chứng** (dùng cho `S6-SEC-NOTITX-1`): `export OWNER_DB_PASSWORD=<POSTGRES_PASSWORD của .env>` cho bước setup + truyền **3 URL tường minh** (URL tường minh THẮNG `LANE_DB`, `db-target.ts:75`) trỏ `mediaos_<lane>` với mật khẩu đã rotate ⇒ 449/449 file chạy thật. **Vá đúng** = đọc credential từ `.env` thay vì hằng số `changeme_*`, và để `lane-db-setup.sh` hỏng **fail-closed** thay vì cảnh báo | **S2** | Độ phủ test / gate | ❌ | ❌ | WO mới (đề xuất ghép `S6-SEC-ROTATE-1` — chính nó gây ra) |
| ~~KI-043~~ | **ĐÓNG 2026-07-28** — `S6-SEC-ROTATE-1`. Đã rotate **5 role** (`mediaos` SUPERUSER · `mediaos_owner` · `mediaos_app` · `mediaos_worker` · `pgbouncer_auth`) sang mật khẩu ngẫu nhiên 32 ký tự. **Bằng chứng hai chiều, đo TỪ HOST qua `localhost:5432`** (đường tấn công thật): 3 literal cũ đều `password authentication failed`, ca đối chứng mật khẩu-bậy cũng bị từ chối (chứng minh đang xác thực `scram-sha-256` chứ không phải `trust`), 5/5 mật khẩu mới nối được. ⚠️ Phép thử bằng `docker exec psql -h 127.0.0.1` **CHỨNG MINH SAI** — `pg_hba` của image có `host all all 127.0.0.1 trust`, mọi mật khẩu đều qua. **Nguồn tái nhiễm đã cắt**: `Invoke-Roles` (`mediaos.ps1`) không còn `ALTER ROLE … '<literal>'` mà uỷ quyền cho `scripts/setup-db-roles.mjs` (chỉ đọc env) — chạy lại `m roles` sau rotate rồi thử lại literal cũ: **vẫn bị từ chối**. **Bind**: 5 cổng `5432/6432/6379/9000/9001` từ `0.0.0.0`+`[::]` → **`127.0.0.1`** (firewall KHÔNG dùng làm bằng chứng: máy có 204 rule inbound allow-any-port). **Literal trên file tracked = 0** (gồm cả docs — không có danh sách miễn trừ). **Chốt hồi quy**: `scripts/check-no-secret-literals.mjs` chạy trong `harness/check.sh` + job `secret-scan` của `security.yml` — bắt cả literal đã biết lẫn port mapping thiếu địa chỉ bind (loại lỗ hổng gitleaks mù vì chuỗi `changeme_*` không *trông* giống secret). Script seed (`demo-seed-{base,full,dashboard}` · `seed-operator`) nay **fail-closed**: không khai DB đích ⇒ exit 1; đích là `mediaos`/`mediaos_dev` ⇒ exit 1 trừ khi khai đúng tên qua `SEED_ALLOW_PROTECTED_DB`. **KHÔNG history-rewrite** (quyết định có chủ đích: literal đã public từ lâu, giá trị phòng thủ sau rotate ≈ 0, chi phí thì thật). Dữ liệu sau rotate: `funtime` 46 user, `/health/db` 200, `check-prod-test-tenants` exit 0 | **S0** | Bảo mật | ✅ | ✅ | **ĐÓNG** — `S6-SEC-ROTATE-1` |
...
`S0 = **0 mở**` (**KI-043 rời danh sách 2026-07-28** — đóng bởi `S6-SEC-ROTATE-1`: rotate 5 role + cắt nguồn tái nhiễm + bind loopback + chốt hồi quy, bằng chứng hai chiều đo TỪ HOST; KI-028 · KI-032 · KI-038 **đều đã đóng VÀ verify trực tiếp trên PROD**, riêng KI-028 phải đóng lại lần hai ngày 2026-07-28) · `S1 = **1 mở**` (KI-027) — **KI-030 rời danh sách 2026-07-27**, đóng bởi `S6-SEC-ORG-1` (3→2); **KI-034 rời danh sách 2026-07-28**, đóng bởi `S6-SEC-NOTITX-1` (2→1); KI-033 **đã vá**; KI-035 **đã vá + hạ xuống `S3`** (hai claim của gate đều sai, xem dòng của nó). KI-027 nay chỉ còn chờ admin enroll 2FA rồi bật cờ, vì gốc rễ KI-036 đã vá ·
`S2 = **8 mở**` (KI-008 · KI-011 · KI-016 · KI-021 · **KI-025** · KI-029 · KI-041 · **KI-045**) — **KI-037 rời danh sách 2026-07-29**, đóng bởi `S6-QA-TENANTWRITE-1` (9→8) · `S3 = **18**` (thêm **KI-046**, mở 2026-07-29 từ chính lưới GHI mới).
**KI-045 mở 2026-07-28** trong lúc thi công `S6-SEC-NOTITX-1` — rotate của `S6-SEC-ROTATE-1` làm gãy
đường `LANE_DB`, tức **hàng rào deny-path/IDOR hiện không chạy được bằng lệnh chuẩn** (8 → 9).
