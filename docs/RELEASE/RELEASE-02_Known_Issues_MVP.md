...
| **KI-075** | **Hai job CHAT gọi đường PHÁT realtime SAU COMMIT mà KHÔNG bọc `try/catch` — hợp đồng "emit không được ném" nằm ở MODULE KHÁC và không có gì ghim nó.** `ChatCallRingingTimeoutJobHandler` (`chat-call-ringing-timeout.job-handler.ts:68` — `emitExpired`) và `ChatCallStaleActiveSweepJobHandler` (`chat-call-stale-active-sweep.job-handler.ts:72` — `emitAutoEnded`) đều đặt lời gọi phát NGOÀI `withTenant` và không bọc. **HOÃN CÓ CHỦ Ý** ở `S10-CHAT-CALLSWEEP-1` (KI-063, merge 2026-08-22) với lý do ghi rõ: job ring-timeout **đang chạy PROD với hình dạng Y HỆT**, vá một mình job mới sẽ làm hai job lệch chuẩn nhau ⇒ muốn vá thì vá **CẢ HAI** ở một WO riêng. ⚠️ **ĐÍNH CHÍNH tên hàm so với ghi chú bàn giao:** job ring-timeout gọi **`emitExpired`** (`action:"missed"`), KHÔNG phải `emitAutoEnded` (`action:"ended"`) — cùng KHUÔN, khác hàm; grep đúng một tên rồi tưởng đã phủ cả hai là đúng lớp bẫy mà số hiệu này sinh ra để chống. **Đo mức độ ĐÚNG, đừng thổi lên — hôm nay CHƯA ném được:** `RealtimeEmitterService.emitChatCall` (`realtime-emitter.service.ts:269-299`) đã tự bọc `try/catch` quanh CẢ `wsChatCallEventSchema.parse` lẫn `server.to().emit()`, còn nhánh `!this.server` là `logger.error` + `return` ⇒ per-call KHÔNG ném; phần duy nhất còn lại trong vòng lặp là `call.startedAt.toISOString()`, mà `chat_calls.started_at` khai `notNull` ở schema ⇒ cũng không ném. **Vậy nợ là gì:** hai job dựa vào một bất biến của **module KHÁC** (`realtime/`) mà **không ca test nào ghim, không dòng chú nào khai**. Ai chuyển `.parse()` ra ngoài `try` của emitter, hay cắm một emitter rethrow, sẽ đổi hành vi của HAI job cách đó hai thư mục mà typecheck vẫn xanh — cùng họ `ui-promises-backend-never-reads` (một bên hứa, bên kia không có gì ép). **Hậu quả KHI bất biến đó vỡ (đọc từ code, không suy diễn):** lời gọi phát nằm SAU commit ⇒ hàng `chat_calls` đã đổi trạng thái THẬT; `run()` reject ⇒ `JobRunner` finalize run-row **`'Failed'`** — nói dối theo chiều ngược (việc nghiệp vụ ĐÃ xong) — và vì cả hai job **idempotent theo thiết kế** (hàng vừa đổi không còn khớp vị từ `status`), nhịp kế khớp **0 hàng** ⇒ **sự kiện mất VĨNH VIỄN, chạy lại job KHÔNG sửa được**. Người dùng giữ khung gọi / chuông của một cuộc gọi đã chết, và CALL **không có đường REST bù** — chính docblock của `emitChatCall` khai điều đó (`chat-calls.controller.ts` chỉ có 4 route `@Post` vòng đời, 0 route ĐỌC để poll). **Bán kính có giới hạn:** một tenant một nhịp — `JobRunner` bọc `try/catch` per-tenant (`job-runner.ts:117-133`) nên tenant kế vẫn chạy. **Việc thật khi làm:** bọc **CẢ HAI cùng lúc** (đối xứng là điều kiện, không phải phong cách), log `error` kèm `jobCode` · `companyId` · số cuộc gọi mất sự kiện, và quyết TƯỜNG MINH `JobRunResult` khi phát hỏng (`failed > 0` + `metadata.emitFailed`, **KHÔNG** nuốt thành `success` trọn vẹn — nuốt là lấy mất tín hiệu duy nhất còn lại), kèm ca ghim bất biến "emit không ném" ở đúng biên `realtime/`. **Workaround:** không có và không cần — bất biến hôm nay còn đúng; đây là nợ **ĐỘ BỀN / hợp đồng biên**, cấp số để lần sau ai đụng `realtime-emitter` biết mình đang đứng trên trần nhà của hai job đang chạy PROD | S4 | Độ bền vận hành (job · realtime) | ❌ | ❌ | WO `S10-CHAT-EMITGUARD-1` (todo) |
