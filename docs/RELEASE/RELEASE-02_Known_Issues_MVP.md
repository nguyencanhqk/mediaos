...
| ~~KI-037~~ | Bộ `tenant-isolation.int-spec` **chỉ SELECT** — không có một ca deny GHI chéo tenant nào. ⟲ số đúng: registry **155 bảng** (không phải 156), **446 ca** (không phải 465/468). | S2 | Độ phủ test | ✅ | ✅ | **ĐÓNG 2026-07-29** — `S6-QA-TENANTWRITE-1`: lưới **446 → 1087 ca** (+4 ca ghi/bảng), `WITH CHECK` **đã chứng minh chạy trên 148/153 bảng** |
| **KI-045** | **459 khoá ngoại MỘT-CỘT** nối hai bảng đều có `company_id`: kiểm tra FK của Postgres **bỏ qua RLS theo thiết kế** ⇒ trong ngữ cảnh tenant A gắn được hàng của mình trỏ sang bản ghi của **B**. Đo catalog: 460 FK một-cột, **đúng 1** composite (vừa thêm ở mig `0533`). Đường ĐỌC hôm nay được `innerJoin` che ⇒ không rò trực tiếp, nhưng liên kết chéo tenant TỒN TẠI THẬT + `ON DELETE CASCADE` bắc cầu sang tenant khác | S3 | Bảo mật (toàn vẹn) | ✅ | ❌ | `S6-SEC-XTENANTFK-1` (mở 2026-07-29) |
...
`S2 = **7 mở**` (KI-008 · KI-011 · KI-016 · KI-021 · **KI-025** · KI-029 · KI-041) — **KI-037 rời danh sách 2026-07-29**, đóng bởi `S6-QA-TENANTWRITE-1` (8→7) · `S3 = **18**` (thêm **KI-045**, mở cùng ngày từ chính lưới mới).
