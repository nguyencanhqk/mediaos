...
| **KI-049** | **`GET /org/teams/:id/members` chiếu `userEmail` + `userFullName` mà KHÔNG ép `data_scope`** — N-1c, phát hiện độc lập bởi 2/3 reviewer ở FULL gate của `S6-SEC-ORGSCOPE-1` (2026-07-28), trước nay chỉ tồn tại dưới dạng **ghi chú văn xuôi** trong khối KI-030 nên KHÔNG được tính vào bất kỳ con số severity nào và KHÔNG nằm trong bước *bug scrub toàn bộ S0/S1/S2* trước RC — cấp số hiệu 2026-07-29 để đếm được. Route gate `@RequirePermission("read","team")`; `OrgService.listTeamMembers` chỉ `withTenant`, không resolve scope ⇒ **cặp quyền gate (`team`) LỆCH với lớp dữ liệu trả về (`user`)** — đúng lớp lỗi mà N-1 vừa vá cho `/org/employees` và N-2 vừa chốt động từ. Vẫn còn nguyên trên master sau `S6-SEC-PERMVERB-1` (#305): PERMVERB đổi động từ danh bạ sang `view:user`, còn route này gate động từ `team` nên không bị chạm. **ĐO PROD 2026-07-29 — phơi nhiễm HÔM NAY = 0, nhưng cấu hình sai đã có trong SEED:** cả 3 role giữ `read:team` đều ở `data_scope=Company` (`SA` 6 user active · `company-admin` 1 · `hr-manager` 0) ⇒ **không ai mất quyền khi siết**; và `teams`=**0**, `team_members`=**0** ⇒ chưa có dữ liệu để rò. Nhưng `hr-manager` giữ `read:team@Company` mà **KHÔNG có `view:user` nào** ⇒ role seeded này bị 403 ở `/org/employees` lại đọc được `userEmail` qua cửa teams — lỗ là THẬT trong cấu hình, chỉ đang thiếu dữ liệu và thiếu người giữ role. Thành phơi nhiễm sống ngay khi ai đó tạo team đầu tiên hoặc gán `hr-manager` | **S2** | Bảo mật (phân quyền) | ❌ | ❌ | `S6-SEC-ORGTEAMSCOPE-1` |
