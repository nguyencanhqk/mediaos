...
| **KI-080** | **`POST /attendance/shift-assignments` trả 500 SYSTEM-ERR-001 khi client gửi `employeeId` mà KHÔNG gửi `assignmentScope` — payload TỰ NHIÊN NHẤT của "gán ca này cho nhân viên này".** Cấp số 25/08 khi thi công `S10-QA-ROUTEHTTP-3` (lane QA **KHÔNG vá**, chỉ ghim + cấp số — luật đã cho ra KI-068). **⚠️ ĐỌC MỨC ĐỘ TRƯỚC: hỏng ĐÚNG CHIỀU AN TOÀN ⇒ KHÔNG phải lỗ bảo mật.** Request bị TỪ CHỐI, không hàng nào được ghi (đã chứng minh bằng `SELECT` sau lượt gọi: 0 hàng). Thiệt hại là **hợp đồng API sai** (đáng lẽ 400 ở biên) + **bơm 500 GIẢ vào giám sát**, y hệt lớp KI-068. **Cơ chế (đọc code + đo bằng HTTP thật, không suy đoán):** `createShiftAssignmentSchema` (`packages/contracts/src/attendance.ts:822`) để `assignmentScope` **mặc định `"Company"`**, và `.refine()` chỉ kiểm chiều THUẬN — *"scope `Department`/`Employee` phải có đúng id tương ứng"*. Nó **KHÔNG kiểm chiều NGƯỢC**: scope `Company` mà vẫn kèm `departmentId`/`employeeId` thì Zod cho qua ⇒ payload đi thẳng xuống DB ⇒ vỡ CHECK `chk_shift_assignments_target` (`(scope='Company' AND department_id IS NULL AND employee_id IS NULL) OR …`) ⇒ `AttendanceShiftService.mapError` gói thành `InternalServerErrorException`. **Bản vá đề xuất:** thêm một `.refine()` cho chiều ngược (scope `Company` ⇒ cả hai id phải vắng) — sửa ở CONTRACT nên FE nhận cùng luật, không phải vá riêng ở service. **Bằng chứng:** ca `🔴 GHIM BUG (KI-080)` — `apps/api/test/integration/routehttp3-attendance-leave.int-spec.ts`. ⚠️ **Ca đó GHIM hành vi SAI CÓ CHỦ Ý:** người vá sẽ thấy nó **ĐỎ** — đó là dấu hiệu vá ĐÚNG; hãy **LẬT** sang `400` + kiểm mã lỗi validation, **TUYỆT ĐỐI không nới assert thành `>=400`** để nó xanh với cả hai (bài học `tests-can-pin-a-hole-open`) | S3 | Hợp đồng API / validate biên | ❌ | ❌ | WO `S10-ATT-SHIFTASSIGNSCOPE-1` |
