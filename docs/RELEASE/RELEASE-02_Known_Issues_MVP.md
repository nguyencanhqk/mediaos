...
| ~~**KI-071**~~ | **ĐÃ ĐÓNG 2026-08-22 — `S10-SEC-ROLEMEMBERROW-1`.** `GET /auth/roles/:id/members` nay bound **TẬP HÀNG** theo `data_scope` của cặp gate `view:user`, không chỉ che CỘT danh tính (KI-053). Điểm ĐÚC vị từ thứ **BA** của cơ chế KI-070 (`permission/role-admin.service.ts#rowScopeFor`, đã ký vào `ROW_SCOPE_MINT_PINS`); `listRoleMembersTx` nhận **object có tên** `{ rowScope, identity }` — hai grant CÙNG kiểu ở hai tham số vị trí liền nhau là chỗ để hoán đổi mà typecheck vẫn xanh. **Ngữ nghĩa `Own` (chốt TRƯỚC khi code):** hàng là một TƯ CÁCH THÀNH VIÊN, cột NGƯỜI là THÀNH VIÊN (`users.id` qua inner join) ⇒ `Own` = *"hàng NÓI VỀ tôi"* — cùng họ `login_logs` (KI-070), KHÁC `audit_logs` (KI-072, `Own` = hàng do tôi GÂY RA). Hệ quả CÓ CHỦ Ý: `@Own` gọi role mình không có chân ⇒ **0 hàng + HTTP 200, KHÔNG 404** (404 ở đó là oracle "role này có/không có bạn trong đó"). **KHÔNG có vế V2/V3** (route không nhận filter query-param nào, contract không có `total`) — điểm bản vá này HẸP HƠN KI-070/072, nói ra để không ai đi tìm hai vế đó rồi tưởng plan bỏ sót. ⚠️ **THAY ĐỔI HÀNH VI THỨ HAI, dòng RIÊNG — fail-soft → FAIL-CLOSED:** cặp bound TRÙNG cặp gate ⇒ nhánh cũ của KI-053 (`scope === null` ⇒ bỏ cột danh tính, VẪN trả hàng) nay **không thể với tới** (tầng hàng ném 403 trước) và đã được **gỡ** thay vì để lại code chết. Là SIẾT, không nới; 0 test nào đang phủ nhánh đó. **BA nguyên nhân 403 phải phân biệt khi trực ca — và KHÔNG có K1:** kill-switch `PERMISSION_GUARD_ENABLED=false` **không mở được** route này vì service còn cổng THỨ HAI `assertCan` gọi thẳng `permissionService.can()`, hàm này KHÔNG đọc cờ (khác hẳn KI-070/072, nơi route chỉ có guard). Còn lại: **K2** cửa sổ cache guard 300s — grant/role bị GỠ *hoặc* một hàng DENY vừa được THÊM; **K3** `data_scope` không chuẩn hoá được ⇒ fail-closed VĨNH VIỄN; **K4** lỗi HẠ TẦNG ở câu scope (`resolveStrongestScope` bắt MỌI exception rồi trả `null`) trong khi `can()` phục vụ hoàn toàn từ cache Valkey — `getCompanyRoleGrantsWithScope` là passthrough KHÔNG cache ⇒ một timeout riêng ở câu scope biến "200 + che cột" thành 403. **Số đo PROD 2026-08-22 (ĐO LẠI lúc đóng, câu đo BAO TRÙM):** ⚠️ câu đo của bản nháp `IN (('view','user'),('*','*'))` **SAI — thiếu hai hình dạng**: `matches()` là `(action = a OR action = '*') AND (resource_type = r OR resource_type = '*')`, **hai vế ĐỘC LẬP** ⇒ BỐN hình dạng khớp, và `('view','*')`/`('*','user')` vừa qua được `assertCan` vừa đóng góp `data_scope`. Câu ĐÚNG `WHERE p.action IN ('view','*') AND p.resource_type IN ('user','*')` trả **6 hàng**: `view:user` giữ bởi 4 vai — `SA` (2 người sống; số "10" lúc seed đếm cả hàng `user_roles` đã soft-delete) · `QUẢN LÝ CẤP CAO` (4) · `company-admin` (2) · `hr` (**0 người giữ**) — cộng `*:*` của `SA` + `QUẢN LÝ CẤP CAO`; **cả 6 đều `@Company`**, phân bố `{Company: 6}`, **0 hàng DENY ở MỌI hình dạng** (điều kiện của "0 403 mới"), 0 giá trị scope ngoài chuẩn ⇒ **không có K3**. `user_roles` sống = **57 hàng / 5 role / 47 user**. ⇒ **0 vai chạm được lỗ, bản vá đóng lỗ TIỀM TÀNG, 0 hồi quy** — nay đã chứng minh cả trên đường wildcard. ⚠️ **Cảnh báo cho người CẤP QUYỀN:** cặp này `is_sensitive = false` (cả `*:*` cũng vậy) ⇒ `exact` THẮNG `wildcard` — cấp `view:user@Own` cho một vai đang giữ `*:*@Company` sẽ **HẠ** vai đó xuống `Own`, và sau bản vá điều đó cắt luôn **TẬP HÀNG**, không chỉ cột. Trước WO này nó chỉ mất email/tên. **RED-proof ĐO ĐƯỢC** (không phải dự đoán), int-spec chạy THẬT trên lane DB `mediaos_rolememberrow`: bản vá **18/18 XANH**; **M-A** (THAY `rowScopeSql(...)` bằng vị từ `true` — **THAY, không XOÁ** khỏi `and()`, xoá làm `WHERE` hụt phần tử ⇒ ca DENY xanh-RỖNG) ⇒ **6 ĐỎ** (A2·A3·R-D2·R-D3·R-T1·R-T2) / 12 xanh; **M-B** (`idCol: users.id` → `users.companyId`, thứ `rowScopeSql` **KHÔNG** bắt được vì hai cột CÙNG bảng `users`) ⇒ **2 ĐỎ** ở int-spec (A2·A3) + **2 ĐỎ** ở tầng KHÔNG-cần-DB (U1·U1c); **M-Q** (`ctx.userId` nhận nhầm `actor.companyId`) ⇒ **1 ĐỎ** ở tầng unit (U2) — con số này là unit-only; theo cấu tạo M-Q cũng làm A2/A3 ĐỎ ở int-spec (`Own` thành `users.id = <companyId>` ⇒ 0 hàng), đừng đọc "1 ĐỎ" như số toàn suite. **M-S** (VỨT `scope` vừa phân giải, hard-code `"Company"` vào `rowScopeFor` — đột biến RẺ NHẤT của cả bản vá) ban đầu cho **7/7 XANH** ở tầng unit và chỉ bị int-spec bắt; FULL gate tìm ra, nay U1 assert cả `scope` ⇒ **1 ĐỎ**. ⚠️ M-A **không** bị tầng unit bắt — brand `IdentityGrant` ép được "caller quên TRUYỀN", KHÔNG ép được "repo nhận rồi quên DÙNG"; đó là ranh giới CÓ TÊN của tầng kiểu, và int-spec là lớp duy nhất đóng nó. ⚠️ **BA ca từng XANH-RỖNG, sửa nhờ plan-review hai vòng — ghi lại vì cả ba là khuôn lặp lại được:** (a) **A2 cũ ghim LỖ MỞ** (`rows.length === 3` + câu "tập HÀNG không đổi") và sửa `length` **không đủ** — khối `for (rows.filter(x => x.userId !== uOwn))` bên trong lặp **0 vòng** khi còn 1 hàng ⇒ hai `expect` KHÔNG BAO GIỜ CHẠY mà ca vẫn xanh, vẫn *đọc như* đang chứng minh tầng CỘT ⇒ phải **XOÁ** khối, thay bằng `toEqual([uOwn])`; (b) **U1/U2 xanh-RỖNG theo cấu tạo** vì route gọi `buildUserScopeConditionOn` **HAI lần với tham số giống hệt** (tầng hàng + tầng cột) ⇒ capture ghi-đè chỉ giữ lời gọi THỨ HAI và `toHaveBeenCalledWith` xanh nếu BẤT KỲ lời gọi nào khớp ⇒ đột biến chỉ sửa tầng HÀNG vẫn xanh; vá bằng "assert MỌI lời gọi + sentinel SQL khác nhau mỗi lời gọi + truy vết `rowScope.cond` về đúng lời gọi mang `idCol === users.id`"; (c) **R-T1/R-T2 xanh-RỖNG** nếu actor `@Team`/`@Department` không là thành viên của chính role họ gọi — người KHÔNG có chân thì `Own` cũng cho 0 hàng, nên "0 hàng" không phân biệt được nhánh `default: false` với nhánh `Own` ⇒ mệnh đề "`Team` HẸP HƠN `Own`" (nợ N-1b) mất bằng chứng. Cùng lớp: **R-D3** cần `roleOther` có thành viên THẬT + ca ALLOW đối chứng **R-A3**. **Kèm theo, ca MỚI cho một khoảng trống có sẵn:** route này trước nay **không có ca chéo-tenant nào ở tầng HTTP** (bản nháp plan trích `permission-soft-delete.int.spec.ts` là SAI — file đó toàn ca tầng repository, 0 lời gọi HTTP) ⇒ thêm **R-X1** (role tenant B ⇒ 404 với actor tenant A), ghim BẤT BIẾN #1. **Rollback:** code-only, **0 migration** ⇒ revert PR đủ cho sự cố CODE; **không có feature-flag** (một cờ tắt-bound-hàng là chính lỗ hổng có công tắc). 403 hàng loạt: kill-switch KHÔNG liên quan (xem trên), lối thoát vận hành DUY NHẤT là `UPDATE role_permissions SET data_scope='Company'` cho cặp `view:user` của vai bị kẹt. Thiếu hàng cho vai `@Own`/`@Team` = hành vi ĐÚNG theo thiết kế, xử lý bằng cấp quyền, không bằng revert. ⚠️ **RANH GIỚI — đọc trước khi tin dấu gạch ngang:** (1) **KHÔNG được đọc thành "không còn lộ membership"**. Đường ĐỌC đã bound, nhưng một **oracle TRONG-tenant qua mã lỗi của đường GHI vẫn còn**: `RoleMembersTab` dựng `memberIds` từ danh sách ĐÃ BỊ SCOPE rồi nuôi `toAssign`/`alreadyMembers` của hai dialog batch, nên vai giữ `assign-role:user@Company` **cộng** `view:user@Own` chạy batch rồi đọc item nào 409 sẽ **dựng lại đúng tập thành viên vừa bị giấu** (kèm bộ đếm "N thành viên" nói dối). **Không có leo thang quyền** — cặp GHI là cặp RIÊNG, sensitive, gate BE (`POST /permissions/users/:userId/roles`) + FE (`PermissionGate ASSIGN_ROLE`) độc lập — và oracle này **độc lập với FE**, vá FE không đóng nó. WO này KHÔNG sửa FE (`paths` không có `apps/app/**`). ⇒ **ĐÃ TÁCH SỐ HIỆU: `KI-073`** (WO `S10-SEC-ROLEMEMBERFE-1`, seed cùng ngày). Cấp số TRƯỚC khi dấu gạch ngang này land là có chủ đích: phần DUY NHẤT còn sống sau bản vá mà chỉ nằm dưới dạng văn xuôi trong một hàng đã gạch thì **vô hình với bug scrub** — đúng lỗi đã mắc với KI-049, và đúng lý do KI-071 được tách khỏi KI-070. (2) `Team`/`Department` = **0 hàng** và lưới scope **KHÔNG đơn điệu** (giữ đồng thời `@Own` + `@Team` ⇒ resolve ra `Team` ⇒ **MẤT** hàng) — sai về phía HẸP, sàn hoá là nợ **N-1b**, phải làm cho CẢ BA đường cùng lúc, không lén vá. (3) Tầng CỘT của KI-053 được **GIỮ** nhưng khả-quan-sát của nó trên route này bị **BAO TRÙM** (hai vị từ dựng từ CÙNG scope/builder/cặp cột ⇒ mọi hàng trả về đều trong scope ⇒ `identityInScope` LUÔN true) — **đừng nghiệm thu cơ chế bound-CỘT qua route này**; bằng chứng độc lập sống ở ca B*/C* (`login-logs`/`security-events`). Hai dòng `IDENTITY_VERDICTS` của route đã được phụ đề đúng nội dung đó. (4) **Nợ MỚI, CÓ TÊN:** nay có **BA** bản sao gần giống của `rowScopeFor` — điều kiện gộp mà docblock `ROW_SCOPE_MINT_PINS` đăng ký trước ("khi KI-071 thêm điểm THỨ BA") **đã thoả**, nhưng gộp là refactor CROWN và phải giải bài toán "gộp mà ratchet vẫn thấy TỪNG bề mặt" TRƯỚC; chờ WO riêng, đừng gộp nhân tiện. (5) `RoleMembersTab.spec.tsx` còn ca ghim "hàng thiếu `email`/`fullName` vẫn render" — server nay **không phát ra được** hình dạng đó trên route này; GIỮ ca (phòng thủ FE vẫn đúng, và nó chống bẫy `server-masking-needs-optional-fe-schema`) nhưng nó mô tả một trạng thái server **bất khả**. **Mô tả khuyết tật gốc, giữ để đọc lịch sử:** **Vế bound-HÀNG của `GET /auth/roles/:id/members` — TÁCH RA từ KI-070 khi đóng nửa nhật ký, cố ý không để nó chìm theo.** `PermissionAdminRepository.listRoleMembersTx` (`role-admin.repository.ts`) chiếu danh sách thành viên một role với `where` = `roleId` + `companyId` + `notDeleted` + chưa-hết-hạn — **0 vị từ scope**. Sau `S6-SEC-IDENTITY-PROJ-1`, email/họ tên đã bị che theo `data_scope` (KI-053 đóng), nhưng một vai giữ `view:user@Own` **vẫn nhận trọn `userId` + `status` + `expiresAt` của MỌI thành viên** — tức vẫn biết ai thuộc role nào. **Vì sao phải có số hiệu riêng:** KI-070 bao HAI bề mặt (hai bảng nhật ký + role-members) và workaround của nó bảo vệ cả hai; gạch KI-070 mà không tách số sẽ **xoá mất dòng workaround duy nhất** đang bảo vệ route này — đúng lỗi mà chính KI-070 tồn tại để chống, và đúng lớp bẫy đã mắc với KI-049 (phát hiện không có số hiệu thì vô hình với bước bug scrub trước RC). ⚠️ **ĐÍNH CHÍNH workaround gốc, đo lại 2026-08-21:** KI-070 ghi *"không cấp `view:audit-log`/`view:role` ở scope hẹp hơn Company"* — vế `view:role` **SAI**: route này gate **`view:user`** (`role-admin.controller.ts` `@RequirePermission("view","user")`, service còn `assertCan(view,user)` nữa), đúng như hàng KI-053 đã đính chính 2026-08-19. Chép nguyên văn sang đây sẽ đẻ ra một workaround trỏ vào **cặp không phải cổng**. **Workaround ĐÚNG:** không cấp **`view:user`** ở scope hẹp hơn Company cho vai nào. **Số đo PROD 2026-08-21:** 4 vai giữ `view:user`, **cả 4 `@Company`** — `SA` (10 người) · `QUẢN LÝ CẤP CAO` (4) · `company-admin` (2) · `hr` (**0 người giữ**) ⇒ chưa ai chạm được. **Việc thật:** cho `data_scope` chặn tập hàng của `listRoleMembersTx` — cùng khuôn `rowScopeSql` + `basis:"scoped-predicate"` mà `S10-SEC-AUDITLOGROW-1` vừa dựng; ⚠️ ở đây cặp bound (`view:user`) **TRÙNG** cặp gate ⇒ dùng luật fail-closed của `rowScopeFor` (null ⇒ 403 + log), KHÔNG dùng luật `resolveOrNull` của tầng cột. ⚠️ Đọc §7 của `docs/plans/S10-SEC-AUDITLOGROW-1.md` trước khi thi công: lưới scope trên `users` KHÔNG đơn điệu (`Team`/`Department` fail-closed 0 hàng, hẹp hơn `Own`), nên bound hàng ở đây sẽ **kế thừa** nghịch lý đó | S3 | Bảo mật (phân quyền) | ❌ | ❌ | **ĐÓNG 2026-08-22** — `S10-SEC-ROLEMEMBERROW-1` |
| ~~**KI-072**~~ | **ĐÃ ĐÓNG 2026-08-22 — `S10-SEC-AUDITLOGROW-2`. ⚠️ PHẠM VI ĐÓNG = ĐÚNG HAI route COMPANY `GET /foundation/audit-logs` (+ `/:id`), KHÔNG phải "bảng `audit_logs` đã bound HÀNG"** — census đường đọc (`grep auditLogs apps/api/src`, đo 22/08) cho **2 đường ĐỌC**: đường foundation (đóng ở đây) và **`ChatOversightRepository.listOversightAudit`** (`chat-oversight.repository.ts:335-399`, `CHAT-API-019`, cặp CHAT + `ChatOversightAuditGuard`) — đường thứ hai VẪN mang đúng hình dạng V2 (`opts.actorUserId` thẳng vào `WHERE` `:352-354`, 0 vị từ row-scope, `users.fullName` qua `leftJoin` trần `:386-389`, `metadata` THÔ `:381`), **chưa có WO/KI, cần số hiệu riêng**. Đọc "KI-072 đóng" thành "`audit_logs` đã kín" là đúng cách KI-054 sinh ra. **Bản vá:** `AuditQueryService.rowScopeFor()` — điểm ĐÚC thứ HAI trong `ROW_SCOPE_MINT_PINS` (cố ý KHÔNG trích chung với `auth-logs-viewer`: gộp hai bề mặt vào một lời gọi `fromScope` làm ratchet không còn nhìn thấy bề mặt `audit_logs`) — + họ `AuditRepository.*ForActorTx` nhận `rowScope` BẮT BUỘC ở CẢ `data` lẫn `count(*)`. Họ cũ ĐỔI TÊN thành `*UnscopedTx` và danh sách hộ tiêu thụ của nó bị PIN (`UNSCOPED_AUDIT_CONSUMER_PINS`, ratchet "chiều 8"): tên một mình không đủ vì họ không-bound vẫn public, và `rowScopePredicateMints()` chỉ thấy điểm ĐÚC, không thấy điểm TIÊU THỤ SAI. **`Own` ở bảng này = "hàng do TÔI GÂY RA"** (`actor_user_id` — bảng chỉ có MỘT cột người), ngữ nghĩa KHÁC `login_logs`; hệ quả có chủ đích là **360 hàng `actor_user_id IS NULL`** (job máy) không thuộc `Own` của ai, ghim bằng ca `N1` — nới thành `OR actor_user_id IS NULL` là mở toang. `/:id` ngoài scope trả **404** `AUDIT_NOT_FOUND` cùng mã với cross-tenant miss (mã lỗi không được thành oracle tồn-tại). **Số đo PROD 22/08 (đo lại lúc merge, không chép số lúc seed):** `audit_logs` **13.204 hàng** · 360 hàng NULL actor · `view:audit-log` giữ bởi 3 vai — `SA` (2 người sống), `QUẢN LÝ CẤP CAO` (4 hàng `user_roles`/3 active), `company-admin` (2) — **cả ba `@Company`**, phân bố `data_scope` = `{Company: 3}`, 0 hàng DENY. ⚠️ **Câu đo đó MỘT MÌNH chưa đủ (K4, FULL gate bắt được):** cổng widget `gateOrThrow` gọi `permission.can()` **không truyền `isSensitive`**, nên vai chỉ giữ wildcard `*:*` qua được cổng widget rồi bị `rowScopeFor` (resolve với `isSensitive:true`, lọc EXACT non-wildcard) trả 403 ⇒ widget biến mất im lặng; mà câu đo lọc `(action,resource_type)=('view','audit-log')` **theo cấu tạo không nhìn thấy** vai đó. Câu đo BAO TRÙM đã chạy: `… IN (('view','audit-log'),('*','*'))` ⇒ hai vai giữ `*:*` (`SA`, `QUẢN LÝ CẤP CAO`) **đều CÓ LUÔN** `view:audit-log` exact @Company; vai có `*:*` mà thiếu cặp exact = **0**. ⇒ **0 vai chạm được lỗ, bản vá đóng lỗ TIỀM TÀNG, 0 hồi quy — nay đã chứng minh cả trên đường wildcard.** *(Đính chính số lúc seed "SA 10 người": đếm cả hàng `user_roles` đã soft-delete.)* **RED-proof ĐO ĐƯỢC** (không phải dự đoán): trước bản vá **6 ĐỎ / 6 XANH**; **M-A** (THAY `rowScopeSql` bằng `` sql`true` `` ở CẢ `buildWhereForActor` VÀ `findByIdForActorTx` — KHÔNG xoá khỏi `conds`, xoá làm `and()` rỗng ⇒ `WHERE false` ⇒ ca DENY xanh-RỖNG) tái lập ĐÚNG **6 ĐỎ** (R1·R2·R3·N1·T1·C1) / 6 XANH; **M-B** (nhầm `idCol` sang `companyId` — thứ `rowScopeSql` KHÔNG bắt được vì hai cột CÙNG bảng) ⇒ **5 ĐỎ**, gồm ca ALLOW của `audit.service.spec.ts` bắt được **không cần DB**. **Hai đột biến nữa do FULL gate tìm ra, ban đầu SỐNG SÓT, nay đã bị giết bằng ca mới:** **M-N** (`: rowCond` → `` : sql`true` `` — nhánh KHÔNG-filter, tức `GET /foundation/audit-logs` không tham số = lời gọi FE bắn ở lần tải đầu) từng cho **12/12 XANH** vì mọi ca đều neo `?objectId=` để chống nhiễu lane-DB — chính cái neo đẻ ra vùng mù; nay ca **R4** (không neo `objectId`) giết nó, ĐỎ đúng 1/13. **M-Q** (truyền `ctx.userId = actor.companyId`) từng lọt vì ca ALLOW chỉ bắt tham số thứ BA (`target`), không bắt `ctx`; nay ĐỎ đúng ca `ctx`. ⚠️ Trong 6 ca XANH chỉ A1·A4·C2·(và C1 sau vá) mang thông tin — A2·A3·G1 xanh dưới CẢ đột biến lẫn bản vá, đừng đọc "6/6" như một cân bằng đối xứng. **Rollback:** code-only, 0 migration ⇒ revert PR đủ cho sự cố CODE; nhưng nếu là **403 hàng loạt** thì kill-switch `PERMISSION_GUARD_ENABLED` **không** mở được route này (K1) — đường thoát DUY NHẤT là `UPDATE role_permissions SET data_scope='Company'` cho cặp `view:audit-log` của vai bị kẹt. **BỐN** nguyên nhân 403 phải phân biệt khi trực ca: K1 kill-switch · K2 cửa sổ cache guard 300s · **K3** `data_scope` trong DB không chuẩn hoá được ⇒ fail-closed VĨNH VIỄN (không phải cửa sổ) · **K4** cổng widget không truyền `isSensitive` ⇒ vai chỉ có `*:*` qua cổng rồi bị `rowScopeFor` chặn (hôm nay 0 vai như vậy — đã đo). **NỢ FAST-FOLLOW do FULL gate mở, CHƯA cấp số:** (a) widget `SYSTEM_LOGS` **biến mất im lặng** khỏi `GET /dashboard/widgets?include_data=true` khi `rowScopeFor` ném 403 — `attachData` bắt `ForbiddenException` rồi `continue` (`dashboard-widget-data.service.ts:206-217`), không `error_state`, không log tầng API, HTTP 200 nguyên vẹn; đường `GET /dashboard/widgets/system-logs/data` thì trả 403 đúng (`:127-129`). Trước bản vá nhánh câm này chỉ trúng khi có race TOCTOU; nay K1/K3 là điều kiện ĐỨNG YÊN ⇒ widget mất vô thời hạn mà chỉ log server biết. Nên phân biệt 403-của-`rowScopeFor` khỏi 403-thiếu-quyền và emit `Degraded`, hoặc tối thiểu thêm counter cạnh `logger.error`. (b) `fetchSystemLogs` gọi `listCompany` **HAI LẦN** (cửa sổ 24h/7d) ⇒ **2 lượt `getCompanyRoleGrantsWithScope` KHÔNG cache** (`permission.cache.ts:91-100` — passthrough cố ý, dựng trên giả định "chỉ dùng ở `/auth/me` bootstrap", giả định mà KI-072 vừa làm sai) mỗi lần render dashboard; `data_scope` không phụ thuộc cửa sổ thời gian nên hai lượt cho CÙNG kết quả. **Đã CHỌN phương án (b) của plan ⟲R1-C11 — giữ hai lượt và ghi chi phí có tên ở đây, không im lặng nhận.** **Mô tả khuyết tật gốc, giữ để đọc lịch sử:** TÁCH RA khi đóng KI-070, cố ý không để nó chìm theo dấu gạch ngang.** `AuditQueryService.listCompany(companyId, query)` — **chữ ký không có `userId` của actor**, nên nó không resolve `data_scope` được kể cả nếu muốn; `withTenant` + RLS chặn CHÉO TENANT chứ không chặn TRONG tenant. Vai giữ `view:audit-log@Own` đọc trọn `audit_logs` của tenant: `actor_user_id`, `action`, `object_type`/`object_id`, `permission_code`, `data_scope`, `ip`, `user_agent`, `request_id`. **Số đo PROD 2026-08-21:** `audit_logs` **13.146 hàng** (12.786 hàng có `actor_user_id`, **13** actor phân biệt) — so với 366 `login_logs` + 65 `user_security_events` mà KI-070 vừa đóng, bề mặt CÒN MỞ lớn gấp **~30 lần** bề mặt ĐÃ đóng. `?actorUserId=` (`auditLogQuerySchema`) đi thẳng vào `eq(auditLogs.actorUserId, …)` — **đúng hình dạng V2** của KI-070: oracle CÓ ĐIỀU KHIỂN, dò được lịch sử hành động của một UUID bất kỳ trong tenant. `/:id` là chiều thứ hai: `getCompanyDetail` trả trọn một hàng theo id, không vị từ scope. ⚠️ **Đừng nhầm với lớp che đã có:** `AuditMaskerService` redact `before`/`after` — nó che GIÁ TRỊ trong hàng, KHÔNG quyết định hàng nào được trả; và route này **không chiếu email/họ tên** nên tầng KI-053/054 (bound CỘT) không áp dụng ở đây, tức lỗ này KHÔNG có lớp giảm nhẹ nào ngoài workaround. **Vì sao phải có số hiệu riêng thay vì một câu trong KI-070:** workaround của KI-070 (*"không cấp `view:audit-log` ở scope hẹp hơn Company"*) là lớp kiểm soát DUY NHẤT đang bảo vệ route này; gạch KI-070 mà không tách số sẽ xoá đúng dòng đó — cùng lỗi đã mắc với KI-049 (phát hiện không có số hiệu thì vô hình với bug scrub trước RC) và cùng lý do KI-071 được tách. **Workaround (GIỮ NGUYÊN, chuyển sang đây):** không cấp `view:audit-log` ở scope hẹp hơn `Company` cho vai nào. **Số đo PROD 2026-08-21:** 3 vai giữ — `SA` (10 người) · `QUẢN LÝ CẤP CAO` (4) · `company-admin` (2) — **cả ba `@Company`**, 0 hàng `DENY` ⇒ chưa ai chạm được. **Việc thật:** cùng khuôn `rowScopeFor` + `rowScopeSql` + `basis:"scoped-predicate"` mà `S10-SEC-AUDITLOGROW-1` vừa dựng; ở đây cặp bound TRÙNG cặp gate (như hai bảng nhật ký) ⇒ dùng luật fail-closed `null ⇒ 403 + log`, KHÔNG dùng luật `resolveOrNull` của tầng cột. ⚠️ **Quyết định phải chốt TRƯỚC khi code, không lén chọn:** `audit_logs` chỉ có MỘT cột người (`actor_user_id`) ⇒ `Own` = *"hàng do tôi gây ra"*, một ngữ nghĩa KHÁC `login_logs` (`Own` = hàng VỀ tôi). Ai vá phải phát biểu ngữ nghĩa đó ra rồi mới dựng vị từ — soi gương ranh giới D7 của `docs/plans/S10-SEC-AUDITLOGROW-1.md`. Kèm theo: thêm điểm ĐÚC thứ hai ⇒ `ROW_SCOPE_MINT_PINS` ĐỎ, phải ký lại qua FULL gate (đó là thiết kế, không phải phiền toái) | S3 | Bảo mật (phân quyền) | ❌ | ❌ | `S10-SEC-AUDITLOGROW-2` — seed 2026-08-22, `status: todo`, **READY** (không phụ thuộc gì); đi TRƯỚC `S10-SEC-ROLEMEMBERROW-1` |
| ~~**KI-073**~~ | **ĐÃ ĐÓNG 2026-08-24 — `S10-SEC-ROLEMEMBERFE-1`. ⚠️ ĐÍNH CHÍNH CƠ CHẾ TRƯỚC KHI ĐỌC PHẦN CÒN LẠI:** cơ chế "chạy batch rồi đọc item nào 409" mà hàng này (và D12 của KI-071) mô tả **KHÔNG TỒN TẠI** — đọc bảng nhánh thật của `PermissionAdminService.assignRole` thì 409 chỉ nổ ở nhánh thua-race `onConflictDoNothing`, còn người ĐÃ là thành viên (không gửi `expiresAt`) rơi vào nhánh **no-op trả 201 + NGUYÊN HÀNG GỐC** (`id`/`grantedBy`/`createdAt` gốc). Kênh thật vì thế **NẶNG HƠN** mô tả: xác định 100%, 1 request/người, `curl` là đủ, và **im lặng ở mọi câu trả lời dương** (no-op = 0 ghi — 0 audit, 0 security-event, 0 outbox). **Bản vá (plan §D2/D4/D5):** (1) BE — thân 201 của `POST /permissions/users/:userId/roles` thu hẹp còn đúng 4 khoá `{userId,roleId,companyId,expiresAt}` echo request, ĐỒNG NHẤT cho mọi actor + cả ba nhánh (bộ chiếu DUY NHẤT `projectAssignResult`, ratchet kiểu `Promise<UserRoleDto>`); audit và `user_security_events` vẫn ghi id hàng THẬT (bất biến #2; đột biến M-F ghim); nhánh no-op GIỮ NGUYÊN (không biến oracle ĐỌC thành khuếch đại GHI — ca O4 ghim 4 bộ đếm). (2) Contract — `userRoleSchema` GỠ HẲN 3 khoá (zod strip khi server cũ còn gửi); `roleMemberListSchema` thêm `complete: z.boolean().catch(false)` — server phát `complete = scope ∈ {Company,System}`, tức bit CÓ THẨM QUYỀN về scope `view:user` của CHÍNH actor; `.catch(false)` để cửa sổ "FE mới + BE cũ" (mặc định trên PROD — Pages tự deploy FE, API tay) rơi về partial-mode CÓ NHÃN thay vì ZodError vỡ trắng tab; cả hai chiều lệch phiên bản tự lành. (3) FE — `complete=false` ⇒ bộ đếm đổi nhãn "N thành viên bạn xem được", dedup theo `memberIds` TẮT (vẫn trừ CHÍNH MÌNH — SoD; vẫn khoá người chưa link tài khoản), dòng "Bỏ qua (đã là thành viên)" thay bằng câu nói rõ giới hạn phạm vi, empty-state riêng "trong phạm vi bạn xem được" (0 hàng là trạng thái MẶC ĐỊNH của `@Own` không có chân — KI-071). **RED-proof ĐO THẬT (lane `mediaos_rolememberfe`, bảng đầy đủ ở plan §3.5/§3.6):** trước vá 10 ca ĐỎ đúng chỗ (O1·O2·S1a·S1b·U1-U3·F1·F3 + 2 ca schema) / 3 neo XANH (O3·O4·F2); sau vá xanh hết; 6 đột biến M-A…M-F đều ĐỎ đúng ca kỳ vọng (M-B: O1 XANH · O2 ĐỎ — O2 tự mang sức nặng; M-D: S1a ĐỎ · S1b XANH). **FULL gate 3/3 PASS 2026-08-24:** security-reviewer PASS (1 MEDIUM + 3 LOW, đã ghi nợ plan §N-6…N-9; vế i18n của MEDIUM vá TRONG PR vì lời hứa sai do chính WO này viết ra) + database-reviewer PASS + silent-failure-hunter PASS (1 MEDIUM = nợ N-5 telemetry). **Số đo PROD 2026-08-24 (script `measure-ki073.mjs`, chỉ-SELECT, 4 hình dạng wildcard × 2 cặp RIÊNG — `assign-role:user` là `is_sensitive` nên `*:*` KHÔNG thoả nổi nó):** mục (2b) **0 vai** giữ đồng thời `assign-role:user` + `view:user` với view hẹp hơn Company (điều kiện = 0 ✅); mục (5) **0 vai** giữ `view:user` ALLOW hẹp hơn Company — đo LẠI, khớp số 22/08; mục (3) **0 hàng** DENY ở mọi hình dạng của hai cặp ⇒ **0 lượt 403 mới**; mục (4) `assign-role:user → is_sensitive=true`, `*:* → false` ⇒ không có nhiễu `effectivelySensitive`. Hệ quả phụ: `QUẢN LÝ CẤP CAO` chỉ giữ `*:*`, KHÔNG có exact `assign-role:user` ⇒ nhánh lọc EXACT (`permission.service.ts:604-606`) khiến vai này **không gọi nổi** đường GHI — tập vai chạm được thật sự là {`SA`, `company-admin`}, cả hai `@Company`. ⇒ bản vá đóng lỗ **TIỀM TÀNG**, 0 vai thực thi được oracle hôm nay, 0 hồi quy. ⚠️ **RANH GIỚI — đọc trước khi tin dấu gạch:** (1) tính chất đạt được là **“không enumerate thành viên IM LẶNG theo chiều DƯƠNG”**, KHÔNG phải "không enumerate im lặng" — kênh `DELETE 404-vs-204` còn sống dưới số hiệu RIÊNG **KI-074** (cấp trước dấu gạch này, đúng tiền lệ KI-071/072); (2) kênh THỜI GIAN (no-op 0 ghi vs fresh 4 ghi) ghi nhận ở KI-074 + plan N-2, không đóng được mà không ghi giả; (3) `Team`/`Department` ⇒ 0 hàng + `complete=false` — nhất quán nhưng KHÔNG sàn hoá (nợ N-3, phải làm cả BA đường cùng lúc); (4) workaround cũ ("không cấp `view:user` hẹp hơn Company cho vai đồng thời giữ `assign-role:user`") **hết cần cho V1** nhưng GIỮ như khuyến nghị vận hành vì KI-074 còn sống. **Mô tả khuyết tật gốc (giữ để đọc lịch sử — cơ chế 409 đã đính chính ở trên):** **Oracle TRONG-tenant qua mã lỗi đường GHI của tab Thành viên role — TÁCH SỐ khi đóng KI-071 (2026-08-22), cố ý không để nó chìm theo dấu gạch ngang.** `S10-SEC-ROLEMEMBERROW-1` đã bound TẬP HÀNG đường ĐỌC `GET /auth/roles/:id/members`, nhưng FE dựng `memberIds` **từ chính danh sách đã bị scope** (`apps/app/src/routes/system/roles/RoleMembersTab.tsx:91`) rồi dùng nó làm bộ lọc dedup cho hai dialog GHI hàng loạt: `toAssign`/`alreadyMembers` của **Thêm theo phòng ban** (`:383-384`) và **theo chức vụ** (`:489-490`), cộng bộ đếm "N thành viên" (`:117`). ⇒ Vai giữ `assign-role:user@Company` **cộng** `view:user@Own` (hoặc `@Team` ⇒ 0 hàng) nhận `memberIds` gần-rỗng, chạy batch rồi **đọc item nào trả 409 để dựng lại đúng tập thành viên mà KI-071 vừa giấu**; bộ đếm cũng nói dối. **Vì sao phải có số hiệu riêng thay vì một câu trong KI-071:** dấu gạch ngang trên KI-071 đọc rất giống "route thành viên role đã kín", và đây là phần DUY NHẤT còn sống sau bản vá — cùng lỗi đã mắc với KI-049 và cùng lý do KI-071 được tách khỏi KI-070 (phát hiện không có số hiệu thì vô hình với bug scrub trước RC). **Mức độ — đọc đúng, đừng thổi lên:** **KHÔNG có leo thang quyền** — cặp GHI là cặp RIÊNG (`assign-role:user`, `is_sensitive=true`), gate BE ở `POST /permissions/users/:userId/roles` và gate FE `PermissionGate ASSIGN_ROLE` đều độc lập với `view:user`; tập dựng lại được **hẹp hơn** thứ chính vai đó đọc được TRƯỚC bản vá; và hôm nay **0 vai** giữ `view:user` ở scope hẹp hơn `Company` (đo PROD 2026-08-22) ⇒ **chưa ai chạm được**. **Ranh giới quan trọng:** oracle này sống ở **mã trả về của API GHI**, KHÔNG ở màn hình ⇒ **vá FE một mình KHÔNG đóng nó**. Vá FE (gate hai dialog bằng `useCanExact` + điều kiện scope `Company`, hoặc lấy `memberIds` từ một đường đếm không-chiếu-danh-tính) chỉ đóng vế bộ đếm sai + 409 ồn ào; đóng vế oracle cần đường GHI trả mã KHÔNG phân biệt "đã là thành viên" với "gán thành công" cho actor ngoài scope đọc. **Workaround đang hiệu lực (lớp kiểm soát duy nhất):** không cấp `view:user` ở scope hẹp hơn `Company` cho vai ĐỒNG THỜI giữ `assign-role:user` | S4 | Bảo mật (rò qua kênh phụ) | ❌ | ❌ | **ĐÓNG 2026-08-24** — `S10-SEC-ROLEMEMBERFE-1` |
| ~~**KI-074**~~ | **ĐÃ ĐÓNG 2026-08-25 — `S10-SEC-ROLEMEMBERDEL-1`, ADR `DECISIONS-11_Role_Membership_Absence_Signal.md`.** Thi công **hướng (b)** đã ký 24/08: `revokeRole` GIỮ **404** cho actor có `view:user` ở scope `Company`/`System`; **204 + ĐÚNG 0 ghi** cho phần còn lại (kể cả scope `null` — "KHÔNG có thẩm quyền" ≠ `Company`). Bit lái hình dạng câu trả lời lấy qua `PermissionService.resolveStrongestScope(actor,'view','user')` gọi **NGOÀI** write-tx (gọi trong tx là `withTenant` LỒNG NHAU) và **KHÔNG truyền `opts`** — truyền `{isSensitive:true}` sẽ ép nhánh exact-only ⇒ mọi vai chỉ giữ `*:*` tụt `null` ⇒ 204-mù im lặng (ghim bằng U5: đúng 1 lời gọi, đúng 4 đối số). **Ba lớp role tách ở nhánh ÂM** (`findAssignableRole` đặt TRONG `if (!existing)` — nâng lên đầu hàm sẽ KHOÁ việc gỡ vai của role đã soft-delete, ghim bằng U9): company-scoped của tenant KHÁC → **404** (RLS `roles_tenant_isolation` + FORCE, `mediaos_app` NOBYPASSRLS — BẤT BIẾN #1) · **system** role (`company_id IS NULL`) → **204** (hành vi ký MỚI: không thuộc tenant nào) · operator-audience → **404**. Cả hai nhánh 404 dùng **chung một chuỗi** `"User does not have this role"` ⇒ 0 bit mới; nền của lập luận đó (POST đã cho bit role-tồn-tại MIỄN PHÍ — hai thông điệp khác nhau + 0 ghi ở cả hai) được ghim bằng ca `D-X4`. **Bằng chứng:** unit `permission-admin.ki074.spec.ts` (U1·U2·U3·U5·U6·U7·U8·U9) + int `role-member-del-oracle.int-spec.ts` (D-S1 · D-A0/A1/A3 · D-N1/N2/N3 · D-X1/X2/X3/X4 · D-W1..W4 — 13/13 trên `LANE_DB`); FULL gate `security-reviewer` + `database-reviewer` + `silent-failure-hunter` **PASS** (0 CRITICAL, 0 HIGH), trong đó security-reviewer tự dựng RED bằng cách revert service về HEAD → unit 4/9 + int 4/13 ĐỎ. ⚠️ `D-X1`/`D-X3` là **ratchet**, KHÔNG phải RED-proof: code cũ trả 404 ở mọi nhánh nên chúng xanh cả trước lẫn sau — đừng đọc "13/13" thành "13 ca đều đỏ trước". ⚠️ **Kênh THỜI GIAN KHÔNG đóng theo** (nhánh ÂM 2 SELECT/0 ghi vs nhánh DƯƠNG 1 SELECT/4 ghi): ghi nhận ở ADR §4, đóng nó đòi ghi audit giả nên KHÔNG hứa. ⚠️ **Cờ `is_sensitive` của catalog nay là một CỔNG của route** — lật `('view','user')` sang `true` biến route thành 204-mù trong im lặng (ADR §6, canh bằng `D-S1`). ✅ **Số đo PROD ĐO LẠI 25/08** (`scripts/measure-ki074-role-member-del.sql`, HAI cặp × 4 hình dạng wildcard, mô phỏng đúng deny-overrides + exact-thắng-wildcard): **6 người giữ `assign-role:user`, CẢ 6 có `view:user` scope mạnh nhất = `Company` ⇒ cả 6 GIỮ 404**; `actors_falling_into_new_204_branch = **0**` ⇒ **(b) là no-op trên thực địa, 0 hồi quy**. Bốn grant chạm cặp gate qua HAI hình dạng: `QUẢN LÝ CẤP CAO` và `SA` chạm **qua `*:*@Company`** (đúng lý do câu đo phải phủ đủ 4 hình dạng, không chỉ cặp exact), `SA`/`company-admin` còn có exact `assign-role:user@Company`; cả bốn vai + `hr` đều mang exact `view:user@Company`. `('view','user').is_sensitive = false` xác nhận lại trên PROD (cổng ở ADR §6 còn nguyên). Catalog PROD KHÔNG có hàng `('view','*')`/`('*','user')` — *đã đo, không có*, khác *chưa đo*. ⚠️ Cấu hình cần canh: `QUẢN LÝ CẤP CAO`/`SA` chạm cặp gate qua `*:*`; cấp thêm cho họ `view:user@Own` sẽ khiến **exact THẮNG wildcard** ⇒ tụt thẳng xuống nhánh 204. ── Bối cảnh gốc: **Oracle thứ HAI của tab Thành viên role — chiều ĐỌC qua `DELETE`, TÁCH SỐ khi đóng KI-073 (2026-08-24), CẤP SỐ TRƯỚC khi dấu gạch ngang land (tiền lệ KI-071/072/073).** `DELETE /permissions/users/:userId/roles/:roleId` phân biệt **404 "User does not have this role"** với **204**: `PermissionAdminService.revokeRole` (`permission-admin.service.ts:161-163` trước WO, nay lệch vài dòng) ném NotFound **TRƯỚC mọi ghi** ⇒ câu trả lời ÂM ("x KHÔNG phải thành viên") để lại **0 hàng forensic, 0 thiệt hại** — gương của V1/KI-073. Chiều DƯƠNG thì **ồn và có giá**: mỗi lần trúng đều ghi `RoleRevoked` + `user_security_events` **và thực sự gỡ vai của nạn nhân** — nhưng actor giữ `assign-role:user` **vá lại được ngay bằng POST**, khiến cặp `RoleRevoked`+`RoleAssigned` sát nhau lẫn vào nhiễu cấp phát bình thường. ⚠️ **Hệ quả cho người đọc dấu gạch trên KI-073:** tính chất mà `S10-SEC-ROLEMEMBERFE-1` đạt được là **"không enumerate thành viên IM LẶNG theo chiều DƯƠNG"** (POST no-op nay trả thân 4 khoá đồng nhất + 0 ghi), **KHÔNG phải** "không enumerate im lặng" — kênh DELETE này còn sống, mỗi câu hỏi dương phải trả giá bằng một lần gỡ-vai thật có dấu vết. **Vì sao KHÔNG đóng trong cùng WO (có nguyên tắc, không phải bỏ sót):** `web-core/auth-users-api.ts:136` ghi rõ 404 là quyết định CÓ CHỦ Ý ("caller xử lý như lỗi rõ ràng, KHÔNG no-op ngầm") — đảo thành 204-mù là lấy mất một tín hiệu vận hành đúng; và ranh phân biệt được chọn là "đóng kênh im lặng, chấp nhận kênh có-dấu-vết". ✅ **CHỦ TRƯƠNG ĐÃ KÝ 2026-08-24 — hướng (b):** GIỮ **404** cho actor có `view:user` ở scope `Company`/`System`; **204** cho phần còn lại. Hai hướng bị loại: (a) 204 đồng nhất — đóng oracle nhưng LẤY MẤT tín hiệu 404 mà `web-core/auth-users-api.ts:136` cố ý dựa vào; (c) chấp nhận rủi ro. Hướng (b) thắng vì **không mất tín hiệu vận hành** (người trực ca luôn ở `Company`) và **đối xứng với cờ `complete`** của KI-073 — cùng một ý: bit CÓ THẨM QUYỀN về scope `view:user` của CHÍNH actor lái hình dạng câu trả lời. ⚠️ **Ba ranh giới của (b), ghi TRƯỚC khi ai thi công:** (1) `resolveStrongestScope` trả `null` (0 grant, hoặc scope không chuẩn hoá được) phải rơi về **204** — `null` là *"KHÔNG có thẩm quyền"*, KHÔNG phải `Company`; (2) chỉ nhánh TRONG-tenant *"user không giữ role này"* đổi sang 204 — nhánh role thuộc **tenant khác GIỮ 404** và phải có ca ghim (BẤT BIẾN #1); (3) 204 ở nhánh ÂM vẫn là **0 ghi** — KHÔNG ghi audit/security-event giả cho "giống nhau", đó là biến oracle ĐỌC thành GHI giả (cùng luật no-op của KI-073). ⚠️ **Hôm nay (b) là no-op trên thực địa** — đo PROD 24/08: 0 vai giữ `view:user` hẹp hơn `Company` ⇒ không ai rơi vào nhánh 204. Là ĐIỂM MẠNH (đóng lỗ tiềm tàng, 0 hồi quy) nhưng cũng là bẫy nghiệm thu: ca DENY phải TỰ GIEO vai `view:user@Own`, không nghiệm thu được bằng dữ liệu PROD. ⚠️ **Kênh THỜI GIAN không đóng theo** (nhánh ÂM 0 ghi vs nhánh DƯƠNG 4 ghi): ghi nhận trong ADR, KHÔNG hứa đóng. Chữ ký này phải vào một ADR `docs/DECISIONS/` khi thi công — tiền lệ KI-065 đóng kèm `DECISIONS-09`. **Workaround:** như KI-073 (không cấp `view:user` hẹp hơn `Company` cho vai đồng thời giữ `assign-role:user`) + khi soi audit, cặp `RoleRevoked`→`RoleAssigned` sát nhau trên cùng `(user, role)` bởi cùng actor là chữ ký của probe này. **Kênh anh em (ghi nhận, không số hiệu riêng):** chênh lệch THỜI GIAN giữa nhánh no-op (0 ghi) và nhánh fresh (insert+audit+event+outbox) của chính POST — không đóng được mà không ghi giả (cùng lớp `attribution-patch-creates-timing-oracle`, plan §4 N-2) | S4 | Bảo mật (rò qua kênh phụ) | ✅ | ✅ | WO `S10-SEC-ROLEMEMBERDEL-1` (**đóng 25/08**) |
| ~~**KI-075**~~ | **ĐÃ ĐÓNG 2026-08-24 — `S10-CHAT-EMITGUARD-1`. ⚠️ ĐỌC MỨC ĐỘ TRƯỚC:** đây là nợ **ĐỘ BỀN / hợp đồng biên**, KHÔNG phải lỗ đang chảy máu — đo lại 24/08 xác nhận hôm nay `emitChatCall` KHÔNG ném được (`realtime-emitter.service.ts:269-299` tự bọc `try/catch` quanh CẢ `wsChatCallEventSchema.parse` LẪN `server.to().emit()`; nhánh `!this.server` là `logger.error` + `return`; `chat_calls.started_at` khai `notNull` nên `.toISOString()` cũng không ném). **ĐÍNH CHÍNH tên hàm — một nửa giá trị của số hiệu này** (ghi chú bàn giao `S10-CHAT-CALLSWEEP-1` viết SAI là *"`emitAutoEnded` ở cả hai job"*): job ring-timeout gọi **`emitExpired`** (`action:"missed"` · `chat-call-ringing-timeout.job-handler.ts`), job gặt gọi **`emitAutoEnded`** (`action:"ended"` · `chat-call-stale-active-sweep.job-handler.ts`). Cùng KHUÔN, khác HÀM — grep đúng một tên rồi tưởng đã phủ cả hai là đúng lớp bẫy số hiệu này sinh ra để chống. **Đã làm gì (vá CẢ HAI cùng lúc — đối xứng là ĐIỀU KIỆN, không phải phong cách):** (1) `emitExpired`/`emitAutoEnded` bọc `try/catch` **PER-ITEM** trong vòng lặp và **trả về SỐ cuộc gọi không phát được** — per-item chứ không bọc cả lời gọi helper, vì bọc ngoài cứu được run-row nhưng bỏ rơi N-1 cuộc gọi sau item hỏng; (2) **cả hai job TIÊU THỤ** con số đó: `success = total − emitFailed` · `failed = emitFailed` · `metadata.emitFailed` (CHỈ có mặt khi > 0, để đường xanh giữ NGUYÊN hình dạng cũ `callsMissed`/`callsAutoEnded`) ⇒ `JobRunner.deriveStatus` cho `'Partial'` khi mất một phần thay vì `'Failed'` nói dối cho một tx ĐÃ commit; (3) log HAI tầng — helper ghi `error` kèm `callId` (biết cuộc gọi nào mất chuông), job ghi `error` kèm `jobCode`·`companyId`·`emitFailed/total`. **KHÔNG nuốt thành `success` trọn vẹn** — sau khi `try/catch` đã che run-row khỏi `'Failed'`, `failed` + `metadata.emitFailed` là **tín hiệu duy nhất còn lại**; bỏ nốt nó đi thì mất chuông thành im lặng tuyệt đối. **Bằng chứng (RED trước, đối chứng ALLOW đủ cả hai vế):** `realtime/realtime-emitter.call.spec.ts` **MỚI** ghim bất biến ở đúng biên `realtime/` — trước WO này `grep emitChatCall apps/api/src/realtime/*.spec.ts` cho **0 kết quả** — 4 ca DENY (`!server` · payload sai schema · `to().emit()` tự ném · danh sách nhận rỗng) + 1 ca ALLOW; đã **kiểm chứng đột biến**: chuyển `.parse()` ra ngoài `try` ⇒ ca DENY-2 ĐỎ. `chat/chat-call-emit-guard.spec.ts` **MỚI** chạy cùng MỘT bảng ca qua `describe.each` cho **CẢ HAI** job (item GIỮA lô ném ⇒ vẫn phát đủ 3/3, `run()` không reject, `{total:3,success:2,failed:1,emitFailed:1}`; cả lô ném ⇒ `success:0` nhưng đếm DB vẫn đọc được; ALLOW lô 3 + lô rỗng so `toEqual` giữ nguyên hình dạng cũ). Ratchet đối xứng trong `chat-realtime-after-commit.spec.ts` bắt buộc CẢ HAI job khai `const emitFailed = this.calls.emit…(` và tiêu thụ vào `failed` — vá một job quên job kia là ĐỎ. **Hình dạng cũ được giữ:** int-spec `chat-call-stale-active-sweep.int-spec.ts` chỉ khẳng định `res.total` — `total` vẫn là đếm hàng DB, không đổi. **Còn lại (KHÔNG thuộc số hiệu này):** 3 món hoãn của `S10-CHAT-CALLSWEEP-1` (audit `closedUserIds` vào `newValues` · ca chéo tenant int-spec · gom lô N+1) chưa có số hiệu, chưa có WO. Quan sát mới khi thi công: `emitLifecycle` trên đường REST `invite` (`chat-calls.service.ts:203`) vẫn KHÔNG bọc — cùng họ nhưng **khác hồ sơ rủi ro** (client nhận 500 và thấy được trạng thái qua chính response POST, không phải mất vĩnh viễn như đường job), nên cố ý để ngoài phạm vi. | S4 | Độ bền vận hành (job · realtime) | ❌ | ❌ | `S10-CHAT-EMITGUARD-1` (đã đóng) |
| ~~**KI-076**~~ | **ĐÃ ĐÓNG 2026-08-25 — `S10-CHAT-EMITGUARD-2`. ⚠️ ĐỌC MỨC ĐỘ TRƯỚC:** nợ **PHÒNG THỦ THEO CHIỀU SÂU**, KHÔNG phải lỗ đang chảy máu — đo lại 25/08 xác nhận `RealtimeEmitterService.emitChatCall` vẫn KHÔNG ném được (`realtime-emitter.service.ts:269-299` bọc `try/catch` quanh CẢ `.parse()` LẪN `server.to().emit()`), **và bất biến đó ĐÃ có ca ghim** từ `S10-CHAT-EMITGUARD-1` (`realtime/realtime-emitter.call.spec.ts`, có kiểm chứng đột biến) ⇒ rủi ro tồn dư THẤP HƠN KI-075 lúc mở. `emitLifecycle` cũng không có vế `.toISOString()` nào để ném (`ChatCallDto.startedAt` đã là `string`). **ĐÍNH CHÍNH đánh giá ban đầu (giữ lại vì đó là một nửa giá trị của số hiệu này):** PR #408 ghi *"khác hồ sơ rủi ro — client thấy được trạng thái qua chính response POST"* — câu đó chỉ đúng cho **ACTOR** và chỉ ở **2/5** hình dạng; **PEER không có response POST nào để đọc**, WS là kênh duy nhất báo cho họ. **Bảng HỒI PHỤC theo từng trạng thái (vế mà đánh giá ban đầu đọc sai):** **(a) `ringing`** (`invite`) → job `CHAT_CALL_RINGING_TIMEOUT` (45s) **+** bước dọn-trước-khi-mời của lời mời kế ⇒ **tự lành**; **(b) `active`** (`accept`) → job `CHAT_CALL_STALE_ACTIVE_SWEEP` (mồ côi grace 2 phút / quá thọ 12h) ⇒ **tự lành CHẬM**; **(c) `rejected` · `cancelled` · `ended`** → **trạng thái CUỐI: KHÔNG job nào khớp** (cả hai job chỉ quét `ringing`/`active`) và CALL **không có route ĐỌC nào để poll bù** ⇒ peer giữ khung gọi của một cuộc gọi đã chết tới khi reload/reconnect — **đúng lớp hậu quả KI-075, không nhẹ hơn**. **Đã làm gì (MỘT điểm sửa, phủ cả NĂM route):** `try/catch` đặt **TRONG thân `emitLifecycle`** (`chat-calls.service.ts`) — helper này là điểm phát của cả `invite` (`:203`, `ringing`) lẫn bốn route vòng đời đi chung `lifecycleTx` (`:558` — `accepted`/`rejected`/`cancelled`/`ended`). **KHÔNG nhân bản guard ra từng route:** `lifecycleTx` được dựng ra CHÍNH ĐỂ chống "quên ở một route", vá lẻ là đi ngược thiết kế đó; và guard tại chỗ giữ ratchet `this.realtime.emitChatCall(` **vẫn ĐÚNG 3** (không hạ trần — [[index-ratchet-must-pin-definition-not-name]]). Kênh tín hiệu = một `logger.error` mang `callId` · `roomId` · `action` · trạng thái DB. **Vế thứ hai — QUYẾT TƯỜNG MINH chuyện đường REST không có chỗ chứa số đếm (KI-076 yêu cầu khai ra):** `emitLifecycle` **GIỮ `: void`**, KHÔNG đổi sang `number` như `emitExpired`/`emitAutoEnded` — đường REST **không có run-row** (`JobRunResult.failed` + `metadata.emitFailed` là của tầng job) và hợp đồng HTTP của 5 route là `ChatCallDto`, nhét "emit hỏng" vào DTO là rò chi tiết vận chuyển vào hợp đồng nghiệp vụ; **trả một con số mà không caller nào tiêu thụ là KẾ TOÁN GIẢ** — nó trông như có tín hiệu trong khi tín hiệu duy nhất là dòng log. `invite:202` vứt số đếm của `emitExpired` bằng `void this.emitExpired(...)` — **tường minh và grep được**, kèm chú giải; helper đã `logger.error` per-item kèm `callId` nên một dòng tổng ở tầng REST chỉ nhân đôi cùng thông tin. Cả ba quyết định nằm trong **docblock**, không chỉ trong plan. **Bằng chứng (RED trước, có đối chứng ALLOW):** `chat/chat-call-rest-emit-guard.spec.ts` **MỚI** — `describe.each` chạy cùng MỘT bảng ca cho **cả 5 route** (dựng `ChatCallsService` THẬT, không fake helper): ALLOW đường xanh phát đúng 1 sự kiện tới đúng `participants[]` + DTO nguyên vẹn · DENY emit ném ⇒ route **KHÔNG reject** và trả **ĐÚNG DTO của đường xanh** (`toEqual`) · DENY ⇒ đúng 1 `logger.error` mang `callId` + `action`; thêm ba ca cho HAI lối phát của `invite` (emit `missed` ném ⇒ `ringing` VẪN phát · emit `ringing` ném ⇒ vẫn trả DTO · ALLOW đối chứng đúng 2 emit thứ tự `missed`→`ringing`) và bốn ratchet cấu trúc (đếm emit vẫn 3 · guard nằm TRONG helper và KHÔNG có bản sao `try { this.emitLifecycle(` ở tầng route · `emitLifecycle` giữ `: void` · `void this.emitExpired(` còn nguyên). **Kiểm chứng RED thật:** trước bản vá **13/22 ca ĐỎ** (10 ca DENY của 5 route + 1 ca lối-phát-thứ-hai + 2 ratchet), 5 ca ALLOW xanh — ca ALLOW bắt buộc, thiếu nó thì đột biến "`emitLifecycle` return ngay dòng đầu" vẫn xanh cả loạt DENY ([[deny-cases-vacuous-without-allow-case]]). **Còn lại (KHÔNG thuộc số hiệu này):** 3 món hoãn của `S10-CHAT-CALLSWEEP-1` (audit `closedUserIds` · int-spec chéo tenant · gom lô N+1) vẫn chưa có số hiệu, chưa có WO. | S4 | Độ bền vận hành (REST · realtime) | ❌ | ❌ | `S10-CHAT-EMITGUARD-2` (đã đóng) |
| ~~**KI-077**~~ | **NĂM tham số `:id`/`:linkId` còn lại của `foundation/files` vẫn nhận chuỗi tự do — cùng cơ chế KI-068, khác KÊNH (param thay vì body).** Cấp số 24/08 khi thi công `S10-FND-BODYVALIDATE-1`: WO đó đóng KI-068 ở kênh BODY và, khi tự đọc lại diff, phát hiện **bản sao của chính lỗ đó nằm cách bản vá một dòng** ở kênh PARAM. **Hai route GHI đã ĐO và ĐÃ VÁ trong WO đó** (`@Param("id", ParseUUIDPipe)`): `POST /foundation/files/:id/links` — `:id` rác làm `linkFileInputSchema.parse({...body, fileId: id})` ném `ZodError` THÔ ⇒ **500 + `error.type="ZodError"`**; `POST /foundation/files/:id/confirm` — `:id` rác đi tới cột `uuid` của DB rồi nổ `22P02` ⇒ **500 + `error.type="Error"`** (hai hình dạng lỗi KHÁC nhau, cùng một hậu quả). **Phần CÒN LẠI của KI này — NĂM tham số, CHƯA ĐO** (nói rõ để không lẫn đo với đoán): `getOne` `:90` · `downloadUrl` `:100` · `download` `:113` · `unlink` `:145` (`:linkId`) · `remove` `:153`. Chúng là READ/DELETE nên chưa nằm trong phạm vi một WO nào; suy từ hình dạng code thì cùng đường DB `22P02`, nhưng **chưa có một lượt HTTP nào chứng minh**. **Khuôn vá đã có sẵn CÙNG CÂY:** `api-keys.controller.ts:71` dùng `@Param("id", ParseUUIDPipe)` từ trước — `foundation/files` lại là chỗ bỏ sót, y hệt kiểu nó là module duy nhất từng thiếu `*.dto.ts`. **Mức độ:** hỏng ĐÚNG CHIỀU AN TOÀN (request vẫn bị từ chối) ⇒ KHÔNG phải lỗ bảo mật; hậu quả là hợp đồng API sai + payload rác bơm 500 GIẢ vào giám sát — y như KI-068. **Việc thật khi làm:** ĐO 5 route bằng HTTP trước, rồi vá; và cân nhắc ratchet cho kênh param giống ratchet kênh body (`body-validation-ratchet.unit-spec.ts`) — không có ratchet thì route thứ sáu sẽ mọc lại. | S4 | Hợp đồng API / xử lý lỗi | ✅ | ✅ | **ĐÓNG 2026-08-25** — `S10-FND-PARAMUUID-1`. **ĐO TRƯỚC KHI VÁ** (`test/integration/files-param-uuid.int-spec.ts`, `LANE_DB=mediaos_paramuuid`, supertest qua đúng stack `main.ts`): **cả 5 tham số trả 500 `SYSTEM-ERR-001` + `error.type="Error"`** — giả thuyết "cùng đường DB `22P02`" được XÁC NHẬN, không còn là suy luận. Sau vá (`@Param(..., ParseUUIDPipe)`): **400 ở BIÊN**, và mỗi ca deny có ca ALLOW đối chứng (UUID hợp lệ nhưng không tồn tại vẫn ĐI QUA biên tới service) ⇒ bản vá không chặn nhầm request hợp lệ. **ĐÍNH CHÍNH docblock sai:** route `unlink` ghi ":id (file) khoanh phạm vi" — KHÔNG ĐÚNG, handler không khai `@Param("id")` và service nhận đúng `linkId`; cô lập tenant giữ được bởi `findByIdTx(user.companyId, linkId, tx)`, tức bởi ngữ cảnh tenant chứ không phải bởi `:id`. Câu cũ sai theo hướng làm người đọc YÊN TÂM HƠN thực tế nên đã sửa; `:id` CỐ Ý không thêm pipe (validate một tham số không ai đọc là validate dữ liệu không tồn tại) và có ca ghim sự thật đó. **RATCHET kênh PARAM** (`test/foundation/param-uuid-census.ts` + `param-uuid-ratchet.unit-spec.ts`, AST, spec TĨNH): module đã vá đòi **0**; toàn API chịu **TRẦN** — xem KI-078 dưới đây về vì sao là trần chứ không phải 0. |
| **KI-078** | **216 tham số `:id`/`*Id` khác trên toàn API chưa có validate ở biên — CHƯA ĐO, cấp số thay vì vá mù.** Số đo 25/08/2026 (AST, `test/foundation/param-uuid-census.ts`, quét `apps/api/src/**/*.controller.ts`): **312** `@Param` tổng · **298** id-like · **77** đã có pipe ⇒ **221 chưa có** (226 trước bản vá của `S10-FND-PARAMUUID-1`). KI-077 kê 5 tham số trong MỘT module; census cho thấy **cùng hình dạng tồn tại 221 lần**. ⚠️ **221 KHÔNG có nghĩa là 221 bug** — chỉ 5 chỗ từng được đo bằng HTTP thật; 216 chỗ còn lại chưa ai chạm, và suy chúng cũng 500 là đúng thứ `done_when` của `S10-FND-PARAMUUID-1` cấm. Cũng KHÔNG phải mọi id-like đều cần `ParseUUIDPipe`: phép lọc theo TÊN sai được cả hai chiều (một `:key` chứa UUID sẽ lọt; một `:tenantId` dạng slug bị đếm oan). **Mức độ:** hỏng đúng chiều an toàn ⇒ KHÔNG phải lỗ bảo mật; hậu quả là hợp đồng API + 500 GIẢ bơm vào giám sát. **Đang được giữ bằng:** ratchet TRẦN (`param-uuid-ratchet.unit-spec.ts` ca (2)) chặn MỌC THÊM, kèm ca (3) ép hạ trần mỗi khi vá bớt — không giả vờ đã dọn xong. **Việc thật khi làm:** chọn nhóm route theo rủi ro, ĐO bằng HTTP trước, vá, hạ trần. | S4 | Hợp đồng API / xử lý lỗi | ❌ | ❌ | Cấp số 2026-08-25 khi thi công `S10-FND-PARAMUUID-1` — chưa có WO |
| ~~**KI-079**~~ | **MỞ VÀ ĐÓNG CÙNG PR 2026-08-25** (`S10-GOV-IDUNIQUE-1`). **⚠️ ĐỌC MỨC ĐỘ TRƯỚC: nợ SỔ SÁCH, KHÔNG phải lỗ bảo mật** — không rò dữ liệu, không hỏng runtime, không leo quyền. Cấp số hiệu vì lớp lỗi này **ĐÃ THU HỌC PHÍ HAI LẦN trong một tuần** và cả hai lần đều phải vá TAY. **Khuyết tật:** ba họ định danh của dự án — mã Work Order trong `harness/backlog.mjs`, số ADR trong `docs/DECISIONS/`, số hiệu `KI-0NN` trong chính sổ này — đều **KHÔNG có cổng ép duy nhất nào**. **Ca 1 (mã WO, đo 25/08):** `backlog.mjs` từng có ĐÚNG hai khối `id: "S10-QA-ROUTEHTTP-2"` (khối ship 18/08 "12 route risk≥5" + khối seed KI-025 ngày 24/08). Trùng id là lỗi **HAI TẦNG**: tầng backlog (`gen-status`/`gen-plan-index` đếm theo id) và **tầng LEDGER** — `harness/lib/wo-state.mjs#statusOverlay()` trả `Map<woId, bucket>` tức **khoá theo id**, nên dấu `finished` của khối ship áp thẳng lên khối seed ⇒ khối seed nằm trong danh sách "Đã xong" của STATUS và **KHÔNG BAO GIỜ** lọt `isReady`. Hệ quả thật: **một Work Order biến mất khỏi hàng đợi mà không phát ra lấy một dòng cảnh báo**. Vá tay: PR #417. **Ca 2 (số ADR, đo 25/08):** master trước đợt merge cao nhất `DECISIONS-09`; PR #414 thêm `DECISIONS-10_Role_Membership_Absence_Signal.md`, PR #416 thêm `DECISIONS-10_Catalog_FK_Company_Guard_Trigger.md` — **khác TÊN FILE ⇒ git merge SẠCH**, không xung đột, cả hai vào master im lặng ⇒ hai quyết định kiến trúc cùng số, mọi trích dẫn "DEC-010" về sau là mơ hồ. Vá tay lúc merge (#414 → `DECISIONS-11`). **Bản vá:** `harness/id-uniqueness.test.mjs` — ba cổng C1 (mã WO) · C2 (số ADR) · C3 (số KI ở §1 sổ này), `node --test`, zero-dep, **không cần DB/build ⇒ chạy trên MỌI PR kể cả docs-only** (lý do đặt ở `harness/` thay vì `apps/api/test/foundation/`). Ghim **ĐỊNH NGHĨA chứ không phải TÊN**: số trích từ tên file bằng regex và so theo **giá trị số** (`DECISIONS-9` đụng `DECISIONS-09`), không hard-code danh sách số đang có. **Số đo trên master `3e32d4cb`: 10/10 XANH** — WO 398 id / 0 trùng · ADR 11 file 01→11 / 0 trùng · KI 78 mã 001→078 / 0 trùng, 0 hụt. **RED-proof (gieo trùng THẬT vào cả ba nguồn):** 3 ĐỎ / 7 XANH đúng ba cổng; thêm 2 mutation chứng minh lớp chống-xanh-rỗng cắn (`touch docs/DECISIONS/ADR-nonconforming.md` ⇒ C2b ĐỎ; đổi tiêu đề `## 1.` ⇒ C3b ĐỎ). **Đã đăng ký vào CẢ BA danh sách tường minh** (`ci.yml` job `tooling` · `api.yml` bước "Tooling tests" · `check.sh` step `tooling-tests`) — cùng lượt kéo luôn `harness/chunk-bisect.test.mjs` vốn chỉ chạy ở `check.sh` mà **CI chưa từng chạy**. Phạm vi KHÔNG làm, khai rõ: `docs/plans/INDEX.md` **TỰ SINH** từ backlog ⇒ C1 đã phủ, cổng riêng chỉ nhân đôi tiếng ồn; lớp "THIẾU" (KI có ở §2 mà thiếu hàng §1) khác lớp "TRÙNG" nên ngoài phạm vi. | S3 | Quản trị / sổ sách | không | không | Harness |
| ~~**KI-080**~~ | **ĐÃ ĐÓNG 2026-08-26 — `S10-ATT-SHIFTASSIGNSCOPE-1`. ⚠️ PHẠM VI ĐÓNG RỘNG HƠN LÚC MỞ: HAI route, không phải một.** Rà điều kiện nghiệm thu "cùng lớp" đào ra `createRuleSchema` (`packages/contracts/src/attendance.ts:762`) có **ĐÚNG CÙNG MỘT `.refine()` một-chiều** — chép nguyên văn, chỉ đổi `assignmentScope`→`ruleScope` — cách bản vá **60 dòng trong CÙNG MỘT FILE**, và `attendance_rules` cũng có CHECK hai chiều `chk_attendance_rules_target`. **Đo bằng HTTP thật trên lane DB `mediaos_shiftassignscope` (không suy đoán):** `POST /attendance/rules {ruleCode,name,employeeId,effectiveFrom}` (khuyết `ruleScope`) ⇒ **500 SYSTEM-ERR-001**, `cause: violates check constraint "chk_attendance_rules_target"` — bản sao chưa từng được cấp số, sẽ sống sót nếu chỉ vá đúng cái đã ghim (bẫy "bản sao cách bản vá MỘT DÒNG" đã cho ra `S10-FND-PARAMUUID-1`). **SỐ ĐO TRƯỚC/SAU (cả hai route, cùng payload):** `POST /attendance/shift-assignments {shiftId,employeeId,effectiveFrom}` **500 SYSTEM-ERR-001 → 400 VALIDATION-ERR-001**; `POST /attendance/rules {ruleCode,name,employeeId,effectiveFrom}` **500 SYSTEM-ERR-001 → 400 VALIDATION-ERR-001**. HỆ QUẢ GHI **không đổi** ở cả hai: `SELECT` sau lượt gọi vẫn **0 hàng** trước và sau — đây là đổi **HỢP ĐỒNG**, không phải đổi hành vi ghi (khẳng định "hỏng đúng chiều an toàn" lúc mở **đúng**, giữ nguyên). **Đã làm gì:** một helper chung `scopeTargetMatches()` (+ `SCOPE_TARGET_MESSAGE`) ở `packages/contracts/src/attendance.ts` **mirror CHECK của DB 1:1**, dùng cho cả hai `.refine()`. Vá ở **CONTRACT** chứ không ở service — FE dùng chung schema nên vá riêng ở service là để FE tiếp tục gửi sai. **LUẬT ĐẶT RA (đừng đọc lụt):** mirror **ĐÚNG BẰNG** CHECK, **không chặt hơn** — nhánh `'Employee'` của CHECK KHÔNG cấm `department_id` nên refine cũng không cấm; chặt hơn CHECK = từ chối payload mà DB chấp nhận = trôi theo chiều ngược lại. Muốn siết thì siết CHECK trước bằng migration. `.default("Company")` **giữ nguyên** — cái sai là refine, không phải default; đổi default là đổi hợp đồng của mọi client đang gửi ĐÚNG. **RÀ CÙNG LỚP — kết quả ĐO, không phải đoán:** quét mọi CHECK dạng "scope↔cột neo" trong `db/schema/**` rồi lần ngược về đường ghi ⇒ **ĐÚNG HAI chỗ có lỗ, cả hai đã vá ở WO này; KHÔNG có món nào phải cấp số mới.** `chk_goals_level_anchor` **sạch** — `createGoalSchema` không có `.refine()` nào nhưng `GoalsValidationService` (`goals-validation.service.ts:120-142`) kiểm **CẢ HAI** chiều (`need` phải có **VÀ** `extra` phải rỗng) ⇒ 422. `chk_dashboard_widget_configs_role_user_scope` **không với tới được từ client** — không có route tạo; chỉ `PATCH /dashboard/configs/:id` với body `.strict()` KHÔNG chứa `config_scope`/`role_id`/`user_id`; hàng do `dashboard-config.seeder.ts` tạo. `finance.ts` (entry_kind · target_type) thuộc module **park (out-of-scope)** ⇒ không đụng. **BÀI HỌC:** cái phân biệt GOAL sạch với ATT hỏng KHÔNG phải "kiểm ở contract hay ở service" mà là **có ai kiểm chiều NGƯỢC hay không**. **Ca test:** ca ghim `🔴 GHIM BUG (KI-080)` đã **LẬT** sang `toBe(400)` + `VALIDATION-ERR-001` (**không** nới thành `>=400` — assert nới xanh với CẢ hai hành vi tức là ghim lỗ hổng MỞ), giữ nguyên assert **0 hàng**; thêm ca `assignmentScope:'Company'` **tường minh** + `employeeId` **THẬT** (để 400 không thể bị nhầm là lỗi FK) ⇒ 400; thêm cặp **ALLOW+DENY** cho `POST /attendance/rules` (`ruleScope:'Employee'`+`employeeId` ⇒ 201 — không có vế ALLOW thì ca 400 là xanh-RỖNG); spec contract MỚI `packages/contracts/src/attendance-scope-target.spec.ts` (29 ca) phủ **TRỌN BẢNG CHÂN TRỊ** của CHECK cho cả hai schema, **có kiểm chứng đột biến** (bỏ `&& !hasEmp` ở nhánh `Department` ⇒ 3 ca ĐỎ). **HỘ TIÊU THỤ đã census trước khi đổi** (500→400 là đổi hành vi quan sát được): `ShiftAssignmentFormDialog.tsx:77-86` **và** `RuleFormDialog.tsx:117-118` (hộ của route thứ HAI) đều **tự lọc id theo scope** nên không bao giờ sinh tổ hợp mâu thuẫn; 4 int-spec còn lại gửi `Company`+không id hoặc `Employee`+`employeeId`; `attendance-shift.service.spec.ts` gọi thẳng service (bỏ qua Zod) ⇒ **không hộ nào dựa vào hành vi 500**. **Lưới:** 592 ca ATT + 579 ca contracts XANH trên lane DB; `pnpm typecheck` 10/10. Kế hoạch: `docs/plans/S10-ATT-SHIFTASSIGNSCOPE-1.md`. | S3 | Hợp đồng API / validate biên | ❌ | ❌ | `S10-ATT-SHIFTASSIGNSCOPE-1` (đã đóng) |
| **KI-081** | **`GET /leave/types` (route ĐỌC chính tắc) KHÔNG trả `annualQuota`, dù contract khai nó là trường BẮT BUỘC và `POST`/`PATCH /leave/types` đều nhận + trả nó.** Cấp số 25/08 khi thi công `S10-QA-ROUTEHTTP-3`. **⚠️ ĐỌC MỨC ĐỘ TRƯỚC: trôi hợp đồng đọc/ghi, KHÔNG phải lỗ bảo mật** — không rò dữ liệu, chỉ **THIẾU** dữ liệu. **Cơ chế:** `leaveTypeSchema` (`packages/contracts/src/leave.ts:58`) khai `annualQuota: z.number().nullable()`; `LeaveService` map đúng trường này khi GHI (`leave.service.ts:232`), nhưng đường ĐỌC đi qua `LeaveReadService.toLeaveTypeView()` (`leave-read.service.ts:206`) — một view RỘNG HƠN (`description` · `deductBalance` · `balanceUnit` · `allowHalfDay` · `maxDaysPerRequest`…) lại **bỏ sót đúng `annualQuota`**. **Hệ quả ĐO ĐƯỢC:** giá trị ghi bằng `PATCH /leave/types/:id` (phản hồi PATCH trả `annualQuota:15`, cột DB `annual_quota=15`) **không đọc lại được** qua route đọc chính tắc ⇒ màn cấu hình nghỉ phép không hiển thị lại được hạn mức vừa đặt nếu chỉ dùng route này. **Bằng chứng:** ca `🔴 GHIM BUG (KI-081)` — `apps/api/test/integration/routehttp3-attendance-leave.int-spec.ts`. ⚠️ Ca GHIM hành vi SAI CÓ CHỦ Ý: vá xong nó ĐỎ ⇒ **LẬT** `toBeUndefined()` thành `toBe(<giá trị>)`, đừng xoá ca | S4 | Hợp đồng API / trôi DTO | ❌ | ❌ | WO `S10-LEAVE-TYPEQUOTA-1` |
...
| ~~**KI-068**~~ | **4 route ghi trả `500 SYSTEM-ERR-001` thay vì `400` khi body sai hợp đồng — validate KHÔNG chạy ở biên.** Đo bằng HTTP thật (không suy đoán): `POST /api/v1/api-keys` với `scopePermissionIds: []` ⇒ **500**, `error.type = "ZodError"`. Cơ chế 3 bước: (1) handler khai `@Body() dto: CreateApiKeyRequest` — đó là **TYPE** (`z.infer`), KHÔNG phải class `createZodDto` ⇒ `ZodValidationPipe` không có schema để chiếu ⇒ body đi thẳng vào handler, KHÔNG được validate ở biên; (2) handler tự `createApiKeyRequestSchema.parse(dto)` ném **ZodError THÔ**; (3) `AllExceptionsFilter` chỉ hiểu `ZodValidationException` của nestjs-zod (`getZodError()`), không hiểu `ZodError` thô ⇒ rơi vào nhánh 500. ⚠️ Comment tại `api-keys.controller.ts:45` KHẲNG ĐỊNH "ZodValidationPipe đã chạy" — SAI, đúng lớp bẫy `ui-promises-backend-never-reads`. **Cùng hình dạng** (census `@Body()` nhận TYPE + handler tự `.parse()`): `POST /foundation/files/upload` · `POST /foundation/files/:id/confirm` · `POST /foundation/files/:id/links` — **3 route này suy từ hình dạng code, CHƯA đo bằng HTTP** (nói rõ để không lẫn đo với đoán). Đối chiếu cách làm ĐÚNG: `profile-change-request.controller.ts:58` dùng `@UsePipes(new ZodValidationPipe(schema))`. **Hỏng đúng chiều an toàn** (request vẫn bị từ chối, fail-closed) ⇒ KHÔNG phải lỗ bảo mật; hậu quả: client không phân biệt được "payload sai" với "server hỏng", và mọi payload sai bơm 500 GIẢ vào giám sát/cảnh báo. **Ghim:** `apps/api/test/integration/invite-apikeys-http.int-spec.ts` — ca `🔴 GHIM BUG (KI-068)` assert CẢ status 500 LẪN `error.type='ZodError'`; ai vá sẽ thấy nó ĐỎ và phải **LẬT** sang `expect(400)`, cấm nới thành `expect([400,500])` ✅ **ĐÓNG 2026-08-24 — `S10-FND-BODYVALIDATE-1`.** Cả 4 route nay trả **400 ở BIÊN**. Vá theo khuôn nhà (hướng (a)): dựng class `createZodDto` ở `api-keys/api-keys.dto.ts` + `foundation/files/files.dto.ts` — `api-keys` và `foundation/files` là hai module DUY NHẤT từng thiếu file `*.dto.ts`, tức bỏ sót chứ không phải ngoại lệ có chủ ý. **SỐ ĐO (AST, `test/foundation/body-validation-census.ts`): trước 193 handler GHI / 189 validate ở biên / **4** KHÔNG → sau **193 / 193 / 0**.** ⟲ **ĐÍNH CHÍNH mẫu số:** bản seed ghi `177/173/4` — con số **4 và danh sách 4 route thì ĐÚNG**, nhưng mẫu số sai: census seed viết bằng **regex** và bỏ sót **16** handler. Bản AST là số dùng được; regex còn đếm sai 4→2 rồi 4→5 trước khi ra được 4 (đúng lớp bài học đã ghi ở `identity-projection-census.ts`). **3/4 route đã LẬT từ suy-luận sang ĐO BẰNG HTTP:** `test/integration/files-http-validate.int-spec.ts` đo trước-vá cả ba trả 500 + `ZodError`, nay 400. **RATCHET** `body-validation-ratchet.unit-spec.ts` (spec TĨNH, chạy cả khi không có `LANE_DB`) ghim "0 handler GHI bỏ qua validate ở biên", neo theo ĐỊNH NGHĨA (đối chiếu metatype với tập `createZodDto`) chứ không theo tên route; **đã kiểm chứng đột biến** — trả một tham số về TYPE ⇒ ĐỎ đúng handler. Ca ALLOW canh hai hồi quy mà chính bản vá có thể đẻ ra: `confirm` body RỖNG vẫn hợp lệ, và `link` KHÔNG cần `fileId` trong body (`LinkFileDto` `.omit({fileId:true})`; handler vẫn ép `fileId` từ route). Comment nói dối ở `api-keys.controller.ts:45` đã gỡ. ⚠️ **KI-068 đóng ở kênh BODY, KHÔNG phải mọi kênh:** khi đọc lại diff đã phát hiện bản sao cùng cơ chế ở kênh PARAM ngay trong các handler vừa vá — hai route GHI đã đo + vá kèm (`@Param("id", ParseUUIDPipe)`), **năm tham số READ/DELETE còn lại tách thành `KI-077`** thay vì vá mù. Đừng đọc dấu gạch của KI-068 thành "foundation/files đã sạch input". | S3 | Hợp đồng API / xử lý lỗi | ❌ | ❌ | **ĐÓNG 2026-08-24** — `S10-FND-BODYVALIDATE-1` |
...
| ~~**KI-010**~~ | Endpoint cũ `GET /employees` chưa phân trang thật (mới chặn bằng cap 2000) | S3 | Sản phẩm | ✅ | ✅ | **ĐÓNG 2026-08-25** — `S10-HR-EMPPAGE-1`. `page`/`per_page` qua **schema Zod ĐÃ CÓ SẴN** (`employeeListQuerySchema` tồn tại từ trước nhưng controller chưa hề dùng — nó nhận 4 `@Query()` RỜI, đúng cách file đó trôi tới hiện trạng); envelope `paginated(...)` mang `pagination.total`. **`LIMIT`/`OFFSET` ở SQL**, không cắt mảng trong JS, và `total` là `count(*)` trên CÙNG mệnh đề `where` (sau filter + sau scope) chứ không phải `rows.length`. Trần **2000 GIỮ NGUYÊN** — WO thay cắt-CÂM bằng cắt-CÓ-BÁO, không thay cái trần; hằng số chuyển về contracts để BIÊN và SQL dùng chung một nguồn. `per_page` vượt trần ⇒ **400 field-level ở biên** (khuôn `loginLogListQuerySchema`), không kẹp im lặng — client xin 99999 mà nhận 2000 không lời giải thích là một dạng cắt câm khác. **Vế FE (phần đắt nhất, không phải BE):** census hộ tiêu thụ cho thấy `apps/app` dùng đường `/hr/employees` (đã có phân trang từ trước), hộ DUY NHẤT của `/employees` là `apps/console`. Nhưng `apiFetch` bóc `.data` và **VỨT** block `pagination` ⇒ thêm `apiFetchPaginated` vào `web-core` (đường song song, opt-in, dùng chung trọn vòng đời SSO) để console đọc được `total` — không có nó thì `total` có trên dây mà không ai đọc được, đúng nghĩa làm nửa vời. |
...
| ~~**KI-025**~~ | **ĐÃ ĐÓNG 2026-08-25 — `S10-QA-ROUTEHTTP-3`. SỐ ĐO MỚI: 500/500 route (100%) có bằng chứng test HTTP thật; CHƯA phủ = 0 ở MỌI mức risk.** ⚠️ **ĐỌC HAI CẢNH BÁO TRƯỚC KHI TRÍCH CON SỐ NÀY.** (a) **100% là 100% của "ĐÃ BỊ CHẠM", KHÔNG phải độ phủ HÀNH VI** — phép đo khớp verb×path ở CẤP FILE nên nó vẫn là **CẬN TRÊN** đúng như đã khai từ đầu; đóng KI này nghĩa là *mọi route đều đã có ít nhất một lượt HTTP thật đi qua guard + Zod + envelope + filter*, KHÔNG nghĩa là API đã được kiểm đủ. (b) **Chính THƯỚC ĐO đã được vá trong cùng WO** ⇒ con số 100% KHÔNG so sánh trực tiếp được với 383/499 (18/08). **BA lớp lỗi của phép đo, đo được 25/08:** **(1) DƯƠNG-TÍNH-GIẢ, chiều nguy hiểm** — census quét file trên ĐĨA còn vitest chạy theo danh sách của nó, nên 6 spec nằm trong `test.exclude` của `vitest.config.ts` (module park/hoãn Phase) vẫn được tính là bằng chứng: **7 route của `WorkflowController`** (`start` · `steps/:id/start` · `steps/:id/submit` · `approval-requests/:id/approve` · `.../request-revision` · `GET /workflow/:instanceId` · `GET /workflow/approval-requests`) mang dấu "covered" bằng những lượt HTTP **CHƯA TỪNG XẢY RA**, nguồn duy nhất là `test/workflow-lifecycle.e2e-spec.ts`. **(2) ÂM-TÍNH-GIẢ** — `PATH_LITERAL_RE` gộp `${}` vào cùng lớp ký tự với path ⇒ một dấu `!` (`${pending!.id}`), `[`, `(` trong biểu thức là ĐỦ để cả literal không khớp gì (3 route). **(3) ÂM-TÍNH-GIẢ + CODE CHẾT** — lớp ký tự thiếu `?` ⇒ mọi literal có query-string bị bỏ, và dòng `literal.split("?")[0]` ngay trong `normalizeLiteralPath` **chưa từng chạy** (3 route). **Vì vậy điểm xuất phát ĐÚNG của đợt này là 382/500 (76,4%) chứ không phải 386/500** — 386 là số của thước đo còn hỏng trên master `90d26aee`. **Bản vá thước đo (cùng PR):** tách `${[^{}]*}` thành nhánh riêng · thêm `?=&%+,` vào lớp ký tự · thêm `VITEST_EXCLUDED_SPECS` + `isVitestExcluded()` vào `isHttpTestFile`. **Chống tái sinh:** 4 ca tự-kiểm gọi thẳng `extractEvidence`/`isHttpTestFile` trên chuỗi dựng sẵn + 1 ca ĐỌC `vitest.config.ts` thật rồi so hai danh sách exclude (ghim ĐỊNH NGHĨA, không ghim tên). **RED-proof 3/3:** gieo lại từng lỗi cũ ⇒ đúng ca tương ứng ĐỎ (`MUT-A` query · `MUT-B` `${}` · `MUT-C` desync exclude). **Ratchet siết:** `MAX_UNCOVERED_TOTAL = 0` (CỔNG CHÍNH mới — thêm BẤT KỲ route nào ở BẤT KỲ mức risk mà không có test HTTP thật ⇒ CI ĐỎ tại PR đó) · `MIN_COVERED_COUNT` 383 → **500** · `MAX_UNCOVERED_HIGH_RISK` giữ 0 (thông điệp lỗi riêng cho nhóm nguy hiểm). **Công việc đã làm:** 7 file `test/integration/routehttp3-*.int-spec.ts`, **105 ca**, phủ 118 route còn nợ — mỗi cụm có ca ALLOW 2xx chứng minh bằng **HỆ QUẢ đọc lại** (không assert theo status code), ca DENY 403 role RỖNG đặt SAU ALLOW, ca DTO 400 ở biên, và ca cross-tenant có **cạnh đối chứng** (token B làm được việc trên tài nguyên của CHÍNH nó) để 404 không thể xanh vì route chết. **Hai BUG THẬT đào ra ⇒ TÁCH SỐ RIÊNG, KHÔNG vá trong lane QA: KI-080 · KI-081** (đúng luật đã cho ra KI-068). **Nguyên văn chẩn đoán 18/08 giữ lại làm bản ghi:** **≥116/499 đường dẫn API (≥23,2%) không có bằng chứng test HTTP nào chạm** (đo lại **18/08** bằng `route-http-coverage.e2e-spec.ts` sau khi `S10-QA-ROUTEHTTP-2` land; mốc trước: ≥129/499 ngày 14/08, và 98/346 là số CŨ đã bỏ). **Nhóm rủi ro cao đã đóng: route risk≥5 chưa phủ = 0** (12 → 0) và ratchet `MAX_UNCOVERED_HIGH_RISK` **siết từ 12 xuống 0** ⇒ thêm route risk≥5 không test là ĐỎ ngay tại PR. Giữ **S2**: 23,2% đường dẫn vẫn chưa có bằng chứng nào, đóng hay hạ mức là kết luận của SỐ chứ không phải mong muốn. ⚠️ Số "đã phủ" **383/499 (76,8%) là CẬN TRÊN, không phải số sự thật** — scan khớp verb-set × path-set ở **cấp FILE**, không cấp câu lệnh, và "chạm" không chứng minh có ca ALLOW ⇒ sai số dồn về phía false-positive "covered" ⇒ độ phủ THẬT ≤ 74,1% và **129 là cận DƯỚI của khoảng trống thật**. Rủi ro: phủ ở tầng service nên guard/DTO/envelope của route chưa từng chạy. Đợt 14/08 phủ HTTP thật **5 route risk≥5 (đều có ca ALLOW 2xx thật) + 2 route risk=3** — đính chính câu "5/18 route risk≥5" của vòng trước, xem §KI-025; `PATCH /settings/security-policy` **KHÔNG tính đã phủ** (route chết, chỉ có ca ghim 403 — **KI-065**; **KI-065 ĐÓNG 19/08/2026 ⇒ route này nay CÓ ca ALLOW 2xx thật và tính là đã phủ**). **Đợt 18/08 (`S10-QA-ROUTEHTTP-2`) đóng nốt 12 route risk≥5 còn nợ** — `users/invite` · `api-keys` create/revoke · `auth/users/:id` password-reset/restore/soft-delete · `permissions` assign-role/revoke-role/object-grant PUT+DELETE · `auth/roles/:id` delete/revoke-permission — mỗi route có ca ALLOW 2xx chứng minh bằng HỆ QUẢ + DENY 403 + DTO 400 ở biên + cross-tenant 404 (49 ca, 3 file `invite-apikeys-http` · `authusers-admin-http` · `permadmin-roles-http`). Nợ còn lại là nhóm risk≤3 | S2 | Độ phủ test | ❌ | ❌ | ~~WO `S10-QA-ROUTEHTTP-2`~~ **XONG 25/08 bởi `S10-QA-ROUTEHTTP-3`** |
...
| ~~**KI-048**~~ | **Hàng `blocked` giờ HIỆN trong màn admin, và tốc độ sinh chúng do KẺ TẤN CÔNG điều khiển** — hệ quả phái sinh của `S6-SEC-LOGINLOG-2`, phát hiện bởi `security-reviewer` ở FULL gate. **Lượng ghi KHÔNG đổi** (những dòng đó vốn đã được ghi, chỉ là dưới `company_id NULL` nên không ai thấy) ⇒ **delta dung lượng = 0**; cái đổi là **khả năng thấy**. Một khi bucket `(slug,email,ip)` đã khoá, MỌI request kế tiếp trong `LOGIN_LOCKOUT_SEC` (900s) sinh một hàng **có chủ** với chi phí server gần bằng 0 (không argon2), trong khi trước đó muốn có hàng có-chủ thì phải qua rate-limiter, mỗi lần tốn một lượt băm. Cộng ba yếu tố: `login_logs` nằm trong `PROTECTED_TABLES` (`retention.service.ts:49`) ⇒ **không bao giờ được thu hồi**; `loginLogListQuerySchema` (`packages/contracts/src/auth.ts`) **không có filter `failure_reason`** ⇒ admin không lọc nhiễu ra được; `total`/paging của AUTH-API-401 phồng vô hạn. ⇒ Kẻ tấn công vô danh có thể **chôn tín hiệu thật dưới nhiễu ngay trong chính màn hình mà KI-044 vừa khôi phục**. Hướng vá đề xuất: gộp (coalesce) hàng `blocked` theo bucket theo cửa sổ khoá — vá luôn cả giới hạn "sàn thủng khi tải cao" ghi ở KI-044 | S3 | Bảo mật (quan sát) | ✅ | ✅ | **ĐÓNG 2026-08-25** — `S10-SEC-LOGINLOG429-1`. Hai vế: (1) **LỌC** — `loginLogListQuerySchema` += `failure_reason` (`z.string()` chứ KHÔNG `z.enum`: mã lý do là dữ liệu append-only lịch sử, enum hoá làm hàng mang mã cũ/mới không lọc được) → repository + viewer + ô nhập ở `LoginLogsPage`. (2) **GỘP** — `LoginRateLimiter.claimFirstOfWindow` (setNx + fallback Map in-memory, TTL = `LOGIN_LOCKOUT_SEC`) ⇒ trần **1 hàng/bucket/cửa sổ** thay vì vô hạn. GỘP = **KHÔNG GHI THÊM**, tuyệt đối không UPDATE (bảng bị REVOKE UPDATE/DELETE). Khoá gộp **soi gương bucket đang khoá** — `isLoginRateLimited` nay trả `"acct"`/`"ip"` thay vì boolean, vì `accountKey` KHÔNG chứa `ip`: gộp theo `{slug\|email\|ip}` sẽ bỏ sót credential-stuffing rải nhiều nguồn, đúng ca nặng nhất. Vị trí kiểm gộp ghim TRONG `try` SAU `startedAt` để sàn thời gian còn phủ lượt bị gộp (không đẻ lại oracle KI-044). **Tổn thất đã cân nhắc, KHÔNG phải quên:** sổ trả lời "CÓ bị chặn trong cửa sổ này", không trả lời "BAO NHIÊU lần" — ghi số đếm cần UPDATE ⇒ cấm; số đếm sống ở bucket rate-limit. |
| ~~**KI-047**~~ | **Bốn đường 429 KHÁC không ghi một dòng `login_logs` nào** — phát hiện khi khoanh ranh giới KI-044. Trong `apps/api/src/auth/**` có 5 chỗ ném `TOO_MANY_REQUESTS` (tra bằng `grep -n TOO_MANY_REQUESTS`, **KHÔNG neo số dòng** — chúng trôi mỗi lần sửa file): trong `auth.service.ts` là nhánh rate-limit của `login()` (đường **DUY NHẤT** ghi `login_logs`), của `verifyTwoFactorLogin` (bước-2), của `disableTwoFactor`, của `changePassword`; cộng một chỗ trong `two-factor.service.ts`. Bốn chỗ sau **không** gọi `recordLoginAttempt`. Đáng kể nhất là **bước-2 2FA** (bucket rate-limit `rlKey` tiền tố `2fa`) — dò mã TOTP 6 số là brute-force thật, hiện chỉ có `securityAlerts.emit`, **không có dòng nào ở AUTH-API-401**, dù `claims.companyId` đang nằm sẵn trong tay (khác hẳn KI-044, ở đó lý do là chưa resolve kịp). ⇒ Sau khi KI-044 đóng, admin thấy được brute-force **mật khẩu** nhưng vẫn mù với brute-force **mã 2FA**. Cùng lớp "mất tầm nhìn của bên phòng thủ", KHÔNG phải rò rỉ ⟲ **ĐO LẠI 24/08/2026 (seed Đợt 3) — SỐ ĐÃ TRÔI, dùng số này:** nay có **SÁU** điểm ném `TOO_MANY_REQUESTS` trong `apps/api/src/auth/**`, không phải 5 ⇒ **NĂM đường không ghi**, không phải bốn. Điểm MỌC THÊM sau ngày mở: `step-up/step-up.service.ts` (`stepUp`). **Đính chính tên hàm:** `verifyTwoFactorLogin` không tồn tại — hàm thật là `completeTwoFactorLogin` (`auth.service.ts:452`), grep theo tên cũ ra 0 kết quả. Không có ratchet thì đường thứ 7 sẽ mọc y như `step-up` đã mọc. | S3 | Bảo mật (quan sát) | ✅ | ✅ | **ĐÓNG 2026-08-25** — `S10-SEC-LOGINLOG429-1`. Vá theo **LUẬT**, không vá từng chỗ: *đường DỰNG NÊN cái khoá phải để lại vết; đường ĐANG BỊ KHOÁ ghi 0 hàng* (ghi ở nhánh đã-khoá là mời bồi hàng vào bảng không thu hồi được — chính KI-048). ⇒ `completeTwoFactorLogin` ghi `login_logs` ở BA nhánh (`TwoFactorChallengeReplay` gộp theo `jti` · `TwoFactorInvalid` không gộp · `TooManyAttempts` gộp); `disableTwoFactor`/`changePassword`/`confirmEnable` ghi `REAUTH_FAILED` (mã MỚI, `user_security_events`) ở nhánh **SAI**; `stepUp` **KHÔNG đụng** — nhánh khoá ghi 0 hàng là nửa (a) của bản vá A09 chống bồi hàng, ghi vào đó là hoàn tác nó. **PHÁT HIỆN NGOÀI KHUNG KI GỐC:** `completeTwoFactorLogin` trước đây ghi `login_logs` **CHỈ khi thành công** — challenge hỏng · replay · 429 · mã sai · công ty ngừng đều 0 dòng, cộng bước-1 nhánh cấp challenge cũng 0 dòng ⇒ **với tài khoản bật 2FA, AUTH-API-401 chỉ chứa THÀNH CÔNG**. Vá riêng nhánh 429 sẽ để lại lỗ LỚN HƠN cái vừa vá. **RATCHET** (thứ KI-047 đòi): census AST `test/foundation/login-log-429-census.ts` + `login-log-429-ratchet.unit-spec.ts` — đo ở mức **NHÁNH** (lời gọi ghi phải là hậu duệ của `Block` trong cùng nhất chứa `throw`), 4 waiver mỗi cái có **neo DƯƠNG**; điểm ném thứ 7 mọc lên không khai ⇒ ĐỎ. Không migration (`failure_reason` và `event_type` đều không có CHECK). |
...
| **KI-055** | 🟡 **ĐÃ VÁ 2026-08-25, CHỜ XÁC NHẬN PROD — `S10-SEC-FKCATALOG-1` (mig `0547`).** **Lỗ tồn dư lớp G sau `S6-SEC-XTENANTFK-1`**: 11 cặp FK trỏ tới bảng **catalog toàn cục** (`company_id` NULLABLE) KHÔNG vá được bằng composite FK — vá là chặn luôn tham chiếu hợp lệ tới hàng dùng chung (đã chứng minh: `Key (company_id, role_id)=(A, <role hệ thống>) is not present in table "roles"`). **Tác hại ĐO ĐƯỢC** (rls-tenant-isolation-tester, FULL gate 2026-07-31): A gán được role của B ⇒ B xoá role của chính B thì hàng `user_roles` mang `company_id = A` biến mất (CASCADE bắc cầu chéo tenant). **CÁCH VÁ:** trigger `enforce_company_id_catalog_fk` (`SECURITY DEFINER`, `search_path` khoá) trên 8 bảng con — cha phải CÙNG TENANT **HOẶC** là hàng toàn cục (`company_id IS NULL`); cơ chế + 2 hướng bị loại + giới hạn: `DECISIONS-10`. Vá kèm dư nợ của `0531`: `dashboard_widgets` nay bất biến `company_id`. **TRƯỚC/SAU (lane `mediaos_fkcatalog`, chain 0000→0547, 2026-08-25):** hành vi **11/11 cặp GHI THÀNH CÔNG hàng lệch tenant → 11/11 bị chặn `23503 catalog_fk_tenant_mismatch`**; tổ hợp #4 (con toàn cục → cha có chủ) **2/2 lọt → 2/2 chặn**; ca ALLOW (gán role hệ thống / widget-template toàn cục) **vẫn ghi được** — không lặp lại thất bại của composite FK. **Dữ liệu:** 13 câu đo drift = **0/0** TRƯỚC và SAU trên lane (và 0 trên PROD ở phép đo TRƯỚC) ⇒ tiền kiểm của `0547` không kích hoạt, **không xoá/sửa hàng nào** (BẤT BIẾN #2). 4 script seed (`seed-admin` · `seed-operator` · `demo-seed-base` · `demo-seed-full`) chạy PASS trên lane đã áp `0547`. **Chốt chống mọc thêm:** waiver `fk-tenant-verdicts.ts` **11 → 0** · ratchet (b) mở thành BA trạng thái + 3 ca mới (l) guard XOR waiver còn hạn & FK một-cột còn sống · (m) hàm giữ `SECURITY DEFINER`/`search_path`/**chủ hàm BYPASSRLS** (mất vế này ⇒ fail-open IM LẶNG) · (n) `dashboard_widgets` immutable · file mới `catalog-fk-tenant-guard.int-spec.ts` (4 tổ hợp × 11 cặp + 2 ca cứng `user_roles` + `cascade-unreachable`). ✅ **ĐÃ ĐO TRÊN BẢN SAO PROD GIỮ QUYỀN 2026-08-25** (`mediaos_prodclone` = `pg_dump -Fc` + `pg_restore` KHÔNG `--no-owner`/`--no-acl`, cùng cụm nên role sang đủ; **đối chiếu clone ↔ PROD khớp tuyệt đối**: 158 bảng RLS · 158 FORCE · 175 policy · 466 grant `mediaos_app` · 214 migration): **13 câu đo = 0 TRƯỚC và 0 SAU**; sau khi áp `0547` → 215 migration · 11 trigger guard ACTIVE · 9 trigger bất biến · `prosecdef=t` · `search_path=pg_catalog, pg_temp` · **chủ hàm `mediaos` BYPASSRLS=true** (chặn đúng R12 fail-open im lặng) · PUBLIC EXECUTE=0 · 67 hàng `user_roles` thật còn nguyên (migration không đụng dữ liệu). ⚠️ **VÌ SAO VẪN CHƯA ĐÓNG HẲN:** vá là **FORWARD-ONLY** (không hồi tố hàng cũ) và còn thiếu điều kiện (3) — **FULL gate PASS + ratchet (l)(m)(n) xanh ở CI thật + người chốt merge** (PR #416, cố ý KHÔNG auto-merge). ⚠️ Hệ quả đo được: PIN `PROVEN_WITH_CHECK_FLOOR` của `tenant-isolation.int-spec` hạ **148 → 147** (A/B cùng lane: guard BẬT 147/156, DISABLE 151/156) — 4 bảng bị guard chặn TRƯỚC `WITH CHECK`; là **CHE**, không phải MẤT | S3 | Bảo mật (toàn vẹn) | ✅ | ❌ | **VÁ 2026-08-25** — `S10-SEC-FKCATALOG-1`, chờ xác nhận PROD |
...
**KI-010 — ĐÓNG 2026-08-25 (`S10-HR-EMPPAGE-1`).** R2 của cụm này nay đã xong: `GET /employees` có
phân trang thật (`page`/`per_page` + `pagination.total`), cap 2000 giữ nguyên làm TRẦN `per_page`.
⇒ **cả ba khuyến nghị R1/R2/R3 của `S5-PERF-1` nay đều đã đóng** (KI-009 13/08 · KI-011 30/07 ·
KI-010 25/08) — cụm này không còn món nào treo. Ghi ra đây vì `done_when` của WO đòi đối chiếu lại
CẢ CỤM, không chỉ món của mình: ba khuyến nghị đóng ở ba thời điểm khác nhau rất dễ để lại một dòng
tổng kết nói sai.
...
**Cập nhật 2026-08-25 (`S10-GOV-IDUNIQUE-1`):** `S2` **không đổi** — **KI-079 mở và đóng trong cùng
PR**, mức `S3`, loại quản trị/sổ sách. Cấp số hiệu dù vá xong ngay, vì khuyết tật **đã có mặt thật
và đã thu học phí HAI LẦN trong một tuần** (#417 gỡ mã WO trùng; lượt merge #414 đổi số ADR trùng),
cả hai lần đều phát hiện bằng mắt người chứ không bằng cổng. Ba điểm đáng giữ làm tiền lệ:

**(1) "Git merge sạch" KHÔNG phải bằng chứng không trùng.** Cả hai ca bệnh đều đi qua git mà không
sinh một xung đột nào: hai file ADR khác TÊN nhưng cùng SỐ là hai file khác nhau dưới mắt git, và
hai khối backlog cùng `id` nằm cách nhau hàng nghìn dòng. Danh tính của dự án nằm ở **nội dung**
(mã WO, số ADR, số KI) chứ không ở đường dẫn — mà git chỉ bảo vệ đường dẫn. Mọi họ định danh do
người cấp phát tuần tự đều cần một cổng RIÊNG, không được mượn git làm cổng.

**(2) Trùng mã WO là lỗi HAI TẦNG, và tầng thứ hai mới là tầng giết việc.** Tầng nhìn thấy được là
backlog hiện hai dòng cùng mã. Tầng không nhìn thấy được là ledger: `statusOverlay()` trả
`Map<woId, bucket>` ⇒ **khoá theo id** ⇒ dấu `finished` của khối đã ship áp thẳng lên khối chưa
làm, khối chưa làm rơi vào "Đã xong" và vĩnh viễn không lọt `isReady`. Vì vậy cổng này phải ĐỎ chứ
không được là cảnh báo — hệ quả của nó là **mất việc**, không phải nhiễu. Cùng họ với
`blocked-status-is-the-only-machine-readable-stop`: cảnh báo trong văn xuôi thì auto-loop mù.

**(3) Cổng chống-trùng có HAI cách xanh-rỗng, không phải một.** Cách quen thuộc là thiếu ca
thử-ngược (gieo trùng mà không ai kiểm là cổng có ĐỎ không). Cách thứ hai kín hơn: **regex trích
trượt hết** thì tập rỗng cũng "không trùng" ⇒ xanh vĩnh viễn, và nó xảy ra một cách hoàn toàn tự
nhiên khi ai đó đổi quy ước đặt tên hoặc đổi tiêu đề mục. Nên mỗi cổng ở đây có thêm một ca `*b`
assert **số phần tử trích được > 0 VÀ không còn phần tử nào không trích được** — cả hai ca `*b` đều
đã được chứng minh là CẮN bằng mutation thật (`touch docs/DECISIONS/ADR-nonconforming.md` ⇒ C2b ĐỎ;
đổi `## 1. Bảng tổng hợp` → `## Bảng tổng hợp` ⇒ C3b ĐỎ), không phải chỉ nhìn nó xanh.
