...
| **KI-076** | **NĂM route REST của CALL cũng gọi đường PHÁT realtime SAU COMMIT mà KHÔNG bọc — và với BA trong năm, mất sự kiện là VĨNH VIỄN y hệt KI-075.** Cấp số 24/08 khi thi công `S10-CHAT-EMITGUARD-1`: món này bị để ngoài phạm vi WO đó **và ban đầu bị đánh giá NHẸ ĐI** — xem đính chính ngay dưới. **Điểm đo (`chat-calls.service.ts`, đo 24/08):** `invite` gọi `emitLifecycle(..., "ringing")` ở `:203`; bốn route vòng đời còn lại (`accept`·`reject`·`cancel`·`hangup` — `CHAT-API-027…030`) dùng CHUNG `lifecycleTx` và phát ở `:558`. Cả hai điểm nằm NGOÀI `withTenant` (đúng luật) nhưng **không bọc `try/catch`**, giống hệt hình dạng hai job trước khi vá. ⚠️ **ĐÍNH CHÍNH đánh giá ban đầu** (ghi trong PR #408 là *"khác hồ sơ rủi ro — client thấy được trạng thái qua chính response POST, không mất vĩnh viễn như đường job"*): câu đó **chỉ đúng cho ACTOR, và chỉ đúng ở 2/5 hình dạng**. Peer KHÔNG có response POST nào để đọc — kênh WS là kênh duy nhất báo cho họ. Bảng hồi phục thật: **(a) `ringing`** (`invite`) → job `CHAT_CALL_RINGING_TIMEOUT` quét sau 45s **+** bước dọn-trước-khi-mời của lời mời kế ⇒ **tự lành**. **(b) `active`** (`accept`) → job `CHAT_CALL_STALE_ACTIVE_SWEEP` quét mồ côi (grace mặc định 2 phút) / quá thọ (12h) ⇒ **tự lành CHẬM**. **(c) `rejected` · `cancelled` · `ended`** → **trạng thái CUỐI: KHÔNG job nào khớp** (cả hai job chỉ quét `ringing`/`active`), và CALL **không có route ĐỌC nào để poll bù** ⇒ peer giữ khung gọi/chuông của một cuộc gọi đã chết **tới khi reload hoặc reconnect** — đúng lớp hậu quả của KI-075, không nhẹ hơn. **Mức độ HÔM NAY — đọc đúng, đừng thổi:** vẫn **KHÔNG ném được**, vì `RealtimeEmitterService.emitChatCall` tự nuốt mọi lỗi. Khác KI-075 ở một điểm QUAN TRỌNG: bất biến đó **nay đã được GHIM** ở `realtime/realtime-emitter.call.spec.ts` (mở ở `S10-CHAT-EMITGUARD-1`, có kiểm chứng đột biến). Nên rủi ro tồn dư không còn là *"ai đó sửa emitter mà không biết"* mà là *"ai đó CỐ Ý đổi hợp đồng emitter và sửa luôn ca ghim"* — **thấp hơn hẳn KI-075 lúc mở**. Đây là nợ phòng-thủ-theo-chiều-sâu, KHÔNG phải lỗ đang chảy máu. **Vế thứ hai, do chính bản vá `S10-CHAT-EMITGUARD-1` tạo ra:** `invite` gọi `emitExpired` ở `:202` — hàm này **nay trả về số cuộc gọi không phát được**, nhưng đường REST **vứt giá trị đó**. Ở đường job con số ấy đi vào `JobRunResult.failed` + `metadata.emitFailed`; đường REST **không có run-row nào để chứa nó**, nên kênh duy nhất còn lại là dòng `logger.error` kèm `callId` bên trong helper. Đó có thể là câu trả lời ĐÚNG, nhưng phải **quyết tường minh và viết ra**, nếu không nó trông y hệt một sơ suất và lần review sau sẽ mở lại đúng cuộc tranh luận này. **Việc thật khi làm:** bọc cả HAI điểm phát (`:203` và `:558`), quyết tường minh chuyện REST không có chỗ chứa số đếm (chấp nhận log-only thì KHAI ra), và ghim bằng ca chứng minh **route vẫn trả 2xx + DTO** khi emit ném — hiện tại nó trả **500 cho một giao dịch ĐÃ commit**, tức actor tưởng thất bại rồi thử lại và ăn **422 `CALL_NOT_ACTIONABLE`** (`mustTransition` từ trạng thái đã đổi). **Workaround:** không có và không cần — bất biến hôm nay còn đúng VÀ đã có ca ghim. | S4 | Độ bền vận hành (REST · realtime) | ❌ | ❌ | WO `S10-CHAT-EMITGUARD-2` (todo) |
