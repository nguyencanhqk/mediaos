...
| --- | --- | --- |
| **Phase 2** | **PAYROLL** — tiền lương | Wave **`S13-PAYROLL`** (v1 — owner duyệt 31/08/2026, [kế hoạch](<../plans/S13-PAYROLL-WAVE.md>)) → **`S15-PAYROLL-V2`** (v2 — owner ký PAY-DEC-011..020 ngày **02/09/2026**, [kế hoạch](<../plans/S15-PAYROLL-V2-WAVE.md>)). **✅ đã có bộ tài liệu v2**: [SPEC-11](<../SPEC/SPEC-11 PAYROLL.md>) · [DB-13](<../DB/DB-13 PAYROLL Database Design.md>) · [API-18](<../API Design/API-18_PAYROLL_API_Design.md>) · [phân quyền §9g](<../permission-matrix-spec.md>) · [ADR thư viện: DECISIONS-14](<../DECISIONS/DECISIONS-14_Payroll_V2_Library_Stack.md>). **v1 ĐÃ SHIP** (7 bảng · 35 route · 6 màn · widget chi phí lương kỳ — mig `0564`–`0568`). **v2 chưa build** (mig `0570+`): **ALTER 3 bảng** (DB-1 lấy 2 · DB-2 lấy 1) + **11 bảng mới** · **50 route** · **11 màn** · **17 cặp quyền** (tất cả sensitive) · **16 mã lỗi** · **4 event** · **2 widget** · FSM kỳ **7 → 8 trạng thái**. ⚠️ **PARK-PAYROLL-001 ĐÃ THU HẸP — v2 LẤY LẠI năm mục**: engine BHXH/BHYT/BHTN/TNCN luỹ tiến (P2-PAY-05-002/003, PAY-DEC-014) · PDF phiếu lương (P2-PAY-07-003) · export payslip batch/signed-URL (P2-PAY-08-002) — cả hai theo PAY-DEC-019 · variance report (P2-PAY-08-004) · report theo phòng ban (P2-PAY-08-003) — cả hai dưới dạng **7 báo cáo** (PAY-DEC-018). Phần còn lại chuyển sang **PARK-PAYROLL-002** (danh sách **ĐÓNG** — [SPEC-11 §5.2b](<../SPEC/SPEC-11 PAYROLL.md>)): *(a) còn lại của PARK-001* — **khiếu nại phiếu lương (dispute/resolve)** · multi-currency · chu kỳ trả ngoài tháng (biweekly/weekly) · loại lương giờ/khoán · **workflow duyệt NHIỀU CẤP** (v2 vẫn một cấp four-eyes, PAY-DEC-007) · remote/work-trip rule (P2-PAY-04-004) · tệp đính kèm điều chỉnh; *(b) gạt MỚI khi khảo benchmark MISA AMIS* — **Doanh số · KPI · Sản phẩm làm đầu vào tính lương: NGOÀI PHẠM VI SẢN PHẨM, KHÔNG phải hoãn** (de-media-fy đã loại KPI/doanh thu theo kênh — CLAUDE.md §1; **đừng seed lại ở wave sau**) · **Phân bổ lương** (cost center — kế toán chi phí, finance OUT) · **lịch gửi báo cáo định kỳ** (cần scheduler + kênh gửi; v2 chỉ xuất theo yêu cầu) · **Trợ lý AI** · **thư viện mẫu cloud** · «lấy lại dữ liệu» từ AMIS · **đa pháp nhân** (Phase 5 / SaaS) · **mẫu bảng lương theo TỪNG nhân viên / theo vị trí** (v2 chỉ toàn công ty hoặc theo `org_unit` — PAY-DEC-013) · **lương tối thiểu vùng NHIỀU VÙNG (I–IV)** (công ty đơn-vùng, `payroll_statutory_rates` lưu MỘT mức; đa vùng cần cột `region` + bản đồ `org_unit → vùng`) |
...
| **Phase 4 (kéo lên)** | **SOCIAL** — mạng xã hội nội bộ | Wave **`S16-SOCIAL`** (owner duyệt 02/09/2026, SOC-DEC-001..010, [kế hoạch](<../plans/S16-SOCIAL-WAVE.md>)). **✅ đã có bộ tài liệu** (18/09/2026, `S16-SOCIAL-DOC-1`): [SPEC-16](<../SPEC/SPEC-16 SOCIAL.md>) · [DB-17](<../DB/DB-17 SOCIAL Database Design.md>) · [API-19](<../API Design/API-19_SOCIAL_API_Design.md>) · [phân quyền §9h](<../permission-matrix-spec.md>) · [UI-07 §34b](<../UI/UI-07_Module_Workspace_Template_Design.md>) biến thể cổng thông tin 3 cột. **Cấp phát ĐÓNG:** 19 bảng `feed_*` · 12 màn · 53 route · 14 cặp quyền (0 sensitive) · 22 mã lỗi · 9 event `NOTI-EVENT-028..036` · 2 widget. **Chưa build** — thi công `DOC-1 → DB-1 → {A: BE-1→FE-1 ‖ B: DB-2→BE-2→FE-2 ‖ C: BE-3→FE-3} → QA-1 → DASH-1`, cộng `FBPOST-1` song song. ⚠️ **Mã `SOCIAL` hai bề mặt** — app vệ tinh đăng bài Facebook (wave S9) thành **tiện ích con**, resource mới tiền tố `feed-` (SOC-DEC-002, [DECISIONS-08 §6b](<../DECISIONS/DECISIONS-08_Social_Satellite_App.md>)). **PARK-SOCIAL-001** (ngoài v1, chừa thiết kế): đăng lại có trích dẫn · khảo sát nhiều câu (form builder) · story/video ngắn · push mobile · dịch tự động · sự kiện + RSVP · **đăng chéo ra Facebook qua fbpost** · trợ lý AI tổng hợp tin · widget doanh số/KPI (**ngoài phạm vi sản phẩm** sau de-media-fy — đừng seed lại). **PARK-SOCIAL-002**: giới thiệu ứng viên — cần đường `Own` cho nhân viên gửi giới thiệu vào `candidates` nguồn `REFERRAL`; **là việc của RECRUIT**, không của SOCIAL |
| Phase 5 | **MOBILE** · **AI** · **INTEGRATION** | |

> **PAYROLL ĐÃ MỞ và v1 đã ship** — nó vẫn là **crown-jewel** ở mọi dòng code (lương = dữ liệu nhạy cảm nhất). Áp
> `CLAUDE.md` §6 FULL gate + test deny-path TRƯỚC cho **mọi WO của wave v2**, không làm như module CRUD thường.
>
> ⚠️ **Ba chỗ rủi ro nhất của v2** (SPEC-11 §25.1) — không nằm ở khối lượng: **(1)** ranh giới TS↔SQL sau PAY-DEC-012 — số học lên TS (`decimal.js`) nhưng **clamp `net ≥ 0` ở LẠI SQL**; **(2)** `Published` là trạng thái MỚI **nằm GIỮA** hai trạng thái cũ ⇒ sai bất kỳ đường đọc nào cho ra **200 với danh sách RỖNG** (không lỗi, không log); **(3)** máy công thức là **bề mặt tấn công mới nhất trong vùng crown** — grammar ĐÓNG, **không `eval` / `new Function`**, ngân sách node tất định (không dùng timeout đồng hồ làm cổng).
>
...
>
> ✅ **Đã xử lý ở v1** (`S13-PAYROLL-DB-1`, mig `0564`–`0565`): 16 cặp di sản **đã gỡ + thu hồi grant**, `hr-manager` về **0 cặp PAYROLL**. **v2 cộng thêm 17 cặp, TẤT CẢ `is_sensitive = true`** ⇒ mỗi cặp phải khai vào **CẢ HAI** danh sách ở BACKEND (`SENSITIVE_CAPABILITY_ALLOWLIST` **và** `SENSITIVE_SCREEN_GATE_PAIRS`) — khai một bên quên bên kia = **màn ẩn với chính người được cấp quyền**, không lỗi, không log.
