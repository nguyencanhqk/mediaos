...
| Hook | Sự kiện | Làm gì | Kích hoạt từ |
| --- | --- | --- | --- |
| **format-on-write** | PostToolUse `Edit\|Write` | prettier/eslint --fix *trên file vừa đổi* | G1 (có repo) |
| **typecheck-changed** | PostToolUse | `tsc --noEmit` phạm vi gói bị đổi | G1 |
| **anti-bandaid-guard** | PreToolUse `Edit\|Write` | **chặn** `catch {}` rỗng · `@ts-ignore` · `eslint-disable` · `.skip(`/`.only(` trong test · `// TODO fix` mới ở vùng đỏ | G1-7 |
| **test-on-stop** | Stop | chạy `pnpm test` (đổi gì test nấy) + báo cáo đỏ/xanh | G2 |
| **regression-tenant** | Stop / sau diff vùng đỏ | chạy lại test isolation 2-tenant | G2-5 |
...
| Mốc | Bật cái gì |
| --- | --- |
| **G0 (giờ)** | Playbook này + 3 guard hook (đã có). Định tuyến model thủ công. |
| **G1** | CI (lint+type+test) · format-on-write · typecheck-changed · anti-bandaid-guard · nhánh+conventional commit. |
| **G2** | test-on-stop · regression-tenant · FULL gate cho mọi diff RLS/audit/secret. |
| **G3+** | santa-method cho permission · vòng tự-sửa build-fix cho vùng xanh. |
| **G5+** | song song module CRUD (worktree) · review đa chiều song song. |
| **G2/G5/G7** | `ecc:harness-audit` rà toàn bộ harness. |
