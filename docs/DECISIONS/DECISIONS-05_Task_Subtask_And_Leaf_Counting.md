...
- **Lý do — đây là một ranh giới QUYỀN đã ghi thành văn, đã xác minh trên code:**
  ...
  Gọi thẳng `aggregateReportTx` từ widget ⇒ người chỉ có `read:project` nhận đúng con số mà route report cố ý khoá sau cặp SENSITIVE + scope riêng ⇒ **gate sensitive thành trang trí**. Thêm điểm nhọn: `aggregateReportTx` còn tính `assigneeWorkload` kèm `employeeName` (**PII**) — widget sẽ fetch rồi vứt, cách một lần refactor là thành rò thật.
