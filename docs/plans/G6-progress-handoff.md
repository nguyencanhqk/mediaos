...
> Cập nhật lần cuối: 2026-06-09 · Branch: **`feat/g6-media`** · HEAD: **`259586c`** (G6-2: **2a–2h XONG**; **e2e G4-7 fix `259586c` → full-suite XANH 443/2skip → đủ điều kiện merge**)
...
- **2c** `apps/api/src/crypto/` (EnvelopeCipher AES-256-GCM + KmsProvider Local/Vault + SecretEncryptionService, AAD pinned, app-gen UUID).
  ...
  - ✅ **2g XONG (`d8ef592`):** rotation worker, **thuần code, KHÔNG migration**. `reWrapAccount`/`reWrapAll` qua `workerDb` (mediaos_worker) + `assertWorkerRoleSafe` (mirror OutboxWorker, chặn bypass-RLS) → `unwrapDek`→`reWrapDek`→UPDATE **chỉ** `encrypted_dek`/`kms_key_id`/`last_rotated_at` (column-grant); resumable (`kms_key_id IS DISTINCT FROM target`); zeroize DEK; `auth_reset_token` từ chối rotate.
  ...
  - ▶️ **KẾ TIẾP (cập nhật):** G6-2 đã hết micro-step build.
    ...
    - ✅ **(item 5) MERGE feat/g6-media → master: ĐỦ ĐIỀU KIỆN** (e2e G4-7 đã xanh, fix `259586c`). Full QA: `pnpm typecheck` **4/4 XANH**; `pnpm test` = **443 pass / 2 skip / 0 fail** (api 390 · web 48 · contracts 5). 17 fail G4-7 đã hết.
      - ✅ **BLOCKER G4-7 RESOLVED (`259586c` · test-only · 0 dòng production):** ROOT-CAUSE THỰC (đã **bác bỏ** nghi phạm G5 `1cd20c3` ban đầu) = e2e **stale** sau refactor media G6, có **2 nguyên nhân**: (1) `POST /projects` được gate `@RequirePermission('create','project')` ở **G6-3 `e335795`** → user e2e không seed role → **403** (gốc cascade); (2) route `POST /projects/:id/content` **bị xoá** ở **G6-4 `da67d1a`** → đổi thành `POST /content {projectId,title}` → **404**. Migration `0019_g5_permissions_seed.sql` thuần additive (position/employee/company) — KHÔNG đụng project, loại trừ. **Fix:** `seedUserRole(...002 project-manager)` cho cả 2 tenant (role có sẵn create:project+create:content từ 0005) + sửa 2 call content sang `POST /content`. Test đỏ từ G6-3 (commit msg `e335795` đã ghi nhận e2e đỏ), full-suite chưa từng chạy nên không ai thấy.
      - ⚠️ **Scope merge:** branch chứa **G3–G6** (master đang ở G2; `origin/master` tip = feat(G2)). Merge = release cả khối, không chỉ G6-2. **Merge `--no-ff` local, KHÔNG push** (commit doc này nằm trong khối merge; branch 61 ahead master).
