...
- **2c** `apps/api/src/crypto/` (EnvelopeCipher AES-256-GCM + KmsProvider Local/Vault + SecretEncryptionService, AAD pinned, app-gen UUID).
  ...
  - ▶️ **KẾ TIẾP (cập nhật):** G6-2 đã hết micro-step build.
    ...
    - ⏸️ **(item 5) MERGE feat/g6-media → master: DEFER (CHƯA merge).** Full QA pre-merge: `pnpm typecheck` **4/4 XANH**; `pnpm test` = **373 pass / 17 fail / 2 skip** — **toàn bộ 17 fail nằm trong 1 file `test/workflow-lifecycle.e2e-spec.ts` (G4-7)**, các package/file khác (gồm MỌI test G6-2) xanh.
      - 🎫 **BLOCKER ticket (G4-7 e2e — KHÔNG phải G6-2, pre-existing):** root = `POST /projects` trả **403 Forbidden** (expected 201) ở ngay setup → 16 fail còn lại cascade (no projectId → 404/400/`undefined.find`/500). Deterministic cả khi chạy isolation. G6-2 KHÔNG đụng `src/workflow`/`src/projects`/file này; 2e0 fail-closed chỉ kích hoạt cho `isSensitive&&requiresReauth` (reveal) nên không thể gây 403 create-project (permission unit 80/80 xanh). **Nghi phạm: G5 `1cd20c3` (org/positions/employees) đổi seed role/permission** → e2e G4-7 vỡ từ đó nhưng full-suite chưa từng chạy nên không ai thấy. **Cần điều tra seed role create-project của tenant trong e2e + migration permission G5.**
      - ⚠️ **Scope merge:** branch chứa **G3–G6** (master đang ở G2; `origin/master` tip = feat(G2)). Merge = release cả khối, không chỉ G6-2. FF được (master 0 ahead / branch 59 ahead). Khi e2e G4-7 xanh → merge `--no-ff` local, KHÔNG push (chờ user).
