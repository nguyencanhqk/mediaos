...
| ~~N-2~~ **ĐÓNG** (`S6-SEC-PERMVERB-1`) | **Chốt một động từ giữa `read:user` (legacy) và `view:user` (canonical §13)** + backfill cặp quyền | **Đã chốt `view:user`** — ADR [DECISIONS-06 D-41](<../DECISIONS/DECISIONS-06_Permission_Verb_Canonical.md>). **KHÔNG cần migration**: đo PROD 2026-07-29 cho thấy pha EXPAND đã xảy ra từ `0444` — cả 7 user sống giữ cặp danh bạ (`SA` 6 + `company-admin` 1) đều có **CẢ HAI** động từ @Company ⇒ đổi gate là no-op. Vế "backfill 3 role" của nợ này **bị bác bỏ bởi số đo**: `manager` đúng thiết kế §13 (Manager `-`), `hr-manager` là media-era ngoài §13, chỉ `hr` lệch thật và **đã có sẵn** `view:user`. RED→GREEN ở `test/integration/org-directory-permission.int-spec.ts` |
...
| N-4 | **`/system/permissions` là consumer thứ hai của `/org/employees`**, gate theo `assign-role:user \|\| grant-object-permission` chứ không cặp danh bạ; nút "Thử lại" ở đó không bao giờ thắng 403 | Hôm nay 0 gap (2 quyền đó chỉ thuộc SA + company-admin, vốn có **cả** `read:user` lẫn `view:user` — vẫn đúng sau `S6-SEC-PERMVERB-1`), nhưng role tuỳ biến tạo qua chính màn RBAC đó mở lại được |
