...
- [x] Wave 3 — CS-10 ✅ **LANDED master local 2026-06-18** (chưa push). SOLO crown. Merge `4835159` (feat `77359ef`) off base `2590e5a`, **mig 0410 idx 111/when 1717500460000**. Bảng `user_invites` (RLS+FORCE, `token_hash` sha256, `password_hash` argon2, partial-unique `(company,lower(email)) WHERE pending`, audit CHECK +`user_invite`, perms `invite:user`+`approve:user` sensitive → system-admin). Module `user-invites`: invite/accept/approve/reject. **Máy trạng thái:** pending(đã mời+email) → accepted(đặt mật khẩu) → approved(tạo `users` ACTIVE) | rejected. **users row CHỈ sinh ở approve** = cổng-duyệt THẬT (login lọc `deleted_at` KHÔNG lọc `status` → không thể gate bằng cờ; nên KHÔNG đụng login). accept @Public sessionless (resolve company từ slug `resolve_company_by_slug`), email-domain check tái dùng **CS-9 `assertEmailDomainAllowedTx`**, gửi email best-effort tái dùng **CS-8 mail config** (decrypt SMTP password JIT). Token single-use (UPDATE-guard `WHERE pending AND accepted_at IS NULL`), hết hạn 72h, lỗi accept ĐỒNG NHẤT. FE console: 2 tab Chờ duyệt/Yêu cầu kích hoạt + Mời/Duyệt/Từ chối (`InvitesPanel` trong trang Đối tượng). **Verify (worktree cô lập, lane DB `mediaos_cs10`):** chain `0000→0410` sạch; api **2647 pass/0 fail**/5 skip · console **142** · contracts **206** · typecheck 5 pkg sạch. **FULL gate (2 reviewer độc lập):** security = SAFE-TO-LAND; silent-failure/santa = SAFE-AFTER-FIXES → crown property (approve nguyên tử, không orphan user) VERIFIED + vá MEDIUM **hash argon2 NGOÀI tx** (2-tx, giảm DoS @Public) + bổ sung 4 int-test (double-approve/reject/re-invite/expired).
  - **GOTCHA land (shared-worktree hazard):** phiên SONG SONG khác (`feat/projects-pm-app`, PM-1 apps/projects mig 0420) `git checkout -b` GIÁN ĐOẠN cùng working tree → HEAD nhảy sang branch họ, thay đổi CS-10 + PM-1 trộn uncommitted. Xử lý: **worktree CÔ LẬP `mediaos-cs10-land`** off `feat/cs10-user-invites` (base sạch 2590e5a) → replay CHỈ file CS-10 (copy untracked + re-apply hot-file edit trên bản sạch) → verify → commit → merge master. KHÔNG đụng tree PM-1 (HEAD họ giữ nguyên). Worktree mới THIẾU `.secrets/local-kek.bin` (gitignore ko theo `worktree add`) → 13 file false-RED crypto → `cp` KEK là xanh.
  - **DEBT (non-block):** (1) accept hash mật khẩu lưu trên invite giữa accept→approve (argon2, không plaintext); (2) **resend email** khi SMTP fail = reject→mời lại (không có endpoint resend — token single-delivery); (3) trang activation người-được-mời (apps/auth) CHƯA build — email link + accept API là hợp đồng, trang mỏng theo sau; (4) email normalize lệch tầng (citext + lower() index đều phủ, low-risk); (5) page Đối tượng gate `read:employee` để vào → tab invite cần cả `read:employee` (admin có cả hai). ⚠️ **main tree (PM-1) CÒN file CS-10 uncommitted lẫn** — sau khi master có CS-10, phiên PM-1 nên bỏ (checkout/clean) các file CS-10 thừa trong tree họ (đã ở master).
