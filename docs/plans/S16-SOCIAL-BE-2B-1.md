...
| `DB` H-2 · `SFH` L-3 | HIGH | `bumpPollOptionVotes` nhận `pollId` + `AND poll_id = # Plan S16-SOCIAL-BE-2B-1 — BÌNH CHỌN (`SOCIAL-API-040..044` + nhánh `type='poll'` của `002` + system-job đóng poll)
...
. Trước đó vị từ chỉ có `company_id`+`id` nên **không thể** phát hiện vế "thuộc poll" mà chính thông điệp lỗi của nó khẳng định — lưới nằm ở call-site, không ở hàm |
...
| # | Việc | Trạng thái |
| --- | --- | --- |
| **S2** | `vote_count` vào script đối soát + SPEC-16 §13.6 thành 6 cột | ⏳ **Gate làm S2 GẤP HƠN:** `DB` H-2 chỉ ra một lỗ chờ làm `vote_count` lệch DƯƠNG mà **không script nào đối soát VÀ không hàm `recountPollTx` nào tồn tại** để sửa khi đã lệch (`restorePostTx` có đường tính lại cho `feed_posts`, poll thì không) |
| ~~S4~~ | ~~Actor audit của job~~ | ✅ ĐÓNG bằng phép đo U1 (xem §2.3) |
| **S5** | Phân trang OFFSET cho `040` | ⏳ Gate thêm **HAI** dữ kiện: (a) `040` trả **MẢNG TRẦN**, không envelope `{data,page,limit,total}` như `030` (khuôn mà chính plan khai) ⇒ FE-2 **không dựng được pager**, không phân biệt trang cuối với trang rỗng; (b) `FEED_PAGE_MAX=10_000` × `limit≤100` ⇒ `OFFSET 999_900` **không index nào phục vụ**, mở sẵn cho mọi người giữ `view:feed` |
| **O-1** 🔴 | **Audit job: MỘT dòng cho cả lô** (`objectId = closed[0].postId`). Hệ quả: poll thứ 2..N **không tra được** bằng `object_id` của chính nó (chỉ nằm trong `metadata.postIds`, thứ mọi màn lọc audit theo đối tượng không đọc); `044` ghi 1 dòng/poll ⇒ CÙNG một `action` có HAI hình dạng audit. T8 chốt vậy để chống bom rác (tiền lệ `leave-accrual` «~526k dòng/năm») và lý lẽ đó ĐÚNG | ⏳ `SFH` M-5 đề nghị: 1 dòng/poll khi `≤50`, rơi về dòng tổng hợp khi vượt. **Đổi hợp đồng trên bảng append-only ⇒ không tự quyết** |
| **O-2** 🔴 | **Tác giả KHÔNG có `employee_profiles` có nhận NOTI-035 không?** `employeeIdOf` trả `string \| null` ⇒ tài khoản console/tích hợp **tạo được** bài+poll, nhưng INNER JOIN `employee_profiles` ⇒ họ **không bao giờ** nhận thông báo. Docblock cũ chỉ giải thích ca **nghỉ việc**, không ca **chưa bao giờ là nhân viên** | ⏳ Gate đã vá vế IM LẶNG (thêm `logger.warn`). Đổi INNER→LEFT JOIN là đổi **chính sách người nhận** ⇒ chữ ký owner |
| **O-3** | **`optionIds: []` xoá hết phiếu rồi trả 200.** ⚠️ **HAI reviewer KHÔNG đồng ý nhau:** `SFH` M-3 gọi là lỗi câm, đòi 422 dẫn sang `042`. `santa-A` nói **có chủ đích, đã khai tường minh ở `social-api-polls.ts`**, và **xác minh cả 5 route dùng CÙNG cặp `view:feed`** ⇒ KHÔNG lách quyền `042` | ⏳ Orchestrator nghiêng về `santa-A`: giữ hành vi, thiếu sót thật là **không có ca test**. Owner chốt |
...
| **O-5** | **`santa-A` F6 — `C-2` chỉ assert Σ, không assert phân bố từng option.** Lỗi "trừ sai option" (`-1` vào B thay vì A) cho `Σ` đúng, `COUNT(*)` đúng, `CHECK >= 0` không chạm ⇒ **C-2 XANH** mà kết quả bình chọn sai NGƯỜI. Code hiện tại đúng ⇒ đây là lỗ của **LƯỚI** | ⏳ Rẻ: `counters()` trả thêm map `optionId → vote_count` + `COUNT(*) GROUP BY option_id`, assert từng cặp |
| **O-6** | **H-8 (`Promise.all` ba snapshot ở `043`)** — chưa vá. Ba lựa chọn: `repeatable read` cho tx đọc · gộp `totalVoters` thành scalar subquery trong cùng câu · `FOR SHARE` | ⏳ Dữ liệu KHÔNG hỏng, chỉ response tạm thời nói sai (refresh là hết) |
| **O-7** | Nợ `lock_timeout`/`statement_timeout` cho tx của job (`SFH` M-6). Chia lô đã bó cửa sổ về 200 hàng, nhưng chưa có timeout ⇒ chờ khoá vẫn vô hạn. Tiền lệ có sẵn ở `db.service.ts` | ⏳ |
...
- **Rò cử tri: sạch.** `totalVotersTx` trả con SỐ; `myVoteOptionIdsTx` ghim `userId = actor`;
...
- **`@Idempotent()` trên `044` KHÔNG làm điều docblock/D12 nói** (`Sec` F7): decorator chỉ
  `SetMetadata`, không header ⇒ **không khoá server-side nào**. Không phải lỗ bảo mật (scope key đã
  gắn company+user+path+fingerprint) và chống-double-submit thật đến từ `closeManualTx`
  `WHERE status='open'` + `RETURNING` (P-11 đã chứng minh). ⇒ **lời khai sai trong crown-jewel**, cần
  sửa docblock + API-19 §6.6 + D12.
...
### 13.5 🔴 NỢ TEST — chưa đóng, phải giao WO

> Owner chốt 23/09 (chi phí phiên): lượt gate này **chỉ vá CODE, không viết test**. Ghi tường minh ở
> đây để không ai đọc «gate đã chạy» thành «đã có lưới».
