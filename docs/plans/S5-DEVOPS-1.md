...
- **FE = TỰ ĐỘNG qua Cloudflare Pages** — job `deploy` trong `.github/workflows/apps-frontend.yml` (push master, matrix app đổi, environment `production`, project `web-mediaos`/`auth-mediaos`/`console-mediaos` khớp `06-deploy-pages.ps1`). Gate bằng repo var `DEPLOY_FE_ENABLED`. **Provision ĐÃ XONG 2026-07-12** (environment + `CLOUDFLARE_ACCOUNT_ID` + secret `CLOUDFLARE_API_TOKEN` + `DEPLOY_FE_ENABLED=true` + fork-PR approval `all_external_contributors`). Redeploy tay không cần code đổi: `gh workflow run "Apps — Frontend CI" --ref master` (workflow_dispatch, mặc định cả 3 app).
