...
revision: 6 — PASS (5 vòng review đối kháng: tiền đề sai · 6 rủi ro nới FSM · bypass quyền auto-map · cửa thứ hai PATCH · desync lúc sinh POST — xem §Sửa sai)
lanes:
  - id: pipeline-adr
...
acceptanceChecks:
  ...
  - "Migration 0500 bước b: task có project_id + task_status='Done' ⇒ state_id trỏ state nhóm 'completed'; 'In Review' ⇒ nhóm 'started'; 0 task có project_id còn state_id NULL. Đếm TRƯỚC/SAU trên lane DB."
  ...
  - "Kéo thẻ sang cột nhóm 'completed' ⇒ task_status='Done' + completed_at set + 1 TaskStateChanged + 1 TaskStatusChanged + event phát. Kéo sang cột CÙNG nhóm ⇒ chỉ 1 TaskStateChanged, KHÔNG event status rác."
...
testTasks:
  ...
  - "Int: kéo sang cột completed ⇒ task_status Done + completed_at NOT NULL + 1 event TASK_STATUS_CHANGED + audit_logs ĐÚNG 2 dòng (1 TaskStateChanged + 1 TaskStatusChanged, action phân biệt)."
  - "Int: kéo giữa 2 cột CÙNG state_group ⇒ task_status không đổi, chỉ 1 TaskStateChanged, 0 event status (chống rác)."
