...
| --- | --- | --- | --- |
| **W1** | Head migration THẬT lúc rebase cuối (M2 nói con số sẽ trôi) | `git fetch origin && git log --oneline origin/master -1 && tail -20 apps/api/migrations/meta/_journal.json` | **ĐO 23/09/2026 trên `5f8434c0`**: tag head = `0585_s16socialbe1b_noti_news_template_fix`, idx = `252`, when = `1717587374000` ⇒ migration WO này = **`0586`**, journal **`idx: 253`**, **`when: 1717587375000`** (`when` phải TĂNG NGẶT — sai ⇒ áp sai thứ tự / bỏ qua im lặng trên DB dùng chung). **Chỉ đo lại nếu rebase cuối thấy head đổi.**<br>✅ **ĐO LẠI T0 23/09/2026 sau khi #534 MERGE + rebase lên `70869365`: head KHÔNG đổi** — `git diff --name-only HEAD...origin/master -- apps/api/migrations/` RỖNG, `git show origin/master:.../\_journal.json` vẫn dừng ở `idx 252` / `0585` / `when 1717587374000`. ⇒ **`0586` · `idx 253` · `when 1717587375000` GIỮ NGUYÊN**. Đo lại lần nữa ở §9 mục 0 trước khi mở PR (R15). |
| **W2** | PR #534 (BE-2B-1) đã merge chưa, có mang contracts poll/idea/kudos không | `gh pr view 534 --json state,mergedAt` | ~~ĐO 23/09/2026 sáng: `OPEN`, chưa merge~~ → 🔄 **ĐỔI KẾT QUẢ, ĐO LẠI T0 23/09/2026: `MERGED` lúc `2026-09-23T07:27:05Z`** (commit master `70869365`). Nhánh FE-1 **đã rebase** lên nó (`242188b6`). **Nó KHÔNG mang migration** (W1) ⇒ băng `0586` vẫn trống. **Phạm vi FE-1 KHÔNG mở rộng theo** (điều kiện đã ghi sẵn trong chính ô này): composer giữ **đúng 2 nút** (ca **C4** ghim bằng test) — nút poll là việc của `S16-SOCIAL-FE-2` (N3).<br>🔴 **NHƯNG LÝ DO CỦA D2 ĐÃ HẾT HIỆU LỰC MỘT NỬA — phải sửa D2, không được im lặng.** `feedCreatableTypeSchema` (`social-api.ts:88`) **nay là `["share","news","poll"]`**, KHÔNG còn `["share","news"]` như plan viết. Nghĩa là BE **CHẤP NHẬN** `poll`; lập luận «render nút rồi ăn 422 là UI dối» **không còn áp cho poll** (vẫn áp cho `idea`/`kudos` — 2 loại đó vẫn bị `feedCreatableTypeSchema` từ chối). ⇒ giữ 2 nút bây giờ là **quyết định PHẠM VI của owner**, không phải ràng buộc kỹ thuật. Xem **D2 đã sửa**. |
| **W3** | ~~`titleKey` i18n khai ở đâu~~ — 🔴 **PLAN NÓI SAI CHỖ, phát hiện ở T4 23/09/2026** | `pnpm --filter @mediaos/web-core exec vitest run src/lib/registry.spec.ts` | 🔴 **SAI: `routeTitle.*` và `app.*`/`appDesc.*` của entry `ROUTE_REGISTRY` phải khai ở `packages/web-core/src/i18n/locales/vi/nav.ts`, KHÔNG phải ở khối đè `nav:{routeTitle:{…}}` của `apps/app/src/i18n/index.ts`.** Bằng chứng: `registry.spec.ts:16` import `viNav from "../i18n/locales/vi/nav"` và `:1380` assert **mọi** `APP_REGISTRY.nameKey/descKey` dịch được trong CHÍNH bundle đó — ca này **ĐÃ ĐỎ THẬT** khi thêm tile `fbpost` (`thiếu nav.app.fbpost: expected undefined to be truthy`) rồi xanh lại (162/162) sau khi khai đúng chỗ. Khối đè ở `apps/app` là cho `RouteMeta` **CỤC BỘ trong `router.tsx`** (`notiEvents`/`chat`/`taskOverdue`…), không phải cho `ROUTE_REGISTRY` — mọi `payroll*`/`system*` đều nằm ở web-core. Làm theo bản đầu của W3 thì breadcrumb in nguyên chuỗi key và tile `fbpost` làm `registry.spec` ĐỎ.<br>_(giữ lại bản đầu để đối chiếu)_ **3 việc**: ① tạo `apps/app/src/i18n/locales/vi/social.ts` (`export default {…}`) + `import socialVi` và `social: socialVi,` trong `apps/app/src/i18n/index.ts`; ② `nav:routeTitle.<key>` cho 5 route — **deep-merge qua khối `nav: { routeTitle: {…} }` ở CUỐI `i18n/index.ts`** (khuôn đã có sẵn cho `notiEvents`/`chat`); ③ `nav:app.social`/`nav:appDesc.social` đã có sẵn từ S9 (tile dùng) — chỉ đổi văn nếu cần. Chỉ có locale `vi`. |
...
| **W6** | Trần `order` đang dùng trong `ROUTE_REGISTRY` để chọn dải cho SOCIAL không đụng module khác | `grep -n "order:" packages/web-core/src/lib/registry.ts \| tail -20` | **ĐO T0 23/09/2026:** trần `ROUTE_REGISTRY.order` hiện tại = **`91.1`** (PAYROLL `payroll.reports`); FOUNDATION dùng lại dải 70–77 ⇒ `order` KHÔNG duy nhất toàn cục. ⇒ **dải trống chọn cho SOCIAL = `92`–`97`** (6 entry: 92 Bảng tin · 93 Tin tức · 94 Đã lưu · 95 chi tiết bài · 96 trang cá nhân `$employeeId` · 97 `/feed/profiles/me`).<br>🔴 **PHÁT HIỆN THÊM (không có trong plan) — ràng buộc thật nằm ở `APP_REGISTRY`, KHÔNG ở `ROUTE_REGISTRY`:** `registry.spec.ts:1372` (`"thứ tự order tăng dần trong registry"`) assert `APP_REGISTRY.map(a=>a.order)` phải tăng dần **THEO VỊ TRÍ MẢNG**; `ROUTE_REGISTRY` **không** có ca tương đương. ⇒ tile `fbpost` của **D13②** phải đặt **vật lý giữa** `social` (`order: 90`) và `assets` (`order: 100`) với `order` trong khoảng đó (chốt **`90.5`**) — chèn cuối mảng sẽ làm spec này ĐỎ. |
| **W7** | 🔴 **Đường đăng ký tệp cho SOCIAL** — xem R1/§2 D8. Có endpoint `POST /social/files/upload-url` + `/confirm` không? | `grep -rn "upload-url" apps/api/src/social/ apps/api/src/foundation/files/` | **ĐO LẠI T0 23/09/2026 (SAU khi #534 merge): KẾT QUẢ RỖNG — vẫn KHÔNG CÓ.** ⇒ **D8 giữ nguyên**: cắt đính kèm khỏi FE-1, nợ N1 → `S16-SOCIAL-BE-1C`. Không mở lại phạm vi. |
| **W8** | Bộ emoji CHAT export tên gì để composer reaction dùng lại | `grep -rn "CHAT_REACTION_EMOJIS\|chatReactionEmojiSchema" packages/contracts/src/chat.ts \| head` | **ĐO T0 23/09/2026 — KHÔNG phải import từ `chat.ts`.** Contracts **ĐÃ CÓ SẴN** `feedReactionEmojiSchema` ở `packages/contracts/src/social.ts` (WO DB-1), nó **tái dùng** `chatReactionEmojiSchema` = `z.enum(["like","love","haha","wow","sad","angry"])` (`chat.ts:215`). `putFeedReactionSchema` (`social-api.ts:450`) đã dùng nó. ⇒ **FE import `feedReactionEmojiSchema` từ `@mediaos/contracts`**, TUYỆT ĐỐI không chép lại danh sách 6 mã (bộ này đã sống ở 3 chỗ: CHECK `chat_message_reactions_emoji_chk` · hằng drizzle `CHAT_REACTION_EMOJIS` (`communication.ts:427`) · enum contracts — chép là đẻ nguồn thứ tư). ⚠️ `feed_reactions.emoji` **KHÔNG có CHECK ở DB** (DB-17 §6.3) ⇒ không có lưới DB đỡ nếu FE gửi mã lạ, chỉ Zod của BE. |
| **W9** | Số spec/ca hiện tại của `apps/app` để so trước–sau | `pnpm --filter @mediaos/app test --reporter=dot 2>&1 \| tail -20` (đọc dòng `EXIT=`) | **ĐO T0 23/09/2026 trên `242188b6`** (sau khi build `contracts` + `web-core`): **295 file / 3011 ca, tất cả PASS**, `EXIT=0`, 32.99s. Đây là mốc so sánh cuối WO — số file/ca phải TĂNG, và `EXIT` phải vẫn `0`. |
...
| **D2** | **Composer chỉ 2 nút: «Chia sẻ» (`share`) và «Tin tức» (`news`)**. Nút «Tin tức» bọc `<PermissionGate action="manage" resourceType="feed-news">` | 🔄 **LÝ DO SỬA Ở T0 23/09/2026 — nửa lập luận cũ đã chết cùng lúc #534 merge (W2).** Bản plan viết `feedCreatableTypeSchema = z.enum(["share","news"])`; **đo lại: nay là `["share","news","poll"]`** (`social-api.ts:88`). ⇒ chia làm hai vế: **(a) `idea` + `kudos`** — vẫn bị contracts **từ chối**, nên render nút là UI dối ⇒ lập luận cũ còn nguyên hiệu lực; **(b) `poll`** — BE **nhận** rồi, nên bỏ nút poll là **lựa chọn PHẠM VI** (`S16-SOCIAL-FE-2` / N3 giữ trọn cụm bình chọn: nút + form soạn lựa chọn + thẻ bài dạng poll + màn bỏ phiếu), **KHÔNG phải bất khả thi kỹ thuật** — ghi đúng như vậy trong PR, đừng viện lý do sai. Ship nửa cụm poll (nút mở form mà thẻ bài chưa render được kết quả) là nợ tệ hơn không ship. UI-07 §34b.2 ghi `*` = nút Tin tức gate `manage:feed-news` | ① 5 nút disabled — SPEC-16 §14 đòi "forbidden = **ẩn**, không hiện rồi báo lỗi". ② 5 nút + tooltip "sắp có" — vẫn là 3 lời hứa không giữ được. ③ **Thêm nút poll ngay ở FE-1** (mới khả thi từ #534) — loại vì cụm poll cần 4 mảnh FE đi cùng nhau, và `PostCard` của FE-1 chưa có nhánh render `type:"poll"` ⇒ bài tạo ra sẽ hiện như thẻ rỗng |
...
| C27 | **Bài `type:"poll"` lọt vào bảng tin** (R16 — MỚI ở T0) | `PostCard` nhận bài `{type:"poll", body:null}` ⇒ **KHÔNG ném**, vẫn render tác giả + thời gian + thanh reaction + nút bình luận | assert **không** có nút/khối soạn bình chọn nào render (chống scope creep ngược: suy biến an toàn ≠ hiện thực poll); và `body:null` ⇒ khối body **vắng**, không phải chuỗi `"null"`/`"undefined"` |
...
| **R15** | **Số migration trôi** vì PR #534 merge trước | Xung đột số / journal | W1 đo lại lúc rebase cuối; đánh số ở bước cuối cùng, không ở bước đầu. ✅ **Đã xảy ra và đã xử ở T0**: #534 merge lúc 07:27, nhưng **không mang migration** ⇒ số không trôi |
| **R16** | 🔴 **MỚI (T0 23/09/2026) — bài `type:"poll"` CÓ THẬT trong bảng tin dù composer FE-1 không tạo được nó.** #534 đã ship `SOCIAL-API-040..044`; bất kỳ client nào (hoặc FE-2 sau này) tạo bài poll là nó **nằm trong cùng danh sách** mà `FeedPage` của FE-1 render | `PostCard` gặp `type` nó không biết ⇒ hoặc ném, hoặc vẽ thẻ rỗng không có nội dung (`body` được phép **NULL** cho `poll`/`kudos` theo `chk_feed_posts_body_required`) | **KHÔNG bỏ qua bằng cách giả định "sẽ không có".** `PostCard` phải **suy biến an toàn**: `type` lạ ⇒ vẫn vẽ phần chung (tác giả · thời gian · reaction · bình luận · menu ⋯), `body === null` ⇒ **không** render khối body, và **không** ném. Ca **C27** ghim. Đây KHÔNG phải mở phạm vi poll (không nút, không form, không kết quả bỏ phiếu — N3 giữ nguyên) mà là chống vỡ danh sách |
