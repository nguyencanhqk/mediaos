...
**Branch:** `feat/g7-workflow` (main dir `c:/dev 2/MediaOS`) · 1a…3c + ⑥ gate + 4a + **4b `187bb3c`** **đã push (origin đồng bộ)** · **DB dev ở migration 0035** (4b KHÔNG migration). Viên kế = **4c** (eval hook + migration **0036** permissions seed).
...
- **4a + 4b XONG.** Viên kế = **4c** (eval hook emit `step.evaluation_required` khi bước `requires_evaluation` approved; consumer G8 tiêu thụ sau / log dead-letter; migration **0036** permissions seed — PHẢI hyphen `create/update/publish/read:workflow-template` + `apply:workflow-instance`). Eval cols 0035 đang inert tới 4c.
