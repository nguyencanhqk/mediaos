...
**Bảng append-only 🔁:** `audit_logs` · `login_logs` · `attendance_logs` · `leave_balance_transactions` · `employee_status_histories` · `task_activity_logs` · `notification_delivery_logs` · `candidate_stage_events` (mig `0559`, S12-RECRUIT-DB-1) · **`payslips` · `payslip_items` · `payslip_acknowledgements`** (PAYROLL — mig `0564`, S13-PAYROLL-DB-1: hai bảng đầu giữ khuôn di sản, `payslip_acknowledgements` được **REVOKE UPDATE** để về cùng khuôn sổ chỉ-INSERT) — tất cả: app role SELECT+INSERT duy nhất, 0 UPDATE/DELETE.

> ⚠️ **KHÔNG bảng PAYROLL nào có `GRANT DELETE` cho `mediaos_app`** — cả 7 bảng (gồm 4 bảng soft-delete `salary_profiles` · `payroll_periods` · `payroll_period_lines` · `bonus_penalties`) nằm trong `RetentionService.PROTECTED_TABLES`, vì retention phát lệnh DELETE sẽ ăn `42501` **uncaught** và hỏng CẢ LƯỢT cleanup của tenant.
...
- **PAYROLL (Phase 2 — [DB-13](<DB/DB-13 PAYROLL Database Design.md>) · [SPEC-11](<SPEC/SPEC-11 PAYROLL.md>)): tầng DB ĐÃ TỒN TẠI nhưng theo hướng CŨ — wave `S13-PAYROLL` RECONCILE (chưa chạy).** Sáu bảng `salary_profiles` · `payroll_periods` · `payslips` · `payslip_items` · `bonus_penalties` · `payslip_acknowledgements` (`schema/payroll.ts`, mig `0091`–`0132`) có RLS ENABLE+FORCE + policy tenant; `payslips`/`payslip_items` đã đúng khuôn append-only (`GRANT SELECT, INSERT`). **0 route, 0 thư mục `apps/api/src/payroll/`, 0 dòng `app.module.ts`.** Cụm này **RỜI §A5 ngày 31/08/2026** — nó KHÔNG còn là "hướng cũ cần dọn" mà là **nền dữ liệu của module Phase 2 có thiết kế**, xử lý theo bản đồ reconcile [DB-13 §5](<DB/DB-13 PAYROLL Database Design.md>): GỠ **19 cột** (KPI · đa loại lương · đa chu kỳ · đa tiền tệ · chuỗi `adjustment`/`void` · nhánh khiếu nại) + **11 CHECK** chết theo + **3 trigger** ép FSM cũ (`payroll_period_status_guard` · `bonus_penalty_guard` · `payslip_ack_status_guard`); ĐỔI `payroll_periods.status` 3 → **7 giá trị** (SPEC-01 §17.15) và `bonus_penalties.status` sang PascalCase (§17.17); THÊM composite tenant FK cho toàn band G12 (ra đời trước khuôn `0535`) + **CHECK four-eyes** `approved_by <> submitted_by`; THÊM **1 bảng MỚI** `payroll_period_lines` (bảng lương nháp — tách khỏi `payslips` để giữ append-only mà vẫn tính lại được). **Quyền:** 19 cặp quyền lương di sản ở `0005`/`0092`/`0097`/`0099`/`0132`/`0180` — **16 cặp bị GỠ + thu hồi mọi grant**, `hr-manager` về **0 cặp PAYROLL** (Phương án B); hai cặp `approve-payroll-period`/`publish-payroll-period` (`0132`) đang để `is_sensitive=false` nên **kế thừa được qua wildcard** — lỗ được vá bằng cặp mới `is_sensitive=true`. Role hệ thống mới `payroll-officer` (`…0015`, **2FA**, không canonical). **Chưa build:** toàn bộ BE/FE/QA/DASH.
