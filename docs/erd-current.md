...
>
> ⚠️ **Sau PAYROLL v2: 7 + 11 = 18 bảng phải nằm trong `PROTECTED_TABLES`.** Mười một bảng mới ([SPEC-11 §8.2](<SPEC/SPEC-11 PAYROLL.md>) B) — `salary_profile_items` · `payroll_employee_settings` · `payroll_dependents` · `salary_components` · `payroll_templates` · `payroll_template_components` · `payroll_statutory_rates` (7 bảng ở `S15-PAYROLL-DB-1`) · `payroll_advances` · `payroll_payment_batches` · `payroll_payment_lines` · `payroll_budgets` (4 bảng ở `S15-PAYROLL-DB-2`) — **chưa migrate** (mig `0570+`). Việc **thêm chúng vào `PROTECTED_TABLES` thuộc chính WO DB đó, cùng lượt với migration tạo bảng** — một bảng không có `GRANT DELETE` mà đứng ngoài danh sách là một lượt cleanup của tenant chết vì `42501` uncaught.
