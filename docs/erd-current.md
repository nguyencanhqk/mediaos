...
> ⚠️ **Code hiện thực hoá đang LỆCH & LẪN bảng hướng cũ** (media/finance…; *payroll đã RỜI nhóm này 31/08/2026 — xem A4*). Phần đối chiếu code↔thiết kế: **[§Phụ lục A](#phụ-lục-a--trạng-thái-hiện-thực-hoá-code--thiết-kế)**. Liệt kê bảng thật trong DB (gồm parked): xem lịch sử git của file này hoặc đọc thẳng `apps/api/src/db/schema/`.
> **Cập nhật:** 2026-06-26.
...
**Bảng append-only 🔁:** `audit_logs` · `login_logs` · `attendance_logs` · `leave_balance_transactions` · `employee_status_histories` · `task_activity_logs` · `notification_delivery_logs` · `candidate_stage_events` (mig `0559`, S12-RECRUIT-DB-1) · **`payslips` · `payslip_items` · `payslip_acknowledgements`** (PAYROLL — mig `0564`, S13-PAYROLL-DB-1: hai bảng đầu giữ khuôn di sản, `payslip_acknowledgements` được **REVOKE UPDATE** để về cùng khuôn sổ chỉ-INSERT) — tất cả: app role SELECT+INSERT duy nhất, 0 UPDATE/DELETE. ⚠️ **PAYROLL v2 (wave S15 — [SPEC-11 §8.2](<SPEC/SPEC-11 PAYROLL.md>) · DB-13 §12–§14) KHÔNG thêm bảng append-only mới**: 11 bảng mới của v2 đều là bảng nghiệp vụ soft-delete, không phải sổ chỉ-INSERT ⇒ danh sách 🔁 này **giữ nguyên**. Cái đổi theo v2 là cụm `PROTECTED_TABLES` ngay dưới.

> ⚠️ **KHÔNG bảng PAYROLL nào có `GRANT DELETE` cho `mediaos_app`** — **7 bảng của v1** (gồm 4 bảng soft-delete `salary_profiles` · `payroll_periods` · `payroll_period_lines` · `bonus_penalties`) nằm trong `RetentionService.PROTECTED_TABLES` (`apps/api/src/foundation/retention/retention.service.ts:45`), vì retention phát lệnh DELETE sẽ ăn `42501` **uncaught** và hỏng CẢ LƯỢT cleanup của tenant.
>
> ⚠️ **Sau PAYROLL v2: 7 + 11 = 18 bảng phải nằm trong `PROTECTED_TABLES`** — CẢ `payroll_template_components`, dù nó là bảng PAYROLL **duy nhất CÓ `GRANT DELETE`**.
>
> 🔻 **Bài học, ghi lại vì nó suýt thành lỗ (S15-PAYROLL-DB-1, security review 11/09/2026):** bản đầu của WO đó để bảng này NGOÀI danh sách, lập luận rằng «tiêu chí của tập là *thiếu* `GRANT DELETE` ⇒ retention ăn `42501` uncaught, bảng này có DELETE nên không thuộc tiêu chí». Lập luận đó là **phân loại, không phải an toàn** — tiêu chí THẬT ghi ở đầu docblock của chính tập là «bảng `runCleanup` **TUYỆT ĐỐI KHÔNG được xoá**». Đường khai thác cụ thể: `POST /foundation/retention-policies` nhận `entityType` **chuỗi tự do** (contract chỉ ép regex, không có allowlist bảng) · `_deleteEligible` lọc theo **`created_at < cutoff`, KHÔNG theo `deleted_at`** ⇒ nó hard-delete hàng cấu hình **ĐANG SỐNG** · `mediaos_app` CÓ DELETE ⇒ lệnh chạy thật. Hậu quả: mẫu còn nhưng danh sách thành phần RỖNG ⇒ «bảng 0 cột mà không lỗi»; mẫu do tenant tạo + mọi `formula_override`/`is_visible`/`sort_order` **mất vĩnh viễn** (bảng không có soft-delete; `audit_logs` chỉ giữ vết lần SỬA). Thêm vào tập KHÔNG đụng API-053 (route tự phát DELETE, không đi qua retention) ⇒ chi phí bằng 0.
>
> **Đã migrate (mig `0570`, `S15-PAYROLL-DB-1`) — CẢ 7 bảng đã vào `PROTECTED_TABLES`** (+ pin trong `retention.service.spec.ts`): `salary_profile_items` · `payroll_employee_settings` · `payroll_dependents` · `salary_components` · `payroll_templates` · `payroll_statutory_rates` · `payroll_template_components`.
> **Đã migrate (mig `0572`, `S15-PAYROLL-DB-2`, 14/09/2026) — 4 bảng track C đã vào `PROTECTED_TABLES`** (+ pin spec): `payroll_advances` · `payroll_payment_batches` · `payroll_payment_lines` · `payroll_budgets` ⇒ đủ **18**. Cả 4 **0 `GRANT DELETE`**, `mediaos_worker` 0 quyền; ba trigger chốt cuối `payroll_payment_batch_freeze` (kỳ của đợt bất biến · đợt `Completed` terminal) · `payroll_payment_line_guard` (dòng của đợt `Completed` đóng băng kể cả `deleted_at` · cấm INSERT/chuyển sang đợt `Completed` · phiếu cùng user + cùng kỳ với đợt · đọc đợt `FOR SHARE`) · `payroll_advance_freeze_guard` (khuôn `bonus_penalty_freeze_guard` + bind/nhả consume chỉ khi kỳ ∈ `{CollectingData, Calculated}`).
...
**Bảng SỔ RECRUIT (mig `0559`, S12-RECRUIT-DB-1 — DB-14 §6.3/§6.6–6.8):** `candidate_stage_events` (APPEND-ONLY tuyệt đối — xem danh sách 🔁 trên) · `interview_participants` (chỉ `SELECT, INSERT` — đổi người = huỷ lượt + tạo lượt mới) · `interview_feedbacks` (UPDATE chỉ `rating, comment, recommendation, updated_at`; chốt cuối unique per (interview, interviewer)) · `offers` (UPDATE chỉ `title, start_date, salary, note, status, responded_at, updated_at, updated_by`; chốt cuối partial unique 1 offer `Draft/Sent`/ứng viên + CHECK `responded_pair`). Không bảng RECRUIT nào có DELETE cho app role. Verify đúng-bằng qua `aclexplode` trong `0559`; chốt hồi quy `s12-recruit-db1-invariants.int-spec.ts`.
...
`companies` · `users` · `roles` · `permissions` · `user_roles` · `role_permissions` · `user_sessions` · `password_reset_tokens` · `login_logs` · `audit_logs` · `files` · `file_links` · `system_settings` · `company_settings` · `sequence_counters` (→ `sequence_counters`) · `public_holidays` · `modules` · `positions` · `profile_change_requests` (+ `employee_code_configs`) · **ASSET (mig `0549`, 29/08/2026):** `asset_categories` · `assets` · `asset_assignments` · `asset_maintenances` · `asset_inventories` · `asset_inventory_items` (DB-15 §6; `employees` → `employee_profiles` theo A2). · **ROOM (S11-ROOM-DB-1, mig `0552`–`0555`, 29/08/2026 — [DB-16](<DB/DB-16 ROOM Database Design.md>)):** `meeting_rooms` (tái dụng + ALTER: +`equipment`/`description`/`requires_approval`/`is_active`/`sort_order`/`updated_*`/`deleted_by`, `capacity` NOT NULL CHECK > 0, unique `lower(name)` partial, gỡ `is_virtual`) · `room_bookings` · `room_booking_attendees` — `schema/rooms.ts`; 10 composite tenant FK; role hệ thống `office-admin` (…0013, không canonical) + 5 cặp `room`/`room-booking` + 22 grant §9e; NOTI `ROOM_BOOKING_CONFIRMED/CANCELLED/REMINDER`; audit `room_booking`. `modules.ROOM` GIỮ `is_active=false` tới `S11-ROOM-FE-1`. 4 bảng di sản `meetings`/`meeting_attendees`/`meeting_notes`/`meeting_tasks` **đã DROP** (0553, 0 hàng) + 6 cặp quyền `meeting*` xoá cứng. · **RECRUIT (S12-RECRUIT-DB-1, mig `0559`–`0561`, 31/08/2026 — [DB-14](<DB/DB-14 RECRUIT Database Design.md>)):** `job_openings` · `candidates` · `candidate_stage_events` (append-only) · `candidate_notes` · `interviews` · `interview_participants` · `interview_feedbacks` · `offers` — `schema/recruit.ts`; 27 composite tenant FK; role hệ thống `recruiter` (…0014, không canonical) + 16 cặp §9f (7 sensitive) + 42 grant; NOTI 4 event `RECRUIT_*` DedupeKey; audit 4 object_type. `modules.RECRUIT` GIỮ `is_active=false` tới `S12-RECRUIT-FE-1`. · **PAYROLL v1 (S13-PAYROLL-DB-1, mig `0564`–`0568` — [DB-13](<DB/DB-13 PAYROLL Database Design.md>) · [SPEC-11](<SPEC/SPEC-11 PAYROLL.md>)):** 6 bảng di sản G12 **đã reconcile** `salary_profiles` · `payroll_periods` · `payslips` · `payslip_items` · `bonus_penalties` · `payslip_acknowledgements` + 1 bảng **mới** `payroll_period_lines` — `schema/payroll.ts`; composite tenant FK phủ toàn band G12 (`0564`), 3 trigger FSM cũ DROP, `payroll_periods.status` 3 → **7 giá trị** + CHECK four-eyes; role hệ thống `payroll-officer` (…0015, **2FA**, không canonical) + 17 cặp (13 sensitive) + **thu hồi 16 cặp di sản**; NOTI-EVENT-020..023 (`0566`); widget `PAYROLL_COST` (`0568`); `modules.PAYROLL` **đã bật** (`0567`, `S13-PAYROLL-FE-1`). 🔁 **PAYROLL v2 lô DB-1 ĐÃ migrate** (mig `0570`–`0571`, `S15-PAYROLL-DB-1`, 11/09/2026): ALTER `salary_profiles` (+5 cột §12.1 — `salary_type` **dựng lại** theo PAY-DEC-015 với nghĩa GROSS/NET, CHECK mang tiền tố bảng để không lẫn với `employee_profiles.salary_type`) · ALTER `payroll_period_lines` (+3 cột §12.4) · **7 bảng mới** `salary_profile_items` · `payroll_employee_settings` · `payroll_dependents` · `salary_components` · `payroll_templates` · `payroll_template_components` · `payroll_statutory_rates` (26 composite tenant FK · `btree_gist` + `EXCLUDE` chống chồng NPT · `payroll_template_components` là bảng PAYROLL **duy nhất có `GRANT DELETE`**) · backfill EXPAND `allowances` → `salary_profile_items` (mã `PC_nnn`, cột jsonb **GIỮ**) · **17 cặp quyền v2 + 31 grant ⇒ 63** · `audit_logs.object_type` **+10 ⇒ 14**. Catalog thành phần lương + tỉ lệ luật định + mẫu mặc định seed **RUNTIME per-company** (`PayrollMasterDataSeeder`) — KHÔNG trong migration (mig `0445` cấm seed company-scoped ở migrate-time). ⚠️ **Lô DB-2 (`payroll_periods` §12.3 + 4 bảng track C) CHƯA migrate** — xem §A4.
...
- **RECRUIT (Phase 2 — [DB-14](<DB/DB-14 RECRUIT Database Design.md>) · [SPEC-12](<SPEC/SPEC-12 RECRUIT.md>)): tầng DB ĐÃ BUILD 31/08/2026 (`S12-RECRUIT-DB-1`, mig `0559`–`0561`, `schema/recruit.ts`).** 8 bảng `job_openings` · `candidates` · `candidate_stage_events` (append-only §9) · `candidate_notes` · `interviews` · `interview_participants` · `interview_feedbacks` · `offers` — RLS+FORCE, 27 composite tenant FK, role hệ thống `recruiter` (…0014, không canonical) + 16 cặp (7 cặp `candidate` `is_sensitive=true`) + 42 grant §9f; NOTI `RECRUIT_JOB_ASSIGNED/INTERVIEW_SCHEDULED/STAGE_CHANGED/CANDIDATE_HIRED` (DedupeKey); audit `job_opening`/`candidate`/`interview`/`offer`. Hàng `modules.RECRUIT` GIỮ `is_active=false` tới `S12-RECRUIT-FE-1`. **Tầng BE ĐÃ BUILD 31/08/2026 (`S12-RECRUIT-BE-1`, `apps/api/src/recruit/` — 32 route API-17 qua 5 controller, guard 2 tầng theo bảng hằng `recruit-route-pairs.const.ts`, masking PII/salary ở `recruit.mapper.ts`, convert 3 pha tái dùng `HrWriteService.allocateEmployeeCode` + `createEmployeeFromCandidateTx` mới, điểm chiếu danh tính duy nhất `recruit-people.repository.ts`, NOTI registrar 4 event ở `notifications/`).** FE/QA chưa build; chốt hồi quy `s12-recruit-db1-invariants.int-spec.ts` + `recruit-be1-*.int-spec.ts` + `recruit-two-layer-guard-census.unit-spec.ts`.

- **PAYROLL — v1 ĐÃ BUILD đủ (DB + BE + FE + DASH), v2 MỚI CÓ THIẾT KẾ ([DB-13](<DB/DB-13 PAYROLL Database Design.md>) · [SPEC-11](<SPEC/SPEC-11 PAYROLL.md>)).** **v1 (wave `S13-PAYROLL`, mig `0564`–`0568`):** 7 bảng (6 bảng di sản G12 đã reconcile theo DB-13 §5 + `payroll_period_lines` mới) — đã chuyển sang §A1; **35 route** `PAYROLL-API-001..035` ở `apps/api/src/payroll/` (`S13-PAYROLL-BE-1`/`BE-2`), **6 màn** `PAY-SCREEN-001..006` ở `apps/app/src/routes/payroll/` (`S13-PAYROLL-FE-1`), widget `PAYROLL_COST` (`S13-PAYROLL-DASH-1`). Cụm này **RỜI §A5 ngày 31/08/2026** — nó KHÔNG còn là «hướng cũ cần dọn» mà là nền dữ liệu thật của một module Phase 2 đang chạy. **v2 (wave `S15-PAYROLL-V2`, owner ký PAY-DEC-011..020 ngày 02/09/2026) — THIẾT KẾ ĐÃ CÓ, CODE CHƯA CÓ:** [SPEC-11 v2](<SPEC/SPEC-11 PAYROLL.md>) §5.1b/§8.2 + DB-13 §12–§14 chốt **ALTER 3 bảng, chia cho HAI WO** (**DB-1 lấy 2**: `salary_profiles` +`salary_type`/`pit_payer`/`insurance_salary`/`probation_salary`/`pay_ratio_pct` §12.1 · `payroll_period_lines` +`component_values_json`/`template_fingerprint`/`gross_up_iterations` §12.4 — **DB-2 lấy 1**: `payroll_periods` +`template_id`/`paid_by`/`paid_at` §12.3, **cùng chuỗi 4 bước nguyên tử** nới `status_check` → backfill `Paid → Published` → siết hai CHECK cặp) và **11 bảng mới** (`salary_profile_items` · `payroll_employee_settings` · `payroll_dependents` · `salary_components` · `payroll_templates` · `payroll_template_components` · `payroll_statutory_rates` · `payroll_advances` · `payroll_payment_batches` · `payroll_payment_lines` · `payroll_budgets`), cộng **50 route** `036..085` · **11 màn** `PAY-SCREEN-007..017` · **17 cặp quyền mới (tất cả sensitive)** · **16 mã lỗi** `ERR-018..033` · **4 event** `NOTI-EVENT-024..027` · **2 widget** `PAYROLL_BUDGET`/`PAYROLL_ADVANCE_PENDING` · `payroll_periods.status` **7 → 8 giá trị** (thêm `Published` **nằm GIỮA** hai trạng thái cũ, kèm backfill `Paid → Published`). **Chưa migrate** — mig **`0570+`** (`0569` đã bị `S14-RECRUIT-FILEGRANT-1` lấy; WO DB vẫn phải đọc `apps/api/migrations/meta/_journal.json` **lúc chạy** và lấy `idx = max + 1`). 11 bảng mới phải vào `RetentionService.PROTECTED_TABLES` cùng lượt migration — xem §9. **Chưa build:** toàn bộ DB-1/DB-2 · BE-1..5 · FE-1..4 · QA-1 · DASH-1 · vỏ UI chung `S15-UI-SHELL-1`.

- **SOCIAL (Phase 4 kéo lên — [DB-17](<DB/DB-17 SOCIAL Database Design.md>) · [SPEC-16](<SPEC/SPEC-16 SOCIAL.md>)): MỚI CÓ THIẾT KẾ, CODE CHƯA CÓ (wave `S16-SOCIAL`, owner duyệt 02/09/2026 SOC-DEC-001..010; bộ tài liệu viết 18/09/2026 ở `S16-SOCIAL-DOC-1`).** **19 bảng** `feed_*` chia hai WO migration nối tiếp: **`S16-SOCIAL-DB-1` (Track A, 10 bảng)** `feed_posts` · `feed_comments` · `feed_reactions` · `feed_mentions` · `feed_tags` · `feed_post_tags` · `feed_saved_posts` · `feed_post_views` (append-only) · `feed_post_acks` (append-only) · `feed_reports`; **`S16-SOCIAL-DB-2` (Track B, 9 bảng)** `feed_groups` · `feed_group_members` (vai trò là HÀNG `owner`/`admin`/`member`) · `feed_polls` · `feed_poll_options` · `feed_poll_votes` · `feed_ideas` · `feed_kudos` · `feed_kudos_recipients` · `feed_kudos_badges`. RLS+FORCE trước dữ liệu, composite tenant-FK, **14 cặp quyền** tiền tố `feed-` (0 sensitive) + **43 grant** §9h, **9 event** `NOTI-EVENT-028..036`, audit `object_type` UNION-ADD `feed_post`/`feed_comment`/`feed_group`/`feed_report`, `file_links.object_type` UNION-ADD `feed_post`/`feed_comment`. Migration **chưa chạy** — dự kiến `0577+` (head lúc viết = idx 243 / `0576`), **đo `_journal.json` lúc chạy**; lane migration S15 và S16 **không** chạy cùng ngày. Hàng `modules.SOCIAL` GIỮ `is_active=false` tới `S16-SOCIAL-FE-1`.

  > ⚠️ **Mã `SOCIAL` có hai bề mặt — đừng đọc nhầm khi tra code.** App vệ tinh **đăng bài Facebook** (wave S9, `apps/fbpost` + `apps/api/src/integrations/social/`, SQLite riêng, dịch vụ cổng 3500 LIVE) dùng cùng mã module và **giữ nguyên** ba cặp `social-post`/`social-account`. Mạng xã hội nội bộ là module **mới** ở `apps/api/src/social/` với resource tiền tố **`feed-`** — lý do kỹ thuật: mig `0544` verify đếm grant `social*` của `employee` và sẽ RAISE nếu bị chạm (SOC-DEC-002, [DECISIONS-08 §6b](<DECISIONS/DECISIONS-08_Social_Satellite_App.md>)).

  **Chưa build:** toàn bộ DB-1/DB-2 · BE-1..3 · FE-1..3 · QA-1 · DASH-1 · FBPOST-1. 19 bảng mới phải vào `RetentionService.PROTECTED_TABLES` cùng lượt migration.
...
- ~~**payroll** (`payroll.ts`)~~ — **RỜI nhóm này 31/08/2026**: sáu bảng payroll là **nền dữ liệu của module PAYROLL Phase 2** ([SPEC-11](<SPEC/SPEC-11 PAYROLL.md>) · [DB-13](<DB/DB-13 PAYROLL Database Design.md>) · [API-18](<API Design/API-18_PAYROLL_API_Design.md>), owner duyệt 31/08/2026), xử lý bằng **RECONCILE** ở wave `S13-PAYROLL` — xem A4. Không còn là "di sản chưa quyết".
