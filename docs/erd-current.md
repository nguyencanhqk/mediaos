...
- **kpi/evaluation/meeting/workflow** (`kpi.ts`/`evaluation.ts`/`meeting.ts` · `workflow.ts`/`approval.ts` engine) + **break-glass** (`break-glass.ts`).
  - **⟲ 27/08/2026 — `S10-CLEAN-WORKFLOWPARK-1` đã dọn NỬA CODE của cụm workflow, BẢNG GIỮ NGUYÊN.** Đã GỠ: `WorkflowController` (13 route `/workflow/*`) · `WorkflowTemplatesController` (16 route `/workflow-templates/*`) · `workflow.service.ts` · cả stack `workflow-templates.*` · `dag-validator` · `dag-result.adapter` ⇒ **bề mặt HTTP của cụm = 0** (ca đo: `apps/api/test/foundation/workflow-surface-removed.unit-spec.ts`). CÒN LẠI có chủ đích: `approval.service.ts` · `workflow-fsm` · `workflow.repository.ts` · `lock-propagation` · `workflow-dag`, vì `apps/api/src/approval/` (ĐANG MOUNT) uỷ quyền cấp duyệt CUỐI cho `ApprovalService`. **Bảng `workflow_*`/`approval_*`/`defects` KHÔNG bị đụng** — DROP bảng là lane `db-migration` + FULL gate, và đi kèm nó là quyết định gỡ luôn `approval/` (xem **KI-082**).
