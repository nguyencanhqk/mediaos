...
- **ROOM (Phase 3, thiết kế 29/08/2026 — [DB-16](<DB/DB-16 ROOM Database Design.md>) · [SPEC-14](<SPEC/SPEC-14 ROOM.md>)):** **tái dụng** `meeting_rooms` (mig `0052`, `meeting.ts` — ALTER thêm thiết bị/`requires_approval`/`is_active`, gỡ `is_virtual`) + 2 bảng **chưa build** `room_bookings` · `room_booking_attendees` (EXCLUDE gist chống trùng lịch) — thi công ở `S11-ROOM-DB-1` (migration `0552+` dự kiến, **sau** ASSET). Cùng WO **DROP** 4 bảng di sản `meetings` · `meeting_attendees` · `meeting_notes` · `meeting_tasks` (đo 29/08/2026: 0 hàng cả 5 bảng, 0 code) — `meeting.ts` → `rooms.ts`. Khi build: `room_bookings` vào danh sách **không DELETE / UPDATE cấp cột** ở §9; `room_booking_attendees` chỉ INSERT; organizer/attendees trỏ `users` bằng composite tenant FK (đã có `UNIQUE (company_id, id)` từ `0535`).
