...
>
> 🔁 **Sau PAYROLL v2: 7 + 10 = 17 bảng trong `PROTECTED_TABLES`, KHÔNG phải 18** (đính chính 11/09/2026, `S15-PAYROLL-DB-1`). Con số 18 giả định cả 11 bảng mới đều thiếu `GRANT DELETE`; đo lại thì **`payroll_template_components` CÓ `DELETE`** — ngoại lệ có chủ đích của DB-13 §13.6 (`PUT /payroll/templates/:id/components` đặt lại cả danh sách trong một tx; soft-delete sẽ tích luỹ hàng chết vô hạn trên một bảng cấu hình thuần). Tiêu chí của `PROTECTED_TABLES` là «**không** có `GRANT DELETE` ⇒ retention ăn `42501` uncaught», nên bảng đó **không thuộc** tập này; nhét vào là làm hỏng chính tiêu chí đang giữ tập đó đọc được.
>
> **Đã migrate (mig `0570`, `S15-PAYROLL-DB-1`) — 6 bảng vào `PROTECTED_TABLES`:** `salary_profile_items` · `payroll_employee_settings` · `payroll_dependents` · `salary_components` · `payroll_templates` · `payroll_statutory_rates`. Bảng thứ 7 `payroll_template_components` **cố ý vắng** (lý do ghi thành comment ngay tại chỗ trong `retention.service.ts`).
