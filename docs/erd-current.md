...
**Bảng append-only 🔁:** `audit_logs` · `login_logs` · `attendance_logs` · `leave_balance_transactions` · `employee_status_histories` · `task_activity_logs` · `notification_delivery_logs` · **`candidate_stage_events`** (mig `0559`, S12-RECRUIT-DB-1 — app role `SELECT, INSERT` DUY NHẤT: 0 UPDATE, 0 DELETE ở CẢ ACL cấp bảng lẫn cấp cột; `acted_by` → `users` là composite FK **`NO ACTION`** để RI action không ghi đè sổ).
...
**Bảng RECRUIT không DELETE / UPDATE cấp cột (mig `0559`, S12-RECRUIT-DB-1 — DB-14 §6):** `candidate_stage_events` (append-only tuyệt đối — xem trên) · `interview_participants` (chỉ `SELECT, INSERT`: đổi người = huỷ lượt + tạo lượt mới) · `interview_feedbacks` (UPDATE chỉ `rating, comment, recommendation, updated_at`) · `offers` (UPDATE chỉ `title, start_date, salary, note, status, responded_at, updated_at, updated_by`; `chk_offers_responded_pair` buộc vào terminal là MỘT câu UPDATE đặt `status` + `responded_at` cùng lúc). 4 bảng còn lại (`job_openings` · `candidates` · `candidate_notes` · `interviews`) `SELECT, INSERT, UPDATE` — **0 DELETE trên cả 8 bảng**; `candidates` GRANT UPDATE cấp bảng là chấp nhận CÓ CHỦ ĐÍCH (DB-14 §6.2: `stage`/`employee_id` phải ghi được qua chính app role). Verify đúng-bằng qua `aclexplode` trong `0559`; chốt hồi quy `s12-recruit-db1-invariants.int-spec.ts`.
...
- **RECRUIT (Phase 2 — [DB-14](<DB/DB-14 RECRUIT Database Design.md>) · [SPEC-12](<SPEC/SPEC-12 RECRUIT.md>)): NỀN DỮ LIỆU ĐÃ BUILD 31/08/2026 (`S12-RECRUIT-DB-1`, mig `0559` DDL · `0560` seed role/quyền/audit · `0561` NOTI; `schema/recruit.ts`) — xem A1.** 8 bảng mới `job_openings` · `candidates` · `candidate_stage_events` (append-only) · `candidate_notes` · `interviews` · `interview_participants` · `interview_feedbacks` · `offers` — đã build đúng DB-14 §6 (mig `0559`). Nền sạch: 0 bảng di sản tuyển dụng (chỉ 2 giá trị enum trong `finance.ts`/`media.ts` đã park — A5); hàng `modules.RECRUIT` pre-seed inactive từ `0435`, GIỮ `false` tới `S12-RECRUIT-FE-1` (guard `0560` forward-compatible: chỉ kiểm TỒN TẠI, KHÔNG assert cờ). `candidate_stage_events` đã vào danh sách bảng append-only §9. **Chưa build:** module NestJS (`S12-RECRUIT-BE-1`), màn hình (`S12-RECRUIT-FE-1`), widget «phễu tuyển dụng» (`S12-RECRUIT-DASH-1` — cố ý tách khỏi WO DB, DB-14 §9).
