...
| NOTI-EVENT-020   | PAYROLL_PERIOD_SUBMITTED | Bảng lương kỳ được gửi duyệt | **Người duyệt hợp lệ** theo `PayrollApproverReader` — CÙNG bộ giải với `PAYROLL-ERR-017` (SPEC-11 §13.1/§17), **KHÔNG** tra role `company-admin` riêng (JOIN tầng ROLE bắt được cả **role tuỳ biến** giữ cặp `approve`; hai bộ giải lệch nhau đẻ đúng thất bại mà 017 sinh ra để chặn — xem §15.10); trừ actor |
...
| NOTI-EVENT-024   | PAYROLL_ADVANCE_SUBMITTED | Tạm ứng lương được gửi duyệt | Người duyệt hợp lệ của tạm ứng — giải theo cặp `('approve','payroll-advance')`, cùng KHUÔN `PayrollApproverReader` (SPEC-11 §17.1), trừ actor |
| NOTI-EVENT-025   | PAYROLL_ADVANCE_APPROVED  | Tạm ứng lương được duyệt      | Nhân sự thụ hưởng (`payroll_advances.user_id`) + người tạo, trừ actor |
| NOTI-EVENT-026   | PAYROLL_ADVANCE_REJECTED  | Tạm ứng lương bị từ chối      | Nhân sự thụ hưởng + người tạo, trừ actor — kèm lý do |
| NOTI-EVENT-027   | PAYROLL_PAYMENT_BATCH_COMPLETED | Đợt chi trả lương đã hoàn tất (kỳ → `Paid`) | Người giữ `('view','payment-batch')` trong company, trừ actor |

> **Dải mở rộng hậu-MVP:** 010–012 cấp cho **ASSET** (SPEC-13 §17, 28/08/2026), 013–015 cấp cho **ROOM** (SPEC-14 §17, 29/08/2026) — wave S11-OFFICE; **016–019 cấp cho RECRUIT** (SPEC-12 §17, 31/08/2026 — wave S12-RECRUIT); **020–023 cấp cho PAYROLL** (SPEC-11 §17, 31/08/2026 — wave S13-PAYROLL); **024–027 cấp cho PAYROLL v2** (SPEC-11 §17.1, 11/09/2026 — wave **S15-PAYROLL-V2**). GOAL/LMS/CHAT trước đó chỉ là event mở rộng (§15.1–15.6 kiểu), **không** chiếm mã chuẩn. Module kế lấy **028+** sau khi đo lại.
...
### 15.10 PAYROLL events *(Phase 2 — wave S13-PAYROLL, SPEC-11 §17, seed ở S13-PAYROLL-DB-1; **+ wave S15-PAYROLL-V2, SPEC-11 §17.1, seed ở S15-PAYROLL-DB-2**)*

| Mã event                  | Sự kiện                          | Người nhận                                                     | Nội dung gợi ý                                             |
| ------------------------- | -------------------------------- | -------------------------------------------------------------- | ---------------------------------------------------------- |
| PAYROLL_PERIOD_SUBMITTED  | Bảng lương kỳ gửi duyệt          | **Người duyệt hợp lệ** giải bằng `PayrollApproverReader` — **CÙNG bộ giải với cổng `PAYROLL-ERR-017`** (SPEC-11 §13.1/§17): JOIN ở tầng ROLE (`user_roles ⋈ roles ⋈ role_permissions ⋈ permissions` trên cặp `('approve','payroll-period')`, `effect='ALLOW'`, còn hiệu lực), `recipient.mode='UserIds'`, trừ actor. **KHÔNG tra role `company-admin` riêng** — JOIN tầng ROLE bắt được **cả role tuỳ biến** được cấp cặp `approve`, còn hai bộ giải lệch nhau đẻ **đúng** thất bại mà 017 sinh ra để chặn (017 cho qua nhưng không ai nhận thông báo). Hệ quả vẫn giữ: **KHÔNG gửi `payroll-officer`** — role đó không có cặp `('approve','payroll-period')` (four-eyes), nhận link là đâm vào 403 | Bảng lương kỳ {period_month} đang chờ bạn duyệt             |
...
| **PAYROLL_ADVANCE_SUBMITTED** *(v2)* | Tạm ứng lương được tạo (`Pending`, commit) | Người duyệt hợp lệ của tạm ứng — giải theo cặp `('approve','payroll-advance')`, **cùng KHUÔN `PayrollApproverReader`** (JOIN ở tầng ROLE, `recipient.mode='UserIds'`), trừ actor | Có đơn tạm ứng lương đang chờ bạn duyệt |
| **PAYROLL_ADVANCE_APPROVED** *(v2)* | Tạm ứng lương được duyệt | Nhân sự thụ hưởng (`payroll_advances.user_id`) **+ người tạo**, trừ actor | Đơn tạm ứng lương của bạn đã được duyệt, khấu trừ vào kỳ {deduct_period_month} |
| **PAYROLL_ADVANCE_REJECTED** *(v2)* | Tạm ứng lương bị từ chối | Như trên, trừ actor | Đơn tạm ứng lương của bạn bị từ chối: {note} |
| **PAYROLL_PAYMENT_BATCH_COMPLETED** *(v2)* | Đợt chi trả hoàn tất (kỳ → `Paid`, commit) | Người giữ `('view','payment-batch')` trong company, trừ actor | Đợt chi trả lương kỳ {period_month} đã hoàn tất |

* `module_code = 'PAYROLL'`, `notification_type = 'Payroll'` — CHECK của **cả hai** bảng `notification_events` và `notifications` **ĐÃ được mig `0566` nới** ở wave S13 (DB-13 §10 bước C). ⇒ WO DB của v2 (`S15-PAYROLL-DB-2`) **ĐO rồi NO-OP có chủ đích** kèm `RAISE NOTICE`, **KHÔNG viết ALTER rỗng** để "cho chắc" (SPEC-11 §17.1).
* `priority`: **020 Normal · 021 Normal · 022 High · 023 High · 024 Normal · 025/026 High** (ảnh hưởng trực tiếp tiền của một cá nhân) **· 027 Normal**. `isEnabled=true`, **`isSystemEvent=false` cả 8** — **PAYROLL v1 và v2 đều KHÔNG có system job** (mọi event đều event-driven).
* ⚠️ **Payload TUYỆT ĐỐI KHÔNG chứa số tiền — kể cả số tạm ứng của chính người nhận.** 020/021/022/023 chỉ `periodMonth` + lý do từ chối (022) + deep-link (`/payroll/periods/{id}` cho 020/021/022, `/me/payslips` cho 023); 024/025/026 chỉ `{ advanceId, deductPeriodMonth, reason?/note?, link }` với deep-link **`/payroll/advances/{id}`** (người duyệt) và **`/me/payroll-advances`** (người thụ hưởng); 027 chỉ `periodMonth` + `batchId`. 025/026 gửi tới chính nhân sự thụ hưởng nên số tiền *của họ* nghe có vẻ vô hại, nhưng luật là ĐÓNG: NOTI đi qua nhiều kênh (email/push) và **không có tầng masking riêng** (SPEC-11 §17/§17.1/§18). Nhân viên bấm vào xem số ở `/me/payroll-advances`.
* `dedupeKey` **CONTENT-DERIVED — KHÔNG dùng `auditLogId`** *(đính chính 01/09/2026, SPEC-11 §17: `AuditService.record` trả **`void`**, không có id để ghép — bản trước của mục này và comment trong mig `0566` viết `{auditLogId}` khi chưa đo tầng audit)*. Nửa sau của khoá lấy từ **`RETURNING` của chính câu UPDATE đổi trạng thái**, nên tính chất «theo LẦN thao tác, không theo cặp kỳ+người» vẫn giữ nguyên. **Phần do PRODUCER sinh** (engine tự ghép tiền tố `${eventCode}:` qua `NotificationDedupeService.computeKey` — registrar **không** tự thêm `eventCode`, kẻo khoá mang tiền tố đôi):
  * `PAYROLL_PERIOD_SUBMITTED` → `{periodId}:{submittedAtIso}` · `PAYROLL_PERIOD_APPROVED` → `{periodId}:{approvedAtIso}` · `PAYROLL_PERIOD_REJECTED` → `{periodId}:{updatedAtIso}` *(`reject` KHÔNG có cột `rejected_at`)* · `PAYSLIP_PUBLISHED` → `{payslipId}`.
  * **v2:** `PAYROLL_ADVANCE_SUBMITTED` → `{advanceId}:{createdAtIso}` · `PAYROLL_ADVANCE_APPROVED` → `{advanceId}:{decidedAtIso}` · `PAYROLL_ADVANCE_REJECTED` → `{advanceId}:{decidedAtIso}` · `PAYROLL_PAYMENT_BATCH_COMPLETED` → `{batchId}` *(một đợt báo đúng một lần)*.
  * Khoá theo `{periodId}`/`{advanceId}` **trần** là "once-ever" ⇒ chu trình reject → sửa → gửi lại **không bao giờ báo lần hai** (engine `DedupeKey` không có bucket thời gian). Catalog seed **`dedupe_strategy = 'DedupeKey'` ngay seed đầu** cho cả 8 — mặc định `'None'` biến `dedupeKey` thành chuỗi trang trí (bài học `0479`/`0507`/`0538`).
* 🔁 **`PAYSLIP_PUBLISHED` (023) ĐỔI ĐIỀU KIỆN PHÁT ở v2 — mã, dedupe key và người nhận KHÔNG đổi:** v1 phát tại chuyển tiếp `Approved → Paid`; **v2 phát tại `Approved → Published`** (`publish`, `PAYROLL-API-014`) vì PAY-DEC-017 đã tách `Published` khỏi `Paid` (SPEC-01 §17.15). Bám theo tên trạng thái mà dời sang `complete-batch` ⇒ nhân viên **chỉ được báo sau khi ngân hàng xong** — trái đúng ý PAY-DEC-017. Đây là **thay đổi hành vi im lặng nhất của wave**; QA có ca ghim: `publish` ⇒ **có** outbox 023, `complete-batch` ⇒ **không** có 023 (SPEC-11 §17.1).
* Người nhận đi **THEO PAYLOAD outbox**, resolve **một lần** lúc ghi, không resolve lại lúc giao — hai bộ giải lệch nhau đẻ đúng thất bại mà cổng sinh ra để chặn (SPEC-11 §13.1/§17).
* Phát qua OutboxNotificationBridge; `registerSource()` fail-loud lúc boot nếu catalog chưa có đủ **8** mã này ⇒ seed phải đi trước WO backend (`S13-PAYROLL-BE-2` cho 020–023, `S15-PAYROLL-BE-4` cho 024–027).
...
1. SPEC-09: Trung tâm cá nhân & Cài đặt tài khoản (ME).
2. SPEC-10: Tiền lương.
