...
> [SPEC-01 Tổng quan](<SPEC-01 Tổng quan.md>) · [SPEC-02 AUTH](<SPEC-02 AUTH.md>) · [SPEC-03 HR](<SPEC-03 HR.md>) · [SPEC-04 ATT](<SPEC-04 ATT.md>) · [SPEC-05 LEAVE](<SPEC-05 LEAVE.md>) · **SPEC-06 TASK** · [SPEC-07 DASH](<SPEC-07 DASH.md>) · [SPEC-08 NOTI](<SPEC-08 NOTI.md>) · [SPEC-09 ME](<SPEC-09 ME.md>) · [SPEC-10 GOAL](<SPEC-10 GOAL.md>) · [SPEC-11 PAYROLL](<SPEC-11 PAYROLL.md>) · [SPEC-12 RECRUIT](<SPEC-12 RECRUIT.md>) · [SPEC-13 ASSET](<SPEC-13 ASSET.md>) · [SPEC-14 ROOM](<SPEC-14 ROOM.md>) · [SPEC-15 CHAT](<SPEC-15 CHAT.md>)
>
...
Board **chỉ hiển thị công việc cha**; công việc con nằm bên trong task cha ([DECISIONS-05](<../DECISIONS/DECISIONS-05_Task_Subtask_And_Leaf_Counting.md>) **D-36** — việc con còn bị ép `state_id = NULL` và giữ NULL, xem §14.21).
...
| parent_task_id  | ID             | Không                 | Task cha — **đã hỗ trợ** (§14.21, [DECISIONS-05](<../DECISIONS/DECISIONS-05_Task_Subtask_And_Leaf_Counting.md>) D-31: đúng 1 cấp, cùng dự án với cha) |
...
Board **chỉ hiện công việc cha**. Công việc con xem trong task cha — [DECISIONS-05](<../DECISIONS/DECISIONS-05_Task_Subtask_And_Leaf_Counting.md>) **D-36** (§14.21). Tab "Danh sách" của vỏ workspace dự án cũng chỉ hiện cha (parity Bảng↔Danh sách).
...
> **Đổi 20/07/2026 — mẫu số đếm theo LÁ.** Từ khi có công việc con, các chỉ số của báo cáo dự án (`countsByStatus` · số quá hạn · tải theo người) đếm **task lá**, không đếm task cha có việc con. Quy tắc đầy đủ + hệ quả đã chấp nhận: §14.21 và [DECISIONS-05](<../DECISIONS/DECISIONS-05_Task_Subtask_And_Leaf_Counting.md>) **D-34**.

---

### 14.21 TASK-FUNC-021: Công việc con (subtask)

> **Nguồn quyết định = [DECISIONS-05](<../DECISIONS/DECISIONS-05_Task_Subtask_And_Leaf_Counting.md>)** (D-31…D-41). Mục này đặc tả hành vi nghiệp vụ; lý do và các phương án đã loại nằm ở ADR, không nhân bản về đây.

#### Mục tiêu

Cho phép phân rã một công việc thành các **công việc con** — mỗi việc con là một task thật, có **người thực hiện và hạn riêng**, trạng thái riêng.

#### Người dùng

* Người tạo/quản lý công việc cha (theo quyền `TASK.TASK.UPDATE` và vai trò dự án — [DECISIONS-04](<../DECISIONS/DECISIONS-04_Task_Per_Project_Role.md>) D-24).
* Người thực hiện của từng việc con.

#### Mô hình

* Việc con là task thật trên `tasks.parent_task_id` (**D-31**), **KHÔNG** phải checklist mở rộng.
* **Đúng 1 cấp:** task được chọn làm cha phải là task gốc (`parent_task_id IS NULL`). Không có tầng 3.
* Việc con **bắt buộc cùng dự án với cha** (cả hai không thuộc dự án nào cũng hợp lệ).
* **Checklist giữ song song, KHÔNG gộp:** badge checklist và badge việc con là hai chỉ số độc lập. *Checklist* = hạng mục con trong đầu MỘT người; *việc con* = việc có chủ + hạn riêng.

#### Luồng chính

1. Người dùng mở màn chi tiết công việc (TASK-SCREEN-007) của một task **gốc**.
2. Panel "Công việc con" hiển thị danh sách con: tiêu đề · người thực hiện · hạn · trạng thái · cờ quá hạn, sắp theo thứ tự thủ công.
3. Thêm việc con: nhập tiêu đề, chọn người thực hiện, chọn hạn. Hệ thống tạo task mới với `parent_task_id` = task đang xem, dự án = dự án của cha, trạng thái khởi tạo `Todo`.
4. Sửa nhanh / đổi trạng thái / xoá từng việc con.
5. Đổi thứ tự việc con bằng nút lên/xuống (TASK-API-702).
6. Mở chi tiết một việc con: panel này **ẩn**, thay bằng dòng "Thuộc công việc cha: …".

#### Quy tắc

1. **Bất biến cây (D-33)** — khi gán cha: cha phải tồn tại, cùng company, chưa xoá; cha phải là task gốc; task đang **có con** thì không được trở thành con; không tự làm cha chính mình. Backend kiểm **sau khi đã khoá hàng** trong cùng giao dịch.
2. **Ẩn khỏi board (D-36)** — việc con không thành thẻ Kanban; `state_id` của việc con luôn NULL, **kể cả sau khi đổi trạng thái**. Tab "Danh sách" của dự án cũng chỉ hiện cha. Đưa một thẻ đang nằm trong cột thành việc con ⇒ thẻ rời board và cột đó giảm 1, có ghi lịch sử.
3. **Gỡ khỏi cha** ⇒ task trở thành gốc **không cột**; người dùng tự kéo vào cột sau. Hệ thống không đoán cột mặc định.
4. **Dự án của cây là bất biến (D-36a)** — không đổi dự án của task đang có việc con (phải gỡ việc con ra trước), và không đổi dự án riêng của một việc con.
5. **Tiến độ thẻ cha** — thẻ Kanban và màn chi tiết hiển thị `đã xong / tổng` việc con; mẫu số **không tính việc con đã huỷ**. Task không có việc con thì không hiện chỉ số này.
6. **Xoá cha ⇒ xoá lan (D-38)** — xoá mềm cha kéo theo xoá mềm **mọi** việc con còn sống (kể cả việc con đã huỷ), trong **cùng một giao dịch**, **tất-cả-hoặc-không**. Có ≥1 việc con nằm ngoài phạm vi GHI của người thao tác ⇒ **từ chối toàn bộ**, không xoá gì, trả về danh sách việc con bị chặn.
7. **Huỷ cha KHÔNG huỷ con (D-41)** — chuyển cha sang `Cancelled` không tự huỷ việc con.
8. **Phạm vi xem (D-39)** — xem được cha là xem được **toàn bộ** việc con của nó. Nhưng **quyền GHI không thừa hưởng**: sửa/đổi trạng thái/xoá riêng một việc con vẫn kiểm quyền trên chính việc con đó. Việc con ngoài tầm với hiển thị **chỉ đọc**, không có liên kết và không có nút thao tác.

#### Quy tắc đếm — LÁ (D-34)

* **"Lá"** = công việc chưa xoá và **không có việc con nào còn sống & chưa huỷ**. Task không có việc con thì chính nó là lá.
* Áp cho **đúng ba nơi, cùng một công thức**: dashboard (`mv_dashboard_task_status`) · báo cáo tiến độ dự án (§14.20) · widget tiến độ dự án.
* **KHÔNG áp cho:** Kanban board · "Việc của tôi" · "Việc quá hạn" · tổng hợp cá nhân · cảnh báo · job nhắc hạn.
* **Hệ quả đã chấp nhận, phải ghi chú trên giao diện (D-34/D-37):**
  1. **Danh sách ≠ con số** — các màn danh sách việc phải xử lý hiện **cả cha lẫn con**, nên người dùng có thể thấy dashboard 12 mà danh sách 15.
  2. Thêm việc con **đầu tiên** vào một task ⇒ **tổng không đổi**; việc con **thứ hai** mới +1.
  3. Người chỉ ôm task cha (mọi việc con giao người khác) hiện **0** trên biểu đồ tải.
  4. **Huỷ việc con cuối cùng làm tổng tăng 1** — cha hết việc con còn hiệu lực nên quay lại làm lá. Đây là hành vi đúng, không phải lỗi.

#### Lọc theo người phụ trách (D-40)

Lọc board theo người X giữ thẻ **cha** khi người thực hiện của cha là X **hoặc** cha có việc con (chưa huỷ) giao cho X. Việc con không thành thẻ riêng. Thẻ có thể hiện dù X không phải người thực hiện của chính thẻ đó — badge tiến độ việc con trên thẻ là dấu hiệu giải thích.

#### Trường hợp lỗi

| Mã lỗi | Trường hợp | HTTP |
| ------ | ---------- | ---- |
| TASK-ERR-043 | Công việc cha không tồn tại / đã xoá / khác công ty | 404 |
| TASK-ERR-044 | Công việc được chọn làm cha đã là việc con (vượt 1 cấp) | 400 |
| TASK-ERR-045 | Công việc đang có việc con nên không thể trở thành việc con | 400 |
| TASK-ERR-046 | Việc con không cùng dự án với công việc cha | 400 |
| TASK-ERR-047 | Có việc con nằm ngoài phạm vi ghi ⇒ không xoá được công việc cha | 403 |

#### Tiêu chí nghiệm thu

1. Tạo việc con dưới một việc con ⇒ `TASK-ERR-044`; gán cha cho task đang có con ⇒ `TASK-ERR-045`; cha ở dự án khác ⇒ `TASK-ERR-046`; cha thuộc công ty khác ⇒ `TASK-ERR-043`.
2. Tạo việc con rồi đổi trạng thái nó sang `Done` ⇒ `state_id` **vẫn NULL** và số đếm từng cột trên board **không đổi**.
3. Thẻ cha hiển thị `đã xong / tổng` việc con; việc con đã huỷ không vào mẫu số; badge checklist vẫn hiện độc lập.
4. Xoá cha ⇒ cha và mọi việc con cùng có `deleted_at`, mỗi việc con có một dòng lịch sử; nếu có việc con ngoài phạm vi ghi ⇒ `TASK-ERR-047` và **không** bản ghi nào bị xoá.
5. Dự án có 1 task lẻ `Todo` + 1 cha `Todo` có 2 việc con (1 `Done`, 1 `Todo`) ⇒ báo cáo đếm `Todo = 2`, `Done = 1` (không phải 3/1); dashboard và widget tiến độ cho **cùng con số**.
6. Số của "Việc của tôi" · "Việc quá hạn" · tổng hợp cá nhân · cảnh báo · job nhắc hạn **không đổi** khi thêm việc con.
7. Lọc board theo người X ⇒ thẻ cha có việc con giao X xuất hiện dù người thực hiện của cha là người khác.
...
| parent_task_id  | UUID/Integer  | Không                | Task cha — **đã hỗ trợ** ([DECISIONS-05](<../DECISIONS/DECISIONS-05_Task_Subtask_And_Leaf_Counting.md>) D-31; bất biến cây: D-33) |
| priority        | String        | Có                   | Low/Medium/High/Urgent                    |
...
| TASK-API-701 | GET    | /api/v1/tasks/{id}/subtasks            | Danh sách công việc con | TASK.TASK.VIEW        |
| TASK-API-702 | PATCH  | /api/v1/tasks/{id}/subtasks/reorder    | Đổi thứ tự công việc con | TASK.TASK.UPDATE     |

> **Về mã `TASK-API-701` / `TASK-API-702`.** Cấp ngày 20/07/2026 cho hai route công việc con (§14.21, chi tiết API-06 §13.7/§13.8, quyết định [DECISIONS-05](<../DECISIONS/DECISIONS-05_Task_Subtask_And_Leaf_Counting.md>)). Dùng **dải `7xx` còn trống** thay vì chèn tiếp `218` để không đụng dải `21x` vốn đang mang ngữ nghĩa "board & cột pipeline" — hai route này thuộc cấu trúc cây công việc, không thuộc board. Cả hai **không sinh cặp quyền mới**: tái dùng `TASK.TASK.VIEW` / `TASK.TASK.UPDATE`, phạm vi kiểm trên **công việc CHA**.
>
...
> | TASK-ERR-043 (cha không tồn tại/khác công ty) | `TASK-ERR-TASK-NOT-FOUND` |
> | TASK-ERR-044 / 045 / 046 (vi phạm bất biến cây việc con) | `TASK-ERR-SUBTASK-INVALID` |
> | TASK-ERR-047 (việc con ngoài phạm vi ghi) | `TASK-ERR-FORBIDDEN` |
...
| TASK-ERR-043 | Công việc cha không tồn tại              | Không tìm thấy công việc cha                        |
| TASK-ERR-044 | Cha đã là việc con (vượt 1 cấp)          | Chỉ hỗ trợ một cấp công việc con                    |
| TASK-ERR-045 | Công việc đang có việc con               | Công việc đang có việc con không thể trở thành việc con |
| TASK-ERR-046 | Việc con khác dự án với cha; đổi dự án của cây (D-36a); danh sách sắp xếp không khớp tập việc con | Việc con phải cùng dự án với công việc cha          |
| TASK-ERR-047 | Việc con ngoài phạm vi ghi               | Bạn không có quyền xóa một số việc con của công việc này |

> **Cấp 20/07/2026 — `TASK-ERR-043…047` (công việc con).** Chi tiết ngữ nghĩa + HTTP status ở §14.21; quyết định gốc: [DECISIONS-05](<../DECISIONS/DECISIONS-05_Task_Subtask_And_Leaf_Counting.md>) §4. Cao nhất trước đợt này là `TASK-ERR-042`, dải `043-047` còn trống khi cấp.
...
14. ~~Có cần hỗ trợ sub-task trong MVP không, hay checklist là đủ?~~ **ĐÃ CHỐT 20/07/2026 — [DECISIONS-05](<../DECISIONS/DECISIONS-05_Task_Subtask_And_Leaf_Counting.md>) D-31: CÓ sub-task, checklist GIỮ SONG SONG** — việc con là task thật trên `parent_task_id`, **đúng 1 cấp**, có người thực hiện + hạn riêng (owner chốt 18/07: mỗi dòng con phải có người và hạn ⇒ checklist không đủ). Checklist vẫn là badge độc lập, không bị thay thế. Đặc tả: §14.21. Kéo theo quy tắc **đếm-lá** cho dashboard/báo cáo (D-34) — con số và danh sách cố ý khác nhau (D-37).
