...
| RECRUIT-ERR-008 | 409 | **Convert bị chặn**: ứng viên chưa có offer nào (`kind = no-offer`) · có offer nhưng không cái nào `Accepted` (`kind = offer-not-accepted`) · đã có employee link (`kind = already-converted` — chốt cuối UNIQUE `candidates.employee_id`, race 2 request đúng-1-thắng) |
| RECRUIT-ERR-009 | 404 / 422 | Interviewer/tham chiếu nhân sự: nhân viên **không tồn tại trong company** → **404** (giống không tồn tại — chống oracle chéo tenant); tồn tại nhưng **không `active`** → **422** (`kind = employee-not-found` / `employee-inactive`); recruiter phụ trách không phải user sống trong company → **404** |
...
3. Tạo hồ sơ nhân viên qua **API nội bộ MỚI, tx-aware** của HR: `HrWriteService.createEmployeeFromCandidateTx(tx, actor, input)` — **BẮT BUỘC viết mới ở WO BE, KHÔNG gọi `createEmployee` hiện có** (đo 31/08/2026, plan-review B1: `createEmployee` (a) tự mint user account khi có `email` — vi phạm REC-DEC-005; (b) đòi scope `('create','employee')`+`('create','user')` mà role `recruiter` không có; (c) tự mở transaction riêng + cấp mã ở tx riêng ⇒ orphan employee khi race). Hợp đồng của hàm mới: **nhận `tx` từ convert** (không tự mở), **không có nhánh provision user**, cấp `employee_code` qua SequenceService ensure-on-miss **trong cùng tx**, ghi audit HR (`object_type='employee'`) trong cùng tx. **Map trường tường minh**: `full_name` → họ tên · `email` → email cá nhân (**nullable — ứng viên không email vẫn convert được**) · `phone` → SĐT (input của hàm mới có `phone`) · `job_openings.org_unit_id` → đơn vị · `job_openings.position_id` → vị trí · `offers.start_date` → ngày vào làm · trạng thái nhân viên = mặc định luồng HR (`Probation`/`Onboarding` — SPEC-03). **Lương offer KHÔNG map**.
