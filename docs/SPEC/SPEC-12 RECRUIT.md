...
> [SPEC-01 Tổng quan](<SPEC-01 Tổng quan.md>) · [SPEC-02 AUTH](<SPEC-02 AUTH.md>) · [SPEC-03 HR](<SPEC-03 HR.md>) · [SPEC-04 ATT](<SPEC-04 ATT.md>) · [SPEC-05 LEAVE](<SPEC-05 LEAVE.md>) · [SPEC-06 TASK](<SPEC-06 TASK.md>) · [SPEC-07 DASH](<SPEC-07 DASH.md>) · [SPEC-08 NOTI](<SPEC-08 NOTI.md>) · [SPEC-09 ME](<SPEC-09 ME.md>) · [SPEC-10 GOAL](<SPEC-10 GOAL.md>) · [SPEC-11 PAYROLL](<SPEC-11 PAYROLL.md>) · **SPEC-12 RECRUIT** · [SPEC-13 ASSET](<SPEC-13 ASSET.md>) · [SPEC-14 ROOM](<SPEC-14 ROOM.md>) · [SPEC-15 CHAT](<SPEC-15 CHAT.md>)
>
...
| `('upload','candidate-file')` | `RECRUIT.CANDIDATE-FILE.UPLOAD` | **true** | đăng ký · xác nhận · **gắn** tệp CV vào ứng viên (API-035..037). KHÔNG mở đường ĐỌC (đã là `view:candidate`), KHÔNG mở đường gỡ/xoá | — | — | Company | Company |

Ghi chú bắt buộc:

- **Đúng 17 cặp; 8 cặp `is_sensitive = true`** — 7 cặp resource `candidate` (REC-DEC-003) + `('upload','candidate-file')` (S14-RECRUIT-FILEGRANT-1, CV là PII ứng viên); 9 cặp còn lại `false` — chốt cùng seed, không flip sau (bẫy `canonical-seed-pin-regression`). Cặp sensitive **phải khai vào allowlist capability ở BACKEND cùng WO BE** — thiếu là màn quản trị biến mất với chính role được grant (`sensitive-capability-allowlist-hides-admin-screens`).
...
- **Đường GHI tệp CV dùng cặp RIÊNG `('upload','candidate-file')`** (S14-RECRUIT-FILEGRANT-1). Hai lý do, cả hai đo được:
  - **Vì sao không dùng `('update','candidate')`:** cặp đó mở luôn PII — dòng "người giữ cặp này thấy email/phone KHÔNG che" ở bảng trên. Cấp nó cho `hr` chỉ để đính CV sẽ bỏ mask PII cho TOÀN role `hr`, hệ quả không ai yêu cầu.
  - **Vì sao resource là `candidate-file` chứ không phải `candidate`:** mig `0560` RAISE theo SỐ ĐẾM grant trên đúng 5 resource RECRUIT (`recruit`/`job-opening`/`candidate`/`interview`/`offer`), và int-spec `s12-recruit-db1-invariants` **đọc `0560_*.sql` từ đĩa rồi chạy lại**. Đặt cặp mới trên `candidate` ⇒ migration ĐÃ SHIP nổ khi replay, exception ném từ trong SQL nên không sửa được ở phía test. Đây là ràng buộc KỸ THUẬT, không phải sở thích đặt tên.
...
- **Ma trận seed = 45 hàng** `role_permissions`: `employee` 0 · `manager` 3 (`access`@Own · `view:interview`@Own · `feedback:interview`@Own) · `hr` 8 (`access`@Own · 4 cặp view@Company · `convert`@Company · `feedback`@Own · `upload:candidate-file`@Company) · `company-admin` 17 · `recruiter` 17 (`access`@Own · `feedback`@Own · 15 cặp còn lại @Company). Migration verify fail-loud đúng số; `super-admin` không enumerate. _(42 → 45 ở mig `0569`: cặp GHI tệp CV cho 3 role.)_
...
| RECRUIT-API-033 | `GET /candidates/:id/files` | `('view','candidate')` | tệp CV đã gắn vào ứng viên; **mảng TRẦN** (không phong bì phân trang); chỉ tệp có `file_links` sống trỏ ĐÚNG ứng viên này |
| RECRUIT-API-034 | `GET /candidates/:id/files/:fileId/download-url` | `('view','candidate')` | URL ký TTL-ngắn; `fileId` không link sống vào ĐÚNG `:id` ⇒ **404** (chống IDOR, không dùng 403 để khỏi thành oracle); ghi `file_access_logs` |
| RECRUIT-API-035 | `POST /candidates/:id/files/upload-url` | `('upload','candidate-file')` | 200; đăng ký metadata + presigned-PUT. Client CHỈ khai `originalName`/`declaredMimeType`/`sizeBytes` — `visibility`/`moduleCode`/`entityType`/`entityId` do SERVER đặt (`.strict()` ⇒ khai thêm là 400) |
| RECRUIT-API-036 | `POST /candidates/:id/files/:fileId/confirm` | `('upload','candidate-file')` | 200 idempotent; **owner-check chạy TRƯỚC** — confirm hộ tệp người khác ⇒ 403 |
| RECRUIT-API-037 | `POST /candidates/:id/files/:fileId/link` | `('upload','candidate-file')` | 201; gắn tệp vào ứng viên (`linkType='Document'`, `accessScope='Company'`, `purpose='CV'`). Gắn lần hai cho cùng cặp ⇒ **403** (không phải 409) — xem §18 |

> ⚠️ **Tệp CV có bề mặt RIÊNG trong RECRUIT kể từ S14-RECRUIT-FILEGRANT-1** (API-033..037). Trước đó chúng đi qua Foundation Files (API-09) generic, gate `*:foundation-file` — cặp mà seed chỉ cấp cho `company-admin`, nên `recruiter`/`hr` **không đính/tải được CV**. Cách đóng KHÔNG phải cấp cặp `foundation-file` cho họ: `view:foundation-file` là cổng màn quản trị `System > Files` và `GET /foundation/files` không gác per-file ⇒ cấp = mở trình duyệt tệp TOÀN TENANT.
>
> Tệp vẫn nằm ở Foundation (`files`/`file_links`, `module_code='RECRUIT'`, `entity_type='candidate'`, private); RECRUIT chỉ bọc thêm một lớp controller có gate riêng. Đường **ĐỌC** gate bằng chính cặp gác màn ứng viên `('view','candidate')`; đường **GHI** gate bằng cặp riêng `('upload','candidate-file')` (§11, mig 0569). Tải về ghi `file_access_logs`. Thêm endpoint sau phải cấp mã `RECRUIT-API-038+` và đo lại dải bằng grep.
>
> **37 mã = 37 route HTTP** (không mã nào gói 2 route). Route-census đếm route — regen với 37.
...
- **Che PII ở server (REC-DEC-003):**
  ...
  - CV: Foundation Files **private**, truy cập qua bề mặt RECRUIT riêng (API-033..037 — S14-RECRUIT-FILEGRANT-1). Đường **đọc** gate bằng chính cặp `('view','candidate')`; đường **ghi/đính tệp** gate bằng cặp riêng `('upload','candidate-file')` **HOẶC** `('create','candidate')`/`('update','candidate')` (§11); mỗi lần tải ghi `file_access_logs` (KHÔNG ghi `audit_logs` — `FileService` sở hữu vết này, xem §16 dòng 527).
  - **`originalName` của tệp CV KHÔNG mask (quyết định tường minh, đừng mở lại):** §18 buộc che email/phone ở **DTO có cấu trúc**, nhưng đường tải gate bằng `('view','candidate')` nghĩa là người đọc được tên tệp cũng đọc được **toàn văn CV**. Che tên tệp trong khi phục vụ chính tệp đó là hình thức, không phải kiểm soát.
  - **Gắn tệp (API-037) ép NĂM vế ngoài cặp quyền** — `RecruitCandidateFileResolver.canLinkFile`: (1) `fileId` có mặt · (2) caller **sở hữu** tệp · (3) `upload_status='Uploaded'` · (4) `scan_status ∈ {Clean, NotRequired}` · (5) tệp **CHƯA TỪNG** có link nào. Vế 5 là hàng rào chống bypass thu hồi: `FileService.link` không kiểm chủ sở hữu, và gắn lại một tệp ĐÃ GỠ vào bất kỳ ứng viên nào sẽ làm `decideForLinkedFile` hết rơi nhánh links-rỗng ⇒ mở lại đường tải VĨNH VIỄN. Hệ quả CHỦ ĐÍCH: gắn lần hai cho cùng cặp trả **403** (không phải 409 `DUP_LINK`), và tệp gỡ nhầm phải **tải lên lại**, không tái-link được.
  - **Quy kết audit tệp CV — tin hàng `FileLinked`, KHÔNG tin hàng `FileUploaded`:** bước 035 ghi
    `audit_logs` với `entityId` = ứng viên trên URL lúc ĐĂNG KÝ, còn bước 037 ghi hàng `FileLinked` với
    ứng viên thật sự được gắn. Hai hàng có thể LỆCH nhau: route 036 `confirm` không ràng tệp vào đúng
    `:id` (không ràng được cho rẻ — `files` không có cột entity), nên cùng một caller sở hữu tệp và có
    `upload:candidate-file@Company` (vốn phủ mọi ứng viên) có thể đi qua URL của ứng viên khác. Không
    phải escalation — người đó chỉ cần đăng ký một tệp mới là xong — nhưng người đọc audit về sau phải
    biết cột nào là căn cứ.
  - **Bất đối xứng CÓ CHỦ ĐÍCH:** `('upload','candidate-file')` KHÔNG mở `canUnlinkFile`/`canDeleteFile` — hai đường đó vẫn đòi `('create','candidate')`/`('update','candidate')`. Nghĩa là `hr` **gắn được** CV nhưng **không gỡ/xoá được**. Quyền hẹp hơn, không phải lỗ. Nợ vận hành kèm theo (xoá CV theo yêu cầu chủ thể dữ liệu) ghi ở `RELEASE-02`.
...
- Cặp sensitive (7 cặp `candidate` + `('upload','candidate-file')`) khai **allowlist capability ở BACKEND** cùng WO BE (`capability-allowlist-hides-admin-screens`) — cả `SENSITIVE_CAPABILITY_ALLOWLIST` lẫn `SENSITIVE_SCREEN_GATE_PAIRS`; FE gate bằng `useCanExact`, KHÔNG `useCan` (wildcard không kế thừa cặp sensitive).
...
10. `recruiter`/`hr` tải CV lên qua API-035→036→037 ⇒ 200/200/201, trong khi CẢ HAI vẫn **403** trên mọi route `/foundation/files*` (WO không cấp cặp `foundation-file`); gắn tệp người khác hoặc tệp đã-gỡ-link ⇒ **403**. Tải CV → `file_access_logs` +1 hàng; `GET /candidates/export` → audit +1 hàng; caller thiếu `('export','candidate')` **hoặc** thiếu `('view','candidate')` ⇒ 403 (§18 — hai cặp đều bắt buộc); filter khớp > 10.000 dòng ⇒ **422 RECRUIT-ERR-015**.
