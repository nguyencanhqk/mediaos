...
| `project` | `goals.project_id` | soi gương `projects.progress_percent` |
...
- Tiến độ dự án lấy từ `projects.progress_percent` (TASK tính, GOAL đọc).
...
| Thông báo | `goal.assigned` · `goal.finalized` qua OutboxNotificationBridge |
...
| NOTI | phát `goal.assigned` / `goal.finalized` qua outbox bridge | GOAL → NOTI |
...
- `is_sensitive` đề xuất `false` cho tất cả; riêng `('finalize','goal')` cân nhắc `true` — **chốt khi làm S5-GOAL-DB-1** (nhớ pin canonical-seed).
...
| Mã lỗi | Quy tắc |
| --- | --- |
| GOAL-ERR-001 | Level↔neo không khớp: `department` phải có `department_id` (không có project/employee); `project` phải có `project_id`; `employee` phải có `employee_id`. Sai → 422 |
...
- **project**: progress = `projects.progress_percent` của `project_id` (TASK tính theo đếm-lá DECISIONS-05, GOAL chỉ đọc).
...
Project recompute progress_percent    → recompute goal mode=project của project đó
...
| --- | --- | --- |
| `goal.assigned` | tạo/cập nhật goal cấp employee mà owner ≠ actor (trưởng phòng giao) | employee được giao |
| `goal.finalized` | chốt kỳ goal | owner goal (+ trưởng đơn vị nếu khác actor) |

Phát qua **OutboxNotificationBridge** (INT-1): enqueue trong transaction, map eventCode verbatim, dedupe + delivery log, cùng company. Check-in reminder định kỳ: phase sau (§5.2).
...
- Job đối soát đêm chạy trong hàng đợi BullMQ hiện có, idempotent.
