...
| PAYROLL-ERR-021 | 422 | **Gross-up NET không hội tụ** sau 30 vòng, hoặc căn cứ lặp ≤ 0 (`kind = grossup-not-converged`); `details[]` nêu `userId` · `iterations` · `reason` (`not-converged` \| `non-positive-base`). 🔁 **KHÔNG mang «sai số còn lại»** — đó là số tiền, trong khi 007 là route GHI gác bằng `('calculate','payroll-period')` (không phải cặp đọc tiền) và API-18 §6.5 cấm tiền trong `details` (`S15-PAYROLL-BE-3` §3.6). **Kỳ giữ nguyên trạng thái, KHÔNG ghi dòng nào** (toàn bộ tx rollback) |
...
| PAYROLL-ERR-023 | 409 | **Mẫu bảng lương của kỳ**: đổi mẫu khi kỳ **> `CollectingData`** (`kind = template-locked`) · `calculate` khi kỳ **chưa gắn mẫu** hoặc mẫu đã **xoá mềm** (`kind = template-missing`; nhánh sau kèm `details.reason = template-deleted`) · mẫu đã **ngưng dùng** (`kind = template-inactive`) · 🔁 mẫu **`scope = org_unit`** gắn vào kỳ (`kind = template-scope-unsupported` — kỳ tính cho CẢ công ty, ngữ nghĩa nhiều mẫu/kỳ chưa định nghĩa ⇒ fail-closed, `S15-PAYROLL-BE-3` §3.10) |
...
> - **023** `template-code-exists` (trùng mã mẫu — chốt cuối UNIQUE). *(`template-in-use` — xoá/ngưng mẫu đang gắn kỳ — ĐÃ hiện thực ở `S15-PAYROLL-BE-4`, xem khối dưới.)*
...
>
> 🔁 **Kind bổ sung ở `S15-PAYROLL-BE-3` (máy tính lương v2, 15/09/2026)** — mã không đổi, danh sách vẫn ĐÓNG:
>
> - **018** `system-component-drift` (nội dung hàng `is_system` lệch hằng seeder — cổng CỨNG lúc gắn/tính vì assert (7) của seeder chỉ log; `details.components` = MÃ, không chuỗi công thức) · `template-input-missing` (mẫu thiếu hoặc ghi đè một trong `THUONG · PHAT · TAM_UNG · NGHI_KHONG_LUONG` — khoản bị gắn/đánh dấu đã dùng mà không vào lương; kiểm lúc gắn 002/004 VÀ lúc tính) · `profile-item-unknown-component` lúc TÍNH mang thêm `userId` (item hồ sơ mã ngoài `profile_item` của mẫu, vd `PC_nnn` di sản — KHÔNG âm thầm = 0).
> - **020** `negative-total` (`TONG_THU_NHAP`/`TONG_KHAU_TRU` âm — chặn trước khi ra cột; CHECK `payroll_period_lines_amounts_check` chỉ là lưới cuối).
> - **022** `statutory-rate-missing` — hiện thực (CẤM tính với tỉ lệ 0 khi công ty chưa có bản hiệu lực).
> - **012** `self-approval` từ CHECK **`bonus_penalties_four_eyes_check`** (mig `0575`) — chốt cuối RACE của 027, cùng mã + kind với tiền-kiểm ở service.
>
...
> 🔁 **Kind bổ sung khi hiện thực track C (`S15-PAYROLL-BE-4`, 15/09/2026)** — mã không đổi, danh sách vẫn ĐÓNG (plan `docs/plans/S15-PAYROLL-BE-4.md` §0 · §0b):
>
> - **017** `no-eligible-completer` — 067 lập đợt khi công ty KHÔNG có người nào KHÁC actor giữ `('manage','payment-batch')`@Company (four-eyes 072 sẽ không bao giờ thoả — fail-fast lúc LẬP, không kẹt kỳ `Published`; cùng lớp `no-eligible-approver` ở `submit`).
> - **023** `template-in-use` — 052 xoá mềm HOẶC `isActive:false` mẫu đang được ≥ 1 kỳ **chưa xoá mềm** tham chiếu (bất kể trạng thái kỳ); `details.periods` = số kỳ. Đếm bằng SELECT thường dưới khoá catalog độc quyền — KHÔNG khoá hàng kỳ.
> - **025** `self-approval` = **người TẠO HOẶC người THỤ HƯỞNG** (`created_by === actor` OR `user_id === actor`) — CHECK `payroll_advances_four_eyes_check` chỉ soi `created_by`, vế thụ hưởng CHỈ có ở service (tiền-kiểm dưới `FOR UPDATE`). Thứ tự kiểm: `advance-already-deducted` TRƯỚC `advance-not-pending`.
> - **026** `advance-period-frozen` áp ở **060/062** khi kỳ đích `∉ {Draft, CollectingData}` (nguyên văn hàng 060); ở **063** chặn khi `∉ {Draft, CollectingData, Calculated}` — cho duyệt ở `Calculated` (lượt tính LẠI nhặt được) và trả **`warnings: ["recalculate-required"]`**; chặn từ `Reviewing` vì snapshot đã đóng băng. Tag `period-frozen` của trigger T3 ⇒ cùng mã/kind.
> - **027** thêm bốn kind: `batch-code-exists` (trùng `code` — UNIQUE `payroll_payment_batches_company_code_uq`; mã tự sinh `CT-<YYYYMM>-<BANK|CASH>-<8 hex>`) · `batch-four-eyes` (072 `completed_by === created_by`) · `payee-no-bank-account` (067/069 `userIds` tường minh + đợt `bank` + người thiếu TK; tự nạp thì bỏ qua + `warnings no-bank-account:<n>`) · `line-already-paid` (069 `removeUserIds` gặp dòng `paid_at NOT NULL` — gỡ được là nhả `payslip_uq` cho phiếu vào đợt khác = chi HAI lần; **v2 KHÔNG có đường bỏ đánh dấu đã chi**). Tag T1 `insert-completed|period-immutable|frozen` và T2 `frozen|insert-into-completed|move-to-completed` ⇒ 027 `batch-already-completed`; T2 `not-found|cross-user|cross-period` và T1 `has-active-lines` **cố ý KHÔNG map** (service phải chặn trước — tới DB là bug ⇒ 500 có chủ đích).
> - **028** `batch-empty` — đường phát duy nhất (072 trên đợt 0 dòng sống).
> - **030** `import-invalid` mang `details.reason` ∈ {`no-file`, `file-too-large`, `file-type`, `parse`, `no-worksheet`, `header`, `no-data-rows`, `rows`}; lỗi theo dòng ở `details[] {field:"row:<n>"}` (cắt 50 + `errorRows`), **không bao giờ lặp lại số tiền**; `import-unknown-user` ưu tiên nếu có ≥ 1 mã NV lạ. **003** `period-frozen` TÁI DÙNG ở 076 khi kỳ `∉ {Draft, CollectingData, Calculated}`.
> - Ba CHECK Zod đã mirror ĐÚNG BẰNG (`payroll_payment_batches_completed_pair_check` · `payroll_payment_lines_bank_pair_check` · `payroll_advances_deducted_bound_check`) ⇒ **400 `VALIDATION-ERR-001`** lưới cuối (`details.constraint`), không 500. FK `payroll_advances_user_id*` · `payroll_payment_lines_user_id*` · `payroll_budgets_org_unit_id*` ⇒ 404 sentinel 010.
...
**Hiện thực ở máy công thức v2 (`S15-PAYROLL-DB-1B`, mig `0574`):** công thức seed `LUONG_CO_BAN = MIN(SYS_BASE_SALARY * (SYS_PRESENT_DAYS + SYS_UNPAID_LEAVE_DAYS) / SYS_WORK_DAYS, SYS_BASE_SALARY) * SYS_PAY_RATIO / 100` — tử số CỘNG `unpaid`, kẹp trần bằng `MIN(…, base)`; `NGHI_KHONG_LUONG` giữ nguyên độ lớn. 🔁 **`S15-PAYROLL-BE-3` (owner O-5, mig `0575`, seedVersion v4):** `NGHI_KHONG_LUONG` đổi thành **`kind = earning` công thức ÂM** `-(SYS_BASE_SALARY * SYS_PAY_RATIO / 100 * SYS_UNPAID_LEAVE_DAYS / SYS_WORK_DAYS)` — để là `deduction` thì `THU_NHAP_CHIU_THUE` (không trừ `deduction`, §13.6 E) tính TNCN trên tiền nghỉ không lương NV KHÔNG nhận (ca 22tr · 18 công + 2 nghỉ không lương · phụ cấp 1tr: TNCN 750.000 thay vì 550.000). `ROUND_HALF_UP` của decimal.js đối xứng quanh 0 ⇒ trị tuyệt đối không đổi từng xu; phiếu vẫn có dòng «Nghỉ không lương −X». ⚠️ **v2 có thể lệch v1 đúng 0,01 đ ở ca HOÀ nửa xu, theo hướng ĐÚNG**: v1 (SQL) chia `(present + unpaid) / work` TRƯỚC, cắt còn ~20 chữ số rồi mới nhân ⇒ ở biên `.xx5` làm tròn XUỐNG; v2 nhân tử số trước rồi chia MỘT lần ⇒ khớp số học chính xác 100% (51.584 ca đo, plan `S15-PAYROLL-DB-1B` §5.2.a). Đối soát v2 bằng số học chính xác, **KHÔNG** bằng v1.
...
- 🔁 **`S15-PAYROLL-BE-3` (owner O-1, 15/09/2026):** đường công thức SQL cố định v1 **ĐÃ GỠ**. Seeder runtime tạo `MAU_MAC_DINH` bằng công thức **v2** (có luật định + TNCN) — **không** tái tạo v1 và **không tự gắn vào kỳ nào**. Tính lại một kỳ v1 ⇒ gắn mẫu qua 004 khi kỳ ≤ `CollectingData`, số ra theo v2. Phiếu đã sinh của kỳ v1 đọc nguyên vẹn: nhánh sinh `payslip_items` bản đồ 7 dòng cũ GIỮ ở 013 cho dòng `component_values_json = '{}'`.
- 🔁 **Phiếu lương dòng v2 (owner O-3):** `payslip_items` sinh cho **MỌI thành phần góp vào net** (khác 0), từ SNAPSHOT lúc tính — `earning`/`tax_exempt` ⇒ `+value` · `deduction`/`statutory_employee` ⇒ `−value` · `tax` ⇒ `−value` CHỈ khi `pit_payer = EMPLOYEE`; BỎ `aggregate` + `statutory_employer`. Ẩn cột (`is_visible = false`) **vẫn sinh item** (chỉ vào `meta.isVisible`) — chỉ sinh item cho thành phần hiện là vỡ bất biến `SUM(items) = gross − deduction + adjustment`. `meta = {componentCode, kind, isVisible}`.
...
- **Nhân sự thử việc**: căn cứ = `probation_salary` nếu có; `pay_ratio_pct` áp lên **lương**, **KHÔNG** áp lên căn cứ đóng BH (hai đại lượng khác nhau; nhân nhầm là sai thẳng vào số nộp bảo hiểm). 🔁 **Chốt đọc `S15-PAYROLL-BE-3` §3.11:** `SYS_INSURANCE_SALARY = COALESCE(probation_salary, insurance_salary, SYS_BASE_SALARY)` — thử việc đứng TRƯỚC lương BH; không tự suy ngày thử việc từ HR.
- 🔁 **Hiện thực cờ tham gia (`S15-PAYROLL-BE-3`, owner O-2):** zero **TỈ LỆ theo dòng**, không đổi seed, không hard-code mã thành phần — `joins_social_insurance = false` ⇒ `TL_BHXH_NV · TL_BHYT_NV · TL_BHTN_NV · TL_BHXH_DN · TL_BHYT_DN · TL_BHTN_DN · TL_KPCD := 0`; `joins_union = false` ⇒ `TL_DOAN_PHI := 0`; **không có hàng settings ⇒ cả hai `false`** (default cột). Snapshot `rates` ghi tỉ lệ SAU khi zero. **KPCĐ gắn cờ BH** (tính trên quỹ lương đóng BH), KHÔNG gắn cờ công đoàn.
...
- Ký hiệu: `b` = giá trị `SYS_BASE_SALARY` thử; `F(b)` = chạy **toàn bộ** đồ thị với `SYS_BASE_SALARY := b` trên đầu vào **«công đủ»** rồi trả `TONG_THU_NHAP − TONG_KHAU_TRU` (KHÔNG clamp, KHÔNG điều chỉnh tay); `N` = NET mục tiêu.
- 🔁 **Đầu vào «công đủ» của vòng lặp (`S15-PAYROLL-BE-3`, owner O-4):** `SYS_PRESENT_DAYS = SYS_WORK_DAYS` · nghỉ/trễ = 0 · `SYS_PRORATE = 1` · `SYS_PAY_RATIO = 100` · thưởng/phạt/tạm ứng = 0; GIỮ phụ cấp hồ sơ + số NPT + cờ tham gia BH thật. Căn cứ BH vắng ⇒ **`b`** (không phải `N`). Lượt cuối chạy đầu vào THẬT với `b*` ⇒ `b*` độc lập ngày công và NV đi làm nửa tháng lĩnh ≈ nửa NET (ca đối soát `docs/QA/evidence/S15-PAYROLL-BE-3-DOI-SOAT.md`).

```text
b₀      = N                                   -- điểm xuất phát — KHÔNG phải chặn dưới: phụ cấp hồ sơ > N làm F(b₀) > N ⇒ b₁ < 0
b_{k+1} = b_k + (N − F(b_k))
b_{k+1} ≤ 0                                   ⇒ 422 ERR-021 (reason non-positive-base)
dừng khi |N − F(b_k)| ≤ 1 đ                   (hội tụ; iterations = k)
trần    30 vòng                               (không hội tụ ⇒ 422 ERR-021, reason not-converged)
```

> 🔁 Đính chính `S15-PAYROLL-BE-3` (plan-review m1): bản cũ ghi «lương cơ bản ≥ NET luôn đúng, nên b₀ = N là chặn dưới» — SAI khi mẫu giữ phụ cấp hồ sơ (N 5tr, `PHU_CAP` 6tr ⇒ b₁ < 0 ⇒ 021 cho CẢ kỳ). Nợ ghi nhận (không làm ở BE-3): cảnh báo sớm ở route hồ sơ lương khi phụ cấp của hồ sơ NET vượt NET mục tiêu.
...
>
> - **060/062** body `{ userId, amount, deductPeriodMonth, reason }` / `{ amount?, deductPeriodMonth?, reason?, delete? }` `.strict()`; `created_by` từ JWT. **063/064** body `{ note? }` / `{ note }` (bắt buộc). Route ghi trả **`{ id, status, warnings }`** (0 khoá tiền); 063 khi kỳ đích đang `Calculated` ⇒ `warnings ["recalculate-required"]`. 060 khi không có người giữ `approve:payroll-advance` khác actor ⇒ vẫn 201 + `warnings ["no-eligible-approver"]` (không enqueue NOTI-024).
> - **067** body `{ payrollPeriodId, method, code?, payDate?, note?, userIds?[] }` `.strict()` — **KHÔNG có trường tài khoản**: snapshot TK sinh ở server từ `payroll_employee_settings` (set-based, số TK không là tham số). `userIds` vắng ⇒ tự nạp MỌI phiếu của kỳ chưa có dòng sống ở đợt nào, lọc theo `method` (`bank` bỏ người thiếu TK + `warnings no-bank-account:<n>`; `net = 0` ⇒ giữ dòng + `zero-net:<n>`). Tiền-kiểm C3 (`no-eligible-completer`) TRƯỚC khi khoá kỳ. Trả `{ id, warnings }`.
> - **069** body `{ status?: Draft|Ready, payDate?, note?, addUserIds?[], removeUserIds?[], markPaidUserIds?[] }` `.strict()` — `status` là enum RIÊNG (nhận `Completed` ⇒ 400; `Completed` chỉ qua 072). «Sửa snapshot» = gỡ + thêm lại; `markPaidUserIds` ghi `paid_at = now()`; audit `{added,removed}` mang `last4`.
> - **072** body `{ payDate?, confirmAllPaid? }`; `confirmAllPaid:true` ghi `paid_at` cho mọi dòng chưa chi trong CÙNG tx trước khi kiểm; trả `{ id, batchStatus, periodStatus, unpaidPayees }`; thứ tự khoá kỳ → đợt; NOTI-027 chỉ khi phủ đủ (dedupe `{periodId}`). Kỳ `Locked` di sản mang `legacyPaidTrail: true` ở DTO kỳ (003) — FE không hiện «đã chi».
> - **070/071** dòng chi: DTO chỉ `bankAccountLast4` (từ SNAPSHOT); tệp UNC 7 cột cố định `STT · Mã NV · Họ tên · Số tài khoản · Ngân hàng · Số tiền · Nội dung` (`Luong <YYYY-MM> <mã NV>` ASCII), `cash` xuất được (hai cột TK rỗng), tên tệp `unc-<code>.xlsx`, ô văn bản bắt đầu `= + - @` được tiền tố dấu nháy đơn; > 10.000 dòng ⇒ 422 016. `bank_name_snapshot` không có chi nhánh.
> - **073** «thực hiện» = Σ `payslips.gross` kỳ `Published/Paid/Locked` cùng `fiscalYear`, gom theo `employee_profiles.org_unit_id` **HIỆN TẠI** (đọc lúc gọi, không lưu — người chuyển phòng làm số năm cũ đổi chỗ), hàng `orgUnitId = null` = toàn công ty; `variance = planned − actual`; chưa gồm phần BH doanh nghiệp. **074/075** trả `{ id, warnings }`.
> - **076** khuôn 5 cột theo VỊ TRÍ `Mã NV · Loại · Số tiền · Lý do · Kỳ (YYYY-MM)` (`Loại` nhận `bonus/penalty` hoặc `Thưởng/Thu nhập/Phạt/Khấu trừ`; `Kỳ` phải bằng `period_month` của kỳ trong URL); `?dryRun=true` mặc định; trả `payrollWriteResultSchema` (`affectedLines`, `warnings` gồm `dry-run` · `possible-duplicate:<n>`). **077** tệp mẫu sinh từ CHÍNH khuôn cột.
...
> ⚠️ **`054` trả `values` của thành phần `fixed` = CHÍNH `fixedAmount` của catalog** (security-review `S15-PAYROLL-BE-2` LOW-5). DTO `051` cố ý bỏ `fixedAmount`, nhưng xem trước tính trên nó. Bất biến «`('manage','payroll-template')` ⇒ `('view','salary-component')`» chỉ được assert cho **role seed** (mig `0571`) — role tuỳ biến tạo sau có thể giữ cặp đầu mà thiếu cặp sau. **Rủi ro CHẤP NHẬN — owner xác nhận 14/09/2026:** `fixedAmount` là định mức **cấp công ty** (vd phụ cấp ăn ca), không phải số tiền của một cá nhân; cả hai cặp đều sensitive + sàn Company. Muốn đóng: `054` đòi thêm `('view','salary-component')` bằng một method tường minh riêng (census hai tầng đổi CÙNG commit).
