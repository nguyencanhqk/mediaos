...
| SPEC-11 | [PAYROLL - Tiền lương](<SPEC/SPEC-11 PAYROLL.md>) | PAYROLL — *Phase 2, **v2.0 Approved 02/09/2026** (wave S15-PAYROLL-V2, PAY-DEC-011..020); v1.0 Approved 31/08/2026 (wave S13-PAYROLL). v1+v2 sống chung MỘT file — PAY-DEC-011 không tách SPEC mới* |
...
| SPEC-16 | [SOCIAL - Mạng xã hội nội bộ](<SPEC/SPEC-16 SOCIAL.md>) | SOCIAL — *Phase 4 kéo lên, **Approved 18/09/2026** (wave S16-SOCIAL, SOC-DEC-001..010). Mã `SOCIAL` = mạng xã hội nội bộ; app vệ tinh đăng bài Facebook là **tiện ích con** (SOC-DEC-002)* |

> PAYROLL (SPEC-11) · RECRUIT (SPEC-12) · ASSET (SPEC-13) · ROOM (SPEC-14) · CHAT (SPEC-15) giữ đúng số đã khoá tại SPEC-01 §7.2/§8 — không dồn số. Đánh số DB/API: ASSET/ROOM = **DB-15/16 · API-14/15** (OFFICE-DEC-001 — DB-13/14 dành cho PAYROLL/RECRUIT ở IMPLEMENTATION-10); RECRUIT = **DB-14 · API-17** (REC-DEC-001 — API-16 đã bị Permission Audit Report chiếm); PAYROLL = **DB-13 · API-18** (PAY-DEC-001 — API-13 vốn dự định cho PAYROLL đã bị CHAT chiếm).
...
| DB-13 | [PAYROLL Database Design](<DB/DB-13 PAYROLL Database Design.md>) | PAYROLL — *Phase 2, **v2 Approved*** — **v1** (**RECONCILE 6 bảng di sản G12** + 1 bảng mới `payroll_period_lines`) **đã migrate `0564`–`0568`**; **v2 §12–§14: ALTER 3 bảng (DB-1 lấy 2 · DB-2 lấy 1) + 11 bảng mới, CHƯA migrate** — mig **`0570+`** (`0569` đã bị `S14-RECRUIT-FILEGRANT-1` lấy; WO DB vẫn phải đọc `_journal.json` lúc chạy) |
| DB-14 | [RECRUIT Database Design](<DB/DB-14 RECRUIT Database Design.md>) | RECRUIT — *Phase 2, Approved* (8 bảng mới, đã migrate `0559`–`0561`) |
...
| DB-17 | [SOCIAL Database Design](<DB/DB-17 SOCIAL Database Design.md>) | SOCIAL — *Phase 4, Approved* (**19 bảng mới** `feed_*` — Track A 10 · Track B 9, **chưa migrate**; mig dự kiến `0577+`, đo `_journal.json` lúc chạy) |
...
| API-18 | [PAYROLL API Design](<API Design/API-18_PAYROLL_API_Design.md>) | PAYROLL — *Phase 2, **v2 Approved*** — **85 route**: `PAYROLL-API-001..035` **đã hiện thực** (`S13-PAYROLL-BE-1/BE-2`), **`036..085` (50 route) CHƯA hiện thực** (wave S15). API-13 vốn dự định cho PAYROLL đã bị CHAT chiếm |
| API-19 | [SOCIAL API Design](<API Design/API-19_SOCIAL_API_Design.md>) | SOCIAL — *Phase 4, Approved* — **53 route** `SOCIAL-API-001..053`, **chưa hiện thực** (wave S16) |
...
| **CHAT** | PRD §8.1 (Phase 4) | [SPEC-15](<SPEC/SPEC-15 CHAT.md>) *(Approved)* | [DB-12](<DB/DB-12 CHAT Database Design.md>) — bảng đã tồn tại thật từ migration `0010`/`0050`, mở rộng ở `0538`/`0539`/`0542`/`0543` | [API-13](<API Design/API-13_CHAT_API_Design.md>) — endpoint đã ship qua các wave S7-CHAT/S8-CHAT-UX/S7-CALL; wave S17-CHAT-UX2 (API-13 §5.1d) đang chạy | [UI-09 §13a](<UI/UI-09_Module_UI_Design.md>) | FRONTEND — đã ship v1 (`S7-CHAT-FE-*`) + S8-CHAT-UX + S7-CALL; wave S17-CHAT-UX2 (v2 bố cục/drawer/responsive) đang chạy, `apps/app/src/routes/chat*` | BACKEND — đã ship v1 (`apps/api/src/chat/`, S7-CHAT-BE-*) + S8-CHAT-UX-BE-* + S7-CALL-BE-*; wave S17-CHAT-UX2-BE-1/BE-2 đang chạy |
...
| **PAYROLL** | PRD §8.1 (Phase 2) | [SPEC-11](<SPEC/SPEC-11 PAYROLL.md>) **v2** *(Approved 02/09/2026, wave S15-PAYROLL-V2)* — v1 Approved 31/08/2026 | [DB-13](<DB/DB-13 PAYROLL Database Design.md>) **v2** — v1: **RECONCILE 6 bảng di sản G12** (`salary_profiles`/`payroll_periods`/`payslips`/`payslip_items`/`bonus_penalties`/`payslip_acknowledgements`) + 1 bảng mới `payroll_period_lines` — **đã migrate `0564`–`0568`**; **v2: ALTER 3 bảng (DB-1 lấy 2 · DB-2 lấy 1) + 11 bảng mới, chưa migrate — mig `0570+`** · [DB-09](<DB/DB-09 Database Index Query Pattern Performance Design.md>) §8.19 index · [DB-10](<DB/DB-10_Migration_Plan_Initial_Seed_Data_Database_Design.md>) seed + thu hồi 16 cặp quyền di sản | [API-18](<API Design/API-18_PAYROLL_API_Design.md>) **v2 — 85 route**: 001–035 đã hiện thực, **036–085 (50 route) chưa hiện thực** | UI — wireframe v1 đã duyệt ở [hồ sơ wave](<plans/S13-PAYROLL-WAVE-review.html>) · **v2: 11 màn `PAY-SCREEN-007..017` (SPEC-11 §9.1) + workspace [UI-07 §21.8](<UI/UI-07_Module_Workspace_Template_Design.md>)** | FRONTEND — **6 màn v1 đã ship** (`S13-PAYROLL-FE-1`, `apps/app/src/routes/payroll/`) + **11 màn v2 dự kiến** (`S15-PAYROLL-FE-1..4`, vỏ UI chung `S15-UI-SHELL-1`) | BACKEND — **35 route v1 đã ship** (`apps/api/src/payroll/`, `S13-PAYROLL-BE-1`/`BE-2`) + **50 route v2 dự kiến** (`S15-PAYROLL-BE-1..5`) |
...
| **SOCIAL** | PRD §8.1 (Phase 4) | [SPEC-16](<SPEC/SPEC-16 SOCIAL.md>) *(Approved 18/09/2026, wave S16-SOCIAL)* | [DB-17](<DB/DB-17 SOCIAL Database Design.md>) — **19 bảng mới** `feed_*` (chưa migrate, mig `0577+` đo journal lúc chạy) | [API-19](<API Design/API-19_SOCIAL_API_Design.md>) — **53 route** (stub DTO chi tiết ở `S16-SOCIAL-BE-1..3`) | UI — [UI-07 §34b](<UI/UI-07_Module_Workspace_Template_Design.md>) biến thể **cổng thông tin 3 cột** · wireframe ở [hồ sơ wave](<plans/S16-SOCIAL-WAVE-review.html>) | FRONTEND — dự kiến (`S16-SOCIAL-FE-1..3`, `apps/app/src/routes/social/` + `layouts/portal/`) | BACKEND — **chưa có code** (`S16-SOCIAL-BE-1..3`; app vệ tinh fbpost ở `apps/api/src/integrations/social/` giữ nguyên) |
...
| DEVOPS-16 | [Runbook Gán Role Module trên PROD](<DEVOPS/DEVOPS-16_Module_Role_Assignment_Runbook.md>) | `S14-OPS-MODULEROLE-1` — census + gán + nghiệm thu 4 role wave S11/S12/S13 (`asset-manager` · `office-admin` · `recruiter` · `payroll-officer`); công cụ đo `scripts/s14-audit-module-roles.mjs` (chỉ SELECT) | Đã có (mới) |
