...
| SPEC-11 | [PAYROLL - Tiền lương](<SPEC/SPEC-11 PAYROLL.md>) | PAYROLL — *Phase 2, Approved 31/08/2026 (wave S13-PAYROLL)* |
...
| DB-13 | [PAYROLL Database Design](<DB/DB-13 PAYROLL Database Design.md>) | PAYROLL — *Phase 2, Approved* (**RECONCILE 6 bảng di sản G12** + 1 bảng mới `payroll_period_lines`, chưa migrate) |
...
| API-18 | [PAYROLL API Design](<API Design/API-18_PAYROLL_API_Design.md>) | PAYROLL — *Phase 2, Stub — Approved* (API-13 vốn dự định cho PAYROLL đã bị CHAT chiếm) |
...
| **PAYROLL** | PRD §8.1 (Phase 2) | [SPEC-11](<SPEC/SPEC-11 PAYROLL.md>) *(Approved 31/08/2026)* | [DB-13](<DB/DB-13 PAYROLL Database Design.md>) — **RECONCILE 6 bảng di sản G12** (`salary_profiles`/`payroll_periods`/`payslips`/`payslip_items`/`bonus_penalties`/`payslip_acknowledgements`) + 1 bảng mới `payroll_period_lines`, chưa migrate · [DB-09](<DB/DB-09 Database Index Query Pattern Performance Design.md>) §8.19 index · [DB-10](<DB/DB-10_Migration_Plan_Initial_Seed_Data_Database_Design.md>) seed + thu hồi 16 cặp quyền di sản | [API-18](<API Design/API-18_PAYROLL_API_Design.md>) (stub — DTO chi tiết ở `S13-PAYROLL-BE-1/BE-2`) | UI — wireframe đã duyệt ở [hồ sơ wave](<plans/S13-PAYROLL-WAVE-review.html>) | FRONTEND — dự kiến (`S13-PAYROLL-FE-1`, `apps/app/src/routes/payroll/`) | BACKEND — **chưa có code** (`S13-PAYROLL-BE-1` sau `S13-PAYROLL-DB-1`) |
