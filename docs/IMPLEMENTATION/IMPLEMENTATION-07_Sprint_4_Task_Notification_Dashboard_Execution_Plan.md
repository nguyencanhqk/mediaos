...
| Notification event | TASK_ASSIGNED, TASK_ASSIGNEE_CHANGED, TASK_STATUS_CHANGED, TASK_PRIORITY_CHANGED, TASK_DUE_DATE_CHANGED, TASK_COMMENT_CREATED, TASK_COMMENT_MENTIONED, TASK_DUE_SOON, TASK_OVERDUE, PROJECT_MEMBER_ADDED, PROJECT_MEMBER_REMOVED, PROJECT_CLOSED (mã chuẩn theo §9.5; event chưa bật MVP để `enabled=false`, không bỏ khỏi catalog) |
...
| `TASK_COMMENT_MENTIONED` | Comment có mention | Người được mention |
...
| `TASK_COMMENT_MENTIONED` | TASK | NOTI | **Canonical.** Alias cũ: `TASK_MENTIONED` (đang dùng ở seed §8.4) -> phải đổi sang `TASK_COMMENT_MENTIONED` |
...
- **Seed §8.4**: bổ sung các event đang thiếu so với producer (`TASK_ASSIGNEE_CHANGED`, `TASK_PRIORITY_CHANGED`, `TASK_DUE_DATE_CHANGED`, `PROJECT_MEMBER_REMOVED`, `PROJECT_CLOSED`) và đổi `TASK_MENTIONED` -> `TASK_COMMENT_MENTIONED`. Nếu một event không có template/không bật trong MVP thì khai báo `enabled=false` chứ không bỏ khỏi catalog.
- **Producer §9.4**: chỉ phát các mã có trong registry; không phát mã lạ.
- **Template §10.4**: mỗi event `enabled=true` phải có template VI; dùng đúng canonical code (đã dùng `TASK_COMMENT_MENTIONED`, đúng).
...
| `TASK_COMMENT_MENTIONED` | Bạn được nhắc đến trong task | {actor_name} đã nhắc đến bạn trong {task_title} |
...
> Cột `Event` phải dùng mã chuẩn theo **Event code registry §9.5** (đã dùng `TASK_COMMENT_MENTIONED`, đúng — không dùng alias `TASK_MENTIONED`). Mỗi event `enabled=true` trong seed §8.4 cần có template VI tương ứng.
...
-> TASK phát TASK_COMMENT_MENTIONED
