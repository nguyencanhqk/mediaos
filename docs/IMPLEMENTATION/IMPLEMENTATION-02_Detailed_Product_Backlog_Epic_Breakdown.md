...
>
> Bổ sung 2026-08-28/29 (wave S11-OFFICE, Phase 3): **EPIC-17 ASSET** (§8.18, 10 story / 53 point, SPEC-13 — IMP02-STORY-153..162) · **EPIC-18 ROOM** (§8.19, 8 story / 40 point, SPEC-14 — IMP02-STORY-163..170). Bổ sung 2026-08-31 (wave S12-RECRUIT, Phase 2): **EPIC-19 RECRUIT** (§8.20, 10 story / 50 point, SPEC-12 — IMP02-STORY-171..180) — đã ship `S12-RECRUIT-DB-1..DASH-1`. Bổ sung 2026-08-31 (wave S13-PAYROLL, Phase 2): **EPIC-20 PAYROLL** (§8.21, 10 story / 63 point, SPEC-11 — IMP02-STORY-181..190) — **v1 đã ship 9/9 WO #454–#462**. Bổ sung 2026-09-11 (wave S15-PAYROLL-V2, Phase 2): **EPIC-20 PAYROLL v2** (§8.21, **+14 story / +115 point**, SPEC-11 v2 — IMP02-STORY-191..204) ⇒ EPIC-20 hiệu dụng **24 story / 178 point**. Tổng hiệu dụng: **204 story / 1454 point** (190 + 14 story; 1339 + 115 point).
>
...
| IMP02-STORY-155 | Asset Manager | Là Asset Manager, tôi muốn cấp phát tài sản cho nhân viên trong một bước và in biên bản. | P0 | 8 | `In Stock → Assigned`; lượt `Active` + partial unique 1 lượt/tài sản (race 2 request ⇒ 1 lượt, 409 không 500); chỉ nhân viên `active` cùng company (404/422); `Idempotency-Key` **do client sinh** khi mở form (interceptor dùng chung, SPEC-13 §12); audit + `ASSET_ASSIGNED`; biên bản render FE (ASSET-DEC-002, ASSET-FUNC-004, ASSET-SCREEN-004). |
...
## 8.21 EPIC-20: PAYROLL - Tiền lương

> **Bổ sung 2026-08-31** theo SPEC-11 PAYROLL (wave S13-PAYROLL, Phase 2 «HR nâng cao» — owner duyệt 31/08/2026; **v1 đã ship 9/9 WO #454–#462**). Story PL-01..10 của `docs/plans/S13-PAYROLL-WAVE.md` §4 ánh xạ 1-1 sang IMP02-STORY-181..190. Trace: `S13-PAYROLL-DOC-1 → DB-1 → BE-1 → BE-2 → FE-1 → QA-1 → DASH-1`.
>
> **Bổ sung 2026-09-11 — EPIC-20 giờ gồm CẢ HAI wave.** Wave **S15-PAYROLL-V2** (owner ký 02/09/2026, PAY-DEC-011..020 — SPEC-11 §22.1, bản **v2**) thêm story **PL-11..PL-24** của `docs/plans/S15-PAYROLL-V2-WAVE.md` §4, ánh xạ 1-1 sang **IMP02-STORY-191..204** (14 story / **115 point**) ⇒ EPIC-20 hiệu dụng **24 story / 178 point** (v1: 10 story / 63 point + v2: 14 story / 115 point). Trace WO S15: `S15-PAYROLL-DOC-1 → S15-UI-SHELL-1 → {Track A: DB-1 → BE-1 → FE-1 ‖ Track B: BE-2 → BE-3 → FE-2 ‖ Track C: DB-2 → BE-4 → FE-3 ‖ Track D: BE-5 → FE-4} → S15-PAYROLL-QA-1 → S15-PAYROLL-DASH-1` (DB-2 nối tiếp DB-1 — lane migration duy nhất). Cấp phát v2 **ĐÓNG** (SPEC-11 §5.1b): **11 bảng mới + ALTER 3 bảng (DB-1 lấy 2 · DB-2 lấy 1 — §12.3 là chuỗi 4 bước nguyên tử của DB-2)** · **11 màn** `PAY-SCREEN-007..017` · **50 route** `PAYROLL-API-036..085` · **16 mã lỗi** `PAYROLL-ERR-018..033` · **17 cặp quyền MỚI, tất cả sensitive** (permission-matrix **§9g.2** ⇒ tổng **34 cặp, 30 sensitive**; **+31 grant `role_permissions` ⇒ tổng 63**) · **4 event** `NOTI-EVENT-024..027` · **2 widget DASH** · **FSM kỳ 7 → 8 trạng thái** (thêm `Published`; bản v2 của SPEC-11 §13.1 **THAY** bản v1 — «Mục tiêu» và «Phạm vi kỹ thuật chính» dưới đây mô tả **v1**).

**Mục tiêu:** Khép trục HR ở đầu ra — hồ sơ lương versioned là nguồn duy nhất, thưởng/phạt nhập tay có duyệt, kỳ lương FSM 7 trạng thái gắn khoá kỳ công ATT, máy tính lương chạy ở SQL với snapshot đóng băng và breakdown giải-thích-được, duyệt một cấp **four-eyes**, phiếu lương append-only phát hành cho nhân viên tự xem + xác nhận, export XLSX có audit, masking lương ở server theo Phương án B, và widget DASH «chi phí lương kỳ».

> ⚠️ **PAYROLL không phải nền trắng.** 6 bảng + **19 cặp quyền** đã tồn tại từ đợt G12 hướng cũ (mig `0091`–`0132`, `0005`, `0180`) với **0 route**. Phần lớn rủi ro của epic này nằm ở **reconcile + thu hồi quyền**, không ở code mới — xem SPEC-11 §11.2 và DB-13 §5.

| Story ID | Actor | User Story / Technical Story | Priority | Point | Acceptance Criteria tóm tắt |
| --- | --- | --- | --- | ---: | --- |
| IMP02-STORY-181 | Payroll Officer | Là Payroll Officer, tôi muốn quản lý hồ sơ lương của nhân sự theo phiên bản có ngày hiệu lực để tính lương đúng theo từng giai đoạn. | P0 | 5 | `salary_profiles` **versioned theo `effective_date`** là nguồn DUY NHẤT cho tính lương (PAY-DEC-003; `employee_profiles.base_salary` không tham gia); trùng ngày ⇒ 409 PAYROLL-ERR-014 (unique là chốt cuối); mask ở server + **audit lượt đọc**; lịch sử phiên bản đầy đủ (PAYROLL-FUNC-001, PAY-SCREEN-004). |
| IMP02-STORY-182 | Payroll Officer | Là Payroll Officer, tôi muốn nhập thưởng/phạt/khấu trừ theo kỳ có lý do và trạng thái duyệt để chúng chảy đúng vào bảng lương. | P0 | 5 | FSM `Pending → Approved/Rejected` (§17.17); `reason` **NOT NULL**, reject bắt buộc `decision_note`; **tự duyệt bị chặn** ⇒ 409 PAYROLL-ERR-012; hàng đã consume khoá sửa ⇒ 013; chỉ hàng `Approved` cùng kỳ chưa consume mới được gộp (PAYROLL-FUNC-002, PAY-SCREEN-005). |
| IMP02-STORY-183 | Payroll Officer | Là Payroll Officer, tôi muốn tạo kỳ lương tháng gắn với kỳ công đã khoá và thấy cảnh báo dữ liệu còn thiếu trước khi tính. | P0 | 8 | FSM **7 trạng thái** `Draft → CollectingData → Calculated → Reviewing → Approved → Paid → Locked` (§17.15) ép ở service; trùng tháng ⇒ 409 008; `calculate` đòi `attendance_periods` **`locked`** ⇒ 409 002; `readiness` liệt kê nhân sự thiếu hồ sơ lương/thiếu công (**cảnh báo mềm**, 0 người đủ điều kiện ⇒ 422 009); `Locked` khoá ngược đường chỉnh công ATT (`ATT-ERR-024`) (PAYROLL-FUNC-003/005/011, PAY-SCREEN-001/002). |
| IMP02-STORY-184 | Payroll Officer | Là Payroll Officer, tôi muốn hệ thống gom số công và ngày nghỉ có/không lương của từng người theo kỳ để làm đầu vào tính lương. | P0 | 5 | `collect` gom từ `attendance_records` + đơn nghỉ đã duyệt tách theo `leave_types.paid`; đối soát hiển thị per nhân sự; gom lại được khi kỳ chưa `Approved`; đầu vào ghi vào `input_snapshot_json` lúc tính (PAYROLL-FUNC-004). |
| IMP02-STORY-185 | Payroll Officer | Là Payroll Officer, tôi muốn tính bảng lương nháp với con số không trôi và giải thích được từng khoản. | P0 | 8 | Công thức PAY-DEC-004 chạy **set-based ở SQL** (`numeric(18,2)`, pro-rate/làm tròn/`GREATEST(gross−deduction,0)` ở SQL — **cấm số thực JS**); ghi `payroll_period_lines` + `input_snapshot_json` **đóng băng**; đổi ATT/LEAVE/hồ sơ lương sau đó không làm đổi số; 2 `calculate` song song → 1 thắng (row lock); fixture đối soát tay khớp **từng đồng** (PAYROLL-FUNC-006, PAY-SCREEN-002). |
| IMP02-STORY-186 | Payroll Officer / Company Admin | Là Company Admin, tôi muốn duyệt bảng lương do người khác tính, và người tính không được tự duyệt. | P0 | 8 | **Four-eyes ba tầng**: cặp `('approve','payroll-period')` **không grant** `payroll-officer` (403) + service so `submitted_by ≠ approved_by` (409 PAYROLL-ERR-005) + CHECK `payroll_periods_four_eyes_check` ở DB; điều chỉnh dòng có **lý do bắt buộc**, chỉ khi `Calculated` (≥ `Approved` ⇒ 003); reject bắt buộc comment → `Calculated`; `reopen` cần cặp riêng + lý do + audit, **chặn khi đã sinh phiếu** (004); NOTI-EVENT-020/021/022 (PAYROLL-FUNC-007/008, PAY-SCREEN-002). |
| IMP02-STORY-187 | Nhân viên | Là nhân viên, tôi muốn xem phiếu lương của mình khi kỳ đã phát hành và xác nhận đã nhận. | P1 | 8 | `generate-payslips` tại `Approved` copy đóng băng sang `payslips` + `payslip_items` (**append-only**), race ⇒ 409 006 (`UNIQUE (company, kỳ, người)`); `publish` (`Approved → Paid`) mở cho nhân viên + `PAYSLIP_PUBLISHED` từng người; trạng thái phiếu là **dẫn xuất** `Generated`/`Published`/`Acknowledged` (§17.16) — nhân viên **không thấy** phiếu `Generated`; cross-employee ⇒ **404**; xác nhận lần hai ⇒ 409 015 (PAYROLL-FUNC-009/010/012, PAY-SCREEN-003/006). |
| IMP02-STORY-188 | Payroll Officer | Là Payroll Officer, tôi muốn xuất bảng lương ra Excel và mọi lượt xuất đều có dấu vết. | P1 | 5 | XLSX theo kỳ; assert **CẢ HAI cặp** `('export','payroll')` + `('view-line','payroll-period')`; **audit bắt buộc**, payload **không có số tiền**; > 10.000 dòng ⇒ 422 PAYROLL-ERR-016. PDF/batch = Phase sau (PARK-PAYROLL-001) (PAYROLL-FUNC-013). |
| IMP02-STORY-189 | Payroll Officer / QA | Là người giữ dữ liệu lương, tôi muốn số lương chỉ tới đúng người có quyền và mọi lượt xem của người khác đều để lại vết. | P0 | 8 | **17 cặp** §9g (13 sensitive) + allowlist capability BACKEND; trường tiền **vắng khoá** khi thiếu quyền (không `null`/`0`), FE `.optional()`; danh sách kỳ **không chở tổng**; 6 đường đọc lương ghi `audit_logs` **cùng tx** (`/me/payslips*` **không** ghi); **THU HỒI 16 cặp quyền lương di sản** — `hr-manager` về **0 cặp**, vá lỗ `approve`/`publish` `is_sensitive=false` ăn theo wildcard; deny-path per-pair có ca ALLOW đối chứng; cross-tenant 404 (PAYROLL-FUNC (SPEC-11 §18), §11.2). |
| IMP02-STORY-190 | Company Admin / BOD | Là quản lý, tôi muốn widget dashboard «chi phí lương kỳ» (tổng gross/net + headcount + trạng thái kỳ gần nhất). | P2 | 3 | `GET /payroll-periods/summary` gác bằng cặp ĐỌC **nhạy cảm** `('view-line','payroll-period')` + **SÀN scope Company** (payload CHỨA số tiền — không dùng được cặp `view:payroll-period`); widget catalog BE + wire slug FE `DashboardWidgetGrid` + test slug-map; không quyền thì không gọi API (PAYROLL-FUNC-014, `S13-PAYROLL-DASH-1`). |
| IMP02-STORY-191 | Payroll Officer | Là Payroll Officer, tôi muốn xem danh sách nhân viên hưởng lương và chi tiết 5 tab ngay trong PAYROLL để làm lương mà không phải mở module HR. | P0 | 8 | `/payroll/employees` + chi tiết (PAY-SCREEN-007) đọc **chiếu HR bó hẹp qua route PAYROLL** (`PAYROLL-API-036/037`, PAY-DEC-016) — **KHÔNG** cấp cặp HR cho `payroll-officer`; **5 tab** (Thông tin chung · Lịch sử lương · Bảo hiểm–Công đoàn · Thuế TNCN · Gia đình), tab thiếu cặp thì **ẩn tab** chứ không render tab rỗng; `taxCode` **vắng khoá** khi caller không có `('view','salary-profile')`; số TK **chỉ 4 số cuối**; **audit lượt đọc** (PAYROLL-FUNC-015). |
| IMP02-STORY-192 | Payroll Officer | Là Payroll Officer, tôi muốn hồ sơ lương phiên bản mới ghi được NET/GROSS, lương đóng BH, lương thử việc, tỉ lệ hưởng và phụ cấp/khấu trừ có định mức. | P0 | 8 | `salary_profiles` **dựng lại `salary_type ∈ {GROSS, NET}`** (PAY-DEC-015) + đối tượng đóng TNCN + lương đóng BH + lương thử việc + tỉ lệ hưởng; phụ cấp/khấu trừ chuyển từ `allowances` **jsonb** sang bảng `salary_profile_items` theo **expand-contract** (gỡ cột jsonb là WO riêng SAU khi đo 0 đường đọc); versioned theo `effective_date` giữ nguyên là nguồn DUY NHẤT cho tính lương; mask ở server + audit lượt đọc (PAYROLL-FUNC-018, PAY-SCREEN-007). |
| IMP02-STORY-193 | Payroll Officer | Là Payroll Officer, tôi muốn khai thiết lập BH/công đoàn, tài khoản ngân hàng và người phụ thuộc của từng nhân sự để tính đúng khoản đóng và giảm trừ. | P0 | 8 | `payroll_employee_settings` **1 hàng/nhân sự** (upsert + `Idempotency-Key`) và `payroll_dependents` có khoảng hiệu lực, **cấm chồng lấp** cùng một NPT ⇒ **409 PAYROLL-ERR-032** (chốt cuối `EXCLUDE USING gist`, `23P01` map 409 **không 500**); số TK **mask 4 số cuối ở MỌI DTO**, audit **không kèm số TK**; PII mới của PAYROLL (SPEC-11 §3.12) ⇒ IDOR cross-employee có cụm ca riêng (PAYROLL-FUNC-016/017). |
| IMP02-STORY-194 | Payroll Officer | Là Payroll Officer, tôi muốn xem bảng công tổng hợp của kỳ trước khi bấm tính lương. | P1 | 3 | `GET /payroll-periods/:id/timesheet` (`PAYROLL-API-043`, PAY-SCREEN-008) gác `('view-line','payroll-period')`, **chỉ ĐỌC**, nguồn ATT (công · phép có/không lương · phút trễ) + hiện trạng khoá kỳ công; **tái dùng `computeInputsTx`** của v1 — KHÔNG viết truy vấn thứ hai (hai bộ số lệch nhau là lớp lỗi đã ghi ở SPEC-11 §13.1); **không chở số tiền**; pagination + audit (PAYROLL-FUNC-019). |
| IMP02-STORY-195 | Payroll Officer | Là Payroll Officer, tôi muốn quản lý catalog thành phần lương với công thức được kiểm cú pháp, vòng lặp và giới hạn ngay khi lưu. | P0 | 13 | `salary_components` = **hệ thống (seed, không xoá, sửa được công thức) + tự thêm**; parser/evaluator **grammar ĐÓNG** có EBNF (`+ − × ÷`, `( )`, so sánh, `IF`, `MIN/MAX/ROUND/ABS`, tham chiếu mã thành phần) — **KHÔNG `eval`/`Function`**, **KHÔNG** gọi hàm tuỳ ý; lỗi lúc **LƯU** ⇒ **422 018** (`formula-syntax`/`unknown-ref`/`unknown-function`/`too-long`/`too-deep`/`too-many-nodes`, `details[]` nêu **vị trí ký tự**) · vòng phụ thuộc ⇒ **422 019** nêu **chu trình đầy đủ** `A → B → C → A`; xoá thành phần hệ thống · thành phần đang được mẫu tham chiếu · trùng `code` · `code` đụng không gian tên `SYS_` ⇒ **409 024**; **ngân sách node TẤT ĐỊNH** (không lấy timeout đồng hồ làm cổng) + fuzz test parser (PAYROLL-FUNC-020, PAY-SCREEN-009). |
| IMP02-STORY-196 | Payroll Officer | Là Payroll Officer, tôi muốn tạo mẫu bảng lương (chọn thành phần, nhãn cột, công thức ghi đè, ẩn/hiện, thứ tự) và gắn vào kỳ. | P0 | 8 | `payroll_templates` + `payroll_template_components`; phạm vi mẫu **toàn công ty hoặc theo `org_unit`** (PAY-DEC-013 — theo vị trí/từng nhân viên là Phase sau); **kỳ gắn ĐÚNG MỘT mẫu lúc tạo**, đổi mẫu chỉ khi kỳ ≤ `CollectingData` — ngoài ra **409 023** (`template-locked` · `template-missing` · `template-inactive`); **Xem trước với dữ liệu giả** (`PAYROLL-API-054`), không chạm dữ liệu thật; ca test «sửa công thức/mẫu sau khi tính **không** đổi số» (snapshot `component_values_json` là nguồn giải thích) (PAYROLL-FUNC-021, PAY-SCREEN-010). |
| IMP02-STORY-197 | Hệ thống | Là hệ thống, tôi muốn tính lương theo mẫu của kỳ với BHXH/BHYT/BHTN/KPCĐ/đoàn phí, TNCN luỹ tiến 7 bậc, giảm trừ gia cảnh và NET gross-up để phiếu lương giải thích được từng đồng. | P0 | 13 | Engine biểu thức chạy ở **TS với `decimal.js`** (PAY-DEC-012 — **cấm `Number` trên tiền**), topo-sort theo đồ thị phụ thuộc, kết quả ghi `numeric(18,2)` + **snapshot `component_values_json` per dòng**; **SQL GIỮ bất biến** (`SUM(items) = net` · CHECK ≥ 0 · UNIQUE) — **clamp ở SQL, KHÔNG ở JS**; tỉ lệ lấy bản hiệu lực **tại ngày cuối kỳ**, thiếu/khuyết bậc ⇒ **422 022**; NET gross-up ≤ **30 vòng**, không hội tụ ⇒ **422 021** và **tx rollback toàn bộ, kỳ giữ nguyên trạng thái**; chia 0 · tràn số · vượt ngân sách node ⇒ **422 020**, kỳ **không đổi trạng thái**; fixture đối soát tay **từng đồng** cho ≥1 nhân sự đủ mọi khoản (GROSS **và** NET) (PAYROLL-FUNC-023, SPEC-11 §13.6–§13.8). |
| IMP02-STORY-198 | Company Admin | Là Company Admin, tôi muốn quản lý bảng tỉ lệ luật định theo ngày hiệu lực để đổi luật không phải đổi code. | P1 | 5 | `payroll_statutory_rates` versioned theo `effective_from` (tỉ lệ NV/DN · trần đóng · 7 bậc TNCN · giảm trừ bản thân/NPT); cặp ghi `('manage','statutory-rate')` **KHÔNG gán `payroll-officer`** (officer chỉ ĐỌC — §9g.2); màn hiện băng cảnh báo **«hệ thống lưu và áp số này, không khẳng định đúng luật»** (SPEC-11 §3.11); seed số hiện hành **do owner xác nhận 02/09/2026**, có ca test **ghim số seed**; công ty đơn-vùng ⇒ lưu **một** mức áp dụng; trùng `effective_from` hoặc sửa bản **đã có kỳ lương dùng** ⇒ **409 PAYROLL-ERR-033** (phải **tạo bản mới**, không sửa tại chỗ) (PAYROLL-FUNC-022, PAY-SCREEN-011). |
| IMP02-STORY-199 | Payroll Officer / Nhân viên | Là Payroll Officer tôi muốn duyệt tạm ứng và để nó tự thành khoản khấu trừ ở kỳ chỉ định; là nhân viên tôi muốn xem tạm ứng của mình. | P1 | 8 | `payroll_advances` FSM `Pending → Approved/Rejected → Deducted`, **four-eyes như thưởng/phạt** — tự duyệt ⇒ **409 025**; gắn vào kỳ đã ≥ `Calculated` ⇒ **409 026** (khoản khấu trừ phải có mặt **TRƯỚC** khi tính); «Tạm ứng của tôi» (`/me/payroll-advances`, **module ME** — không kéo `access:payroll`) gác `('view-own','payroll-advance')`@Own, cross-employee ⇒ **404**; NOTI-EVENT-024/025/026 với `DedupeKey` ngay seed đầu, **payload TUYỆT ĐỐI không có số tiền** (PAYROLL-FUNC-024, PAY-SCREEN-012/017). |
| IMP02-STORY-200 | Payroll Officer | Là Payroll Officer, tôi muốn lập đợt chi trả, xuất tệp UNC cho ngân hàng và đánh dấu đã trả để kỳ lương chuyển sang `Paid`. | P1 | 13 | FSM kỳ **7 → 8 trạng thái** (PAY-DEC-017): `publish` đổi đích `Approved → **Published**`, và **`complete-batch` là chuyển tiếp DUY NHẤT vào `Paid`** (`PAYROLL-API-072`, cặp `('manage','payment-batch')`) — mở tx row-lock **kỳ TRƯỚC, đợt SAU** rồi gọi chính `assertPeriodTransition`; lập đợt từ kỳ chưa `Published` · hoàn tất khi còn dòng chưa chi · hoàn tất lần hai · hai đợt cùng phủ một nhân sự ⇒ **409 027**, không đợt nào hoàn tất ⇒ **409 028**; migration theo **đúng thứ tự 4 bước** (`ADD paid_by/at` → nới `status_check` 8 giá trị → backfill `Paid → Published` → siết `published_pair_check` + thêm `paid_pair_check`); tệp **UNC XLSX là đường DUY NHẤT chở số TK đầy đủ**; `PAYSLIP_PUBLISHED` (023) phát ở **`publish`**, **KHÔNG** ở `complete-batch` (ca ghim ở QA); NOTI-EVENT-027 (PAYROLL-FUNC-025, PAY-SCREEN-013). |
| IMP02-STORY-201 | Company Admin | Là Company Admin, tôi muốn lập ngân sách lương năm theo đơn vị và theo dõi thực hiện. | P2 | 5 | `payroll_budgets` UNIQUE `(năm, đơn vị)` ⇒ **409 029** (race map 409, không 500); cặp ghi `('manage','payroll-budget')` **KHÔNG gán `payroll-officer`** — ngân sách là cam kết tài chính của BOD, officer chỉ ĐỌC để đối chiếu (§9g.2); màn hiện kế hoạch · thực hiện · chênh lệch; là nguồn của widget **PAYROLL-WIDGET-002** (`payroll-budget`, cặp `('view','payroll-budget')` + **SÀN scope Company** ở HAI tầng, FE dùng `useCanExact`) (PAYROLL-FUNC-026, PAY-SCREEN-014). |
| IMP02-STORY-202 | Payroll Officer | Là Payroll Officer, tôi muốn nhập Excel thu nhập/khấu trừ khác theo kỳ thay vì gõ tay từng dòng. | P2 | 5 | Dùng lại **khuôn import Excel của HR**; dữ liệu đổ vào `bonus_penalties` ở trạng thái **`Pending` — KHÔNG tự duyệt** (giữ nguyên four-eyes của thưởng/phạt); sai khuôn cột · vượt trần **5.000 dòng** · có dòng không khớp nhân sự ⇒ **422 030**; **toàn tệp hoặc không dòng nào** — không import một phần; audit lượt import (PAYROLL-FUNC-027). |
| IMP02-STORY-203 | Quản lý / Payroll Officer | Là quản lý, tôi muốn trang Tổng quan PAYROLL và 7 báo cáo lọc theo kỳ/đơn vị, xuất được ra Excel. | P1 | 13 | `/payroll` **là trang gốc module** = Tổng quan (6 khối Recharts + **Lời nhắc** 3 loại: phiếu chưa phát hành · NV chính thức chưa tham gia BH · lương đóng BH ngoài quy định) — người không có cặp được **chuyển hướng** tới màn đầu tiên họ mở được, **không thấy trang trắng**; **7 báo cáo**, danh mục **lọc theo quyền** (báo cáo không mở được thì **không liệt kê**); gác `('view','payroll-report')` + **SÀN scope `Company`** ép ở **HAI tầng độc lập** (metadata + data) + **audit mỗi lượt**, **KHÔNG cache** (cache hit bỏ qua audit); xuất XLSX đòi thêm `('export','payroll')`; quá **50.000 dòng** ⇒ **422 031** (PAYROLL-FUNC-028/029, PAY-SCREEN-015/016). |
| IMP02-STORY-204 | Nhân viên / Payroll Officer | Là nhân viên tôi muốn tải phiếu lương PDF của mình; là Payroll Officer tôi muốn xuất PDF hàng loạt cho cả kỳ. | P2 | 5 | Sinh ở **server** (`pdfmake`, **font Việt nhúng** — không được rơi về font thiếu dấu) **từ snapshot `payslip_items`, KHÔNG tính lại**; tải qua **signed-URL** của file-service; PDF hàng loạt (`PAYROLL-API-085`) assert **CẢ HAI** `('export','payroll')` + `('view-payslip','payslip')`, PDF phiếu của **chính mình** (`084`) chỉ cần `('view-own-payslip','payslip')` — **KHÔNG cấp cặp `export:payslip-pdf`** (PAY-DEC-019 đã ký đường khác); quá **2.000 phiếu** ⇒ **422 031**; audit mọi lượt xuất (PAYROLL-FUNC-030). |

### Phạm vi kỹ thuật chính

- **RECONCILE 6 bảng di sản G12** (DB-13 §5): GỠ **20** cột + 11 CHECK chết theo + **3 trigger** ép FSM cũ (**dựng lại 1 trigger hẹp** đóng băng field tiền của thưởng/phạt); ĐỔI `payroll_periods.status` 3 → 7 giá trị và `bonus_penalties.status` sang PascalCase; THÊM composite tenant FK cho toàn band G12 + CHECK four-eyes. **ĐO `count(*)` trước khi ALTER — ≠ 0 thì DỪNG.**
- **1 bảng MỚI** `payroll_period_lines` (bảng lương nháp) — tách khỏi `payslips` để giữ append-only mà vẫn tính lại được trước duyệt
- 3 bộ trạng thái (SPEC-01 §17.15–17.17, trong đó **§17.16 là DẪN XUẤT**) ép ở service; chốt cuối ở DB: 4 UNIQUE + CHECK four-eyes
- **17 cặp quyền** — 13 sensitive (cặp ĐỌC `view-line` tách khỏi cặp GHI `calculate`) + role hệ thống mới `payroll-officer` (`…0015`, **2FA**, không canonical); **32 grant** §9g; **thu hồi 16 cặp di sản + mọi grant**
- Seed nối head thật (`0564+` dự kiến): reconcile DDL · module giữ inactive · role · thu hồi + seed quyền · audit UNION-ADD (chỉ giá trị còn thiếu) · NOTI catalog 4 event (cả hai bảng CHECK, `DedupeKey`, **payload không có số tiền**)
- Neo `user_id` (không đổi sang `employee_id`) — nguồn công/phép đang khoá theo `user_id`; nợ reconcile đi cùng đợt ATT/LEAVE (`erd-current` §A3)

### Ghi chú dependency

PAYROLL phụ thuộc AUTH (RBAC per-pair + scope; `users`; role 2FA), HR (`employee_profiles` cho danh tính — **không** dùng `base_salary` để tính), ATT (`attendance_periods` phải `locked`; kỳ lương `Locked` khoá ngược đường chỉnh công), LEAVE (`leave_types.paid`), FOUNDATION (`audit_logs` · `@Idempotent()` · `companies.payroll_config_json`) và NOTI (outbox bridge). Lane migration **nối tiếp**: một track duy nhất `DOC-1 → DB-1 → BE-1 → BE-2 → FE-1 → QA-1 → DASH-1`; **BE tách đôi** vì cụm máy tính lương + duyệt + phát hành vượt khẩu độ một phiên vùng đỏ. `plan-reviewer` PASS trên SPEC-11 + DB-13 là cổng mở WO DB. Coverage `payroll/` **≥85%**.
...
| Sprint 13 | SPEC-11 + docs/plans/S13-PAYROLL-WAVE.md | PAYROLL - Tiền lương (Phase 2 «HR nâng cao», hậu go-live) | 181-190 (EPIC-20 PAYROLL) | 63 | Hồ sơ lương versioned, thưởng/phạt có duyệt (chặn tự duyệt), kỳ lương FSM 7 trạng thái gắn khoá kỳ công ATT + cảnh báo thiếu, máy tính lương SQL snapshot-đóng-băng + breakdown, duyệt four-eyes ba tầng, phiếu lương append-only + «phiếu của tôi» + xác nhận, export XLSX có audit, thu hồi 16 cặp quyền lương di sản, 4 event NOTI, widget «chi phí lương kỳ» |
| Sprint 15 | SPEC-11 v2 + docs/plans/S15-PAYROLL-V2-WAVE.md | PAYROLL v2 - máy tính lương lập trình được + bước chi trả thật (Phase 2 «HR nâng cao», hậu go-live) | 191-204 (EPIC-20 PAYROLL v2) | 115 | Màn Nhân viên 5 tab qua chiếu HR bó hẹp, hồ sơ lương NET/GROSS + phụ cấp có định mức, thiết lập BH/công đoàn/TK ngân hàng + người phụ thuộc, bảng công tổng hợp kỳ, catalog thành phần lương + máy công thức grammar đóng (không `eval`), mẫu bảng lương + xem trước, máy tính lương v2 (BHXH/BHYT/BHTN/KPCĐ/đoàn phí + TNCN luỹ tiến 7 bậc + giảm trừ NPT + NET gross-up), tỉ lệ luật định versioned, tạm ứng four-eyes + «tạm ứng của tôi», đợt chi trả + tệp UNC ⇒ FSM kỳ **8 trạng thái** (`Published → Paid`), ngân sách lương, import Excel thu nhập/khấu trừ khác, Tổng quan `/payroll` + 7 báo cáo, PDF phiếu lương, 17 cặp quyền mới (⇒ 34) + 4 event NOTI, 2 widget DASH |
