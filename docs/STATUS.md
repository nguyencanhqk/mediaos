...
> Sinh bởi `harness/gen-status.mjs` lúc **2026-07-31 06:53Z**. Status TỰ ĐỘNG từ ledger (start-on-touch · finish-on-commit); đóng dấu tay: `node harness/ledger.mjs start|done <WO>`. Cơ cấu WO (title/zone/paths/deps) sửa ở `harness/backlog.mjs`.
...
- `S6-SEC-IDENTITY-PROJ-1` Gốc rễ của N-1/N-2/N-1c — buộc TẦNG CHIẾU `users.email`/`users.fullName` phải nhận vị từ scope, thiếu thì VỠ TYPECHECK (không phải trả 0 hàng im lặng)
  - **vì sao chặn**: HOAN CO CHU DICH (owner chot 2026-07-31), KHONG phai ket phu thuoc. Can cu: done_when #6 cua chinh WO - 'S3 KHONG chan RC; dang trong cua so RC thi hoan, dung tron vao duong toi RC'. Cua so RC DANG MO: RELEASE-07 ghi CH…
