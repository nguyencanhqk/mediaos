...
### 🔴 S6-SEC-ORG-1 — KI-030 — gate 3 route đọc /org đang lộ danh bạ toàn tenant cho MỌI user đăng nhập (employees · teams · teams/:id/members); giữ /org/units/tree + /org/roles làm TENANT_READ có chữ ký
- **zone**: red · **skills**: code-review
- **sửa ở đâu (paths)**: `apps/api/src/org/**`, `apps/api/test/**`, `apps/console/src/**`, `docs/_review/**`, `docs/RELEASE/**`, `docs/permission-matrix-spec.md`, `docs/plans/S6-SEC-ORG-1.md`
- **phụ thuộc**: S6-SEC-ROUTEMAP-1✓
...
- `S6-REL-1` Release Candidate build + release notes + Go-live runbook + deployment/rollback rehearsal + monitoring/alerting/support readiness (WS7/WS8/WS9) — crown release ⏳ cần: S6-PERF-DB-1, S6-SEC-ORG-1, S6-SEC-NOTITX-1, S6-SEC-LOGINLOG-1, S6-SEC-MV-1
