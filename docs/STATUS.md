...
> Sinh bởi `harness/gen-status.mjs` lúc **2026-08-04 10:14Z**. Status TỰ ĐỘNG từ ledger (start-on-touch · finish-on-commit); đóng dấu tay: `node harness/ledger.mjs start|done <WO>`. Cơ cấu WO (title/zone/paths/deps) sửa ở `harness/backlog.mjs`.
...
- 🔴 `S7-CHAT-BE-9` 🔒 CHAT-API-019 nhận bộ lọc actorUserId + from/to (giữ keyset) — để CHAT-SCREEN-008 lọc ở SERVER thay vì trên các dòng đã tải
