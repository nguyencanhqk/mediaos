...
> Sinh bởi `harness/gen-status.mjs` lúc **2026-07-23 11:12Z**. Status TỰ ĐỘNG từ ledger (start-on-touch · finish-on-commit); đóng dấu tay: `node harness/ledger.mjs start|done <WO>`. Cơ cấu WO (title/zone/paths/deps) sửa ở `harness/backlog.mjs`.
...
- 🔴 `S5-LMS-APP-3` LOCAL apps/lms — API export tiến độ: GET /api/mediaos/progress?email= Bearer MEDIAOS_SYNC_TOKEN (tái dùng bearerMatches sync-users) → enrollment + % hoàn thành/course + learning time + điểm quiz/exam; cap kích thước + không lộ dữ liệu user khác
...
- `S5-LMS-APP-2` LOCAL apps/lms — SSO-only: cờ env SSO_ONLY=true → đóng register/forgot/reset/resend-otp (route redirect + API 403), /login chỉ còn nút 'Đăng nhập qua MediaOS' ({MEDIAOS_APP_URL}/lms), break-glass ADMIN_EMAILS vẫn login mật khẩu; audit consume SSO → admin_audit_log. LÀM CUỐI WAVE ⏳ cần: S5-LMS-APP-3
