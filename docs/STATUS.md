...
> Sinh bởi `harness/gen-status.mjs` lúc **2026-08-06 18:08Z**. Status TỰ ĐỘNG từ ledger (start-on-touch · finish-on-commit); đóng dấu tay: `node harness/ledger.mjs start|done <WO>`. Cơ cấu WO (title/zone/paths/deps) sửa ở `harness/backlog.mjs`.
...
- 🔴 `S7-CALL-DB-1` Migration CALL: chat_calls + chat_call_participants (company_id + RLS FORCE + append-only) + seed cặp ('call','chat-room')
- 🔴 `S8-CHAT-UX-BE-2` Avatar phòng cho group/department/project — presign wrapper gate ('update','chat-room') sao khuôn ChatFilesService + resolver riêng; direct KHÔNG có avatar riêng (dẫn xuất)
- 🟡 `S8-CHAT-UX-BE-3` Thả cảm xúc: PUT/DELETE /chat/messages/:id/reactions/:emoji + tổng hợp vào DTO tin + phát chat:reaction
...
- `S8-CHAT-UX-FE-2` Mục Ghim + menu ngữ cảnh mỗi hội thoại (ghim · tắt thông báo · đánh dấu chưa đọc · lưu trữ) + avatar trong danh sách + màn đặt avatar ở RoomInfoPanel ⏳ cần: S8-CHAT-UX-BE-2
- `S8-CHAT-UX-FE-3` Khung chat: avatar người gửi + gộp tin liên tiếp cùng người + thanh thả cảm xúc + chỉ báo đang gõ / chấm đang online ⏳ cần: S8-CHAT-UX-BE-3, S8-CHAT-UX-FE-2
- `S8-CHAT-UX-QA-1` Nghiệm thu wave S8-CHAT-UX: deny-path + cross-tenant + coverage ≥80% + xác minh ratchet không bị nới ⏳ cần: S8-CHAT-UX-FE-3
...
| `9c484f9b` | 2026-08-06 | feat(devops): SOCIAL lên domain dangfb.funtimemediacorp.com + prod-status phân biệt 404-thiếu-ingress (#356) |
| `db8c2ca9` | 2026-08-06 | feat(devops): thêm SOCIAL (fbpost) vào menu dev.bat + m prod-update/restart/status (#355) |
| `93f3035d` | 2026-08-06 | docs(devops): DEVOPS-14 §7.1b — SOCIAL đã LIVE trên PROD (đo được) + ghi bẫy stderr-redirect của nssm |
| `57fdb121` | 2026-08-06 | docs(devops): DEVOPS-14 §7.2 — gom bước còn lại về MỘT lệnh tự xin quyền admin |
| `8faaa68c` | 2026-08-06 | docs(devops): DEVOPS-14 §7 — trạng thái triển khai SOCIAL thật (PROD DB đã ở head 213) + cảnh báo cửa sổ tile chết |
