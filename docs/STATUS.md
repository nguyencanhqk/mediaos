...
> Sinh bởi `harness/gen-status.mjs` lúc **2026-07-24 14:03Z**. Status TỰ ĐỘNG từ ledger (start-on-touch · finish-on-commit); đóng dấu tay: `node harness/ledger.mjs start|done <WO>`. Cơ cấu WO (title/zone/paths/deps) sửa ở `harness/backlog.mjs`.
...
- 🟡 `S5-FND-UI-GEN-1` packages/ui — nâng primitive lên thế hệ shadcn mới (button/badge/input/select/card/table): thêm biến thể + cỡ icon, đổi ngôn ngữ focus-ring sang border-ring + ring/50 + ring-3px, data-slot/aria-invalid; GIỮ NGUYÊN các API riêng của MediaOS
