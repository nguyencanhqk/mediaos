...
> Sinh bởi `harness/gen-status.mjs` lúc **2026-07-31 00:11Z**. Status TỰ ĐỘNG từ ledger (start-on-touch · finish-on-commit); đóng dấu tay: `node harness/ledger.mjs start|done <WO>`. Cơ cấu WO (title/zone/paths/deps) sửa ở `harness/backlog.mjs`.
...
| `44736106` | 2026-07-29 | fix(sec): S6-SEC-PERMVERB-1 — N-2: chốt MỘT động từ danh bạ (`view:user`), bỏ gate legacy `read:user` (#305) |
| `e909b972` | 2026-07-29 | fix(sec): S6-SEC-ROTATE-1 — KI-043 (S0, chặn go-live): rotate 5 role Postgres PROD, cắt nguồn tái nhiễm, bind loopback, chốt hồi quy có RED-proof (#309) |
