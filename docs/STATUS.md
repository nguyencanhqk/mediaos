...
- 🟡 `S5-BRAND-BE-1` BE Thương hiệu công ty: wrapper presign logo + favicon trên FileService (pattern ME avatar) — /foundation/company/branding, logo→companies.logo_url (fileId), favicon→company_settings key, KHÔNG migration, KHÔNG quyền mới
...
- `S5-BRAND-FE-1` FE khối 'Thương hiệu' trong /system/company: upload/preview/gỡ logo + favicon qua wrapper BE-1 (pattern AvatarUploadCard 4 pha), gate update:foundation-company + dọn ô URL thô ở console ⏳ cần: S5-BRAND-BE-1
- `S5-BRAND-FE-2` FE áp thương hiệu ra vỏ app: GlobalTopbar hiện logo công ty (fallback wordmark) + hook useFavicon set favicon động sau đăng nhập + sửa favicon tĩnh vỡ /vite.svg ở cả 3 app ⏳ cần: S5-BRAND-BE-1
