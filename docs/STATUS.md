...
> Sinh bởi `harness/gen-status.mjs` lúc **2026-08-03 03:34Z**. Status TỰ ĐỘNG từ ledger (start-on-touch · finish-on-commit); đóng dấu tay: `node harness/ledger.mjs start|done <WO>`. Cơ cấu WO (title/zone/paths/deps) sửa ở `harness/backlog.mjs`.
...
- **sửa ở đâu (paths)**: `apps/api/src/chat/**`, `apps/api/src/employees/**`, `apps/api/src/org/**`, `apps/api/src/tasks/**`, `apps/api/src/scheduler/**`, `apps/api/src/recycle-bin/**`, `apps/api/src/foundation/system-jobs/**`, `apps/api/test/integration/**`, `docs/plans/S7-CHAT-BE-5.md`
...
- 🔴 `S7-FND-LINKFALLBACK-1` FilePolicy: tệp TỪNG có link module thì KHÔNG bao giờ tụt xuống fallback FOUNDATION.FILE.* — lỗ chung của 5 module, phát hiện khi vá S7-CHAT-BE-3
...
- 🔴 `S7-CHAT-BE-7` 🔒 Đường đọc-vượt membership (CHAT-DEC-004): controller+service RIÊNG /chat/oversight/*, chỉ đọc, cặp ('view','chat-oversight'), audit trong CÙNG transaction trước khi trả dữ liệu
...
| sha | ngày | mô tả |
| --- | --- | --- |
| `7fe6f7b8` | 2026-08-03 | docs(chat): S7-CHAT-BE-5 plan rev 3 — chốt 7 mục BLOCK còn treo của rev 2 |
| `73f4862a` | 2026-08-03 | feat(chat): S7-CHAT-BE-4 — tìm kiếm toàn văn tiếng Việt (CHAT-API-015) |
| `8bd4e59f` | 2026-08-03 | feat(chat): S7-CHAT-BE-3 — đính kèm tệp qua FOUNDATION Files + ChatMessageFileResolver |
