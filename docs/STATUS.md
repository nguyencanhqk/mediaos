...
> Sinh bởi `harness/gen-status.mjs` lúc **2026-08-03 09:41Z**. Status TỰ ĐỘNG từ ledger (start-on-touch · finish-on-commit); đóng dấu tay: `node harness/ledger.mjs start|done <WO>`. Cơ cấu WO (title/zone/paths/deps) sửa ở `harness/backlog.mjs`.
...
- **sửa ở đâu (paths)**: `apps/api/src/main.ts`, `apps/api/src/realtime/**`, `apps/api/test/integration/**`, `docs/plans/S7-CHAT-RT-0.md`
...
- 🟡 `S7-INT-OUTBOX-FIFO-1` OutboxWorker dispatch ĐÚNG THỨ TỰ trong cùng lô claim (KI-059): RETURNING của UPDATE không theo ORDER BY của CTE ⇒ event tiêu thụ sai thứ tự, hỏng-im-lặng ở mọi consumer phụ thuộc thứ tự

**CHỜ (kẹt phụ thuộc):**
- `S7-CHAT-FE-1` Nền FE chat: contracts + api-client + store Zustand dùng chung + MỘT kết nối WS duy nhất cho toàn app shell (trang full-screen và panel nổi dùng chung) ⏳ cần: S7-CHAT-RT-0
...
| sha | ngày | mô tả |
| --- | --- | --- |
| `cf6d05d6` | 2026-08-03 | feat(chat): S7-CHAT-RT-1 — realtime CHAT một chiều, join server-side, emit sau commit |
...
| `5dcfd14e` | 2026-08-03 | feat(chat): S7-CHAT-BE-5 — phòng dẫn xuất theo phòng ban/dự án + job đối soát |
