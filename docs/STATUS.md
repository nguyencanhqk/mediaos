...
> Sinh bởi `harness/gen-status.mjs` lúc **2026-07-20 16:33Z**. Status TỰ ĐỘNG từ ledger (start-on-touch · finish-on-commit); đóng dấu tay: `node harness/ledger.mjs start|done <WO>`. Cơ cấu WO (title/zone/paths/deps) sửa ở `harness/backlog.mjs`.
...
- **sửa ở đâu (paths)**: `apps/api/src/db/schema/**`, `apps/api/migrations/**`, `apps/api/src/foundation/seed/**`, `apps/api/test/integration/**`, `docs/plans/S5-GOAL-DB-1.md`
- **done_when (đích hội tụ)**:
...
- `S5-FND-REVOKE-1` Nợ di sản G-era (finding MEDIUM gate S5-GOAL-DB-1): REVOKE DELETE org_units + projects khỏi app role — chặn cửa cascade-xoá goals/goal_updates vòng qua soft-delete (expand-contract 2 release nếu còn caller) ⏳ cần: S5-GOAL-DB-1
...
| sha | ngày | mô tả |
| --- | --- | --- |
| `b8cc7011` | 2026-07-20 | feat(goal): DB core module Mục tiêu — goals + goal_updates (append-only) + tasks.goal_id + seed quyền/counter/audit-CHECK/NOTI-catalog [S5-GOAL-DB-1] |
