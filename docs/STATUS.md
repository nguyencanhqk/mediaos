...
> Sinh bởi `harness/gen-status.mjs` lúc **2026-07-10 05:24Z**. Status TỰ ĐỘNG từ ledger (start-on-touch · finish-on-commit); đóng dấu tay: `node harness/ledger.mjs start|done <WO>`. Cơ cấu WO (title/zone/paths/deps) sửa ở `harness/backlog.mjs`.
...
- 🟡 `S4-NOTI-BE-3` BE Notification admin config (GET events/templates/delivery-logs, PATCH bật/tắt event, cập nhật template) + reminder job TASK_DUE_SOON/TASK_OVERDUE — P1/P2
- 🔴 `S4-DASH-BE-1` BE Dashboard resolver (GET /dashboard/me, /types, /:type) + widget registry + permission/scope gate — crown data-scope
- 🟡 `S4-FE-NOTI-CLEANUP-1` Gỡ dứt điểm NotificationBell (@mediaos/ui) + notification-api legacy (web-core) — code chết gọi route BE đã xoá ở PR #133
...
| sha | ngày | mô tả |
| --- | --- | --- |
| `e31c953` | 2026-07-10 | feat(S4-NOTI-BE-2): NOTI event intake + notification engine (crown trust boundary) (#135) |
| `3ba7a06` | 2026-07-10 | feat(S4-DASH-SEED-1): mig 0484 catalog widget DASH + 7 cặp quyền + default config (#137) |
| `e7b04d6` | 2026-07-10 | fix(ci): chạy int-spec với LANE_DB + MinIO + apply-rule 201→200 (#136) |
| `bc8e979` | 2026-07-10 | chore(backlog): WO S4-FE-NOTI-CLEANUP-1 — gỡ NotificationBell + notification-api chết |
| `0f7f043` | 2026-07-10 | chore(plan): auto-loop Đội 1 reconcile S4-NOTI-BE-2 sang dạng máy-đọc (giữ SAVEPOINT + /send→BE-3) |
| `912f562` | 2026-07-10 | fix(S4-DASH-SEED-1): plan v3 — 7 cặp quyền, grant-matrix vét cạn, lane tuần tự |
| `67b12b4` | 2026-07-10 | feat(S4-FE-NOTI-1): NotificationBadge/Dropdown/List/Detail + MarkRead/MarkAllRead (#134) |
| `10447b0` | 2026-07-10 | fix(plans): 2 cảnh báo plan-reviewer còn sót trong S4-NOTI-BE-2 + S4-DASH-SEED-1 |
| `64d4787` | 2026-07-10 | feat(S4-NOTI-BE-1): My-Notification APIs (own-scope) + mig 0483 delete:notification @Own (#133) |
| `e62fc35` | 2026-07-10 | fix(plans): sửa 6 lỗi plan-reviewer BLOCK trong S4-DASH-SEED-1 + S4-NOTI-BE-2 |
| `5666c74` | 2026-07-10 | fix(harness): S4-NOTI-BE-1 khai thiếu paths → migration permission lọt LIGHT gate |
| `99ba059` | 2026-07-10 | docs(s4): pre-author plan S4-NOTI-BE-2 + S4-DASH-SEED-1, chốt Option B cho gate widget |
