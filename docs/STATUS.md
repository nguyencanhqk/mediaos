...
> Sinh bởi `harness/gen-status.mjs` lúc **2026-07-20 00:09Z**. Status TỰ ĐỘNG từ ledger (start-on-touch · finish-on-commit); đóng dấu tay: `node harness/ledger.mjs start|done <WO>`. Cơ cấu WO (title/zone/paths/deps) sửa ở `harness/backlog.mjs`.
...
- 🔴 `S5-TASK-SUBTASK-1` Công việc con = subtask THẬT (parent_task_id): CRUD + người thực hiện/hạn riêng + ẩn khỏi board + tiến độ thẻ cha = tỉ lệ con hoàn thành
...
| `ccac5bf0` | 2026-07-19 | feat(task): sidebar cây phòng ban + filter dự án theo phòng ban — đợt B (S5-TASK-NAV-TREE-1) (#242) |
| `87f6763c` | 2026-07-19 | feat(task): board pipeline FE — cột state-mode + kéo thả move-state + quản lý cột — lane 8 (fe, CUỐI đợt A) của S5-TASK-PIPELINE-1 (#241) |
| `f0cd0730` | 2026-07-19 | feat(task): dự án mới seed 6 cột pipeline mặc định trong cùng tx — lane 7 (be-seed) S5-TASK-PIPELINE-1 (#240) |
| `1d52158c` | 2026-07-19 | feat(task): board đọc theo cột pipeline (columnMode:'state') + lọc task cha — lane 6 (be-read) của S5-TASK-PIPELINE-1 (#239) |
| `40766241` | 2026-07-19 | feat(task): move-state + method dùng chung gate update-state/auto-map — lane 5 (be-write) của S5-TASK-PIPELINE-1 🔴 (#238) |
| `278e3196` | 2026-07-19 | feat(task): contracts cột pipeline — union columnMode + stateId + move-state schema — lane 4 (contracts) của S5-TASK-PIPELINE-1 (#237) |
| `b96eefbd` | 2026-07-19 | feat(task): migration 0499+0500 — seed update-state:task + nhóm review + backfill state_id — lane 3 (migration) của S5-TASK-PIPELINE-1 🔴 (#236) |
