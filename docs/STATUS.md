...
- 🔴 `S4-TASK-DB-1` Schema + migration TASK core (projects·project_members·tasks·task_assignees·task_watchers·task_comments·task_checklists·task_checklist_items·task_activity_logs) theo DB-06 — RLS+FORCE, soft-delete, index, check-constraint
- 🟢 `S4-FE-REGISTRY-1` FE đăng ký module TASK·NOTI·DASH vào route/sidebar/action registry + query layer wiring (PermissionGate, i18n vi) — nền cho màn Sprint 4
...
- `S4-NOTI-DB-1` Schema + migration NOTI (notification_events·notification_templates·notifications·notification_delivery_logs) theo DB-07 — RLS+FORCE, partial index unread, delivery status ⏳ cần: S4-TASK-DB-1
- `S4-NOTI-SEED-1` Seed notification event catalog (Event code registry §9.5 canonical) + template IN_APP tiếng Việt + permission NOTI + role mapping idempotent ⏳ cần: S4-NOTI-DB-1
...
- `S4-DASH-DB-1` Schema + migration DASH (dashboard_widgets·dashboard_widget_configs·dashboard_widget_cache) theo DB-07 — RLS+FORCE, cache không lưu dữ liệu nhạy cảm chưa mask ⏳ cần: S4-NOTI-DB-1
...
- `S4-FE-TASK-1` FE Project screens: ProjectListPage · ProjectDetailPage · ProjectFormDrawer · ProjectMemberTable (P0/P1) ⏳ cần: S4-TASK-BE-1, S4-FE-REGISTRY-1
- `S4-FE-TASK-2` FE Task screens: TaskListPage · MyTasksPage · TaskDetailPage · TaskFormDrawer · TaskAssignControl · TaskStatusSelect (P0) ⏳ cần: S4-TASK-BE-3, S4-FE-REGISTRY-1
...
- `S4-FE-NOTI-1` FE Notification: NotificationBadge · NotificationDropdown · NotificationListPage · NotificationDetailPage · MarkRead/MarkAllRead · deep link an toàn (P0/P1) ⏳ cần: S4-NOTI-BE-1, S4-FE-REGISTRY-1
- `S4-FE-DASH-1` FE Dashboard shell + P0 widgets: DashboardMePage · DashboardWidgetGrid · WidgetCard · MyTasksWidget · TaskAlertsWidget · NotificationsWidget (P0) ⏳ cần: S4-DASH-BE-2, S4-FE-REGISTRY-1
...
| `b6dae39` | 2026-07-07 | feat(auth): S2-AUTH-PERMUX-1 — tối ưu gán quyền (trạng thái thật + nhóm + bulk + nhân bản vai trò + nhãn Việt) (#120) |
| `63c6c90` | 2026-07-07 | chore(harness): chốt S2-AUTH-ROLEMEM-1 (PR #119 merged, squash 23e1686) + regen STATUS |
| `23e1686` | 2026-07-07 | feat(auth): S2-AUTH-ROLEMEM-1 — tab Thành viên trên RoleDetailPage (xem/gỡ/thêm theo người hoặc phòng ban) (#119) |
