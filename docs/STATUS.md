...
> Sinh bởi `harness/gen-status.mjs` lúc **2026-07-27 05:57Z**. Status TỰ ĐỘNG từ ledger (start-on-touch · finish-on-commit); đóng dấu tay: `node harness/ledger.mjs start|done <WO>`. Cơ cấu WO (title/zone/paths/deps) sửa ở `harness/backlog.mjs`.
...
- 🟡 `S6-QA-CHUNK-1` KI-014 — truy gốc crash ERR_IPC_CHANNEL_CLOSED rồi chuẩn hoá chạy test chia chunk vào check.sh: mở lại cổng verify local (chạm CẢ @mediaos/api và @mediaos/app)
...
- `S6-REL-1` Release Candidate build + release notes + Go-live runbook + deployment/rollback rehearsal + monitoring/alerting/support readiness (WS7/WS8/WS9) — crown release ⏳ cần: S6-PERF-DB-1, S6-QA-CHUNK-1, S6-SEC-ROUTEMAP-1, S6-SEC-ORG-1, S6-SEC-NOTITX-1, S6-SEC-LOGINLOG-1, S6-SEC-MV-1
