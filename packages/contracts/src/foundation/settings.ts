...
/**
 * S2-FND-CONTRACT-1 — Foundation Settings DTO (nguồn sự thật contracts cho /api/v1/foundation/settings*
 * + /company-settings + /system-settings). MIGRATE từ apps/api settings.dto.ts (S1-FND-SETTING-1 /
 * S2-FND-BE-8) GIỮ NGUYÊN shape — apps/api import LẠI + bọc `createZodDto` cục bộ (nestjs-zod là dep của
 * api, KHÔNG của contracts). NAMESPACE ở foundation/ để KHÔNG đè packages/contracts/src/settings.ts (domain
 * company-profile CŨ G5/CS-5 — HOT-FILE §3, tránh va).
 *
 ...
 *
 * BẤT BIẾN #3: KHÔNG field server-only (secret_ref / validation_schema / raw value) — DTO REQUEST/QUERY,
 * z.object STRIP mọi key lạ ⇒ secret pointer / validation definition KHÔNG đi qua contracts ra client.
 */
