 ...
 * 3 namespace `common`/`nav`/`auth`) và đăng ký thêm namespace FEATURE của riêng app Sản xuất
 * (`tasks` / `channels` / `chat` / `dashboard` / `kpi` / `projects` / `workflows` / `home`).
 *
 ...
 * web-core) nên glob chỉ gom các namespace feature còn lại.
 *
