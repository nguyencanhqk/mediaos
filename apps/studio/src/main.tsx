...
/**
 * FS-3 SSO: silent-refresh KHI LOAD (giống apps/web). Có phiên (refresh cookie hợp lệ) → nạp access token +
 * /me vào store → mount app Sản xuất. Không có phiên → điều hướng về app đăng nhập trung tâm (apps/auth) kèm
...
boot().catch((err: unknown) => {
  console.error("[studio] boot() lỗi không mong đợi → điều hướng về app đăng nhập:", err);
  redirectToAuth();
});
