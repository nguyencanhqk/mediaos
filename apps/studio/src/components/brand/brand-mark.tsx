...
export function BrandWordmark({
  ...
  return <span className={cn("brand-gradient-text font-semibold tracking-tight", className)}>{text}</span>;
}

/**
 * Logo đầy đủ: mark + wordmark. Dùng ở app-shell, launcher, login.
...
export function BrandLogo({
  ...
  return (
    <span className={cn("inline-flex items-center gap-2.5", className)}>
      ...
      {showWordmark && <BrandWordmark text={wordmarkText} className={cn(textSize, wordmarkClassName)} />}
    </span>
  );
}
