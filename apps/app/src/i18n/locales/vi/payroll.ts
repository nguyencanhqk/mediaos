...
export default {
  ...
  errors: {
    ...
    componentCodeReserved: "Mã này thuộc không gian tên hệ thống (tiền tố SYS_/TL_/GT_, mã hệ thống, tên hàm) — chọn mã khác.",
    componentInUse: "Thành phần đang nằm trong mẫu bảng lương — gỡ khỏi các mẫu đó trước khi ngưng dùng hoặc xoá.",
    componentValuePair: "Kiểu giá trị không khớp dữ liệu: công thức cần chuỗi công thức, cố định cần số tiền, theo hồ sơ thì để trống cả hai.",
    formulaOverrideNotAllowed: "Không ghi đè công thức được cho thành phần tổng hợp hoặc thành phần lấy theo hồ sơ — bỏ phần ghi đè.",
    formulaTooLong: "Công thức dài quá 500 ký tự — rút gọn hoặc tách thành thành phần trung gian.",
    rateEffectiveDateExists: "Đã có bản tỉ lệ luật định cùng ngày hiệu lực — sửa bản đó hoặc chọn ngày khác.",
    rateInUse: "Bản tỉ lệ đã được kỳ lương tính dùng — không sửa tại chỗ được; tạo bản mới với ngày hiệu lực mới.",
    systemComponentImmutable: "Thành phần hệ thống chỉ đổi được tên và thứ tự — muốn đổi công thức, ghi đè trong mẫu bảng lương.",
    ...
    templateComponentUnknown: "Mẫu chứa thành phần không còn trong danh mục (đã ngưng dùng hoặc xoá) — cập nhật lại danh sách thành phần của mẫu.",
