/**
 * S7-CHAT-FE-2 — cột phải: bảng thông tin phòng (SPEC-15 §9 CHAT-SCREEN-004).
 *
 * HAI tab ở WO này: **Thành viên** · **Tin ghim**. Tab **Tệp** CỐ Ý chưa có — `chatApi` chưa mirror
 * `GET /chat/rooms/:id/files` (FE-1 có test đóng đinh điều đó), nó thuộc `S7-CHAT-FE-4`. Dựng một tab
 * rỗng chờ sẵn là hứa với người dùng một thứ chưa tồn tại.
 *
...
interface RoomInfoPanelProps {
  ...
  loadError: boolean;
  onChanged: () => void;
  /** Nhảy tới một tin ghim trong hội thoại — trả `false` khi tin nằm ngoài phần lịch sử đã tải. */
...
export function RoomInfoPanel({
  ...
  return (
    ...
    >
      ...
      <Tabs
        value={tab}
        onValueChange={(v) => setTab(v as "members" | "pinned")}
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <Button
                          ...
                          onClick={() => setJumpMiss(!onJumpToMessage(message.id))}
                        >
                          {t("info.pinned.jump")}
                        </Button>
                      </div>
