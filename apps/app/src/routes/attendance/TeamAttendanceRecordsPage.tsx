/**
 * TeamAttendanceRecordsPage — bảng công nhóm (ATT-SCREEN-003, S3-FE-ATT-2).
 *
 * Gate: useCanExact('view-team','attendance') — fail-closed.
 * Thiếu quyền → forbidden EmptyState + KHÔNG gọi listTeamRecords (enabled=false).
 * Cùng shell với MyAttendanceRecordsPage (filter / pagination / columns).
 * Masking là việc của server.
 */
...
import { useTeamAttendanceRecords } from "./hooks/useAttendanceRecords";
import {
  ATT_ENGINE_PAIRS,
  ATT_STATUS,
  ATT_PATHS,
...
export function TeamAttendanceRecordsPage() {
  const { t } = useTranslation("attendance");
  const navigate = useNavigate();

  // NHẠY CẢM: useCanExact — KHÔNG wildcard fallback (view-team:attendance is_sensitive).
  ...
  const { data, isLoading, isError, refetch } = useTeamAttendanceRecords(queryParams, canView);
  ...
  if (!canView) {
    return (
      <div className="p-6">
        <EmptyState
          title={t("team.forbidden.title")}
          description={t("team.forbidden.description")}
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <DataTable
        ...
        isLoading={isLoading}
        emptyState={
          <EmptyState title={t("team.empty.title")} description={t("team.empty.description")} />
        }
        pageSize={ATT_RECORDS_PAGE_SIZE}
      />
