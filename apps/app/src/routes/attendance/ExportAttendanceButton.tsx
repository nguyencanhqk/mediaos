 ...
 *
 * Gate: useCanExact('export','attendance') — cặp NHẠY CẢM (is_sensitive) nên PHẢI dùng exact-match
 * fail-closed (mirror LeaveReportsPage / AttendanceReportsPage), KHÔNG dùng <PermissionGate>/useCan
 * wildcard-aware: user chỉ có `*:*` KHÔNG được thấy nút — khớp BE fail-closed, tránh FE-permit/BE-403
 * mismatch. Thiếu cap exact → return null (nút KHÔNG render). Server vẫn là cổng thật (403 nếu bị gọi
 * trực tiếp). Tải nhị phân qua attendanceApi.exportCompanyRecords → apiFetchBlob (refresh-on-401 replay).
 * Có loading (disabled + spinner) + error (inline alert người-đọc); server áp data-scope + cap MAX_ROWS
 * (422 vượt cap → hiện lỗi, KHÔNG tải file cắt im lặng).
 *
...
export function ExportAttendanceButton({ query, fallbackFilename }: ExportAttendanceButtonProps) {
  ...
  const canExport = useCanExact(
    ATT_ENGINE_PAIRS.EXPORT.action,
    ATT_ENGINE_PAIRS.EXPORT.resourceType,
  );
  ...
  // Cặp NHẠY CẢM — không có cap exact `export:attendance` → nút KHÔNG render.
  // (useCanExact fail-closed: `*:*`/wildcard KHÔNG mở cổng; server vẫn 403 nếu bị gọi trực tiếp.)
