/**
 * AttendanceAuditLogsPage — /attendance/audit-logs (S3-FE-ATT-6, ATT-SCREEN-019, CO-S4-006).
 *
 * Viewer audit RIÊNG của ATT (KHÔNG dùng chung route/guard với /system/audit-logs — cặp
 * view:attendance-audit-log RIÊNG, giới hạn server-side vào object-type allowlist của ATT). Field
 * before/after/oldValues/newValues ĐÃ redact ở server (AuditMaskerService) — client chỉ render field
 * top-level (action/objectType/actor/thời gian); KHÔNG tự suy đoán field bị ẩn.
 */
...
import { useAttendanceAuditLogs } from "../hooks/useAttendanceReports";
...
export function AttendanceAuditLogsPage() {
  const { t } = useTranslation("attendance");
  const { t: tc } = useTranslation("common");
  const canView = useCanExact(
    ATT_ENGINE_PAIRS.AUDIT_LOG_VIEW.action,
    ATT_ENGINE_PAIRS.AUDIT_LOG_VIEW.resourceType,
  );
  ...
  const { data, isLoading, isError, refetch } = useAttendanceAuditLogs(queryParams, canView);
