...
/**
 * FS-4 SSO: silent-refresh KHI LOAD (giống apps/console). Có phiên (refresh cookie hợp lệ) → nạp access token
 * + /me vào store → mount vỏ nghiệp vụ (tenant). Không có phiên → điều hướng về app đăng nhập trung tâm
 * (apps/auth) kèm `?redirect=<đích>`. KHÔNG render UI khi chưa có phiên. Hết phiên giữa chừng do api-client
 * xử lý (refresh-on-401 → redirect). App này TENANT (aud=user) — vỏ ứng dụng đích mặc định sau đăng nhập.
 */
...
boot().catch((err: unknown) => {
  console.error("[app] boot() lỗi không mong đợi → điều hướng về app đăng nhập:", err);
  redirectToAuth();
});
