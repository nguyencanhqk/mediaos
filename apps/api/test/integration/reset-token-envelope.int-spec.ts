...
describe.skipIf(!hasDb)('G6-2b RED 12 — forgotPassword reset-token must be enveloped, not plaintext', () => {
  ...
  it('RED 12d — pre-existing plaintext outbox rows are scrubbed (count → 0)', async () => {
    ...
    await direct.query(
      ...
      [
        A.companyId,
        JSON.stringify({ userId: randomUUID(), email: EMAIL, resetToken: `${A.companyId}.legacy-plaintext` }),
      ],
    );
    const res = await direct.query(
