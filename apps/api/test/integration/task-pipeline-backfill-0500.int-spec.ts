...
describe.skipIf(!hasLaneDb)("S5-TASK-PIPELINE-1 — migration 0500 backfill (DB cô lập LANE_DB)", () => {
  let direct: Pool;
  let A: SeededTenant;
  ...
  beforeAll(async () => {
    ...
    await seedState(pCustom, "Quay", "started", 2);
  ...
  it("0500 end-to-end: map (i) · giữ (ii)/(iii) · heal (iv) · review (v) · rename a2 · dồn sort a3 · seed a · idempotent", async () => {
    ...
    try {
      ...
      expect(await stateOf(client, tTodo), "(iii) đúng cột theo status → giữ").toBe(regCol["Todo"]);
      expect(await stateOf(client, tHeal), "(iv) heal lệch nhóm → cột started").toBe(
        regCol["In Progress"],
      );
      expect(await stateOf(client, tReview), "(v) In Review → cột review, KHÔNG phải started").toBe(
        rev.rows[0].id,
      );
      ...
      const snapshot = async () =>
        ...
        JSON.stringify(
          (
            await client.query(
              `SELECT id, state_id FROM tasks WHERE company_id=$1 ORDER BY id`,
              [A.companyId],
            )
          ).rows,
        );
