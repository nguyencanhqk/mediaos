...
describe.skipIf(!hasLaneDb)("S5-GOAL-BE-2 check-in/finalize (DB cô lập, đường thật)", () => {
  ...
  beforeAll(async () => {
    ...
    // CỐ Ý CẤP ĐỦ cặp TASK @Department: ca C1b bên dưới phải bị chặn bởi **PHẠM VI** (việc thuộc phòng
    // khác), KHÔNG phải bởi "thiếu grant" — nếu thiếu grant thì 403 xuất hiện dù có gate hay không và
  ...
  // ── C1b. CỔNG THỨ HAI phía TASK khi gắn/tháo (finding tự soi, FULL gate) ──────
  describe("C1b. gắn task đòi CẢ update:goal LẪN phạm vi ghi TASK", () => {
    it("trưởng phòng B KHÔNG gắn được việc của phòng A vào mục tiêu phòng B (⇒ 404, cột goal_id không đổi)", async () => {
      // Mục tiêu cấp PHÒNG của B: vế `department` của GOAL-ERR-008 chỉ CẢNH BÁO MỀM, nên nếu chỉ gate
      // update:goal thì actor gắn được BẤT KỲ việc nào trong tenant — kể cả việc của phòng khác.
      const goalB = await createGoal({
        name: "Mục tiêu phòng B (gắn việc)",
        level: "department",
        departmentId: ouB,
        status: "Active",
        progressMode: "tasks",
      ...
      // Việc của phòng A, giao cho nhân viên phòng A ⇒ NGOÀI phạm vi ghi TASK của trưởng phòng B.
      const taskOfA = await direct.query(
        `INSERT INTO tasks (company_id, title, task_type, task_status, department_id,
                            main_assignee_employee_id)
         VALUES ($1,'Việc riêng của phòng A','office','Todo',$2,$3) RETURNING id`,
        [A.companyId, ouA, staffEmp],
      );
      const taskId = taskOfA.rows[0].id as string;

      const res = await auth("post", `/goals/${goalB}/tasks`, headToken).send({
        taskIds: [taskId],
      });
      expect(res.status, JSON.stringify(res.body)).toBe(404);

      const row = await direct.query("SELECT goal_id FROM tasks WHERE id = $1", [taskId]);
