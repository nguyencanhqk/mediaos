...
export const IDENTITY_VERDICTS: readonly IdentityVerdict[] = [
  ...
  },
  {
    ...
    basis: "second-assert",
    reason:
      "SOCIAL-API-028/029 — tên NGƯỜI BÁO CÁO trong hàng đợi kiểm duyệt. Căn cứ là cặp quyền RIÊNG `view:feed-report`/`manage:feed-report` (cao hơn `view:feed`) assert ở tầng 2, cộng vị từ phạm vi D6 nằm TRONG chính câu (`scopeCondition`, `switch` vét cạn 5 giá trị `DataScope`, mọi giá trị ngoài `Company|System|Department` ⇒ `sql`false``). CỐ Ý khai `second-assert` chứ KHÔNG `scoped-predicate`: câu này **không** mang `visiblePostCondition` — bypass ĐÃ KÝ (D13/H4-ii) để hàng đợi đọc được cả bài `hidden`/đã xoá mềm — nên nhãn «vị từ scope chặn tập hàng» sẽ hứa nhiều hơn thứ câu truy vấn thật sự làm. ⚠️ ĐIỂM CHIẾU NÀY CHỈ CÒN HIỆU LỰC Ở SCOPE Company/System (SOC-DEC-011, owner ký 22/09/2026). Cột VẪN được chiếu về app, nhưng `toReportDto(r, revealReporter)` trả `reporter: null` khi `SocialAccessService.isCompany(actor.routeScope)` là `false` — tức manager mang `view:feed-report@Department` KHÔNG đọc được danh tính nào. Lý do: vị từ D6 tính theo đơn vị của BÀI, nên khi tác giả bài bị tố chính là trưởng đơn vị đó, họ đọc được tên người vừa tố giác mình — kênh trả đũa trực tiếp. 🔴 **ĐỪNG «dọn» tham số `revealReporter`**: nó là cổng, không phải thừa. Kênh anh em đã bịt cùng lượt: NOTI-036 không forward `actorUserId` sang `notifications.payload` (`PAYLOAD_KEYS_DENIED` ở `social-noti-bridge.registrar.ts`).",
    signedBy: "S16-SOCIAL-BE-1B",
  },
  {
