...
function makeRepo() {
  return {
    ...
    getOrgTree: vi.fn().mockResolvedValue([
      { id: 'div', name: 'Division', children: [{ id: 'dep', name: 'Dept', children: [{ id: 'unit', name: 'Unit', children: [] }] }] },
    ]),
    ...
    createTeam: vi.fn().mockResolvedValue([{ id: TEAM_ID, name: 'Team A', type: 'production_team' }]),
...
describe('OrgService (F3 breadth)', () => {
  ...
  it('createOrgUnit → ConflictException on unique violation', async () => {
    ...
    await expect(service.createOrgUnit(COMPANY_ID, { name: 'Dup', type: 'department' })).rejects.toBeInstanceOf(
      ConflictException,
    );
  });
  ...
  it('createOrgUnit → InternalServerError when insert returns nothing', async () => {
    ...
    const { service } = makeService(repo);
    await expect(service.createOrgUnit(COMPANY_ID, { name: 'X', type: 'department' })).rejects.toBeInstanceOf(
      InternalServerErrorException,
    );
  });
  ...
  it('updateOrgUnit → Conflict on unique violation', async () => {
    ...
    const { service } = makeService(repo);
    await expect(service.updateOrgUnit(COMPANY_ID, UNIT_ID, { code: 'DUP' })).rejects.toBeInstanceOf(
      ConflictException,
    );
  });
  ...
  it('deleteOrgUnit → NotFound when nothing deleted', async () => {
    ...
    const { service } = makeService(repo);
    await expect(service.deleteOrgUnit(COMPANY_ID, UNIT_ID)).rejects.toBeInstanceOf(NotFoundException);
  });
  ...
  it('createTeam → Conflict on unique violation', async () => {
    ...
    const { service } = makeService(repo);
    await expect(service.createTeam(COMPANY_ID, { name: 'Dup', type: 'production_team' })).rejects.toBeInstanceOf(
      ConflictException,
    );
  });
  ...
  it('createTeam → InternalServerError when insert returns nothing', async () => {
    ...
    const { service } = makeService(repo);
    await expect(service.createTeam(COMPANY_ID, { name: 'X', type: 'production_team' })).rejects.toBeInstanceOf(
      InternalServerErrorException,
    );
  });
  ...
  it('addTeamMember resolves with row', async () => {
    const { service } = makeService();
    const row = await service.addTeamMember(COMPANY_ID, TEAM_ID, { userId: USER_ID, roleName: 'member' });
