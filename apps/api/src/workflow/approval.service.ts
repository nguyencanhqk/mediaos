...
import { computeNewlyUnblockedStepIds, isWorkflowComplete, type DagContext } from "./workflow-dag";
import {
...
function mapFsmError(err: unknown): never {
  ...
  if (err instanceof ApprovalRequestNotPendingError) throw new ConflictException(err.message);
  throw err;
}
...
export class ApprovalService {
   ...
   */
  async approve(companyId: string, requestId: string, actorId: string, comment?: string) {
    ...
    return this.db
      .withTenant(companyId, async (tx) => {
        ...
        if (!instance)
          throw new NotFoundException(`Instance not found: ${step.workflowInstanceId}`);
        ...
        if (!closedRequest)
          throw new InternalServerErrorException("Failed to close approval request");
        ...
        const instanceStepRows = await this.repo.findStepsByInstanceIdInTx(
          companyId,
          instance.id,
          tx,
        );
        ...
        for (const candidateId of unblockedCandidateIds) {
          if (lockedCandidateIds.has(candidateId)) {
            ...
            this.logger.warn(
              "approve fan-out: candidate still locked by another revision — not opening",
              {
                candidateId,
        ...
        for (const openStepId of newlyOpenedStepIds) {
          ...
          const openDef = defStepRows.find(
            (d) => d.nodeKey === (openStep.nodeKey ?? openStep.stepCode),
          );
        ...
        const approvedDef = defStepRows.find((d) => d.nodeKey === (step.nodeKey ?? step.stepCode));
        if (!approvedDef) {
          ...
          this.logger.error(
            "approve eval-hook: no def-step matched approved step — eval flag unverifiable",
            {
              stepId: step.id,
