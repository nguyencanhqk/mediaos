...
export const AUDIT_OBJECT_TYPES = [
  ...
  // 0446/0440), append-only #2 nguyên vẹn; INSERT audit KHÔNG vỡ audit_logs_object_type_chk trên Postgres thật.
  "leave_policy",
  // S3-ATT-BE-5 (mig 0464): remote/onsite-work request workflow — create/submit/approve/reject/cancel
  // ghi audit object_type='remote_work_request' audit-in-tx app-tenant (RemoteWorkRequestService). before/
  // after = snapshot request (request_type/start-end date/attendance_mode/location/reason/approver/
  // watcher_user_ids/status) — KHÔNG salary/PII/secret (BẤT BIẾN #3 — masker che nếu lọt). 0464 UNION
  ...
  // S5-HR-IMPORT-BE-1 (mig 0496): HR bulk employee import — HrEmployeeImportService session-audit ghi 1 dòng
  // object_type='employee_import' {fileName, ok, fail} audit-in-tx app-tenant (mỗi employee tạo qua
  // createFromImportTx cũng ghi 'create' object_type='employee'). before/after CHỈ metadata phiên import
  // (fileName/ok/fail/rows) — KHÔNG salary/identity/PII vào before/after (BẤT BIẾN #3 — masker che nếu lọt).
  // Import hàng loạt danh bạ = hành động quan trọng (SPEC-01 §16.3). 0496 UNION ADD-only vào CHECK (clone
