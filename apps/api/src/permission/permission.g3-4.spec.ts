...
describe("PermissionGuard", () => {
  ...
  function makeGuard(reflector: Reflector): import("./guards/permission.guard").PermissionGuard {
  ...
  it("(10) passes when can() returns ALLOW", async () => {
    const guard = makeGuard(
      makeReflector({ permMeta: { action: "read", resourceType: "project" } }),
    );
  ...
  it("(11) throws 403 when can() returns DENY (deny-explicit)", async () => {
    const guard = makeGuard(
      makeReflector({ permMeta: { action: "delete", resourceType: "project", isSensitive: true } }),
    );
    mockPermSvc.can.mockResolvedValue({
      allow: false,
  ...
  it("(12) throws 403 when can() returns deny-sensitive (sensitive + no explicit grant)", async () => {
    const guard = makeGuard(
      makeReflector({
        permMeta: { action: "view-salary", resourceType: "payslip", isSensitive: true },
      }),
    );
    mockPermSvc.can.mockResolvedValue({
      allow: false,
      reason: "deny-sensitive",
      auditRequired: true,
    } satisfies PermissionDecision);
    const ctx = makeCtx({ user: { id: "u1", companyId: "co1", email: "a@b.com" } });
    await expect(guard.canActivate(ctx)).rejects.toThrow(ForbiddenException);
  ...
  it("(14) privilege-escalation: user without grant-object-permission → 403", async () => {
    const guard = makeGuard(
      makeReflector({
        permMeta: {
          action: "grant-object-permission",
...
describe("CachedPermissionRepository", () => {
  ...
  it("(15) cache miss: queries DB and stores result in Valkey", async () => {
    ...
    // So với chuỗi do permCapKey() THẬT sinh ra — KHÔNG hạ xuống expect.any(String)/stringContaining
    // lỏng: assert lỏng ở đây sẽ đóng đinh một lỗ (khoá mất envScope vẫn xanh).
    expect(mockValkey.set).toHaveBeenCalledWith(permCapKey("co1", "u1"), expect.any(String), 300);
