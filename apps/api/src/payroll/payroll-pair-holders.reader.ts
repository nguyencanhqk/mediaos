 ...
 * tham số hoá theo `routeKey`. Là CTE của `PayrollApproverReader` (mig `0566`) tổng quát hoá theo cặp — cùng cách
 * khớp `decideCan` (permission.decide.ts): **DENY thắng tất cả** (kể cả DENY qua wildcard) · ALLOW phải **exact, KHÔNG
 * wildcard** (mọi cặp track C `is_sensitive = true` — mig 0571) · KHÔNG xét `object_permissions` · `user_roles` sống,
 * chưa hết hạn · **SÀN SCOPE Company** (SPEC-11 §13.5 — grant hẹp hơn sẽ 403 ở route, tính họ là «đủ» là xanh giả).
 *
 * Ba caller, ba cổng:
 ...
 *
 * `PayrollApproverReader` GIỮ NGUYÊN (file đã qua FULL gate) — reader này KHÔNG thay nó.
...
export class PayrollPairHoldersReader {
  ...
  async holdersTx(
    ...
    const { action, resourceType } = PAYROLL_ROUTE_PAIRS[routeKey];
