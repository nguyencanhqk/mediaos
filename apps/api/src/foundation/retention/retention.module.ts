 ...
 * `system_job_runs` qua FUNCTION SECURITY DEFINER (mig 0510) trên `workerDb` — KHÔNG dùng RetentionService/
 * DatabaseModule (chạy role mediaos_worker trực tiếp, không có DI-dep) nên chỉ cần khai trong providers để
 * DiscoveryService gom. KHÔNG export (không consumer nào ngoài scheduler).
 *
