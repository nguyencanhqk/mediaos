 ...
 * and wraps responses in the {success,message,data,meta} envelope. PermissionGuard gating is integration/
 * e2e territory (guard is class-level @UseGuards(PermissionGuard) + @RequirePermission per route).
 */
...
describe("FilesController", () => {
  ...
  it("POST /files → upload(user, parsed input) + success envelope", async () => {
    ...
    expect(res).toMatchObject({ success: true, data: { id: "f1" } });
  });

  it("GET /files → list with meta envelope", async () => {
    const res = await h.controller.list(req, { page: 1, limit: 20 } as never);
    expect(h.service.list).toHaveBeenCalledWith(
      USER,
      expect.objectContaining({ page: 1, limit: 20 }),
    );
    expect(res).toMatchObject({ success: true, meta: { total: 1 } });
  });

  it("GET /files/:id → getMetadata(user, id)", async () => {
    const res = await h.controller.getOne(req, "f1");
    expect(h.service.getMetadata).toHaveBeenCalledWith(USER, "f1");
    expect(res).toMatchObject({ success: true, data: { id: "f1" } });
  });

  it("GET /files/:id/download → getDownloadUrl(user, id)", async () => {
    const res = await h.controller.download(req, "f1");
    expect(h.service.getDownloadUrl).toHaveBeenCalledWith(USER, "f1");
    expect(res.data).toMatchObject({ url: "https://x/y" });
  });
