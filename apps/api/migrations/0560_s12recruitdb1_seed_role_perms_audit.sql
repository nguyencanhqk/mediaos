-- Migration 0560: S12-RECRUIT-DB-1 (🔴 RED, zone=red, crown) — SEED nghiệp vụ RECRUIT (DB-14 §9 bước B, mirror 0554):
--   role hệ thống `recruiter` + 16 cặp quyền (7 cặp resource `candidate` is_sensitive=TRUE — REC-DEC-003) +
--   42 grant per-(role,pair) theo ma trận §9f + UNION-ADD 4 giá trị `job_opening`/`candidate`/`interview`/`offer`
--   vào CHECK audit_logs.object_type. THUẦN DATA/DDL-CHECK — KHÔNG db:generate.
--
...
--
-- QUYẾT ĐỊNH CHỐT (plan docs/plans/S12-RECRUIT-DB-1.md §1.4 · DB-14 §9B):
--   D1 modules.RECRUIT ĐÃ TỒN TẠI từ 0435 (Extension, is_active=false, sort_order 9). KHÔNG INSERT, KHÔNG BẬT —
--      guard FORWARD-COMPATIBLE: chỉ RAISE khi hàng KHÔNG tồn tại, KHÔNG assert is_active (bài học
--      module-enable-guard-blocks-next-wo — S12-RECRUIT-FE-1 sẽ bật cờ; ca idempotency replay file này phải
--      sống sót sau đó). Pin migration-smoke EXTENSION_INACTIVE_MODULES ĐÃ có 'RECRUIT' — không sửa ở đây.
--   D2 role `recruiter` (REC-DEC-008): company_id NULL, is_system=true, requires_two_factor=false TƯỜNG MINH,
--      id cố định …0014 (…0013 = office-admin; tiền lệ 0019 hr-manager). KHÔNG canonical — không vào
--      DashCanonicalRole / NOTI_CANONICAL_ROLES / pin auth-seed-canonical-roles. Hiring manager = role
--      `manager` có sẵn, KHÔNG role mới. GHI NHẬN CHO AUDIT: recruiter giữ 7 cặp sensitive @Company với
--      requires_two_factor=false (tiền lệ hr/asset-manager — chốt có chủ đích, đổi ở AUTH nếu chính sách mở rộng).
--   D3 16 cặp SPEC-12 §11: 7 cặp resource `candidate` is_sensitive=TRUE (PII ứng viên), 9 cặp còn lại false.
--      Cặp sensitive ⇒ WO BE-1 PHẢI khai allowlist capability BACKEND trước khi FE render màn quản trị.
--   D4 ma trận 42 hàng (§9f): employee 0 · manager 3 (access@Own · view:interview@Own · feedback:interview@Own) ·
--      hr 7 (access@Own · 4 cặp view@Company · convert@Company · feedback@Own) · company-admin 16 ·
--      recruiter 16 (access@Own · feedback@Own · 14 cặp @Company). Grant vào role canonical `hr`
--      (role hệ thống `hr-manager` KHÔNG nhận grant RECRUIT ở v1 — SPEC-12 §11 ghi chú).
--   D5 audit CHECK: clone NGUYÊN khối 0554 bước (5) (= 0545: neo 2 tầng `object_type = ANY(…)`, fail-closed,
--      NO-LOSS/NO-GAIN — bẫy audit-check-union-parse-anchor-trap). v_new = job_opening + candidate + interview +
--      offer — bản đồ hành động→object_type là danh sách ĐÓNG ở SPEC-12 §12 (ghi chú/feedback/export gom về
--      aggregate; KHÔNG có candidate_note/interview_feedback riêng).
--   D6 HAI NAMESPACE CỐ Ý KHÁC NHAU (plan-review DOC-1 LOW): audit object_type dùng snake (`job_opening`)
--      theo họ giá trị có sẵn trong CHECK; resource cặp quyền dùng dash (`job-opening`) theo quy ước engine
--      (`update-status`/`assign-role`/`move-stage`). Census/grep phải quét CẢ HAI.
--
...
--   runtime giải wildcard *:* nhưng cổng sensitive đòi cặp exact — verify M8 ở int-spec).
--
-- BAND 0560 (lane S12-RECRUIT-DB-1). Journal: idx 227, when 1717587349000 (> 0559 idx 226 / 1717587348000).
--   AUDIT_OBJECT_TYPES (schema/audit.ts) += 'job_opening','candidate','interview','offer' CÙNG commit.
-- ════════════════════════════════════════════════════════════════════════════════════════════════

-- ─────────────── (1) modules.RECRUIT: verify TỒN TẠI, forward-compatible (mirror 0554 bước 1) ───────────────
DO $$
DECLARE v_n int;
BEGIN
...
-- ─────────────── (2) Role hệ thống recruiter (tiền lệ 0550 asset-manager / 0554 office-admin) ───────────────
-- MED-3 (plan-review v2): id cố định …0014 phải CHƯA thuộc role nào khác — ON CONFLICT DO NOTHING sẽ nuốt
-- va chạm id trong im lặng và bước (4) grant nhầm role.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM roles WHERE id = '00000000-0000-0000-0000-000000000014' AND name <> 'recruiter'
  ) THEN
    RAISE EXCEPTION '[0560] id …0014 DA thuoc role khac (khong phai recruiter) — chon id co dinh moi';
  END IF;
END;
$$;
--> statement-breakpoint
INSERT INTO roles (id, company_id, name, description, is_system, requires_two_factor) VALUES
  ('00000000-0000-0000-0000-000000000014', NULL, 'recruiter',
   'Recruiter: chuyên viên tuyển dụng — CRUD vị trí/ứng viên, pipeline stage, phỏng vấn, offer, convert (SPEC-01 §10.7 · SPEC-12)',
   true, false)
...
--     7 cặp resource `candidate` = is_sensitive TRUE (REC-DEC-003 — PII ứng viên).
INSERT INTO permissions (action, resource_type, is_sensitive) VALUES
  ...
  ('create',     'job-opening', false),  -- RECRUIT.JOB.CREATE             tạo vị trí
  ('update',     'job-opening', false),  -- RECRUIT.JOB.UPDATE             sửa · gán recruiter · FSM
  ('view',       'candidate',   true),   -- RECRUIT.CANDIDATE.VIEW         xem ứng viên (PII dạng che)
  ('create',     'candidate',   true),   -- RECRUIT.CANDIDATE.CREATE       tạo hồ sơ + check-duplicate + CV
  ('update',     'candidate',   true),   -- RECRUIT.CANDIDATE.UPDATE       sửa hồ sơ (thấy PII KHÔNG che)
  ...
  ('export',     'candidate',   true),   -- RECRUIT.CANDIDATE.EXPORT       export (audit bắt buộc)
  ('convert',    'candidate',   true),   -- RECRUIT.CANDIDATE.CONVERT      ứng viên trúng tuyển → nhân viên
  ('view',       'interview',   false),  -- RECRUIT.INTERVIEW.VIEW         xem lượt + feedback
  ('manage',     'interview',   false),  -- RECRUIT.INTERVIEW.MANAGE       xếp/sửa/kết thúc/huỷ lượt
  ('feedback',   'interview',   false),  -- RECRUIT.INTERVIEW.FEEDBACK     feedback CỦA MÌNH (Own mọi role)
  ('view',       'offer',       false),  -- RECRUIT.OFFER.VIEW             xem offer (KHÔNG lương)
  ('manage',     'offer',       false)   -- RECRUIT.OFFER.MANAGE           tạo/sửa/FSM offer + thấy lương
ON CONFLICT (action, resource_type) DO NOTHING;
--> statement-breakpoint

-- ─────────────── (4) Grant per-(role, pair) ma trận §9f = 42 hàng (mirror 0554 bước 4) ───────────────
DO $$
DECLARE
  recruit_grants CONSTANT text[][] := ARRAY[
    -- employee: 0 hàng (không grant RECRUIT nào — least privilege)
    -- manager (hiring manager): 3
    ['manager',       'access',     'recruit',     'Own'],
    ['manager',       'view',       'interview',   'Own'],
    ['manager',       'feedback',   'interview',   'Own'],
    -- hr (role canonical — KHÔNG phải hr-manager): 7
    ...
    -- company-admin: 16 (access@Own · feedback@Own · 14 cặp @Company)
    ...
    -- recruiter: 16 (cùng hình company-admin — access@Own · feedback@Own · 14 @Company)
...
BEGIN
  ...
  RAISE NOTICE '[0560] RECRUIT grants: % INSERT moi, % re-scope (4 role x ma tran §9f = 42 hang)', v_seeded, v_rescoped;
END;
$$;
--> statement-breakpoint

-- ─────────────── (5) CHECK audit_logs.object_type += job_opening/candidate/interview/offer — CLONE NGUYÊN KHỐI 0554/0545 ───
DO $$
DECLARE
  v_oid     oid;
  v_con     text;
...
BEGIN
  ...
  IF v_add IS NULL OR array_length(v_add, 1) IS NULL THEN
    RAISE NOTICE '[0560] job_opening/candidate/interview/offer da co trong CHECK — idempotent skip';
    RETURN;
  END IF;
...
-- ════════════════════════════════════════════════════════════════════════════════════════════════
-- (6) VERIFY fail-LOUD (mirror 0554 bước 6): mọi câu đếm role NEO company_id IS NULL AND deleted_at IS NULL.
--     B4 (plan-review): per-row 42 bộ — SAI MỘT HÀNG LÀ ĐỎ; tổng-42 chỉ là sanity chống over-grant.
-- ════════════════════════════════════════════════════════════════════════════════════════════════
DO $$
DECLARE
  v_n  int;
  ...
  rw   record;
BEGIN
  -- (a) đúng 16 cặp: 7 cặp resource candidate is_sensitive=TRUE, 9 cặp còn lại FALSE
  SELECT count(*) INTO v_n FROM permissions
   WHERE resource_type = 'candidate' AND is_sensitive = true
     AND action IN ('view', 'create', 'update', 'move-stage', 'comment', 'export', 'convert');
  IF v_n <> 7 THEN
    RAISE EXCEPTION '[0560] verify: catalog co % cap candidate is_sensitive=TRUE, ky vong 7 (REC-DEC-003)', v_n;
  END IF;
  SELECT count(*) INTO v_n FROM permissions
  ...
  IF v_n <> 9 THEN
    RAISE EXCEPTION '[0560] verify: catalog co % cap RECRUIT khong-nhay-cam is_sensitive=false, ky vong 9', v_n;
  END IF;
  ...
  -- (b2) employee: 0 grant RECRUIT; manager/hr: 0 cặp GHI candidate + 0 manage interview/offer + 0 create/update job-opening
  SELECT count(*) INTO v_n
    FROM role_permissions rp
    JOIN roles r       ON r.id = rp.role_id
    JOIN permissions p ON p.id = rp.permission_id
   WHERE r.name = 'employee' AND r.company_id IS NULL AND r.deleted_at IS NULL
     AND p.resource_type IN ('recruit', 'job-opening', 'candidate', 'interview', 'offer');
  IF v_n <> 0 THEN
    RAISE EXCEPTION '[0560] verify: employee CO % grant RECRUIT — ky vong 0 (least privilege §9f)', v_n;
  END IF;
  SELECT count(*) INTO v_n
    FROM role_permissions rp
    JOIN roles r       ON r.id = rp.role_id
   ...
   WHERE r.name IN ('manager', 'hr')
     AND r.company_id IS NULL AND r.deleted_at IS NULL
     AND rp.effect = 'ALLOW'
     AND ((p.resource_type = 'candidate' AND p.action IN ('create', 'update', 'move-stage', 'comment', 'export'))
       OR (p.resource_type IN ('interview', 'offer') AND p.action = 'manage')
       OR (p.resource_type = 'job-opening' AND p.action IN ('create', 'update')));
  IF v_n <> 0 THEN
    RAISE EXCEPTION '[0560] verify: manager/hr CO % cap GHI ngoai ma tran §9f — over-grant', v_n;
  END IF;

  -- (b3) PER-ROW: từng (role, action, resource, scope) — 42 tổ hợp, mỗi tổ hợp ĐÚNG 1 hàng (B4)
  ...
  LOOP
    ...
    IF v_n <> 1 THEN
      RAISE EXCEPTION '[0560] verify: % %:% @% = % hang, ky vong 1', rw.role, rw.act, rw.res, rw.scope, v_n;
    END IF;
  END LOOP;

  -- (b4) company-admin VÀ recruiter: đúng 16 hàng; 14 cặp @Company ngoài access + feedback; feedback = Own
  ...
  -- (c) role recruiter đúng thuộc tính (ON CONFLICT DO NOTHING không sửa hàng sẵn có sai)
  SELECT count(*) INTO v_n
    FROM roles
  ...
  -- (d) CHECK audit chứa đủ 4 giá trị mới + canary employee/user (NO-LOSS) — regex biên [, { '] v [' , }]
  FOREACH v IN ARRAY ARRAY['job_opening', 'candidate', 'interview', 'offer', 'employee', 'user'] LOOP
    IF NOT EXISTS (
  ...
  -- (e) module RECRUIT TỒN TẠI. CỐ Ý KHÔNG ép is_active (forward-compat — S12-RECRUIT-FE-1 sẽ bật;
  --     hợp đồng "inactive tới khi có màn" canh ở pin EXTENSION_INACTIVE_MODULES của migration-smoke).
  IF NOT EXISTS (
  ...
  RAISE NOTICE '[0560] verify PASS: 16 perm (7 sensitive) + 42 grant per-row §9f + role recruiter + audit CHECK 4 gia tri + module RECRUIT';
END;
$$;
...
--   WHERE rp.permission_id = p.id AND p.resource_type IN ('recruit', 'job-opening', 'candidate', 'interview', 'offer');
