...
// S6-SEC-ROTATE-1 (KI-043): SEED_DIRECT_URL giờ BẮT BUỘC — mặc định cũ là DB PROD bằng mật khẩu literal.
const DIRECT_URL = resolveSeedTarget({ label: "demo-seed-full" });
...
const PEOPLE = [
  {
    email: "an.nguyen@demo.local",
    name: "Nguyễn Văn An",
    unit: "noi-dung",
    pos: "truong-phong-noi-dung",
    role: ROLE.projectManager,
    salary: 25000000,
    etype: "full_time",
  },
  {
    email: "binh.tran@demo.local",
    name: "Trần Thị Bình",
    unit: "noi-dung",
    pos: "bien-kich",
  ...
  {
    email: "hoa.vo@demo.local",
    name: "Võ Thị Hoa",
    unit: "san-xuat",
    pos: "qa",
    role: ROLE.qaReviewer,
    salary: 17000000,
    etype: "full_time",
  },
  {
    email: "khanh.do@demo.local",
    name: "Đỗ Gia Khánh",
    unit: "kenh",
    pos: "quan-ly-kenh",
    role: ROLE.channelManager,
    salary: 20000000,
    etype: "full_time",
  },
  {
...
// Phân cấp: Ban Lãnh Đạo = gốc; các phòng ban trực thuộc → sơ đồ tổ chức có cây thật.
const ORG_UNITS = [
  { code: "ban-lanh-dao", name: "Ban Lãnh Đạo", type: "department", parent: null },
  { code: "noi-dung", name: "Phòng Nội Dung", type: "department", parent: "ban-lanh-dao" },
  { code: "san-xuat", name: "Phòng Sản Xuất", type: "department", parent: "ban-lanh-dao" },
  { code: "kenh", name: "Phòng Vận Hành Kênh", type: "department", parent: "ban-lanh-dao" },
  { code: "nhan-su", name: "Phòng Nhân Sự", type: "department", parent: "ban-lanh-dao" },
  { code: "tai-chinh", name: "Phòng Tài Chính", type: "department", parent: "ban-lanh-dao" },
];
...
const CHANNELS = [
  ...
  {
    code: "podcast-khoi-nghiep",
    name: "Podcast Khởi Nghiệp",
    platform: "podcast",
    niche: "Kinh doanh",
    lang: "vi",
    health: "healthy",
...
const PROJECTS = [
  {
    code: "series-tech-2026",
    name: "Series Đánh giá công nghệ 2026",
    type: "content_production",
    priority: "high",
  },
  {
    code: "tet-2026",
    name: "Chiến dịch nội dung Tết 2026",
    type: "growth_campaign",
    priority: "urgent",
  },
  {
    code: "am-thuc-mien-bac",
    name: "Series Ẩm thực miền Bắc",
    type: "content_production",
    priority: "medium",
  },
...
const CONTENT = [
  {
    code: "ct-iphone16",
    title: "Review iPhone 16 Pro Max chi tiết",
    project: "series-tech-2026",
    status: "in_production",
    pstatus: "in_production",
  },
  {
    code: "ct-laptop-gaming",
    title: "Top 5 laptop gaming đáng mua 2026",
    project: "series-tech-2026",
    status: "review",
    pstatus: "waiting_review",
  },
  {
    code: "ct-galaxy-s25",
    title: "Đánh giá Galaxy S25 Ultra",
    project: "series-tech-2026",
    status: "published",
    pstatus: "published",
  },
  {
    code: "ct-banh-chung",
    title: "Hướng dẫn gói bánh chưng ngày Tết",
    project: "tet-2026",
    status: "draft",
    pstatus: "planning",
  },
  {
    ...
    project: "am-thuc-mien-bac",
    status: "approved",
    pstatus: "approved",
  },
  {
    code: "ct-bun-cha",
    title: "Làm bún chả Hà Nội chuẩn vị",
    project: "am-thuc-mien-bac",
    status: "in_production",
    pstatus: "in_production",
  },
  ...
  try {
    ...
    for (const u of ORG_UNITS) {
      ...
      if (!id)
        id = await selId(
          ...
          [COMPANY_ID, u.name, u.type, u.code],
        );
      unitId[u.code] = id;
    }

    // Wire parent_id sau khi mọi unit đã có id (idempotent — tự vá cả row đã tồn tại
    // từ lần seed trước chưa có phân cấp). Root (parent=null) set NULL tường minh.
    for (const u of ORG_UNITS) {
      const parentId = u.parent ? unitId[u.parent] : null;
      await c.query(`UPDATE org_units SET parent_id=$1 WHERE id=$2`, [parentId, unitId[u.code]]);
    }
    ...
    for (const p of POSITIONS) {
      ...
      if (!id)
        id = await selId(
          c,
          `INSERT INTO positions (company_id,org_unit_id,name,code,level,status) VALUES ($1,$2,$3,$4,$5,'active') RETURNING id`,
          [COMPANY_ID, unitId[p.unit], p.name, p.code, p.level],
        );
      posId[p.code] = id;
    }
    ...
    for (const person of PEOPLE) {
      ...
      if (!id) {
        id = await selId(
          ...
          [COMPANY_ID, person.email, pwHash, person.name],
        );
      } else {
        await c.query(`UPDATE users SET password_hash=$1, full_name=$2 WHERE id=$3`, [
          pwHash,
          person.name,
          id,
        ]);
      ...
      const hasRole = await selId(
        ...
        [id, person.role, COMPANY_ID],
      );
      if (!hasRole) {
        await c.query(
          `INSERT INTO user_roles (user_id,role_id,company_id,granted_by) VALUES ($1,$2,$3,$4)`,
          [id, person.role, COMPANY_ID, ADMIN_ID],
        );
      }
    }

    // ── TRƯỞNG ĐƠN VỊ + QUẢN LÝ TRỰC TIẾP (sơ đồ tổ chức) ─────────────────────
    // Trưởng đơn vị = nhân viên active có cấp chức vụ (positions.level) cao nhất trong đơn vị; thành viên
    // còn lại → direct_manager_id = trưởng đơn vị. Idempotent (chạy lại tự cập nhật), generic theo dữ liệu.
    await c.query(
      `UPDATE org_units ou SET head_user_id = sub.user_id
         FROM (
           SELECT DISTINCT ON (ep.org_unit_id) ep.org_unit_id, ep.user_id
             FROM employee_profiles ep
             LEFT JOIN positions p ON p.id = ep.position_id
            WHERE ep.company_id = $1 AND ep.deleted_at IS NULL AND ep.status = 'active'
              AND ep.user_id IS NOT NULL
            ORDER BY ep.org_unit_id, p.level DESC NULLS LAST, ep.id
         ) sub
        WHERE ou.id = sub.org_unit_id AND ou.company_id = $1 AND ou.deleted_at IS NULL`,
      [COMPANY_ID],
    );
    await c.query(
      `UPDATE employee_profiles ep SET direct_manager_id = ou.head_user_id
         FROM org_units ou
        WHERE ep.org_unit_id = ou.id AND ep.company_id = $1 AND ep.deleted_at IS NULL
          AND ou.head_user_id IS NOT NULL AND ep.user_id <> ou.head_user_id`,
      [COMPANY_ID],
    );

    // Gán admin role company-admin (để menu permission đầy đủ trên web)
    const adminHasRole = await selId(
      c,
      `SELECT id FROM user_roles WHERE user_id=$1 AND role_id=$2 AND company_id=$3`,
      [ADMIN_ID, ROLE.companyAdmin, COMPANY_ID],
    );
    if (!adminHasRole)
      await c.query(
        `INSERT INTO user_roles (user_id,role_id,company_id,granted_by) VALUES ($1,$2,$3,$4)`,
        [ADMIN_ID, ROLE.companyAdmin, COMPANY_ID, ADMIN_ID],
      );

    // ── WORK SCHEDULE (ca mặc định) + LEAVE TYPES ─────────────────────────────
    let scheduleId = await selId(
      c,
      `SELECT id FROM work_schedules WHERE company_id=$1 AND name=$2 AND deleted_at IS NULL`,
      [COMPANY_ID, "Ca hành chính"],
    );
    if (!scheduleId) {
      scheduleId = await selId(
        c,
        `INSERT INTO work_schedules (company_id,name,work_type,start_time,end_time,grace_minutes,is_default,status) VALUES ($1,$2,'fixed','08:30','17:30',15,true,'active') RETURNING id`,
        [COMPANY_ID, "Ca hành chính"],
      );
    }
    const leaveTypes = [
    ...
    for (const lt of leaveTypes) {
      ...
      if (!id)
        id = await selId(
          c,
          `INSERT INTO leave_types (company_id,name,code,paid,annual_quota,status) VALUES ($1,$2,$3,$4,$5,'active') RETURNING id`,
          [COMPANY_ID, lt.name, lt.code, lt.paid, lt.quota],
        );
    ...
    for (const person of attPeople) {
      ...
      for (let d = -7; d <= -1; d++) {
        ...
        const exists = await selId(
          c,
          `SELECT id FROM attendance_records WHERE company_id=$1 AND user_id=$2 AND work_date=$3 AND deleted_at IS NULL`,
          [COMPANY_ID, uid, isoDate(wd)],
        );
        if (exists) continue;
    ...
    for (const lv of leaves) {
      ...
      const exists = await selId(
        c,
        `SELECT id FROM leave_requests WHERE company_id=$1 AND user_id=$2 AND leave_type_id=$3 AND start_date=$4 AND deleted_at IS NULL`,
        [COMPANY_ID, uid, leaveTypeId[lv.type], isoDate(lv.start)],
      );
      if (exists) continue;
      ...
      if (!id) {
        id = await selId(
          ...
          [
            ...
            ch.name,
            ch.platform,
            platformId[ch.platform],
            ch.code,
            ch.niche,
            ch.lang,
            userId[PEOPLE[5].email],
            ch.health,
          ],
        );
      }
    ...
    for (const ct of CONTENT) {
      let id = await selId(
        c,
        `SELECT id FROM content_items WHERE company_id=$1 AND code=$2 AND deleted_at IS NULL`,
        [COMPANY_ID, ct.code],
      );
      if (!id) {
        id = await selId(
          ...
          [
            ...
            ct.status,
            ct.pstatus,
            userId[PEOPLE[1].email],
            channelId["tech-review"],
          ],
        );
      }
    ...
    let wfInstId = await selId(
      c,
      `SELECT id FROM workflow_instances WHERE company_id=$1 AND content_item_id=$2 AND status='active'`,
      [COMPANY_ID, wfContent],
    );
    if (!wfInstId) {
      wfInstId = await selId(
        c,
      ...
      const assignees = [
        userId[PEOPLE[1].email],
        userId[PEOPLE[2].email],
        userId[PEOPLE[4].email],
        userId[PEOPLE[6].email],
      ];
    ...
    let periodId = await selId(
      c,
      `SELECT id FROM payroll_periods WHERE company_id=$1 AND period_month=$2 AND deleted_at IS NULL`,
    ...
    const revenues = [
      {
        ...
        amount: 45000000,
        channel: "tech-review",
        desc: "Doanh thu AdSense kênh Tech Review T5",
      },
      {
        source: "youtube_adsense",
        amount: 28000000,
        channel: "am-thuc-viet",
        desc: "Doanh thu AdSense kênh Ẩm Thực T5",
      },
      {
        source: "sponsorship",
        amount: 60000000,
        channel: "tech-review",
        desc: "Tài trợ booking review điện thoại",
      },
      {
        source: "tiktok",
        amount: 12000000,
        channel: "du-lich-bui",
        desc: "Doanh thu TikTok Creator Fund",
      },
      {
        source: "affiliate",
        amount: 8500000,
        channel: "tech-review",
        desc: "Hoa hồng affiliate link sản phẩm",
      },
    ];
    for (const rv of revenues) {
      const exists = await selId(
        c,
        `SELECT id FROM revenue_records WHERE company_id=$1 AND description=$2 AND entry_kind='original'`,
        [COMPANY_ID, rv.desc],
      );
      if (exists) continue;
      await c.query(
        ...
        [
          COMPANY_ID,
          null,
          channelId[rv.channel],
          rv.amount,
          isoDate(day(-20)),
          rv.source,
          rv.desc,
          userId[PEOPLE[8].email],
        ],
      );
    }
    const costs = [
    ...
    for (const ct of costs) {
      const exists = await selId(
        c,
        `SELECT id FROM cost_records WHERE company_id=$1 AND description=$2 AND entry_kind='original'`,
        [COMPANY_ID, ct.desc],
      );
      if (exists) continue;
      await c.query(
    ...
    const notifs = [
      {
        ...
        body: "Lê Quốc Cường gửi duyệt video 'Top 5 laptop gaming 2026'",
      },
      {
        type: "task_submitted",
        body: "Trần Thị Bình hoàn thành kịch bản 'Review iPhone 16 Pro Max'",
      },
    ...
    for (const n of notifs) {
      const exists = await selId(
        c,
        `SELECT id FROM notifications WHERE company_id=$1 AND user_id=$2 AND body=$3`,
        [COMPANY_ID, ADMIN_ID, n.body],
      );
      if (exists) continue;
      await c.query(
        `INSERT INTO notifications (company_id,user_id,type,body,is_read) VALUES ($1,$2,$3,$4,false)`,
        [COMPANY_ID, ADMIN_ID, n.type, n.body],
      );
    }
