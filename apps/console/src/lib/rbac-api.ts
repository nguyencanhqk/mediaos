...
/**
 * RBAC API client cho apps/console (CS-2 — Phân quyền, tenant plane, aud=user).
 *
 * Mirror apps/admin tenant/rbac (self-service): mọi route đi qua `actor.companyId` của BE (token).
 * Console = 1 công ty ⇒ KHÔNG có companyId trên path; BE ép quyền + scope theo JWT (withTenant + RLS).
 *
 * Hợp đồng route (permission-admin.controller.ts + org.controller.ts):
 *   - GET    /org/roles                               → danh mục vai trò (đọc mở cho user tenant).
 *     ⚠️ BE đã LOẠI TRỪ role operator-audience (platform-admin) — chống leo thang (CS-2 escalation guard).
 *   - GET    /org/employees                           → danh sách user (subject gán role/object-perm).
 *   - POST   /permissions/users/:userId/roles         → gán role  (gate `assign-role:user`, isSensitive).
 ...
 *   - DELETE /permissions/object                      → xoá object-permission (gate idem, 204).
 *
 * ⚠️ BE KHÔNG có read-API cho "role hiện tại của 1 user" hay "danh sách object-permission". UI thao tác
 * theo DANH MỤC (chọn user + role/permission) — KHÔNG hiển thị state grant hiện hữu (mirror admin).
 */
...
export const rbacApi = {
  // ── Danh mục (read) ──────────────────────────────────────────────────────────
  /** GET /org/roles — danh mục vai trò gán được của tenant (operator role đã bị BE loại trừ). */
