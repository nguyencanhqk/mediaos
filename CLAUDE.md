...
5. **DB cô lập khi verify có Postgres:** chạy test trên DB riêng `bash scripts/lane-db-setup.sh <lane>` → `export LANE_DB=mediaos_<lane>` → `pnpm --filter @mediaos/api test`. Không set → fallback `mediaos`. Lý do: drizzle migrator áp migration đơn điệu theo `when` → DB chung bị skip migration band thấp ⇒ test xanh-giả/đỏ-giả. **`harness/check.sh` giờ PHÁT HIỆN việc này tự động:** step test chạy với `TURBO_FORCE=1` (chống turbo trả log CŨ từ cache = xanh-giả) rồi đếm số test-file `describe.skipIf(!hasDb)` bị SKIP (không phải FAIL) qua `harness/lane-db-guard.mjs`. Thiếu `LANE_DB` và số skip > ngưỡng (`INT_SKIP_THRESHOLD`, mặc định 20) → in banner LOUD + đổi kết luận từ "XANH ✅" sang trạng-thái-thứ-ba **"XANH KHÔNG ĐỦ BẰNG CHỨNG"** (mặc định vẫn exit 0 — KHÔNG phá tier thường). Chạy **như CI** (deny-path/IDOR/cross-tenant thật sự thực thi) bằng `bash harness/check.sh --lane-db[=<lane>]` (tự gọi `lane-db-setup.sh`, opt-in, cần Postgres — không có Docker thì cảnh báo chứ không bắt buộc) hoặc tự `export LANE_DB=...` trước. **Trước khi mở PR ở vùng đỏ/nhạy cảm:** `bash harness/check.sh --all` (hoặc `REQUIRE_LANE_DB=1`) — nếu số int-spec skip vượt ngưỡng mà chưa có `LANE_DB`, lệnh **exit 1 (ĐỎ)**, ép deny-path phải chạy thật trước khi merge.
